...
```bash
gh api repos/nighthawk-dev/nighthawk/pulls/<RELEASE>/files \
...
The preview is pasted into chat tools (for example Lark), where relative docs links do not resolve. Rewrite every docs link to its absolute published URL: map `../<path>.md[#anchor]` to `https://nighthawk-dev.github.io/nighthawk/zh/<path>.html[#anchor]` — for example `../configuration/config-files.md#loop-control` → `https://nighthawk-dev.github.io/nighthawk/zh/configuration/config-files.html#loop-control`. Never emit raw relative paths, and never wrap a link in backticks; code-style the link text inside the brackets instead ([`loop_control`](...)).
