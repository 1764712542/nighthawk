...
| Test import | `from '@nighthawk/agent-core/di/test'` | `from '#/_base/di/test'` |
...
| Errors | `from '../../errors'` (central `NighthawkError`, `ErrorCodes`) | `from '#/_base/errors'` + domain co-located `XxxError` — see errors.md |
...
import { NighthawkError, ErrorCodes } from '../../errors';

// v2
import { createDecorator, type ServiceIdentifier } from '#/_base/di/instantiation';
import { Disposable } from '#/_base/di/lifecycle';
import { IInstantiationService } from '#/_base/di/instantiation';
import { NighthawkError, type ErrorCode } from '#/_base/errors';
```
...
**Errors** — move any shared error into a co-located `XxxError extends NighthawkError` with a registered `code` (errors.md). Do not keep throwing v1's central error codes from a v2 domain.
...
```ts
// v1
import { TestInstantiationService } from '@nighthawk/agent-core/di/test';
