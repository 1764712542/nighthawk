...
└── errors.ts                  ← v1-compatible error codes (NighthawkError codes)
```
...
The route translates domain `NighthawkError` codes into protocol `ErrorCode` numbers. Two registries must stay in sync:
