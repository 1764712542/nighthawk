...
Run from the package (or with `--filter @nighthawk/agent-core-v2`):

- `pnpm --filter @nighthawk/agent-core-v2 lint:imports` — import-boundary guard (`scripts/check-import-boundaries.mjs`). Catches v1 imports (`@nighthawk/agent-core`) and kosong subtree violations.
- `pnpm --filter @nighthawk/agent-core-v2 typecheck` — `tsc -p tsconfig.json --noEmit`.
- `pnpm --filter @nighthawk/agent-core-v2 test` — `vitest run`.

## Changesets (when the change ships through the CLI)

If the change is user-facing and ships through the CLI, generate a changeset with the repository's `gen-changesets` skill (root `AGENTS.md` workflow). `agent-core-v2` is an internal package; if its change enters the CLI bundle, the changeset lists `@nighthawk/nighthawk` and describes the real change — do not present an internal-only change as a user-facing feature. Never write a `major` bump without explicit user confirmation.
...
- Do not list internal packages in a changeset when the change enters the CLI bundle — list `@nighthawk/nighthawk` and describe the real change.
