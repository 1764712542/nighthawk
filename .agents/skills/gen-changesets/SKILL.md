---
name: gen-changesets
description: Use when generating changesets in the nighthawk repository — deciding whether to write one, which package to list, the bump level, the wording, and the confirmation workflow.
---

# Generate Changesets

The only user-facing published package is the CLI: `@nighthawk/nighthawk`. All other `@nighthawk/*` packages (sdk, agent-core, kosong, kaos, oauth, telemetry, and so on) are internal.
...
Do write: user-perceivable new features or behavior changes, and internal-package changes that fix a user-useful bug or change CLI output/behavior (list `@nighthawk/nighthawk` for those).
...
- An internal change enters the CLI bundle and is user-perceivable → list `@nighthawk/nighthawk`.
...
- pi-tui exception: pi-tui-only changes list `@nighthawk/pi-tui`; if the same change is also visible to CLI users, write a separate CLI changeset (two files, never mixed).
- nighthawk-inspect and the vis packages never appear in a changeset.
