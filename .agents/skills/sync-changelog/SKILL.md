---
name: sync-changelog
description: Use after a release succeeds, when maintainers need to sync apps/nighthawk/CHANGELOG.md into docs/en/release-notes/changelog.md and docs/zh/release-notes/changelog.md, then open a PR on a dedicated branch.
---
...
`nighthawk` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@nighthawk/nighthawk`, writes its changelog here:
...
- The top of `apps/nighthawk/CHANGELOG.md` contains version blocks that are not yet in `docs/en/release-notes/changelog.md`.
- The `gen-docs` flow does not run this automatically; maintainers must explicitly do it after release.

Do **not** run this before the Release PR is merged. At that point, changesets has not yet written the new version into `apps/nighthawk/CHANGELOG.md`.

## Source And Targets

| File | Role | Edited by |
|---|---|---|
| `apps/nighthawk/CHANGELOG.md` | **Only upstream source**, generated by changesets | Never edit manually |
...
- The released version exists on npm (`npm view @nighthawk/nighthawk versions --json`) or has a matching GitHub Release tag.
- The top of `apps/nighthawk/CHANGELOG.md` is that new version.
...
```bash
rg '^## ' apps/nighthawk/CHANGELOG.md | head -5
...
- The upstream H1 `# @nighthawk/nighthawk` because the docs page already has `# Changelog`.
...
Drop SDK-only and provider-internal detail. This changelog serves `@nighthawk/nighthawk` CLI and web users. Within an entry, keep only what CLI/web users can perceive, and remove sentences that document internals instead of user-visible behavior. Apply this on both the English and Chinese pages:
...
```bash
git log -1 --format=%cs "@nighthawk/nighthawk@<version>"
```
...
- **Name concrete files or config keys when it helps clarity**.
  ...
  - Better: `插件现支持在 nighthawk.plugin.json 中声明生命周期 hooks。`
...
- **Keep usage hints to one short clause**.
  ...
  - Better: `例如 nighthawk web --allowed-host example.com。`
...
```markdown
- Add a --allowed-host flag to nighthawk web that lets extra Host header values pass the DNS-rebinding check, and include allow guidance in the 403 error message. Pass --allowed-host <host> to allow an extra host.
```

Before (literal, wordy):

```markdown
- 为 `nighthawk web` 新增 `--allowed-host` 标志，允许额外的 Host 请求头值通过 DNS 重绑定检查，并在 403 错误消息中包含允许指引。传入 `--allowed-host <host>` 以允许额外的 host。例如 `nighthawk web --allowed-host example.com`。
```
...
```text
docs(changelog): sync <version range> from apps/nighthawk/CHANGELOG.md
```
...
- Synced `<version range>` from `apps/nighthawk/CHANGELOG.md` into `docs/en/release-notes/changelog.md`
...
- Never edit upstream `apps/nighthawk/CHANGELOG.md`.
...
| Mistake | Fix |
|---|---|
| Adding entries directly to the English docs page without reading upstream | Use `apps/nighthawk/CHANGELOG.md` as the source |
...
- The top version in `apps/nighthawk/CHANGELOG.md` is not published on npm or GitHub Releases.
- You are about to edit `apps/nighthawk/CHANGELOG.md`.
