...
export const LEGACY_ENGINE_ENV = "NIGHTHAWK_LEGACY_FLAG";

const TRUTHY_ENV_VALUES = new Set(["1", "true", "yes", "on"]);

/**
 * The single engine-selection decision for the whole extension. A truthy
 * `NIGHTHAWK_LEGACY_FLAG` wins over the `nighthawk.useAgentCoreV1` setting, so
...
function getConfig() {
  return vscode.workspace.getConfiguration("nighthawk");
}
...
export function onSettingsChange(callback: (changedKeys: string[]) => void): vscode.Disposable {
  return vscode.workspace.onDidChangeConfiguration((e) => {
    if (!e.affectsConfiguration("nighthawk")) {
      return;
    }
    ...
    const changedKeys = keys.filter((key) => e.affectsConfiguration(`nighthawk.${key}`));
