...
import { NighthawkRuntime } from "./runtime/nighthawk-runtime";
...
export class BridgeHandler {
  ...
  constructor(
    ...
    try {
      this.runtime = new NighthawkRuntime({
      ...
      const rollbackHint = useAgentCoreV1
        ? ""
        : " You can roll back to the legacy engine: enable the 'nighthawk.useAgentCoreV1' setting and reload the window.";
      throw new Error(
        `Failed to start the NightHawk engine: ${error instanceof Error ? error.message : String(error)}.${rollbackHint}`,
        { cause: error },
      );
    }
  ...
  private trace(id: string, method: string, durationMs: number, ok: boolean): void {
    ...
    console.debug(`[nighthawk-vscode] ${line}`);
    this.writeLog(line);
  }
  ...
  private logRuntimeError(message: string, error?: unknown): void {
    ...
    console.error(`[nighthawk-vscode] ${line}`);
    this.writeLog(line);
  }
}
