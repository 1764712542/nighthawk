...
/**
 * Coordinates legacy legacy-cli migration for the VS Code host while keeping all
      ...
      prompt:
        sources.length === 0
          ...
          : {
              ...
              message:
                "Legacy NightHawk data was found. Migrate config, MCP servers, history, skills, and sessions into NightHawk? Your old data will be kept.",
              actions: [
...
function readLegacyShareDir(
  ...
  if (
    ...
    return {
      kind: "invalid",
      message: "The legacy nighthawk.environmentVariables setting is invalid and was ignored.",
    };
  }
  ...
  for (const source of sources) {
    for (const failure of source.failures) {
      if (failure.code === "run-failed") {
        actions.push(
          `Fix access to ${source.source.sourceHome} or the NightHawk home, then run “NightHawk: Migrate Legacy Data” again.`,
        );
      } else {
        actions.push(
    ...
    if (summary.config.wroteSiblingDueToConflict) {
      actions.push("Review and merge config.migrated-from-legacy-cli.toml.");
    }
    if (summary.config.wroteTuiSibling) {
      actions.push("Review and merge tui.migrated-from-legacy-cli.toml.");
    }
    if (summary.mcp.wroteSiblingDueToConflict) {
      actions.push("Review and merge mcp.migrated-from-legacy-cli.json.");
    }
...
function runMessage(
  ...
  if (status === "nothing-to-migrate") return "No legacy NightHawk data needs migration.";
  if (status === "failed") {
