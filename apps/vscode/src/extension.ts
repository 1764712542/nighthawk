...
import { NighthawkWebviewProvider } from "./NighthawkWebviewProvider";
...
const LEGACY_REAUTH_NOTICE_KEY = "nighthawk.legacyMigration.reauthNotice.v1";
const LEGACY_WARNING_NOTICE_KEY = "nighthawk.legacyMigration.warningNotice.v1";
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    vscode.window.registerWebviewViewProvider("nighthawk.webview", provider, {
  ...
  const commands: Record<string, () => void | Promise<void>> = {
    "nighthawk.clearAllState": async () => {
      await context.globalState.update("nighthawk.config", undefined);
      await context.globalState.update("nighthawk.mcpServers", undefined);
      await context.workspaceState.update("nighthawk.mcpEnabled", undefined);
      await vscode.window.showInformationMessage("NightHawk: Extension UI state cleared.");
    },
    "nighthawk.openInTab": () => {
      provider?.createPanel();
    },
    "nighthawk.openInSideBar": async () => {
      await vscode.commands.executeCommand("nighthawk.webview.focus");
    },
    "nighthawk.focusInput": async () => {
      await vscode.commands.executeCommand("nighthawk.webview.focus");
      provider?.broadcast(Events.FocusInput, {});
    },
    "nighthawk.insertMention": async () => {
    ...
    "nighthawk.newConversation": async () => {
      await vscode.commands.executeCommand("nighthawk.webview.focus");
  ...
  log("NightHawk Code activated");
}

export async function deactivate(): Promise<void> {
  log("NightHawk Code deactivating");
...
async function offerLegacyMigration(
  ...
  const warningNotice =
    discovery.warnings.length === 0
      ? null
      : "Some legacy NightHawk data could not be inspected. Use “NightHawk Code: Migrate Legacy Data” to retry.";
  if (discovery.prompt === null) {
    ...
    if (
      ...
      const action = await vscode.window.showWarningMessage(
        warningNotice ?? "Some legacy NightHawk data could not be inspected.",
        "Show Logs",
      );
...
async function performMigration(
  ...
  log(`${retry ? "Retrying" : "Starting"} legacy NightHawk data migration`);
