...
import { isNighthawkError } from "@nighthawk/nighthawk-sdk";
...
import { normalizeEffort } from "../runtime/nighthawk-runtime";
...
const streamChat: Handler<StreamChatParams, { done: boolean }> = async (params, ctx) => {
  if (!ctx.workDir) {
    ...
    void vscode.window.showWarningMessage("NightHawk: Please open a folder first.", "Open Folder").then((action) => {
...
function emitCaughtError(
  ...
  const code = isNighthawkError(error) ? error.code : "internal";
