...
export const sessionHandlers: Record<string, Handler<any, any>> = {
  [Methods.GetNighthawkSessions]: async (_, ctx): Promise<SessionInfo[]> => {
    if (!ctx.workDir) return [];
    return (await ctx.harness.listSessions({ workDir: ctx.workDir })).map(toSessionInfo);
  },

  [Methods.GetAllNighthawkSessions]: async (_, ctx): Promise<SessionInfo[]> => {
  ...
  [Methods.LoadNighthawkSessionHistory]: async (params: LoadHistoryParams, ctx) => {
    if (!ctx.workDir || !isSessionId(params.nighthawkSessionId)) return [];
    const runtime = await ctx.resumeSession(params.nighthawkSessionId);
    ...
    try {
      ...
      void Promise.resolve(
        vscode.window.showWarningMessage(
          "NightHawk: This conversation opened, but its file change history is unavailable.",
          "Show Logs",
        ),
      )
        .then((action) => {
  ...
  [Methods.DeleteNighthawkSession]: async (params: DeleteSessionParams, ctx): Promise<{ ok: boolean }> => {
  ...
  [Methods.ForkNighthawkSession]: async (params: ForkSessionParams, ctx) => {
