    ...
    if (command.name === "import") {
      ...
      if (result.sensitive) {
        void vscode.window.showWarningMessage(
          "NightHawk: The imported file may contain API keys, tokens, or credentials.",
        );
      }
    } else {
      switch (command.name) {
...
async function exportContext(
  ...
  void vscode.window.showInformationMessage("NightHawk: Session exported.", "Open File").then((action) => {
...
function defaultExportName(sessionId: string, now: Date): string {
  ...
  return `nighthawk-export-${sessionId.slice(0, 8)}-${timestamp}.md`;
}
