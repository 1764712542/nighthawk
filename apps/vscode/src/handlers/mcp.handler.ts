...
export const mcpHandlers: Record<string, Handler<any, any>> = {
    ...
    await vscode.window.withProgress(
      {
        ...
        title: `NightHawk: Authenticating "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          ...
          await vscode.window.showInformationMessage(`NightHawk: OAuth completed for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`NightHawk: OAuth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.ResetAuthMCP]: async ({ name }: NameParams, ctx) => {
    await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: `NightHawk: Resetting auth for "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          await ctx.harness.resetMcpServerAuth(name);
          await vscode.window.showInformationMessage(`NightHawk: Auth reset for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`NightHawk: Reset auth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.TestMCP]: async ({ name }: NameParams, ctx): Promise<MCPTestResult> => {
    void vscode.window.showInformationMessage(`NightHawk: Testing MCP server "${name}"...`);
