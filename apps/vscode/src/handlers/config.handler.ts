...
import {
  effectiveModelAlias,
  type NighthawkConfig as SdkNighthawkConfig,
  type ModelAlias,
  type ThinkingEffort,
...
import type {
  NighthawkConfig as WebviewNighthawkConfig,
  ModelConfig,
  SlashCommandInfo,
...
const openSettings: Handler<void, { ok: boolean }> = async () => {
  await vscode.commands.executeCommand("workbench.action.openSettings", "nighthawk");
  return { ok: true };
};

const getModels: Handler<void, WebviewNighthawkConfig> = async (_, ctx) => {
  const config = await ctx.harness.getConfig({ reload: true });
...
export function toWebviewConfig(config: SdkNighthawkConfig): WebviewNighthawkConfig {
