...
export interface NighthawkRuntimeOptions {
...
export class NighthawkRuntime {
  readonly harness: NighthawkHarness;

  private readonly broadcast: RuntimeBroadcast;
  private readonly captureBaseline: NighthawkRuntimeOptions["captureBaseline"];
  private readonly log: NighthawkRuntimeOptions["log"];
  private readonly sessions = new Map<string, SessionRuntime>();
  private readonly sessionByView = new Map<string, string>();
  private closed = false;

  constructor(options: NighthawkRuntimeOptions) {
    this.broadcast = options.broadcast;
    this.captureBaseline = options.captureBaseline;
    this.log = options.log;
    const createHarness = options.useAgentCoreV1 ? createNighthawkHarness : createNighthawkHarnessV2;
    this.harness =
      options.harness ??
      createHarness({
  ...
  private ensureOpen(): void {
    if (this.closed) throw new Error("NightHawk runtime is closed.");
  }
}
