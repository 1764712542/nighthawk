...
const REQUIRED_WEBVIEW_FILES = [
  'dist/webview.js',
  'dist/nighthawk-banner-dark.svg',
  'dist/nighthawk-banner-light.svg',
  'dist/nighthawk-logo.png',
];
...
export async function verifyVsix(vsixPath, target, options = {}) {
  const extractionRoot = await mkdtemp(join(tmpdir(), 'nighthawk-vsix-audit-'));
  try {
...
function verifyForbiddenFiles(files) {
  for (const file of files) {
    ...
    if (
      lower.includes('nighthawk-agent-sdk') ||
      lower.includes('download-cli') ||
      lower.includes('/bin/nighthawk/') ||
      /(^|\/)uv(?:\.exe)?$/.test(lower)
    ) {
