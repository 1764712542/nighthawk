...
const defaultCachePath = join(tmpdir(), "nighthawk-vscode-test-cache");
...
export async function runExtensionHostSmoke(options = {}) {
  ...
  try {
    ...
    await writeFile(join(paths.workspace, "README.md"), "# NightHawk VSIX Extension Host smoke\n", "utf8");
    ...
    await runTests({
      ...
      extensionTestsEnv: {
        NIGHTHAWK_HOME: paths.nighthawkHome,
        NIGHTHAWK_VSCODE_SMOKE_OS_HOME: paths.osHome,
        NIGHTHAWK_VSCODE_SMOKE_REPORT: paths.report,
        NIGHTHAWK_VSCODE_SMOKE_VSIX: basename(vsixPath),
      },
    });
...
function defaultVsixPath() {
  ...
  return join(appDir, "artifacts", "vsix", `nighthawk-${process.platform}-${arch}.vsix`);
}
...
async function writeHarnessManifest(directory) {
  await writeFile(
    ...
    JSON.stringify({
      name: "nighthawk-vscode-extension-host-smoke",
      displayName: "NightHawk VSCode Extension Host Smoke",
