...
class Bridge {
  ...
  constructor() {
    ...
    if (typeof acquireVsCodeApi === "function") {
      ...
      console.warn("[NightHawk Bridge] Running outside VS Code, using mock");
      this.vscode = {
        postMessage: (msg) => console.log("[NightHawk Mock]", msg),
        getState: () => undefined,
        setState: () => {},
      };
    }
  ...
  getModels() {
    return this.call<NighthawkConfig>(Methods.GetModels);
  }
  ...
  getNighthawkSessions() {
    return this.call<SessionInfo[]>(Methods.GetNighthawkSessions);
  }

  getAllNighthawkSessions() {
    return this.call<SessionInfo[]>(Methods.GetAllNighthawkSessions);
  }
  ...
  loadSessionHistory(sessionId: string) {
    return this.call<UIStreamEvent[]>(Methods.LoadNighthawkSessionHistory, { nighthawkSessionId: sessionId });
  }

  deleteSession(sessionId: string) {
    return this.call<{ ok: boolean }>(Methods.DeleteNighthawkSession, { sessionId });
  }

  forkSession(sessionId: string, turnIndex: number) {
    return this.call<{ sessionId: string } | null>(Methods.ForkNighthawkSession, { sessionId, turnIndex });
  }
