...
export function SessionList({ onClose }: SessionListProps) {
  ...
  const { data: nighthawkSessions = [], loading, mutate } = useRequest(() => bridge.getAllNighthawkSessions());
  ...
  const filteredSessions = useMemo(() => {
    if (!searchQuery.trim()) return nighthawkSessions;
    const q = searchQuery.toLowerCase();
    return nighthawkSessions.filter((s) => s.brief.toLowerCase().includes(q));
  }, [nighthawkSessions, searchQuery]);
