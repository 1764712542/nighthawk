...
const EXTENSION_ID = "nighthawk.nighthawk";
...
exports.run = async function run() {
  const isolatedHome = process.env.NIGHTHAWK_VSCODE_SMOKE_OS_HOME;
  ...
  assert.ok(process.env.NIGHTHAWK_HOME, "NIGHTHAWK_HOME must point at the isolated test home");
  ...
  const config = vscode.workspace.getConfiguration("nighthawk");
  ...
  await vscode.commands.executeCommand("nighthawk.openInTab");
  await waitFor(() => {
    return vscode.window.tabGroups.all.some((group) =>
      group.tabs.some((tab) =>
        tab.input instanceof vscode.TabInputWebview && isNighthawkPanelViewType(tab.input.viewType)));
  }, 5_000, () => `NightHawk Webview tab did not open; tabs=${describeTabs()}`);

  await vscode.commands.executeCommand("nighthawk.showLogs");
  await vscode.commands.executeCommand("nighthawk.reset");
  ...
  assert.ok(process.env.NIGHTHAWK_VSCODE_SMOKE_REPORT, "Extension Host report path must be provided");
  await writeFile(
    process.env.NIGHTHAWK_VSCODE_SMOKE_REPORT,
