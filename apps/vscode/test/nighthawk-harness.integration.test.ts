/**
 * Scenario: the VS Code host and another Node SDK client share one in-process NightHawk home.
 ...
 * Wiring: NighthawkRuntime, NighthawkHarness, core, storage, and HTTP provider adapter are real; only the remote provider is local.
 * The runtime harness follows the extension engine decision (v2 by default, the legacy v1 under NIGHTHAWK_LEGACY_FLAG).
 * Run: pnpm --filter nighthawk exec vitest run test/nighthawk-harness.integration.test.ts
 */
...
import { NighthawkRuntime } from "../src/runtime/nighthawk-runtime";
...
interface RuntimeRig {
  ...
  readonly runtime: NighthawkRuntime;
...
async function createRuntimeRig(extraAliases: readonly string[] = []): Promise<RuntimeRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "nighthawk-vscode-harness-"));
  ...
  const runtime = new NighthawkRuntime({
    version,
    homeDir,
    // The dual-engine CI matrix reruns this suite with NIGHTHAWK_LEGACY_FLAG=1;
...
async function createPlainHarness(homeDir: string): Promise<NighthawkHarness> {
  const harness = createNighthawkHarness({
    homeDir,
    identity: { productName: "nighthawk-cli", version: "test", platform: "nighthawk_cli" },
  });
...
async function createMcpHandlerRig(): Promise<McpHandlerRig> {
  const homeDir = await mkdtemp(join(tmpdir(), "nighthawk-vscode-mcp-handler-"));
...
describe("VS Code NightHawk harness integration (shares one in-process SDK home)", () => {
  ...
  it("keeps project-layer servers in the list refreshed after every mutation", async () => {
    const rig = await createMcpHandlerRig();
    const project = await mkdtemp(join(tmpdir(), "nighthawk-vscode-mcp-project-"));
  ...
  it("exports current context as Markdown under the workspace", async () => {
    ...
    expect(markdown).toContain("# NightHawk Session Export");
