 ...
 * Run: pnpm --filter nighthawk exec vitest run --config vitest.config.ts test/vsix-package.test.ts
 */
...
describe('VSIX package CLI (target planning and validation)', () => {
  it('plans all six supported targets when no target is supplied', async () => {
    const outputDir = await makeTempDir('nighthawk-package-plan-');
    ...
    expect(result.stdout).toContain('nighthawk-darwin-x64.vsix');
    expect(result.stdout).toContain('nighthawk-darwin-arm64.vsix');
    expect(result.stdout).toContain('nighthawk-linux-x64.vsix');
    expect(result.stdout).toContain('nighthawk-linux-arm64.vsix');
    expect(result.stdout).toContain('nighthawk-win32-x64.vsix');
    expect(result.stdout).toContain('nighthawk-win32-arm64.vsix');
  });

  it('accepts a Windows ARM target when the output path contains spaces', async () => {
    const root = await makeTempDir('nighthawk-package-windows-');
...
describe('Extension Development Host setup (isolated local state)', () => {
  it('creates the complete isolated directory layout for a debug launch', async () => {
    ...
    await expect(readFile(join(baseDir, 'workspace', 'README.md'), 'utf8')).resolves.toContain(
      'Isolated NightHawk extension development workspace',
    );
    ...
    await expect(directoryExists(join(baseDir, 'nighthawk-home'))).resolves.toBe(true);
  });
...
async function makeVsixFixture(target: string): Promise<string> {
  const root = await makeTempDir('nighthawk-vsix-fixture-');
  ...
  await Promise.all([
    ...
    writeFile(join(distDir, 'webview.js'), 'globalThis.__nighthawkWebview = true;\n'),
    writeFile(join(distDir, 'nighthawk-banner-dark.svg'), '<svg />'),
    writeFile(join(distDir, 'nighthawk-banner-light.svg'), '<svg />'),
    writeFile(join(distDir, 'nighthawk-logo.png'), 'fixture'),
    writeFile(join(resourcesDir, 'nighthawk-icon-storefront.png'), 'fixture'),
    writeFile(join(resourcesDir, 'nighthawk-icon.svg'), '<svg />'),
  ]);
  return root;
}
