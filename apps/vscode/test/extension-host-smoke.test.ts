 ...
 * Host cannot discover the developer's real legacy NightHawk home.
 * Wiring: real smoke orchestration and filesystem; @vscode/test-electron is the
 * external process/download boundary.
 * Run: pnpm --filter nighthawk exec vitest run --config vitest.config.ts test/extension-host-smoke.test.ts
 */
...
beforeEach(() => {
  ...
  vscodeTest.runTests.mockImplementation(async (options) => {
    await writeFile(
      options.extensionTestsEnv.NIGHTHAWK_VSCODE_SMOKE_REPORT,
...
describe('installed VSIX Extension Host smoke', () => {
  ...
  it('gives the harness separate NightHawk and operating-system homes before activation', async () => {
    const fixture = await makeFixture();
    ...
    expect(env.NIGHTHAWK_HOME).not.toBe(env.NIGHTHAWK_VSCODE_SMOKE_OS_HOME);
    expect(env.NIGHTHAWK_VSCODE_SMOKE_OS_HOME).toContain('os-home');
  });
...
async function makeFixture(): Promise<{ vsixPath: string; cachePath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'nighthawk-extension-host-smoke-'));
  tempDirs.push(root);
  const cachePath = join(root, 'cache');
  const vsixPath = join(root, 'nighthawk-test.vsix');
