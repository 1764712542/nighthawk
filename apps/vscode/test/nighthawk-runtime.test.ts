/**
 * Scenario: the VS Code host owns one NightHawk harness and routes Webviews to shared SDK sessions.
 * Responsibilities: create/resume/switch, per-session settings, multi-view ownership, detach, and disposal.
 * Wiring: NighthawkRuntime and SessionRuntime are real; in-memory Session/NighthawkHarness fakes form the public SDK boundary.
 * Run: pnpm exec vitest run --config apps/vscode/vitest.config.ts test/nighthawk-runtime.test.ts
 */
...
import { NighthawkRuntime, type OpenSessionOptions } from "../src/runtime/nighthawk-runtime";

const sdkFactories = vi.hoisted(() => {
  const v1Harness = { homeDir: "/tmp/nighthawk-runtime-v1-home", close: vi.fn(async () => undefined) };
  const v2Harness = { homeDir: "/tmp/nighthawk-runtime-v2-home", close: vi.fn(async () => undefined) };
  return {
    v1Harness,
    v2Harness,
    createNighthawkHarness: vi.fn(() => v1Harness),
    createNighthawkHarnessV2: vi.fn(() => v2Harness),
  };
});

vi.mock("@nighthawk/nighthawk-sdk", async (importOriginal) => {
  const original = await importOriginal<typeof import("@nighthawk/nighthawk-sdk")>();
  return {
    ...original,
    createNighthawkHarness: sdkFactories.createNighthawkHarness,
    createNighthawkHarnessV2: sdkFactories.createNighthawkHarnessV2,
  };
});
...
function createFakeSession(
  ...
  let status: SessionStatus = {
    model: initial.model ?? "nighthawk-test",
...
function createRuntime(
  ...
  const runtime = new NighthawkRuntime({
    version: "0.6.0",
    harness: sdk.harness,
    broadcast: () => undefined,
...
describe("NightHawk runtime (owns shared SDK sessions for Webviews)", () => {
  it("creates the v2 harness by default and the v1 harness for rollback", async () => {
    const defaults = new NighthawkRuntime({
      version: "0.6.0",
      broadcast: () => undefined,
      captureBaseline: () => undefined,
      log: () => undefined,
    });
    expect(sdkFactories.createNighthawkHarnessV2).toHaveBeenCalledOnce();
    expect(sdkFactories.createNighthawkHarnessV2).toHaveBeenCalledWith({
      homeDir: undefined,
      identity: {
        productName: "nighthawk-vscode",
        version: "0.6.0",
        platform: "nighthawk_vscode",
      },
      uiMode: "vscode",
    });
    expect(sdkFactories.createNighthawkHarness).not.toHaveBeenCalled();
    expect(defaults.harness).toBe(sdkFactories.v2Harness as unknown as NighthawkHarness);
    await defaults.dispose();

    const rollback = new NighthawkRuntime({
      version: "0.6.0",
      useAgentCoreV1: true,
      broadcast: () => undefined,
      captureBaseline: () => undefined,
    ...
    expect(sdkFactories.createNighthawkHarness).toHaveBeenCalledOnce();
    expect(rollback.harness).toBe(sdkFactories.v1Harness as unknown as NighthawkHarness);
  ...
  it("announces the session's actual status to the attaching view so the display matches it", async () => {
    ...
    expect(broadcasts).toContainEqual({
      ...
      data: {
        type: "StatusUpdate",
        payload: { model: "nighthawk-test", thinking_effort: "max", plan_mode: true },
        _sessionId: "saved-1",
      },
      webviewId: "view-1",
    });
