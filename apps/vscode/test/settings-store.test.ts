...
const MODELS = [
  { id: "plain", name: "Plain", provider: "managed:nighthawk", capabilities: [] },
  {
    id: "reasoning",
    name: "Reasoning",
  ...
  { id: "always", name: "Always", provider: "managed:nighthawk", capabilities: ["always_thinking"] },
];
...
describe("Webview model metadata", () => {
  it("keeps same-named models in separate provider groups", () => {
    const groups = groupModelsByProvider([
      { id: "nighthawk/shared", name: "Shared", provider: "managed:nighthawk", capabilities: [] },
      { id: "proxy/shared", name: "Shared", provider: "company-proxy", capabilities: [] },
    ]);
...
describe("Webview thinking mode parity with the TUI", () => {
  it("derives thinking modes from metadata only, mirroring the TUI rules", () => {
    ...
    expect(getModelThinkingMode({ ...base, name: "NightHawk Thinking Pro" })).toBe("none");
