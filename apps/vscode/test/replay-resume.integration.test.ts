 ...
 * Run: pnpm --filter nighthawk exec vitest run --config vitest.config.ts test/replay-resume.integration.test.ts
 */
...
async function createReplayRig(): Promise<ReplayRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "nighthawk-vscode-replay-"));
  ...
  const harness = createNighthawkHarness({
    homeDir,
    identity: { productName: "nighthawk-vscode", version: "test", platform: "nighthawk_vscode" },
  });
