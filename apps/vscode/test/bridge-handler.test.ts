 ...
 * Run: pnpm --filter nighthawk exec vitest run --config vitest.config.ts test/bridge-handler.test.ts
 */
...
const host = vi.hoisted(() => {
  ...
  return {
    ...
    watcher,
    harness,
    createNighthawkHarness: vi.fn(() => harness),
    createNighthawkHarnessV2: vi.fn(() => harness),
    showWarningMessage,
...
vi.mock("@nighthawk/nighthawk-sdk", async (importOriginal) => {
  ...
  return {
    ...original,
    createNighthawkHarness: () => host.createNighthawkHarness(),
    createNighthawkHarnessV2: () => host.createNighthawkHarnessV2(),
  };
});
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), "nighthawk-vscode-bridge-"));
  ...
  host.createNighthawkHarness.mockImplementation(() => host.harness);
  host.createNighthawkHarnessV2.mockImplementation(() => host.harness);
...
describe("Engine startup", () => {
  ...
  it("reports the rollback setting when the default engine cannot start", () => {
    ...
    host.createNighthawkHarnessV2.mockImplementationOnce(() => {
      throw new Error("engine boom");
    });

    expect(constructBridge).toThrow(
      /Failed to start the NightHawk engine: engine boom\..*nighthawk\.useAgentCoreV1/s,
    );
  });

  it("reports no rollback hint when the legacy engine itself cannot start", () => {
    vi.stubEnv("NIGHTHAWK_LEGACY_FLAG", "1");
    host.createNighthawkHarness.mockImplementationOnce(() => {
      throw new Error("legacy boom");
    });

    expect(constructBridge).toThrow(/^Failed to start the NightHawk engine: legacy boom\.$/);
  });
});
...
describe("Webview RPC boundary (validates requests before host dispatch)", () => {
  ...
  it("does not expose the session storage path when listing sessions", async () => {
    host.harness.listSessions.mockResolvedValueOnce([
      {
        id: "session-1",
        workDir: root,
        sessionDir: "/private/nighthawk/sessions/session-1",
        updatedAt: 123,
        title: "Visible title",
      },
    ] as never);
    ...
    const result = await bridge.handle(
      { id: "rpc-1", method: Methods.GetNighthawkSessions },
      "view-1",
    );
    ...
    expect(JSON.stringify(result)).not.toContain("/private/nighthawk/sessions");
  });
  ...
  it("does not expose the session storage path when forking a session", async () => {
    ...
    const target = {
      id: "session-2",
      workDir: root,
      sessionDir: "/private/nighthawk/sessions/session-2",
      updatedAt: 124,
    };
    ...
    const result = await bridge.handle(
      {
        id: "rpc-1",
        method: Methods.ForkNighthawkSession,
  ...
  it("keeps conversation history available when its baseline snapshot disappears", async () => {
    ...
    const result = await bridge.handle(
      {
        id: "rpc-1",
        method: Methods.LoadNighthawkSessionHistory,
        params: { nighthawkSessionId: "session-1" },
      },
      "view-1",
    );
...
describe("Webview config saves (thinking effort persistence parity with the TUI)", () => {
  ...
  function mockConfig(thinking?: { enabled: boolean; effort?: string }) {
    host.harness.getConfig.mockResolvedValue({
      defaultModel: "nighthawk/reasoning",
      thinking,
      models: { "nighthawk/reasoning": effortModel },
    } as never);
  }
  ...
  it("persists a non-top effort as the global default", async () => {
    ...
    const result = await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "nighthawk/reasoning", thinking: true, effort: "high" } },
      "view-1",
    );
  ...
  it("keeps the model's top declared tier session-only", async () => {
    ...
    await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "nighthawk/reasoning", thinking: true, effort: "max" } },
      "view-1",
    );
  ...
  it("leaves the stored effort alone when the pick re-confirms the active effort", async () => {
    ...
    await bridge.handle(
      { id: "rpc-1", method: Methods.SaveConfig, params: { model: "nighthawk/reasoning", thinking: true, effort: "high", effortChanged: false } },
      "view-1",
    );
...
function createResumedSession(id: string, workDir: string) {
  ...
  const summary = {
    id,
    workDir,
    sessionDir: join("/private/nighthawk/sessions", id),
    createdAt: 1,
    updatedAt: 2,
