...
window into a running NightHawk engine (workspaces, sessions, agents, and the
scoped DI registry).
...
1. Start a kap-server with the debug surface mounted (repo dev scripts do this
   ...
   bearer auth.
2. `pnpm --filter @nighthawk/nighthawk-inspect dev` — the Vite dev server proxies
   `/api` to the server (`NIGHTHAWK_SERVER_URL`, default `http://127.0.0.1:58627`)
   and auto-discovers running instances
   (`~/.nighthawk/server/instances`); switch servers from the header dropdown.
