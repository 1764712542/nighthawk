 ...
 * `nighthawk.bearer.<token>` subprotocol (the only credential channel a
...
const WS_BEARER_PROTOCOL_PREFIX = 'nighthawk.bearer.';
...
export class TranscriptWs {
  ...
  private connect(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      this.helloId = `nighthawk-inspect-${Date.now().toString(36)}`;
