...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'nighthawk-inspect-discovery-'));
});
...
describe('resolveNighthawkHomeDir', () => {
  it('honors NIGHTHAWK_HOME, else falls back to ~/.nighthawk', () => {
    expect(resolveNighthawkHomeDir({ NIGHTHAWK_HOME: '/tmp/kh' })).toBe('/tmp/kh');
    expect(resolveNighthawkHomeDir({})).toBe(join(process.env['HOME'] ?? '', '.nighthawk'));
  });
});
