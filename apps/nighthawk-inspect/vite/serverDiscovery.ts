 ...
 *   current builds  `<nighthawk home>/server/instances/<serverId>.json`
 *   pre-registry builds  `<nighthawk home>/server/lock`
 * and persists the bearer token at `<nighthawk home>/server.token` (one token per
 ...
 * `~/.nighthawk/server.token` by hand — no new exposure beyond the local dev
 * session.
 */
...
/** home resolution per request: `NIGHTHAWK_HOME` env, else `~/.nighthawk`. */
export function resolveNighthawkHomeDir(env: NodeJS.ProcessEnv = process.env): string {
  const fromEnv = env['NIGHTHAWK_HOME'];
...
export interface DiscoverOptions {
  /** The dev-proxy target (`NIGHTHAWK_SERVER_URL`); merged as a `proxy` entry. */
...
export async function discoverLocalServers(
  ...
  const home = options.homeDir ?? resolveNighthawkHomeDir();
...
export function serverDiscoveryPlugin(options: { proxyTarget: string }): Plugin {
  ...
  return {
    name: 'nighthawk-inspect-server-discovery',
