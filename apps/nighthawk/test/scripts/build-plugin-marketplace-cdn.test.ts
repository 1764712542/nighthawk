...
describe('buildPluginMarketplaceCdn', () => {
  it('packages WebBridge without publishing other unlisted official directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nighthawk-plugin-cdn-build-'));
    tempRoots.push(root);
    const pluginsRoot = join(root, 'plugins');
    const outDir = join(root, 'out');

    await writePlugin(pluginsRoot, 'listed-plugin');
    await writePlugin(pluginsRoot, 'nighthawk-webbridge');
    ...
    await expect(access(join(outDir, 'official/nighthawk-webbridge.zip'))).resolves.toBeUndefined();
