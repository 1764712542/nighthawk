...
const executableName = process.platform === 'win32' ? 'nighthawk.exe' : 'nighthawk';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `nighthawk-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `nighthawk-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, NIGHTHAWK_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `nighthawk-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  nighthawk-${target}.zip\n`,
    );
  });

  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'nighthawk-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'nighthawk-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'nighthawk-darwin-arm64.zip.sha256'),
      `${checksum}  nighthawk-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@nighthawk/nighthawk@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: '@nighthawk/nighthawk@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'nighthawk-darwin-arm64.zip',
          checksum,
        },
      },
    });
