...
describe('targetTriple', () => {
  ...
  it('honors NIGHTHAWK_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { NIGHTHAWK_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns nighthawk.exe on win32', () => {
    expect(executableName('win32')).toBe('nighthawk.exe');
  });

  it('returns nighthawk on other platforms', () => {
    expect(executableName('darwin')).toBe('nighthawk');
    expect(executableName('linux')).toBe('nighthawk');
  });
});
...
describe('path helpers', () => {
  ...
  it('returns absolute bin path with executable name', () => {
    expect(nativeBinPath('darwin-arm64', 'darwin')).toBe(
      p('dist-native/bin/darwin-arm64/nighthawk'),
    );
    expect(nativeBinPath('win32-x64', 'win32')).toBe(
      p('dist-native/bin/win32-x64/nighthawk.exe'),
    );
  });

  it('returns intermediate artifact paths', () => {
    expect(nativeJsBundlePath()).toBe(p('dist-native/intermediates/main.cjs'));
    expect(nativeBlobPath()).toBe(p('dist-native/intermediates/nighthawk.blob'));
