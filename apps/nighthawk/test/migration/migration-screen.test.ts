...
function makePlan(over: Partial<MigrationPlan> = {}): MigrationPlan {
  return {
    ...
    hasMcp: true,
    hasUserHistory: true,
    oauthCredentials: ['nighthawk.json'],
    workdirs: [],
    detectedPlugins: [],
...
describe('MigrationScreenComponent — ask phase', () => {
  it('ask1 renders the intro block and three options', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.nighthawk',
      targetHome: '/y/.nighthawk',
      onComplete: () => {},
    });
    const out = render(c);
    expect(out).toContain('Migrate from the legacy CLI');
  ...
  it('ask1 summary does not mention legacy-cli login (oauth is not a migrated kind)', async () => {
    // OAuth credentials are deliberately never migrated, so the pre-migration
    // summary must not list "legacy-cli login" alongside the real migratable
    ...
    const out = render(c);
    expect(out).not.toContain('legacy-cli login');
...
describe('MigrationScreenComponent — progress phase', () => {
  it('renders a step checklist and the session counter when in progress', () => {
    ...
    expect(out).toContain('Migrating from the legacy CLI');
...
describe('MigrationScreenComponent — result phase', () => {
  it('renders the report summary including plugin notices', () => {
    ...
    expect(out).toContain('2 legacy CLI plugins');
  });
  ...
  it('surfaces conflict and failure warnings on the result screen', () => {
    ...
    const out = c.render(80).join('\n');
    expect(out).toContain('config.migrated-from-legacy-cli.toml');
    expect(out).toContain('mcp.migrated-from-legacy-cli.json');
  ...
  it('lists sibling-file contents in the config-fallback warning so the user knows what to merge', () => {
    ...
    // to `config.migrated-from-legacy-cli.toml` instead, the result screen must
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedHooks: 0,
            siblingContents: {
              providers: ['openai', 'managed:nighthawk'],
              models: ['gpt4'],
              hooks: 3,
            },
          },
  ...
  it('lists kept config settings on the result screen when legacy-cli differed', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.nighthawk',
      targetHome: '/y/.nighthawk',
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedModels: [],
            droppedKeys: [],
            configConflicts: ['default_model', 'providers.nighthawk'],
            wroteSiblingDueToConflict: false,
            wroteTuiSibling: false,
            migratedHooks: 0,
            droppedHooks: 0,
    ...
    expect(out).toContain('default_model · providers.nighthawk');
  });
