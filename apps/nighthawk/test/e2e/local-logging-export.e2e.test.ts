...
const ENABLED = process.env['NIGHTHAWK_E2E'] === '1';
...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'legacy-cli-log-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'legacy-cli-log-work-'));
  oldHome = process.env['NIGHTHAWK_HOME'];
  oldLogLevel = process.env['NIGHTHAWK_LOG_LEVEL'];
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['NIGHTHAWK_HOME'] = oldHome;
  }
  if (oldLogLevel === undefined) {
    delete process.env['NIGHTHAWK_LOG_LEVEL'];
  } else {
    process.env['NIGHTHAWK_LOG_LEVEL'] = oldLogLevel;
  }
...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    const harness = createNighthawkHarness({
      homeDir,
      identity: createNighthawkHostIdentity('0.1.1'),
    });
    try {
      ...
      await runNighthawkExport([session.id, '-o', defaultZip]);
      ...
      await runNighthawkExport([session.id, '-o', noGlobalZip, '--no-include-global-log']);
...
async function runNighthawkExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`nighthawk export exited ${code}: ${stderr.join('')}`);
    },
  });
  await program.parseAsync(['node', 'nighthawk', 'export', ...args]);
}
