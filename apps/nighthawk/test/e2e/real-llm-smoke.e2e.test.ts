/**
 * Real-LLM smoke. Opt-in only — runs when `NIGHTHAWK_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `NighthawkHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   NIGHTHAWK_E2E_REAL     — set to "1" to enable this suite
 *   NIGHTHAWK_E2E_MODEL    — model alias override (default: config's default)
 *   NIGHTHAWK_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   NIGHTHAWK_E2E_WORKDIR  — workspace directory (default: /tmp/nighthawk-e2e)
 */

import { mkdirSync } from 'node:fs';
import process from 'node:process';

import { createNighthawkHarness, type Event } from '@nighthawk/nighthawk-sdk';
import { describe, expect, test } from 'vitest';

import { createNighthawkHostIdentity, getVersion } from '#/cli/version';

const DEFAULT_PROMPT = 'Reply with a single word: hi';
const DEFAULT_WORKDIR = '/tmp/nighthawk-e2e';
const TURN_TIMEOUT_MS = 60_000;

const ENABLED = process.env['NIGHTHAWK_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through NighthawkHarness',
    async () => {
      const workDir = process.env['NIGHTHAWK_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['NIGHTHAWK_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['NIGHTHAWK_E2E_MODEL'];
      ...
      const harness = createNighthawkHarness({
        identity: createNighthawkHostIdentity(version),
      });
