...
describe('loadPluginMarketplace', () => {
  it('loads a local marketplace file and resolves relative plugin sources', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nighthawk-plugin-marketplace-'));
    ...
    expect(marketplace.plugins.slice(0, 2)).toEqual([
      {
        ...
        version: '1.0.0',
        description: 'Datasource tools',
        source: join(dir, 'nighthawk-datasource'),
        keywords: ['data'],
        homepage: undefined,
      },
      {
  ...
  it('masks same-id catalog rows with the built-in entries', async () => {
    ...
    const webbridge = marketplace.plugins.filter((entry) => entry.id === 'nighthawk-webbridge');
    expect(webbridge).toHaveLength(1);
    expect(webbridge[0]?.source).toBe('capability:nighthawk-webbridge');
    expect(webbridge[0]?.version).toBe('1.12.0');
    expect(marketplace.plugins.some((entry) => entry.id === 'nighthawk-cu')).toBe(true);
  });
  ...
  it('includes Superpowers in the repository marketplace fixture', async () => {
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'nighthawk-datasource',
        tier: 'official',
        source: join(REPO_ROOT, 'plugins/official/nighthawk-datasource'),
      }),
    );
  });
  ...
  it('loads the default CDN marketplace with injectable fetch', async () => {
    ...
    const marketplace = await loadPluginMarketplace({
      workDir: '/tmp/work',
      source: nighthawkPluginMarketplaceUrl(),
      fetchImpl,
    });

    expect(fetchImpl).toHaveBeenCalledWith(nighthawkPluginMarketplaceUrl());
    expect(marketplace.plugins[0]).toEqual(
      expect.objectContaining({
        id: 'nighthawk-datasource',
        displayName: 'NightHawk Datasource',
        source: new URL(
          './official/nighthawk-datasource.zip',
          nighthawkPluginMarketplaceUrl(),
        ).toString(),
      }),
    );
  ...
  it('falls back to the source checkout marketplace when the default CDN cannot be fetched', async () => {
    const previous = process.env[NIGHTHAWK_PLUGIN_MARKETPLACE_URL_ENV];
    delete process.env[NIGHTHAWK_PLUGIN_MARKETPLACE_URL_ENV];
    const fetchImpl = vi.fn(async () => {
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env[NIGHTHAWK_PLUGIN_MARKETPLACE_URL_ENV] = previous;
      }
    }
  });
  ...
  it('keeps the built-in entries when the catalog is unreachable', async () => {
    ...
    expect(marketplace.plugins.map((entry) => entry.id)).toEqual(['nighthawk-cu', 'nighthawk-webbridge']);
  });
  ...
  describe('version derivation from a GitHub source', () => {
    ...
    it('does not derive a version from a non-GitHub URL', async () => {
      const entry = await loadEntry('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/curated/superpowers.zip');
      expect(entry.version).toBeUndefined();
    });
  ...
  it('accepts legacy marketplace type aliases as normal plugins', async () => {
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'nighthawk-webbridge',
        source: join(dir, 'nighthawk-webbridge'),
      }),
    );
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
