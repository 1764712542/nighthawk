...
import { resolveNighthawkOAuthKey } from '@nighthawk/nighthawk-oauth';
import { describe, expect, it } from 'vitest';

const REPO_ROOT = join(import.meta.dirname, '../../../..');
const SERVER_ENTRY = join(REPO_ROOT, 'plugins/official/nighthawk-datasource/bin/nighthawk-datasource.mjs');

describe('nighthawk-datasource MCP server', () => {
  it('exposes the same two generic tools as the Python plugin', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'nighthawk-datasource-plugin-'));
    const nighthawkHome = join(tempDir, 'nighthawk-home');
    let child: ChildProcessWithoutNullStreams | undefined;

    try {
      await mkdir(join(nighthawkHome, 'credentials'), { recursive: true });
      await writeFile(
        join(nighthawkHome, 'credentials', 'nighthawk.json'),
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          NIGHTHAWK_HOME: nighthawkHome,
          NIGHTHAWK_DATASOURCE_API_URL: `http://127.0.0.1:${address.port}`,
        },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('uses env-scoped credentials and derives the datasource URL from NIGHTHAWK_BASE_URL', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'nighthawk-datasource-plugin-'));
    const nighthawkHome = join(tempDir, 'nighthawk-home');
    const requests: unknown[] = [];
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      const scopedCredential = nighthawkEnvCredentialName({ oauthHost, baseUrl });
      ...
      await writeFile(
        join(nighthawkHome, 'credentials', `${scopedCredential}.json`),
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          NIGHTHAWK_HOME: nighthawkHome,
          NIGHTHAWK_BASE_URL: baseUrl,
          NIGHTHAWK_OAUTH_HOST: oauthHost,
          NIGHTHAWK_DATASOURCE_API_URL: undefined,
        },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('retries with a rotated credential when the backend rejects the previous token', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'nighthawk-datasource-plugin-'));
    const nighthawkHome = join(tempDir, 'nighthawk-home');
    const credentialsFile = join(nighthawkHome, 'credentials', 'nighthawk.json');
  ...
  it('returns the complete data-source routing contract when tools are listed', async () => {
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        cwd: REPO_ROOT,
        env: { ...process.env, NIGHTHAWK_HOME: nighthawkHome },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
  ...
  it('appends a request-id / tool-call-id trace line to tool results', async () => {
    ...
    try {
      ...
      expect(text).toContain('[nighthawk-datasource] request-id: backend-req-test · tool-call-id:');
    } finally {
      child?.stdin.end();
      child?.kill();
      await closeServer(server);
...
// truth in @nighthawk/nighthawk-oauth. The credential file name is the OAuth
// key with its `oauth/` prefix stripped.
function nighthawkEnvCredentialName(options: {
  readonly oauthHost: string;
  readonly baseUrl: string;
}): string {
  return resolveNighthawkOAuthKey(options).replace(/^oauth\//, '');
}
