 ...
 * `nighthawk connect`
 *
 * Verifies that the connect sub-command (alias `login`) is registered on the
 * program and that the action drives `harness.auth.login`, prints the device
 * code to stderr, and exits with the right code on success / failure.
 */
...
vi.mock('@nighthawk/nighthawk-sdk', async () => {
  const actual = await vi.importActual<typeof import('@nighthawk/nighthawk-sdk')>(
    '@nighthawk/nighthawk-sdk',
  );
  return {
    ...actual,
    createNighthawkHarness: vi.fn(() => ({
      auth: {
        login: mockLogin,
      },
    })),
...
describe('nighthawk connect', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(createNighthawkHarness).mockClear();
  ...
  it('registers a `connect` subcommand with a `login` alias on the program', () => {
    const program = new Command('nighthawk');
    registerLoginCommand(program);

    const connect = program.commands.find((c) => c.name() === 'connect');
    expect(connect).toBeDefined();
    expect(connect?.aliases()).toContain('login');
    expect(connect?.description()).toMatch(/[Cc]onnect/);
  });

  it('invokes harness.auth.login and exits 0 on success', async () => {
    mockLogin.mockResolvedValue({ providerName: 'nighthawk', ok: true });

    const program = new Command('nighthawk').exitOverride();
    registerLoginCommand(program);

    await expect(program.parseAsync(['node', 'nighthawk', 'login'])).rejects.toThrow(ExitCalled);
  ...
  it('prints device code prompt to stderr', async () => {
    mockLogin.mockImplementation(
      async (
        ...
        return { providerName: 'nighthawk', ok: true };
      },
    );
