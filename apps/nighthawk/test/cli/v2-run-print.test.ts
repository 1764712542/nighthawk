...
const mocks = vi.hoisted(() => ({
  ...
  createNighthawkDefaultHeaders: vi.fn(() => ({})),
  resolveNighthawkHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/nighthawk-test-home'),
  createNighthawkDeviceId: vi.fn(() => 'device-1'),
}));

vi.mock('@nighthawk/agent-core-v2', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@nighthawk/agent-core-v2')>();
  return {
    ...actual,
...
vi.mock('@nighthawk/nighthawk-oauth', async () => {
  ...
  return {
    ...actual,
    createNighthawkDefaultHeaders: mocks.createNighthawkDefaultHeaders,
...
describe('runV2Print', () => {
  ...
  it('binds the profile named by --agent-file when --agent is absent', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nighthawk-agent-file-'));
