...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      ...
      expect(process.title).toBe('nighthawk');
    } finally {
  ...
  it('initializes and flushes telemetry around the upgrade command', async () => {
    ...
    expect(mocks.createNighthawkHarness).toHaveBeenCalledWith(expect.objectContaining({
      homeDir: '/tmp/nighthawk-home',
      telemetry: {
        track: mocks.track,
  ...
  it('formats NightHawk startup errors with structured fields', () => {
    const error = new NighthawkError(
