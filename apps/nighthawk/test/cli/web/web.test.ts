/**
 * Tests for the `nighthawk web` Commander wiring and its subcommands.
 *
 ...
 * output, browser opening, and the rotate-token / deprecated `nighthawk server kill`
...
describe('nighthawk web', () => {
  ...
  it('routes `nighthawk server` and any legacy subcommand to a deprecation notice', async () => {
    for (const argv of [
      ['node', 'nighthawk', 'server'],
      ['node', 'nighthawk', 'server', 'run', '--port', '1'],
      ['node', 'nighthawk', 'server', 'status'],
      ['node', 'nighthawk', 'server', 'ps', '--json'],
    ]) {
      const program = makeProgram();
      let stderr = '';
      ...
      expect(stderr).toContain('`nighthawk server` has been deprecated and no longer works.');
      expect(stderr).toContain('nighthawk web');
      expect(stderr).toContain('nighthawk server kill');
...
describe('`nighthawk web` ready banner', () => {
  it('prints the TUI-style ready panel once listening', async () => {
    ...
    expect(plain).toContain('NightHawk server ready');
    ...
    // the NightHawk sprite stays next to the title.
    expect(plain).not.toContain('╭');
    expect(plain).not.toContain('╰');
    expect(plain).toContain('▄▟███▙▄');
    expect(plain).toContain('█▌◉ ◉▐█');
    expect(plain).toContain('▜█▙▼▟█▛');
    expect(plain).not.toContain('NightHawk server:');

    // Title is above the URLs; Logs/Stop are at the bottom.
    expect(plain.indexOf('NightHawk server ready')).toBeLessThan(plain.indexOf('Local:'));
  ...
  it('uses the TUI dark palette for the ready banner', async () => {
    ...
    expect(out).toContain(color.bold.hex(darkColors.primary)('NightHawk server ready'));
...
describe('ready banner reflects the bind class', () => {
  it('lists Local + Network addresses for a 0.0.0.0 bind (Vite-style)', async () => {
    ...
    expect(raw).toContain('NightHawk server ready');
...
describe('`nighthawk web` opens the browser', () => {
...
describe('`nighthawk web` option threading', () => {
  ...
  it('prints the one-line ready line instead of the full banner with a non-default --log-level', async () => {
    ...
    const plain = stripAnsi(readStdout());
    expect(plain).toContain('NightHawk server: http://127.0.0.1:58627/#token=tok');
    expect(plain).not.toContain('NightHawk server ready');
...
describe('server web asset directory resolution', () => {
  it('uses extracted SEA web assets when available', async () => {
    ...
    expect(resolveServerWebAssetsDir('/cache/nighthawk/dist-web')).toBe('/cache/nighthawk/dist-web');
  });
  ...
  it('returns the assets dir when it is built, dev mode or not', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-web-assets-'));
    try {
      writeFileSync(join(dir, 'index.html'), '<html></html>');
      expect(serverWebAssetsDir({}, dir)).toBe(dir);
      expect(serverWebAssetsDir({ NIGHTHAWK_DEV_SERVER: '1' }, dir)).toBe(dir);
    } finally {
  ...
  it('tolerates missing assets in dev mode (API-only server)', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-web-assets-'));
    try {
      expect(serverWebAssetsDir({ NIGHTHAWK_DEV_SERVER: '1' }, dir)).toBeUndefined();
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe('`nighthawk server kill` (deprecated, legacy servers only)', () => {
  ...
  it('prints "No running legacy NightHawk server." and sends no signal when no lock exists', async () => {
    const { handleLegacyKillCommand } = await import('#/cli/sub/web/legacy-kill');
    const { deps, writes, signals } = makeLegacyKillDeps({ readLock: async () => undefined });

    await handleLegacyKillCommand(deps);

    expect(writes.join('')).toContain('No running legacy NightHawk server.');
...
describe('readLegacyLock', () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'nighthawk-legacy-lock-'));
  });
  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
  });
...
describe('resolveServerToken', () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'nighthawk-server-token-'));
  });
  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
  });
...
describe('`nighthawk web rotate-token`', () => {
  let dir: string;
  let prevHome: string | undefined;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'nighthawk-rotate-'));
    prevHome = process.env['NIGHTHAWK_HOME'];
  ...
  afterEach(() => {
    if (prevHome === undefined) {
      ...
      process.env['NIGHTHAWK_HOME'] = prevHome;
    }
    rmSync(dir, { recursive: true, force: true });
  });
  ...
  it('writes a new token to server.token and prints it', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'web', 'rotate-token']);
