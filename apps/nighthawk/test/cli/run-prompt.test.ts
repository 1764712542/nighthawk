 ...
 * Run: pnpm -C apps/nighthawk exec vitest run test/cli/run-prompt.test.ts
 */
        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'nighthawk -r ses_prompt',
              content: 'To resume this session: nighthawk -r ses_prompt',
            })}\n`,
          );
          return;
        }
        stderr.write(`nighthawk version ${version}\n`);
        stdout.write('• hello world\n\n');
        stderr.write('To resume this session: nighthawk -r ses_prompt\n');
      },
    ),
...
describe('runPrompt', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('NIGHTHAWK_MODEL_OUTPUT_FORMAT', '');
  });
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: nighthawk -r ses_prompt\n');
  ...
  it('selects the profile declared by an explicit agent file for a fresh v1 session', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nighthawk-run-prompt-agent-'));
  ...
  it('uses the CLI model override when creating a fresh prompt session', async () => {
    await runPrompt(opts({ model: 'nighthawk/k2.5' }), '1.2.3-test', {
    ...
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'nighthawk/k2.5' }),
    );
  });
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: nighthawk -r ses_prompt\n',
    );
  ...
  it('allows resuming a concrete session when Windows workdir uses backslashes', async () => {
    const cwd = vi.spyOn(process, 'cwd').mockReturnValue(String.raw`C:\Users\nighthawk\project`);
    mocks.harnessListSessions.mockResolvedValueOnce([
      { id: 'ses_existing', workDir: 'C:/Users/nighthawk/project' },
    ]);
  ...
  it('applies the CLI model override to resumed prompt sessions', async () => {
    await runPrompt(opts({ session: 'ses_existing', model: 'nighthawk/k2.5' }), '1.2.3-test', {
      stdout: { write: vi.fn(() => true) },
      stderr: { write: vi.fn(() => true) },
    });

    expect(mocks.harnessResumeSession).toHaveBeenCalledWith({ id: 'ses_existing' });
    expect(mocks.session.setModel).toHaveBeenCalledWith('nighthawk/k2.5');
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'nighthawk/k2.5' }),
    );
  });
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"nighthawk -r ses_prompt","content":"To resume this session: nighthawk -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('throws when no default model is configured', async () => {
    ...
    await expect(
      ...
      'No model configured. Run `nighthawk` and use /connect to add a provider, then retry; or set default_model in config.toml.',
    );
  ...
  it('emits the version first in text mode on the default v2 engine', async () => {
    ...
    expect(stderr.write).toHaveBeenNthCalledWith(1, 'nighthawk version 1.2.3-test\n');
    expect(stderr.text().startsWith('nighthawk version 1.2.3-test\n')).toBe(true);
    expect(stdout.text()).toBe('• hello world\n\n');
  });
  ...
  it('uses the legacy engine when legacy wins over the experimental flag', async () => {
    ...
    expect(mocks.nighthawkHarnessConstructor).toHaveBeenCalled();
    expect(stderr.text()).not.toContain('nighthawk version');
  });
