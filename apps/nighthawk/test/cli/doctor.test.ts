...
beforeEach(async () => {
  ...
  dir = join(tmpdir(), `nighthawk-doctor-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('nighthawk doctor', () => {
  ...
  it('uses the legacy validator when legacy wins over the experimental flag', async () => {
    ...
    const text = '[providers.nighthawk]\ntype = "nighthawk"\n';
  ...
  it('treats a missing explicit target path as an error', async () => {
    ...
    expect(err).toContain('NightHawk doctor found 1 issue.');
  ...
  it('checks a valid explicit config path routed through commander', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'doctor', 'config', './candidate-config.toml']);
  ...
  it('checks a valid explicit tui path routed through commander', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'doctor', 'tui', './candidate-tui.toml']);
...
`,
    ...
    expect(stdout.join('')).toBe('');
    const err = stderr.join('');
    expect(err).toContain('NightHawk doctor found 2 issues.');
...
`,
    ...
    const err = stderr.join('');
    expect(err).toContain('Validation issues:');
    expect(err).toContain('models.nighthawk.max_context_size:');
  });
});

describe('nighthawk doctor (default v2 config validation)', () => {
  afterEach(() => {
    delete process.env['NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP'];
    delete process.env['NIGHTHAWK_LOOP_MAX_ATTEMPTS_PER_STEP'];
  });
...
`,
  ...
  it('reports TOML syntax errors with line and column', async () => {
    await writeFile(join(dir, 'config.toml'), '[providers.nighthawk\ntype = "nighthawk"\n', 'utf-8');
    const { deps, stderr } = makeDeps();
...
`,
  ...
  it('warns about a deprecated env var that supplies a value', async () => {
    ...
    process.env['NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP'] = '5';
    const { deps, stdout, stderr } = makeDeps();
    ...
    expect(stdout.join('')).toContain(
      'Environment variable NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP is deprecated; use NIGHTHAWK_LOOP_MAX_ATTEMPTS_PER_STEP instead.',
    );
  });
  ...
  it('does not warn about the deprecated env var when the primary one is set', async () => {
    await writeFile(join(dir, 'config.toml'), '[loop_control]\n', 'utf-8');
    process.env['NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP'] = '5';
    process.env['NIGHTHAWK_LOOP_MAX_ATTEMPTS_PER_STEP'] = '5';
    const { deps, stdout } = makeDeps();

    const code = await handleDoctor(deps, { target: 'config' });

    expect(code).toBe(0);
    expect(stdout.join('')).not.toContain('NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP');
  });
});
