/**
 * `nighthawk provider` CLI unit tests. The handlers receive an injected `getHarness`
...
vi.mock('@nighthawk/nighthawk-sdk', async (importOriginal) => {
  ...
  return {
    ...
    createNighthawkHarness: (...args: unknown[]) => {
      harnessRouting.nighthawkHarnessConstructor(...args);
      return harnessRouting.harness;
    },
    createNighthawkHarnessV2: (...args: unknown[]) => {
      harnessRouting.nighthawkHarnessV2Constructor(...args);
      return harnessRouting.harness;
    },
  };
});
...
interface FakeHarness {
  ...
  setConfig: (patch: Partial<NighthawkConfig>) => Promise<NighthawkConfig>;
  removeProvider: (providerId: string) => Promise<NighthawkConfig>;
  close: () => Promise<void>;
}

function makeHarness(initial: NighthawkConfig): {
  harness: FakeHarness;
  current: () => NighthawkConfig;
  setConfigCalls: Array<Partial<NighthawkConfig>>;
  removeCalls: string[];
} {
  // `persisted` simulates the on-disk config; the real RPC's `removeProvider`
  // reads from / writes to disk on every call (see
  // `packages/agent-core/src/rpc/core-impl.ts removeNighthawkProvider`). Tests must
  ...
  // `removeProvider`.
  let persisted: NighthawkConfig = structuredClone(initial);
  const setConfigCalls: Array<Partial<NighthawkConfig>> = [];
  ...
  const harness: FakeHarness = {
    ...
    setConfig: async (patch) => {
      ...
      // Mirror the real `setNighthawkConfig`: deep-merge with undefined keys
      ...
      persisted = next as NighthawkConfig;
...
describe('nighthawk provider add', () => {
  it('imports providers and models from a custom registry, persisting source on each provider', async () => {
    ...
    const { harness, current, setConfigCalls } = makeHarness({ providers: {} } as NighthawkConfig);
  ...
  it('reads the api key from NIGHTHAWK_REGISTRY_API_KEY when --api-key is omitted', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness } = makeHarness({ providers: {} } as NighthawkConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { NIGHTHAWK_REGISTRY_API_KEY: 'sk-env-token' },
    });
...
describe('nighthawk provider remove', () => {
...
describe('nighthawk provider list', () => {
  ...
  it('renders one row per provider with counts and source labels', async () => {
    ...
    expect(out).toMatch(/managed:nighthawk\s+type=nighthawk\s+models=0\s+source=oauth/);
...
describe('registerProviderCommand', () => {
  it('describes the user-facing subcommand and routes flags through commander', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness, current } = makeHarness({ providers: {} } as NighthawkConfig);
    ...
    await tryRun(() =>
      program.parseAsync(
        ['node', 'nighthawk', 'provider', 'add', REGISTRY_URL, '--api-key', 'sk-cli'],
        { from: 'node' },
      ),
    );
  ...
  it('reports write failures on stderr and exits 1 instead of crashing', async () => {
    ...
    harness.removeProvider = async () => {
      throw new Error(
        'Cannot change settings while config.toml is invalid — fix it first (run `nighthawk doctor` for details).',
      );
    };
    const { deps, stderr, exitCodes } = makeDeps(harness);
    ...
    await tryRun(() =>
      program.parseAsync(['node', 'nighthawk', 'provider', 'remove', 'nighthawk'], { from: 'node' }),
    );
...
describe('nighthawk provider catalog list', () => {
...
describe('nighthawk provider catalog add', () => {
  it('imports a provider from the catalog without changing the default model', async () => {
    mockRegistryFetch(CATALOG_BODY);
    const initial: NighthawkConfig = {
      providers: {
        other: { type: 'nighthawk', baseUrl: 'https://x', apiKey: 'k' },
      },
      models: {
        'other/main': {
          provider: 'other',
  ...
  it('rejects an unknown --default-model with a helpful hint', async () => {
    ...
    expect(err).toContain('nighthawk provider catalog list anthropic');
  });
  ...
  it('does not persist thinking.enabled=false for first-time setup with --default-model', async () => {
    ...
    // explicit "off" request. A fresh `nighthawk provider catalog add
  ...
  it('falls back to NIGHTHAWK_REGISTRY_API_KEY when --api-key is omitted', async () => {
    mockRegistryFetch(CATALOG_BODY);
    const { harness, current } = makeHarness({ providers: {} } as NighthawkConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { NIGHTHAWK_REGISTRY_API_KEY: 'sk-env' },
    });
...
describe('nighthawk provider engine routing', () => {
  beforeEach(() => {
    harnessRouting.nighthawkHarnessConstructor.mockClear();
    harnessRouting.nighthawkHarnessV2Constructor.mockClear();
    harnessRouting.harness = makeHarness({ providers: {} } as NighthawkConfig).harness;
  });
  ...
  it('builds the v2 harness by default', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'provider', 'list'], { from: 'node' });

    expect(harnessRouting.nighthawkHarnessV2Constructor).toHaveBeenCalledTimes(1);
    expect(harnessRouting.nighthawkHarnessConstructor).not.toHaveBeenCalled();
  });
  ...
  it('builds the legacy harness when the legacy flag is truthy', async () => {
    ...
    expect(harnessRouting.nighthawkHarnessConstructor).toHaveBeenCalledTimes(1);
    expect(harnessRouting.nighthawkHarnessV2Constructor).not.toHaveBeenCalled();
  });
});
