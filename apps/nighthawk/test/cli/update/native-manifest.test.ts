...
import { nighthawkCdnBinariesBase } from '#/constant/app';
...
const MANIFEST_BODY = JSON.stringify({
  version: VERSION,
  tag: `@nighthawk/nighthawk@${VERSION}`,
  platforms: {
    'win32-x64': {
      filename: `nighthawk-win32-x64.zip`,
...
describe('fetchNativeReleaseManifest', () => {
  ...
  it('throws on a malformed checksum', async () => {
    const body = JSON.stringify({
      version: VERSION,
      platforms: { 'win32-x64': { filename: 'nighthawk-win32-x64.zip', checksum: 'xyz' } },
    });
    await expect(
      fetchNativeReleaseManifest(VERSION, mockFetch({ ok: true, status: 200, body })),
    ).rejects.toThrow();
...
describe('selectPlatformEntry', () => {
  const manifest = {
    version: VERSION,
    platforms: {
      'win32-x64': { filename: 'nighthawk-win32-x64.zip', checksum: 'a'.repeat(64) },
    },
  };
...
describe('url helpers', () => {
  it('builds the manifest and binary URLs from the binaries base', () => {
    expect(nativeManifestUrl(VERSION)).toBe(`${nighthawkCdnBinariesBase()}/${VERSION}/manifest.json`);
    expect(nativeBinaryUrl(VERSION, 'nighthawk-win32-x64.zip')).toBe(
      `${nighthawkCdnBinariesBase()}/${VERSION}/nighthawk-win32-x64.zip`,
    );
  });
});
