...
describe('runUpdatePreflight', () => {
  ...
  it('homebrew: prints manual brew upgrade command, does not spawn', async () => {
    ...
    expect(stdout.join('')).toContain('https://github.com/nighthawk-dev/nighthawk');
    expect(promptForInstallChoice).not.toHaveBeenCalled();
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('global region: install commands and site links follow the region profile', async () => {
    ...
    try {
      ...
      expect(brew.stdout.join('')).toContain('https://github.com/nighthawk-dev/nighthawk');
      expect(mocks.spawn).not.toHaveBeenCalled();
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform });
      refreshNighthawkRegion();
  ...
  it('shows a one-shot notice after a background update succeeds and the new version starts', async () => {
    ...
    expect(rendered).toContain(
      'https://nighthawk-dev.github.io/nighthawk/en/release-notes/changelog.html',
    );
