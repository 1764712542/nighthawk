...
const BINARY_FILENAME = 'nighthawk-linux-x64';
...
describe('stageNativeUpdate', () => {
  ...
  beforeEach(async () => {
    workDir = await mkdtemp(join(tmpdir(), 'nighthawk-stage-test-'));
    exePath = join(workDir, 'bin', 'nighthawk');
  ...
  it('downloads, verifies and records the staged metadata', async () => {
    ...
    expect(result.staged.exeFileName).toMatch(/^nighthawk-0\.7\.0\.\d+\.\d+\.\d+$/);
  ...
  it('cleans orphaned staging files before downloading, preserving live swap claims', async () => {
    ...
    await agedOrphan(join(stagingDir, 'nighthawk-9.9.9'), Buffer.from('orphan-exe'));
    await agedOrphan(join(stagingDir, 'nighthawk-9.9.9.part'), Buffer.from('partial'));
    // A live swap claim referencing its own staged exe must survive.
    const claimExe = 'nighthawk-8.8.8';
    ...
    const youngExe = 'nighthawk-7.7.7';
    ...
    await expect(stat(join(stagingDir, 'nighthawk-9.9.9'))).rejects.toThrow();
    await expect(stat(join(stagingDir, 'nighthawk-9.9.9.part'))).rejects.toThrow();
  ...
  it('cleans orphans with prerelease and build-metadata versions', async () => {
    ...
    const { mkdir } = await import('node:fs/promises');
    await mkdir(stagingDir, { recursive: true });
    await agedOrphan(join(stagingDir, 'nighthawk-1.2.3-rc.1'), Buffer.from('orphan'));
    await agedOrphan(join(stagingDir, 'nighthawk-1.2.3+build.5.exe'), Buffer.from('orphan'));
    await agedOrphan(join(stagingDir, 'nighthawk-1.2.3-rc.1.123.0.part'), Buffer.from('partial'));
    // New-style published name with the unique per-worker infix.
    await agedOrphan(join(stagingDir, 'nighthawk-4.5.6.1234.1700000000000.0'), Buffer.from('orphan'));
    ...
    await expect(stat(join(stagingDir, 'nighthawk-1.2.3-rc.1'))).rejects.toThrow();
    await expect(stat(join(stagingDir, 'nighthawk-1.2.3+build.5.exe'))).rejects.toThrow();
    await expect(stat(join(stagingDir, 'nighthawk-1.2.3-rc.1.123.0.part'))).rejects.toThrow();
    await expect(stat(join(stagingDir, 'nighthawk-4.5.6.1234.1700000000000.0'))).rejects.toThrow();
  });

  it("preserves another worker's staged result when this attempt fails", async () => {
    const stagingDir = getNativeStagingDir(exePath);
    const exeFileName = `nighthawk-${VERSION}`;
...
describe('promoteStagedUpdateToManual', () => {
  ...
  beforeEach(async () => {
    workDir = await mkdtemp(join(tmpdir(), 'nighthawk-promote-test-'));
    exePath = join(workDir, 'bin', 'nighthawk');
  });
...
describe('readStagedNativeUpdate', () => {
  ...
  beforeEach(async () => {
    workDir = await mkdtemp(join(tmpdir(), 'nighthawk-staged-read-test-'));
    exePath = join(workDir, 'bin', 'nighthawk');
  });
