import type { ExperimentalFeatureState } from '@nighthawk/nighthawk-sdk';
...
describe('ExperimentsSelectorComponent', () => {
  it('renders searchable feature toggles with source details', () => {
    ...
    expect(out).toContain('    id micro_compaction · config · NIGHTHAWK_EXPERIMENTAL_MICRO_COMPACTION');
  ...
  it('drafts changes with Space and applies them with Enter', () => {
    ...
    expect(text(selector)).toContain(
      '    id micro_compaction · default · NIGHTHAWK_EXPERIMENTAL_MICRO_COMPACTION · modified',
    );
