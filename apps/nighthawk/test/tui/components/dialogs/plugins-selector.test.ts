...
import type { CapabilityStatus, PluginSummary } from '@nighthawk/nighthawk-sdk';
...
describe('plugins selector dialogs', () => {
  it('trusts only built-in NightHawk CDN plugin paths', () => {
    ...
    expect(pluginTrustLabel({
      ...
      hasErrors: false,
      source: 'zip-url',
      originalSource: 'https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/curated/superpowers.zip',
    })).toBe('curated');
    expect(pluginTrustLabel({
      id: 'nighthawk-cu',
      displayName: 'NightHawk Computer Use',
      ...
      hasErrors: false,
      source: 'zip-url',
      originalSource: 'https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/nighthawk-computer-use/latest/nighthawk-cu-plugin.zip',
    })).toBe('official');
    expect(pluginTrustLabel({
      id: 'demo',
      displayName: 'Demo',
    ...
    expect(pluginTrustLabel({
      ...
      hasErrors: false,
      source: 'local-path',
      originalSource: 'https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/local',
    })).toBe('third-party');
  });
  ...
  it('trusts paths under the distribution base, not just any file on its host', () => {
    ...
    expect(labelFor('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/nighthawk-datasource.zip')).toBe('official');
    expect(labelFor('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/curated/superpowers.zip')).toBe('curated');
    expect(labelFor('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/nighthawk-computer-use/latest/nighthawk-cu-plugin.zip')).toBe('official');
    expect(labelFor('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/nighthawk-computer-use-windows/latest/nighthawk-cu-win-plugin.zip')).toBe('official');
    ...
    expect(labelFor('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/unrelated/plugin.zip')).toBe('third-party');
    expect(labelFor('https://cdn.jsdelivr.net.example.test/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/x.zip')).toBe('third-party');
  });
  ...
  it('recognizes installed plugins by official provenance', () => {
    ...
    expect(isOfficialPluginInstall({
      ...base,
      source: 'zip-url',
      originalSource: 'http://127.0.0.1:58627/nighthawk/plugins/official/nighthawk-datasource.zip',
    })).toBe(false);
    expect(isOfficialPluginInstall({
      ...base,
      source: 'zip-url',
      originalSource: 'https://example.test/nighthawk/plugins/official/nighthawk-datasource.zip',
    })).toBe(false);
  });

  it('shows installed NightHawk Computer Use and WebBridge plugins as official', () => {
    const installed: PluginSummary[] = [
      ...
      },
      {
        ...
        source: 'zip-url',
        originalSource: 'https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/nighthawk-webbridge.zip',
      },
    ];
  ...
  it('treats only the official NightHawk CDN path as a trusted install source', () => {
    expect(isOfficialPluginSource('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/nighthawk-datasource.zip')).toBe(true);
    expect(isOfficialPluginSource('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/nighthawk-computer-use/latest/nighthawk-cu-plugin.zip')).toBe(true);
    expect(
    ...
    expect(isOfficialPluginSource('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/curated/superpowers.zip')).toBe(false);
    expect(isOfficialPluginSource('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/foo.zip')).toBe(false);
    expect(isOfficialPluginSource('https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/unrelated/plugin.zip')).toBe(false);
    ...
    expect(isOfficialPluginSource('http://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/official/x.zip')).toBe(false);
  ...
  it('renders the inline plugin hint on the installed row', () => {
    const datasource = { ...superpowers, id: 'nighthawk-datasource', displayName: 'NightHawk Datasource', skillCount: 1 };
    const { panel } = makePanel({
      installed: [datasource],
      selectedId: 'nighthawk-datasource',
      pluginHint: { id: 'nighthawk-datasource', text: 'pending /new' },
    });
    const out = strip(renderRaw(panel));
    expect(out).toContain('? NightHawk Datasource  enabled  pending /new');
  });
  ...
  it('renders the hardcoded Web Bridge entry on the Official tab while loading', () => {
    ...
    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk WebBridge  open in browser');
  ...
  it('renders a same-id custom catalog row as a normal plugin, without capability state', () => {
    ...
    // nighthawk-webbridge id: without the capability: marker it must render and
    // install as a plain plugin, not borrow capability status.
    const capabilities = [makeCapability({ id: 'nighthawk-webbridge', displayName: 'NightHawk WebBridge' })];
    const entries = [
      {
        id: 'nighthawk-webbridge',
        tier: 'official' as const,
        displayName: 'NightHawk WebBridge (fork)',
    ...
    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk WebBridge (fork)  install');

    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
      entry: expect.objectContaining({ id: 'nighthawk-webbridge', source: 'https://x/fork.zip' }),
    });
  });
  ...
  it('renders capability rows from the engine while the catalog is still loading', () => {
    ...
    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk Computer Use  install');
    expect(out).toContain('NightHawk WebBridge  install');
    expect(out).toContain('Background GUI automation');
    expect(out).not.toContain('id nighthawk-cu');
    expect(out).not.toContain('Official plugin');
    expect(out).not.toContain('open in browser');
    expect(out).toContain('Loading marketplace');

    panel.handleInput('\r'); // index 0 → nighthawk-cu routes to capability install
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
      entry: expect.objectContaining({ id: 'nighthawk-cu', source: 'capability:nighthawk-cu' }),
    });
    panel.handleInput('[B');
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
      entry: expect.objectContaining({ id: 'nighthawk-webbridge', source: 'capability:nighthawk-webbridge' }),
    });
  });
  ...
  it('keeps built-in rows out while the overridden marketplace is loading', () => {
    ...
    const out = strip(renderRaw(panel));
    expect(out).not.toContain('NightHawk Computer Use');
    expect(out).toContain('NightHawk WebBridge  open in browser');
    expect(out).toContain('Loading marketplace');
  });
  ...
  it('opens the Web Bridge webpage on Enter instead of installing', () => {
    ...
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'open-url',
      url: 'https://github.com/nighthawk-dev/nighthawk/tree/main/plugins/official/nighthawk-webbridge',
  ...
  it('installs a catalog official entry after navigating past Web Bridge', () => {
    const { panel, onSelect } = makePanel({ initialTab: 'official' });
    panel.setMarketplace(marketplaceEntries, '/tmp/marketplace.json');
    panel.handleInput('\u001B[B'); // ↓ → nighthawk-datasource
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
      entry: expect.objectContaining({ id: 'nighthawk-datasource' }),
    });
  });
  ...
  it('lets the real catalog entry win over the pinned Web Bridge promo', () => {
    const entries = [
      {
        ...
        tier: 'official' as const,
        displayName: 'NightHawk WebBridge',
        source: 'capability:nighthawk-webbridge',
      },
      ...officialEntries,
    ];
    const { panel, onSelect } = makePanel({ initialTab: 'official' });
    ...
    expect(out.split('NightHawk WebBridge').length - 1).toBe(1);
    expect(out).not.toContain('open in browser');
  ...
  it('installs a Curated entry whose id matches the pinned WebBridge', () => {
    ...
    // nighthawk-webbridge id; on the Curated tab it must install normally, not
  ...
  it('updates the Windows backing plugin through its capability entry', () => {
    const installed = [
      {
        ...superpowers,
        id: 'nighthawk-cu-win',
        displayName: 'NightHawk Computer Use for Windows',
        version: '0.2.13',
      },
    ];
    const capability = makeCapability({ pluginId: 'nighthawk-cu-win' });
    const entry = {
      id: 'nighthawk-cu',
      tier: 'official' as const,
      displayName: 'NightHawk Computer Use',
      version: '0.2.14',
      source: 'capability:nighthawk-cu',
      builtIn: true,
    };
    ...
    expect(strip(renderRaw(panel))).toContain(
      'NightHawk Computer Use for Windows  enabled  update 0.2.13 → 0.2.14',
    );
    panel.handleInput('\r');
  ...
  it('keeps installation state separate from capability readiness', () => {
    const installed = [
      { ...superpowers, id: 'nighthawk-cu', displayName: 'NightHawk Computer Use', version: '0.5.4' },
    ];
    const capabilities = [makeCapability()];
    const entries = [
      {
    ...
    expect(installedOut).toContain('NightHawk Computer Use  enabled');
    ...
    expect(officialOut).toContain('NightHawk Computer Use  installed · v0.5.4');
  ...
  it('uses the Windows backing plugin id for Official installation state', () => {
    ...
    const capabilities = [makeCapability({ pluginId: 'nighthawk-cu-win' })];
    const entries = [
      {
        id: 'nighthawk-cu',
        tier: 'official' as const,
    ...
    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk Computer Use  installed · v0.2.14');
  ...
  it('keeps Enter on the Installed tab consistent with other plugins', () => {
    ...
    expect(onSelect).toHaveBeenCalledWith({ kind: 'details', id: 'nighthawk-cu' });
  });
  ...
  it('keeps unsupported capability diagnostics out of the Installed list', () => {
    ...
    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk Computer Use  enabled');
  ...
  it('does not expose capability readiness, version, or optional issues in the marketplace', () => {
    ...
    const installed = [
      { ...superpowers, id: 'nighthawk-webbridge', displayName: 'NightHawk WebBridge', version: '1.11.3' },
    ];
    const { panel } = makePanel({ installed, capabilities, initialTab: 'official' });
    panel.setMarketplace(
      [{ id: 'nighthawk-webbridge', displayName: 'NightHawk WebBridge', source: 'capability:nighthawk-webbridge', tier: 'official', builtIn: true }],
      '/tmp/marketplace.json',
    );

    const out = strip(renderRaw(panel));
    expect(out).toContain('NightHawk WebBridge  installed');
  ...
  it('toggles MCP servers from the MCP selector', () => {
        ...
        mcpServers: [
          {
            ...
            transport: 'stdio',
            command: 'node',
            args: ['./bin/nighthawk-datasource.mjs'],
            cwd: '/plugins/nighthawk-datasource',
          },
        ],
        diagnostics: [],
      },
    ...
    expect(selections).toEqual([
      { kind: 'toggle', pluginId: 'nighthawk-datasource', server: 'data', enabled: false },
    ]);
  });
  ...
  it('defaults plugin removal confirmation to cancel', () => {
    ...
    expect(out).toContain(' Remove NightHawk Datasource (nighthawk-datasource)?');
  ...
  it('defaults the third-party install trust prompt to exit', () => {
    ...
    expect(out.some((line) => line.includes('NightHawk has not reviewed'))).toBe(true);
