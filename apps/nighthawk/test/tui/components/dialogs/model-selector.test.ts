...
describe('ModelSelectorComponent', () => {
  it('lays out the provider as a right column and marks the current model', () => {
    const picker = new ModelSelectorComponent({
      models: { nighthawk: model('NightHawk K2') },
      currentValue: 'nighthawk',
    ...
    expect(out).toMatch(/❯ NightHawk K2\s+NightHawk ← current/);
    // Provider is no longer inlined in parentheses next to the name.
    expect(out).not.toContain('NightHawk K2 (NightHawk)');
  });
  ...
  it('toggles thinking with Left/Right (not with "/")', () => {
    ...
    picker.handleInput('\r');
    expect(onSelect).toHaveBeenLastCalledWith({ alias: 'nighthawk', thinking: 'off' });
  ...
  it('forces always-thinking models on and unsupported models off', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        always: model('NightHawk Thinking', ['always_thinking']),
        plain: model('NightHawk Plain', ['tool_use']),
      },
      currentValue: 'always',
      currentThinkingEffort: 'off',
      onSelect,
  ...
  it('renders the unavailable thinking segment muted', () => {
    const picker = new ModelSelectorComponent({
      models: { always: model('NightHawk Thinking', ['always_thinking']) },
      currentValue: 'always',
      currentThinkingEffort: 'on',
      onSelect: vi.fn(),
      onCancel: vi.fn(),
  ...
  it('keeps the thinking draft when moving across models', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        plain: model('NightHawk Plain', ['tool_use']),
        thinking: model('NightHawk Thinking', ['thinking']),
      },
      currentValue: 'plain',
      currentThinkingEffort: 'off',
      onSelect,
  ...
  it('defaults a thinking-capable model to On but keeps the current model state', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        current: model('NightHawk Current', ['thinking']),
        other: model('NightHawk Other', ['thinking']),
      },
  ...
  it('fuzzy-filters by typing and reports a match count', () => {
    ...
    expect(out).toContain('NightHawk Turbo');
    expect(out).not.toContain('NightHawk K2');
  ...
  it('invokes onSessionOnlySelect on Alt+S with the effective thinking state', () => {
    ...
    expect(onSessionOnlySelect).toHaveBeenCalledWith({ alias: 'nighthawk', thinking: 'off' });
  ...
  it('renders effort segments with the default effort highlighted', () => {
    const picker = new ModelSelectorComponent({
      models: { nighthawk: effortModel('NightHawk K2', ['low', 'high', 'max'], 'high') },
      currentValue: 'nighthawk',
  ...
  it('cycles efforts with Left/Right and clamps at the ends', () => {
    ...
    picker.handleInput(RIGHT);
    picker.handleInput('\r');
    expect(onSelect).toHaveBeenLastCalledWith({ alias: 'nighthawk', thinking: 'max' });
  ...
  it('always-on effort models hide Off and clamp selection at the last effort', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        nighthawk: effortModel('NightHawk K2', ['low', 'high', 'max'], 'high', ['always_thinking']),
      },
      currentValue: 'nighthawk',
      currentThinkingEffort: 'high',
      onSelect,
  ...
  it('defaults an effort model without a current level to its defaultEffort', () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel('NightHawk Other', ['low', 'high', 'max'], 'max'),
      },
      currentValue: 'current',
      currentThinkingEffort: 'off',
      onSelect,
  ...
  it('falls back to the middle effort when an effort model has no defaultEffort', () => {
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel('NightHawk Other', ['low', 'medium', 'high']),
      },
      currentValue: 'current',
      currentThinkingEffort: 'off',
      onSelect: vi.fn(),
  ...
  it('renders the warning line directly below the key-hint line when provided', () => {
    ...
    expect(lines.findIndex((l) => l.includes('NightHawk K2'))).toBeGreaterThan(hintIdx + 1);
  });
...
describe('ModelSelectorComponent overrides', () => {
  it('uses overridden support_efforts for selectable efforts', () => {
    const picker = new ModelSelectorComponent({
      models: {
        nighthawk: {
          ...effortModel('NightHawk K2', ['low', 'high', 'max'], 'max'),
