...
describe('mediaUrlPartToText', () => {
  ...
  it('renders an internal daemon file reference as a bare placeholder', () => {
    // `nighthawk-file://…?path=…` resolves nowhere for the user and carries the
    // materialization path — never render the wire form.
    expect(
      mediaUrlPartToText('image', 'nighthawk-file://f_1?path=%2FUsers%2Falice%2Fmedia%2Ff_1.png'),
    ).toBe('[image]');
    expect(mediaUrlPartToText('video', 'nighthawk-file://f_2')).toBe('[video]');
  });
