...
import { parseDaemonFileUrl } from '@nighthawk/nighthawk-sdk';
...
function setupTempCache(): { cleanup: () => void } {
  const home = mkdtempSync(join(tmpdir(), 'nighthawk-home-'));
  const prev = process.env[NIGHTHAWK_HOME_ENV];
  process.env[NIGHTHAWK_HOME_ENV] = home;
  return {
    cleanup: () => {
      if (prev === undefined) delete process.env[NIGHTHAWK_HOME_ENV];
      else process.env[NIGHTHAWK_HOME_ENV] = prev;
...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), 'nighthawk-src-'));
}
...
// bare `nighthawk-file://` daemon reference (the paste was uploaded at paste
...
describe('extractMediaAttachments', () => {
  ...
  it('keeps matched-placeholder order with mixed image and video attachments', () => {
    ...
    expect(r.parts).toEqual([
      ...
      { type: 'image_url', imageUrl: { url: 'data:image/png;base64,AQ==' } },
      { type: 'text', text: ' then ' },
      { type: 'video_url', videoUrl: { url: 'nighthawk-file://file-v1' } },
      { type: 'text', text: ' end' },
    ]);
  });
  ...
  it('emits a bare nighthawk-file video_url part for an uploaded video', () => {
  ...
  it('expands an uploaded (fileId) image into a bare nighthawk-file reference', () => {
    const { cleanup } = setupTempCache();
    try {
      const store = new ImageAttachmentStore();
      ...
      expect(r.parts).toEqual([
        { type: 'text', text: 'describe ' },
        { type: 'image_url', imageUrl: { url: 'nighthawk-file://file-1' } },
        { type: 'text', text: ' please' },
      ]);
      expect(parseDaemonFileUrl('nighthawk-file://file-1')).toEqual({ fileId: 'file-1' });
...
describe('resolveOriginalCaptions', () => {
  ...
  it('authors the caption before the bare nighthawk-file reference', () => {
    const dir = makeTempDir();
    try {
      const { store, att } = storeWithOriginal(
      ...
      expect(resolved[1]).toEqual({
        type: 'image_url',
        imageUrl: { url: 'nighthawk-file://file-2' },
      });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
...
describe('pendingMediaIngestions', () => {
  ...
  it('waits for a pending video upload so extraction can use the daemon-ref form', async () => {
    ...
    expect(videoUrlFromParts(r.parts)).toBe('nighthawk-file://file-v1');
  });
