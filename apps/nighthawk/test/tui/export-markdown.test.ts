...
import type { ContentPart, ToolCall } from '@nighthawk/nighthawk-sdk';
import type { ContextMessage, PromptOrigin } from '@nighthawk/nighthawk-sdk';
...
describe('buildExportMarkdown', () => {
  ...
  it('builds complete markdown with frontmatter and overview', () => {
    ...
    expect(md).toContain('# NightHawk Session Export');
  ...
  it('renders an uploaded image daemon ref as [image] in the exported user message', () => {
    // An uploaded image persists as a self-contained `nighthawk-file://` part —
    ...
    expect(md).not.toContain('nighthawk-file');
