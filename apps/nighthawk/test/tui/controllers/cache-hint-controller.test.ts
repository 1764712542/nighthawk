...
const CONFIG: CacheHintConfig = {
  version: 1,
  config: { 'nighthawk-k2': { min_tokens_to_hint: 100000, cache_duration: 600 } },
};
...
function makeHost(
  ...
  const state = {
    ...
    appState: {
      model: 'k2',
      availableModels: { k2: { model: 'nighthawk-k2', provider: 'managed:nighthawk' } },
      availableProviders: { 'managed:nighthawk': { oauth: { key: 'nighthawk' } } },
      sessionId: 's1',
      streamingPhase: 'idle',
      isCompacting: false,
...
function uploadedExtraction(fileId: string, byte: number): ExtractionResult {
  ...
  return {
    parts: [
      ...
      {
        type: 'image_url',
        imageUrl: { url: `nighthawk-file://${fileId}?path=${encodeURIComponent(path)}` },
      },
    ],
    hasMedia: true,
...
describe('CacheHintController scenario 2 (idle submit)', () => {
  ...
  it('does not intercept when the provider is not OAuth-managed', () => {
    ...
    const { host } = makeHost({
      appState: {
        availableProviders: { 'managed:nighthawk': {} }, // apiKey form: no oauth
      },
    });
    const controller = new CacheHintController(host);
    controller.recordActivity();
