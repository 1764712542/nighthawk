...
import { log, type GoalSnapshot } from '@nighthawk/nighthawk-sdk';
...
function loginRequiredError(): Error & { readonly code: string } {
  return Object.assign(new Error('OAuth provider "managed:nighthawk" requires login.'), {
...
describe('NighthawkTUI startup', () => {
  ...
  it('hydrates permission/plan defaults after a session-less v2 login', async () => {
    ...
    vi.mocked(promptPlatformSelection).mockResolvedValue('nighthawk');
  ...
  it("starts TUI without a session when fresh startup needs OAuth login", async () => {
    ...
    expect(driver.state.startupState).toBe('ready');
    expect((driver as any).startupNotice).toContain('Provider session expired. Send /connect to reconnect.');
    expect(driver.state.appState).toMatchObject({
      sessionId: '',
      model: '',
      thinkingEffort: 'off',
  ...
  it('logs login failures with session context', async () => {
    ...
    const session = makeSession();
    const loginError = new Error('Failed to list NightHawk models (HTTP 402).');
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({ providers: [] })),
        login: vi.fn(async () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(
        'login failed',
        expect.objectContaining({
          providerName: 'managed:nighthawk',
          alreadyLoggedIn: false,
          sessionId: 'ses-1',
          error: expect.objectContaining({
            message: 'Failed to list NightHawk models (HTTP 402).',
          }),
        }),
      );
    } finally {
  ...
  it('tracks logout while preserving the active session model', async () => {
      ...
      getConfig: vi.fn(async () =>
        loggedOut
          ...
          : {
              ...
              providers: { 'managed:nighthawk': { type: 'nighthawk' } },
            },
      ),
      auth: {
        status: vi.fn(async () => ({
    ...
    vi.mocked(promptLogoutProviderSelection).mockResolvedValue('managed:nighthawk');
    await handleLogoutCommand(driver as any);

    expect(harness.auth.logout).toHaveBeenCalledWith('managed:nighthawk');
    ...
    expect(harness.track).toHaveBeenCalledWith('logout', { provider: 'managed:nighthawk' });
  });
  ...
  it('keeps the active session when logging out a different provider', async () => {
    ...
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { provider: 'managed:nighthawk', model: 'nighthawk-v1', maxContextSize: 100 },
        },
        providers: {
          'managed:nighthawk': { type: 'nighthawk' },
