...
describe('parseSlashInput', () => {
  it('parses command names and trimmed args', () => {
    ...
    expect(parseSlashInput('/model   nighthawk-k2  ')).toEqual({
      name: 'model',
      args: 'nighthawk-k2',
    });
  });
...
describe('built-in slash command registry', () => {
  ...
  it('keeps swarm mode changes and swarm tasks idle-only', () => {
    ...
    expect((swarm as NighthawkSlashCommand).experimentalFlag).toBeUndefined();
  ...
  it('offers swarm subcommand argument completions', () => {
    ...
    expect(swarmArgumentCompletions('of')).toEqual([
      { value: 'off', label: 'off', description: 'Turn swarm mode off · 关闭集群模式' },
    ]);
  ...
  it('offers tower subcommand argument completions', () => {
    ...
    expect(towerArgumentCompletions('tea')).toEqual([
      { value: 'teardown', label: 'teardown', description: 'Tear down the tower · 拆除 Tower' },
    ]);
  ...
  it('defaults commands without explicit availability to idle-only', () => {
    const command: NighthawkSlashCommand = {
      name: 'example',
      aliases: [],
  ...
  it('sorts commands by priority descending and name ascending', () => {
    const commands: NighthawkSlashCommand[] = [
  ...
  it('registers goal with subcommand-aware availability', () => {
    ...
    expect((goal as NighthawkSlashCommand).experimentalFlag).toBeUndefined();
  ...
  it('gates secondary-model behind the secondary-model experiment, always available', () => {
    ...
    expect((command as NighthawkSlashCommand).experimentalFlag).toBe('secondary-model');
  ...
  it('gates tower behind the tower experiment and the v2 engine', () => {
    ...
    expect(command).toBeDefined();
    expect((command as NighthawkSlashCommand).experimentalFlag).toBe('tower');
    expect((command as NighthawkSlashCommand).requiresEngineV2).toBe(true);
  });
