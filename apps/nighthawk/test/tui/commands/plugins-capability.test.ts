...
import { resetCapabilitiesCache, setCapabilities, type Component } from '@nighthawk/pi-tui';
...
describe('plugins command capability surface', () => {
  ...
  it('routes built-in entries through capabilities only on v2', () => {
    ...
    expect(
      isCapabilityEntry(v2.host, { id: 'nighthawk-cu', source: 'capability:nighthawk-cu', builtIn: true } as never),
    ).toBe(true);
    expect(
    ...
    expect(
      isCapabilityEntry(v2.host, { id: 'nighthawk-cu', source: 'https://example.test/plugin.zip' } as never),
    ).toBe(false);
    // A forged capability: source without the parser-proof flag is a plain row.
    expect(
      isCapabilityEntry(v2.host, { id: 'nighthawk-cu', source: 'capability:nighthawk-cu' } as never),
    ).toBe(false);

    const v1 = fakeHost({});
    expect(
      isCapabilityEntry(v1.host, { id: 'nighthawk-cu', source: 'capability:nighthawk-cu', builtIn: true } as never),
    ).toBe(false);
  });
  ...
  it('removePlugin notes that capability runtimes are left untouched', async () => {
    ...
    await removePlugin(host, 'nighthawk-cu');
    expect(statuses.some((s) => s.includes('Removed nighthawk-cu'))).toBe(true);
    ...
    expect(statuses.some((s) => s.includes('Restart NightHawk before reinstalling'))).toBe(true);
  ...
  it('keeps the runtime note for the Windows backing plugin id', async () => {
    const { host, statuses } = fakeHost({ engineV2: true });
    await removePlugin(host, 'nighthawk-cu-win');
    expect(statuses.some((s) => s.includes('Removed nighthawk-cu-win'))).toBe(true);
    expect(statuses.some((s) => s.includes('runtime binaries were left untouched'))).toBe(true);
  });
  ...
  it('treats only the default catalog (and the dev server) as injectable', () => {
    ...
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        NIGHTHAWK_PLUGIN_MARKETPLACE_URL: 'http://127.0.0.1:60056/marketplace.json',
        NIGHTHAWK_PLUGIN_MARKETPLACE_FROM_DEV_SERVER: '1',
      }),
    ).toBe(true);
    expect(
      isDefaultMarketplaceCatalog(undefined, {
        NIGHTHAWK_PLUGIN_MARKETPLACE_URL: 'https://example.test/marketplace.json',
      }),
    ).toBe(false);
  ...
  it('starts a capability install only when none is running', async () => {
    ...
    await installCapabilityFromPanel(
      idle.host,
      fakePanel().panel,
      { id: 'nighthawk-cu', displayName: 'NightHawk Computer Use', source: 'capability:nighthawk-cu' } as never,
    );
    expect(idle.installCapability).toHaveBeenCalledWith('nighthawk-cu');
  });
  ...
  it('renders visible clickable store URLs after WebBridge installs in a hyperlink-capable terminal', async () => {
    ...
    expect(notices).toContainEqual({ title: 'NightHawk WebBridge is installed.', detail: undefined });
    ...
    expect(rendered).toContain(
      '\u001B]8;;https://chromewebstore.google.com/detail/nighthawk-webbridge/fldmhceldgbpfpkbgopacenieobmligc\u001B\\',
    );
  ...
  it('renders full store URLs after WebBridge installs in a terminal without hyperlinks', async () => {
    ...
    expect(unwrappedVisibleText(transcriptEntries)).toContain(
      'https://chromewebstore.google.com/detail/nighthawk-webbridge/fldmhceldgbpfpkbgopacenieobmligc',
    );
  });
  ...
  it('separates the WebBridge install result from its setup steps with one blank line', async () => {
    ...
    const installed = lines.findIndex((line) => line.includes('NightHawk WebBridge is installed.'));
    const intro = lines.findIndex((line) =>
      line.includes('Two steps left to use NightHawk WebBridge:'),
    );
  ...
  it('shows the engine error when a background capability install fails', async () => {
    ...
    expect(statuses).toContain(
      'NightHawk Computer Use installation failed: Authenticode signature is not valid',
    );
