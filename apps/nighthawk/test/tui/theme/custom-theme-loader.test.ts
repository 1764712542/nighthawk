...
let home: string;
const originalHome = process.env['NIGHTHAWK_HOME'];

beforeEach(() => {
  home = join(tmpdir(), `nighthawk-themes-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
afterEach(() => {
  ...
  if (originalHome === undefined) {
    ...
    process.env['NIGHTHAWK_HOME'] = originalHome;
  }
});
