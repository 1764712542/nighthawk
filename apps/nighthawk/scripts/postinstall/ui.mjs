 ...
 *     `nighthawk-legacy`. The "`nighthawk` now launches the new CLI" claim is
 *     safe to make here because the orchestrator already verified
 *     reachability after this set of removals.
 *   - "Consolidated" — first shim's `nighthawk-legacy` already pointed at
 *     a legacy file (re-migration case); we deleted the duplicate
 *     source and kept the existing target. Same end state, different
 *     mechanism.
 *   - "Couldn't preserve as nighthawk-legacy" — first shim's `nighthawk-legacy`
 *     slot was a user-managed file; we deleted the source `nighthawk` to
...
export function logMigrationDone(outcomes, pm) {
  const reinstallCmd = pmGlobalInstallCommand(pm, '@nighthawk/nighthawk');
  const {
    renames,
    consolidates,
  ...
  const lines = [successHeading('nighthawk now runs the new version'), ''];

  if (renames.length > 0) {
    lines.push(pad('   Renamed your old nighthawk so you can still run it as'));
    lines.push(pad('   nighthawk-legacy:'));
  ...
  if (consolidates.length > 0) {
    lines.push(pad('   Removed an extra copy of your old nighthawk (nighthawk-legacy'));
    lines.push(pad('   was already set up here from before):'));
    for (const c of consolidates) {
      lines.push(pathInBox(c.shimPath));
      lines.push(pathInBox('  (nighthawk-legacy is at ' + c.target + ')'));
    }
    lines.push('');
  }

  if (skippedForeignTarget.length > 0) {
    lines.push(pad('   Removed your old nighthawk (a file you created was already'));
    lines.push(pad('   using the name nighthawk-legacy, so we left it alone):'));
  ...
  if (deletes.length > 0) {
    ...
    lines.push(pad('   new nighthawk if we left them):'));
  ...
  if (blockedHarmless.length > 0) {
    ...
    lines.push(pad('   they won\'t run instead of the new nighthawk:'));
  ...
  //      available as `nighthawk-legacy`. Standard takeover footer.
  //   2. Only skippedForeignTarget (no rename, no consolidate, no
  //      blockedHarmless): user has a file they made called
  //      `nighthawk-legacy`, so we couldn't save the old CLI under that
  ...
  if (preservedSomewhere) {
    lines.push(
      pad('   Now typing `nighthawk` runs the new version. To run the old'),
      pad('   version, type `nighthawk-legacy` instead. Your settings from'),
      pad('   the old version will be moved over the first time you'),
      pad('   run `nighthawk`.'),
      '',
      pad('   Note: if you reinstall the old nighthawk later (e.g. with'),
      pad('   `uv tool`, `pip`, or `pipx`), it will put `nighthawk` back.'),
      pad('   Run this command again to switch to the new one:'),
      pathInBox(reinstallCmd),
      '',
      pad('   If typing `nighthawk` still runs the old version, open a new'),
      pad('   terminal window — your current one may have remembered'),
      pad('   the old path.'),
    );
  } else if (skippedForeignTarget.length > 0) {
    lines.push(
      pad('   Now typing `nighthawk` runs the new version. Your settings'),
      pad('   from the old version will be moved over the first time'),
      pad('   you run `nighthawk`.'),
      '',
      pad('   We couldn\'t save the old nighthawk as `nighthawk-legacy` because'),
      pad('   that name was already taken by a file you\'d created.'),
      pad('   If you need the old nighthawk back, install it again with'),
      pad('   `uv tool install legacy-cli` (or pipx / pip).'),
      '',
      pad('   If typing `nighthawk` still runs the old version, open a new'),
      pad('   terminal window — your current one may have remembered'),
      pad('   the old path.'),
...
export function logMigrationBlocked(blocked, actionable, pm) {
  ...
  const lines = [
    warningHeading('Can\'t switch to the new nighthawk yet'),
    '',
    pad('   There\'s an old nighthawk on your computer that we can\'t change.'),
    pad('   As long as it\'s there, typing `nighthawk` will still run the old'),
  ...
  if (actionable.length > 0) {
    lines.push(
      '',
      pad('   We also found these old nighthawk files. We could remove them'),
...
/**
 * The reachability check found a `nighthawk` ahead of our shim on PATH
 ...
 * user can decide.
 */
export function logForeignNighthawkInTheWay(foreignPath, pm) {
  const reinstallCmd = pmGlobalInstallCommand(pm, '@nighthawk/nighthawk');
  emit(
    renderBox([
      warningHeading('Can\'t switch to the new nighthawk yet'),
      '',
      pad('   There\'s another file called `nighthawk` on your computer that\'s'),
      pad('   not the new CLI and not the old one — it looks like'),
      pad('   something you set up yourself. As long as it\'s there,'),
      pad('   typing `nighthawk` will run it instead of the new version.'),
      '',
      pad('   We found it at:'),
      pathInBox(foreignPath),
      '',
      pad('   To use the new nighthawk, delete or rename that file, then'),
      pad('   install again:'),
      pathInBox(reinstallCmd),
      '',
      pad('   Nothing on your computer was changed.'),
...
/**
 * The legacy `nighthawk` was found, but the directory where the package
 * manager placed the new `nighthawk` shim is not on the user's PATH.
 * Renaming the legacy shim now would leave the user with NO reachable
 * `nighthawk` command — the new one would still not be discoverable by
...
export function logNewCliNotOnPath(detection, pm) {
  ...
  emit(
    renderBox([
      warningHeading('New nighthawk is installed, but your terminal can\'t find it'),
      '',
      pad('   The old nighthawk is still here:'),
      pathInBox(detection.shimPath),
      '',
      pad('   The new nighthawk was installed by ' + pm + ', but it landed in a'),
      pad('   folder your terminal doesn\'t search. (Your terminal looks'),
      pad('   for commands in folders listed in your PATH.) If we removed'),
      pad('   the old nighthawk now, typing `nighthawk` wouldn\'t find anything.'),
      '',
      pad('   Add the new nighthawk\'s folder to your PATH (and save the change'),
      pad('   in your ' + rcLabel + ' so it sticks), then install again:'),
      pathInBox(newPathHint),
      pathInBox(reinstallCmd),
      '',
      pad('   The old nighthawk is still where it was.'),
    ]),
  );
}
