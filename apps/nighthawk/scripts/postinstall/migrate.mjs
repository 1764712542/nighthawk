/**
 * Detection of the previous Python `legacy-cli` shim and the actual
 * filesystem operations that perform (or refuse) the rename.
 *
 * Detection:
 *   - {@link detectLegacyShims}: walks the caller-supplied
 *     `pathString` and returns every legacy `nighthawk` shim along the
 ...
 *     pipx-installed `legacy-cli` has two legacy shims in different
 ...
 *     decide whether an existing `nighthawk-legacy` is itself a legacy CLI
 ...
 * with a misleading "nighthawk now launches the new CLI" notice in front of
...
const LEGACY_BIN = 'nighthawk';
const LEGACY_RENAME = 'nighthawk-legacy';
...
/**
 * Expand `nighthawk` into the set of filenames that resolve as executables
 * on this platform. POSIX → just `['nighthawk']`. Windows → adds every
 * `PATHEXT` extension (so we find `nighthawk.exe`, `nighthawk.cmd`, etc).
 */
...
/**
 * Walk `pathString` and return every legacy `nighthawk` shim along the
...
export async function detectLegacyShims(ownRoot, pathString) {
  ...
  for (const dir of pathEntries(pathString)) {
    for (const name of candidates) {
      ...
      // Defence-in-depth: never touch a `nighthawk` that resolves into our
...
/**
 * Does the file at `p` look like the legacy Python `legacy-cli`?
 *
 ...
 * whether an existing `nighthawk-legacy` is itself a legacy shim (safe to
 * drop the duplicate `nighthawk`) or a user-managed file we must not
 * clobber.
 */
 ...
 * extension so a Windows `C:\…\nighthawk.exe` ends up at `nighthawk-legacy.exe`
 * rather than an extension-less `nighthawk-legacy` that `nighthawk.exe -- legacy`
 * shells won't run.
 */
 ...
 *   `sudo pip install legacy-cli` → `/usr/local/bin/`.
 *
 ...
 * which shim becomes nighthawk-legacy) before any filesystem writes
 * happen.
 *
 ...
 *                        existing `nighthawk-legacy` as the canonical
 ...
 *                        "preserve original nighthawk" invariant fails for
 ...
 *   - we're consolidating onto an existing legacy `nighthawk-legacy`, or
