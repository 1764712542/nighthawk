/**
 * Where the user actually types things, and what `nighthawk` resolves to
 * from there.
 *
 * Covers:
 *
 ...
 *     `nighthawk` into, not just the installer's environment.
 ...
 *   - The reachability check ({@link findFirstResolvableNighthawk}) —
 ...
 *     unknown/foreign `nighthawk` (e.g. a user-written wrapper).
 *
...
/**
 * Expand a basename like `nighthawk` into the set of filenames the OS
 * would actually match on PATH.
 *
 * On POSIX: just `['nighthawk']`.
 *
 * On Windows: `['nighthawk', 'nighthawk.exe', 'nighthawk.cmd', …]` — every
 * extension in `PATHEXT`. Without this, our PATH walk would miss
 * the typical `nighthawk.exe` shim produced by `uv tool install` on
 * Windows.
 */
 ...
 *     --location=global @nighthawk/nighthawk` — verified on npm
 *     11.13.0.
 ...
 * directory so symlinked install layouts (e.g. `<prefix>/bin/nighthawk`
...
//     do on POSIX)
const PACKAGE_NAME_MARKERS = ['@nighthawk/nighthawk'];
 ...
 * `nighthawk` to, AFTER the to-be-removed shims in `actionableShimPaths`
 * are pretended gone. Returns the first match by PATH order:
 *
 *   - { kind: 'own' }                     — our shim wins.
 *   - { kind: 'blocked-legacy', shim }    — a legacy `nighthawk` we
 *     detected but couldn't touch (a "blocked" shim) wins.
 *   - { kind: 'foreign', path }           — something else wins: a
 *     `nighthawk` we didn't recognize as a legacy CLI and didn't generate
 *     ourselves (e.g. a user-managed wrapper script in `~/bin`).
 *   - { kind: 'none' }                    — no `nighthawk` resolves at all.
 *
 ...
 * unknown `nighthawk`.
 */
export async function findFirstResolvableNighthawk(
  ownRoot,
  pathString,
  actionableShimPaths,
 ...
 * `nighthawk` into. A `~/.zshrc`-only PATH entry, a `~/.bash_profile`
...
export async function userShellPath() {
  ...
  return new Promise((resolve) => {
    ...
    const probe =
      'printf "<<<NIGHTHAWK_PATH_BEGIN>>>%s<<<NIGHTHAWK_PATH_END>>>\\n" "$PATH"';
    ...
    child.on('close', (code) => {
      ...
      const match = stdout.match(
        /<<<NIGHTHAWK_PATH_BEGIN>>>([\s\S]*?)<<<NIGHTHAWK_PATH_END>>>/,
      );
 ...
 *     because either source may contain a legacy `nighthawk` we want to
 ...
 *     the user will actually type `nighthawk` into. We do NOT union here:
