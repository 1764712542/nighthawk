 ...
 *
 * Input dir must contain files matching: nighthawk-<target>.zip.sha256
...
// Tag 格式 `@nighthawk/nighthawk@x.y.z` 或 `vx.y.z` 或 `x.y.z`，都归一化到 x.y.z
const version = tag.replace(/^@nighthawk\/nighthawk@/, '').replace(/^v/, '');

const entries = await readdir(inputDir);
const sumFiles = entries.filter((f) => /^nighthawk-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No nighthawk-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // nighthawk-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^nighthawk-/, '').replace(/\.zip$/, '');
