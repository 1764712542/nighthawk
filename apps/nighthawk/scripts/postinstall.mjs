#!/usr/bin/env node
/**
 * Postinstall hook for @nighthawk/nighthawk.
 *
 * Goal: when this package is installed globally, ensure typing `nighthawk`
 * invokes the new TypeScript CLI. The npm `package.json` bin field
 * installs a fresh `nighthawk` shim into the global bin dir; this script
 * removes any pre-existing `nighthawk` shim left behind by the previous
 * Python CLI (installed via `uv tool install`, `pipx install`,
 * `pip install`, etc.) that would otherwise shadow ours via PATH
 * ordering. The renamed shim is kept as `nighthawk-legacy` so users can
 ...
 *   - Does not touch a `nighthawk` we don't recognize as the previous
 ...
 *     rename target like `nighthawk.exe` → `nighthawk-legacy.exe`).
 *
 * ## Code layout
 *
 ...
 *     `nighthawk`-vs-`nighthawk-legacy` classification, the rename / unlink
 *     primitives.
 ...
 * ## Workflow
 *
 * What runs when a user types `npm install -g @nighthawk/nighthawk`
 * (or the yarn / pnpm equivalent):
 *
 *   1. The manager extracts the package and runs lifecycle scripts.
 *      The `bin.nighthawk` mapping in `package.json` tells the manager to
 *      install a `nighthawk` shim under its global bin directory.
 ...
 *      twice.
 *   5. Detect EVERY previous Python `legacy-cli` shim on the detection
 *      PATH (`detectLegacyShims`). Returns `[]` for fresh-install /
 *      no-op. Multiple results happen when the user has installed
 *      `legacy-cli` through more than one Python tool (uv + pipx, or
 ...
 *      `findFirstResolvableNighthawk` walks PATH treating the actionable
 ...
 *        - `foreign` → some `nighthawk` we don't recognize (a user's own
 *          file) wins. Surface `logForeignNighthawkInTheWay` asking the
 *          user to delete or rename their own file; touch nothing.
 *        - `none` → no `nighthawk` on PATH at all (our shim's bin dir
 ...
 *      touch becomes `nighthawk-legacy` (preserves what `nighthawk` referred
 *      to before this install). Each subsequent shim is `unlink`ed —
 *      keeping it as a dormant duplicate adds no value. If the
 *      first shim's `nighthawk-legacy` target is already user-managed,
 *      we delete `nighthawk` anyway (still achieves takeover) and tell
 *      the user we couldn't preserve a fallback. Extension is
 *      preserved on Windows (`nighthawk.exe` → `nighthawk-legacy.exe`).
...
async function main() {
  ...
  // we don't claim "nighthawk works now" when the shim only sits in the
  // installer's env).
  const paths = await postinstallPaths();

  // Step 4: detect EVERY previous Python `legacy-cli` shim on the
  ...
  //     needed)
  //   - an unrelated `nighthawk` we don't recognize (a user's own wrapper
  ...
  const blocker = await findFirstResolvableNighthawk(
    ownRoot,
    paths.reachability,
    actionableShimPaths,
  ...
  if (blocker.kind !== 'own') {
    if (blocker.kind === 'blocked-legacy') {
      ...
      logForeignNighthawkInTheWay(blocker.path, pm);
    } else {
  ...
  // we can touch becomes `nighthawk-legacy` (preserves what the user's
  // `nighthawk` used to refer to). Every subsequent shim is just
...
main().catch((err) => {
  ...
  notify(`[nighthawk] postinstall warning: ${message}`);
});
