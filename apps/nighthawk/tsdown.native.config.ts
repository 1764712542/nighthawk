...
function shouldAlwaysBundle(id: string): boolean {
  ...
  // Everything else is force-bundled, which covers `@nighthawk/*` (incl.
  // vis-server for `nighthawk vis`) plus its transitive `hono` / `@hono/node-server`
  // — so the SEA bundle is self-contained (check-bundle.mjs enforces this).
  return true;
}

function buildTarget(): string {
  return process.env['NIGHTHAWK_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __NIGHTHAWK_VERSION__: JSON.stringify(packageJson.version),
    __NIGHTHAWK_CHANNEL__: JSON.stringify(process.env['NIGHTHAWK_CHANNEL'] ?? ''),
    __NIGHTHAWK_COMMIT__: JSON.stringify(process.env['NIGHTHAWK_COMMIT'] ?? ''),
    __NIGHTHAWK_BUILD_TARGET__: JSON.stringify(buildTarget()),
    __NIGHTHAWK_NATIVE_BUNDLE__: 'true',
  },
  deps: {
