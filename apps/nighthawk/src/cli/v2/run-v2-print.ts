 ...
 *
 * Selected by `runPrompt` unless `NIGHTHAWK_LEGACY_FLAG` is truthy.
 */
...
import { createNighthawkDefaultHeaders, createNighthawkDeviceId } from '@nighthawk/nighthawk-oauth';
import type { GoalUpdated } from '@nighthawk/agent-core-v2/features/goal/goalOps';
...
export async function runV2Print(
  ...
  const identity = createNighthawkHostIdentity(version);
  const hostHeaders = createNighthawkDefaultHeaders({ homeDir, ...identity });
    ...
    if (target.cwd !== undefined && resolve(target.cwd) !== resolve(workDir)) {
      stderr.write(
        `Session "${opts.session}" was created under a different directory.\n` +
          `  cd "${target.cwd}" && nighthawk -r ${opts.session}\n\n`,
      );
...
/**
 * Apply the print-mode (`nighthawk -p`) background-resource policy after the main
