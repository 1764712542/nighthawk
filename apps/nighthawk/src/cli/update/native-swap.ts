 ...
 * prefix (`nighthawk.config.bak`, …) belongs to the user. A `.bak` still mapped
...
async function cleanupStaleSwapClaims(exePath: string): Promise<boolean> {
  ...
  for (const entry of entries) {
    if (!entry.startsWith(`${NIGHTHAWK_NATIVE_STAGED_STATE_FILE_NAME}.swap-`)) continue;
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = deps.spawnImpl(deps.exePath, deps.argv.slice(2), {
        stdio: 'inherit',
        env: { ...deps.env, [NIGHTHAWK_UPDATE_REEXEC_ENV]: '1' },
...
export async function maybeRelaunchWithStagedNativeUpdate(
  ...
  if (isTruthy(deps.env[NIGHTHAWK_UPDATE_REEXEC_ENV])) {
    // Read-once guard: drop it so this session's children (and any nested
    // nighthawk launches from them) do not inherit the swap skip.
    delete deps.env[NIGHTHAWK_UPDATE_REEXEC_ENV];
    return false;
  }
  if (swapInProgress) {
