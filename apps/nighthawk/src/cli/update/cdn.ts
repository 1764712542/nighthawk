...
export interface FetchLatestResult {
  /** Raw newest version — what `nighthawk upgrade` installs, never rollout-gated. */
...
/**
 * Fetch the latest published NightHawk Code version from the CDN.
 *
...
export async function fetchLatestVersionFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, nighthawkCdnLatestUrl());
  if (!response.ok) {
...
async function fetchUpdateManifestFromCdn(fetchImpl: typeof fetch): Promise<UpdateManifest> {
  const response = await fetchWithTimeout(fetchImpl, nighthawkCdnLatestJsonUrl());
  if (!response.ok) {
