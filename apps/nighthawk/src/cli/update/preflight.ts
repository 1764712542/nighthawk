...
import { log, type Logger } from '@nighthawk/nighthawk-sdk';
import type { TelemetryProperties } from '@nighthawk/telemetry';

import {
  nighthawkOfficialInstallUrl,
...
export function spawnForSource(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return { cmd: 'brew', args: ['upgrade', 'nighthawk'] };
    case 'native':
...
function thirdPartySourceNote(): string {
  return (
    ...
    `For the latest updates, use the official installer: ${nighthawkOfficialInstallUrl()}\n`
  );
}
...
function renderBackgroundInstallSuccessNotice(version: string): string {
  ...
  return `NightHawk updated to ${displayVersion}\nChangelog: ${CHANGELOG_URL}\n`;
}
...
/**
 * `NIGHTHAWK_NO_AUTO_UPDATE` (or the legacy `NIGHTHAWK_CLI_NO_AUTO_UPDATE` alias)
 ...
 * from legacy-cli, where the variable gated all auto-update behavior. Accepts
...
export function isAutoUpdateDisabledByEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  return truthy(env['NIGHTHAWK_NO_AUTO_UPDATE']) || truthy(env['NIGHTHAWK_CLI_NO_AUTO_UPDATE']);
}
