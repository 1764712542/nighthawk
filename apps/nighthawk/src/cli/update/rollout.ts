...
import { readNighthawkDeviceId } from '@nighthawk/nighthawk-oauth';
import { resolveNighthawkHome } from '@nighthawk/nighthawk-sdk';
...
export type PassiveUpdateReason =
  ...
  | 'eligible'
  /** NIGHTHAWK_EXPERIMENTAL_FLAG is on: rollout skipped, newest always visible. */
  | 'experimental';
 ...
 *
 * `nighthawk upgrade` must NOT go through this gate — it selects directly from the
 * raw latest version.
 */
...
export function resolveUpdateDeviceId(): string {
  return readNighthawkDeviceId(resolveNighthawkHome()) ?? randomUUID();
}
 ...
 * truthy values as `NIGHTHAWK_NO_AUTO_UPDATE`) — the update preflight runs
 * before the harness exists, so the core flag registry is not consulted.
 * `NIGHTHAWK_NO_AUTO_UPDATE` still wins: disabling updates beats opting in.
 */
export function isRolloutBypassedByExperimentalEnv(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  const value = (env['NIGHTHAWK_EXPERIMENTAL_FLAG'] ?? '').trim().toLowerCase();
