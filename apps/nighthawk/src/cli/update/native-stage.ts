 ...
 *
 * The CDN serves the bare platform binary (e.g. `nighthawk-win32-x64.exe`),
...
import { NIGHTHAWK_NATIVE_STAGED_STATE_FILE_NAME } from '#/constant/app';
...
const StagedNativeUpdateSchema = z
  .object({
     ...
     * `nighthawk upgrade` (vs the passive background downloader): manual stages
...
export function stagedExeFileName(version: string, platform: NodeJS.Platform): string {
  return platform === 'win32' ? `nighthawk-${version}.exe` : `nighthawk-${version}`;
}
 ...
 * infix (`nighthawk-<version>.<pid>.<epoch-ms>.<n>[.exe]`). Once published, a
...
function uniqueStagedExeFileName(version: string, platform: NodeJS.Platform): string {
  ...
  stageTempCounter += 1;
  return platform === 'win32' ? `nighthawk-${version}${infix}.exe` : `nighthawk-${version}${infix}`;
}
 ...
 * persisted. Used when an explicit `nighthawk upgrade` adopts a payload the
 ...
 * executable (`nighthawk-<version>[.<pid>.<epoch-ms>.<n>][.exe]`) or a download
...
function isUpdaterOwnedStagingFile(entry: string): boolean {
  if (!entry.startsWith('nighthawk-')) return false;
  let name = entry.slice('nighthawk-'.length);
...
async function cleanupStagingOrphans(stagingDir: string): Promise<void> {
  ...
  const keep = new Set<string>([NIGHTHAWK_NATIVE_STAGED_STATE_FILE_NAME]);
  for (const entry of entries) {
    // The current record and every swap claim pin the exe they reference.
    if (
      entry !== NIGHTHAWK_NATIVE_STAGED_STATE_FILE_NAME &&
      !entry.startsWith(`${NIGHTHAWK_NATIVE_STAGED_STATE_FILE_NAME}.swap-`)
    ) {
      continue;
    }
    keep.add(entry);
...
export interface StageNativeUpdateOptions {
  ...
  /** True when the stage answers an explicit user-initiated `nighthawk upgrade`. */
 ...
 * disk (repeat `nighthawk upgrade`, or foreground/background overlap). **Throws**
