...
export function createProgram(
  ...
  const program = new Command(CLI_COMMAND_NAME)
    ...
    .addHelpText('after', '\nDocumentation:        https://nighthawk-dev.github.io/nighthawk/\n');
  ...
  program
    .command('upgrade')
    .alias('update')
    .description('Upgrade NightHawk to the latest version.')
    .action(async () => {
      await onUpgrade();
    });
  ...
  // download, or foreground from `nighthawk upgrade` — `--manual` marks the
