...
export async function runShell(
  ...
  const harnessOptions: NighthawkHarnessOptions = {
  ...
  // The agent-core-v2 route is the default (same engine gate as `nighthawk -p`):
  // the harness is the SDK's v2-backed client, so the whole TUI runs on the
  // agent-core-v2 engine unless the legacy flag is set.
  const engineV2 = isNighthawkV2Enabled();
  const harness = engineV2
    ? createNighthawkHarnessV2(harnessOptions)
    : createNighthawkHarness(harnessOptions);
  ...
  const migrationPlan = await detectPendingMigration({
    sourceHome: join(homedir(), '.nighthawk'),
  ...
  const tui = new NighthawkTUI(harness, {
    cliOptions: opts,
    agentProfile,
  ...
  const trackLifecycleForSession = (
    sessionId: string,
    event: string,
    properties?: Parameters<NighthawkHarness['track']>[1],
  ) => {
  ...
  const trackLifecycle = (event: string, properties?: Parameters<NighthawkHarness['track']>[1]) => {
  ...
  // If we crash without going through NighthawkTUI.stop(), the terminal is left in
