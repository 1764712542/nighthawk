...
export async function runPrompt(
  ...
  const cleanupPromptRun = async (): Promise<void> => {
    ...
    // the background if it overruns; the caller (`nighthawk -p`) force-exits shortly
  ...
  try {
    ...
    // Headless goal mode: `nighthawk -p "/goal <objective>"`. The goal driver keeps
...
async function createPromptHarness(
  options: Parameters<typeof createNighthawkHarness>[0],
): Promise<PromptHarness> {
  // The v2 engine is dispatched earlier in `runPrompt` (see the
  // `isNighthawkV2Enabled()` branch) and never reaches here; this is the v1 path.
  return createNighthawkHarness(options);
}
      ...
      stderr.write(
        `${chalk.hex('#E8A838')(
          `Session "${opts.session}" was created under a different directory.\n` +
            `  cd "${target.workDir}" && nighthawk -r ${opts.session}`,
        )}\n\n`,
      );
      throw new Error(
        `Session "${opts.session}" was created under a different directory.`,
...
function runPromptTurn(
  ...
  // A `nighthawk -p` run is not done just because the model ended a turn: an active
