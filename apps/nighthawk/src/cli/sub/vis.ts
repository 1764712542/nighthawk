/**
 * `nighthawk vis` sub-command.
 *
 * CLI glue only: resolves the nighthawk home, starts the in-process session
...
export async function handleVis(deps: VisDeps, opts: VisOptions): Promise<void> {
  ...
  // Lazily load the embedded single-file SPA so normal `nighthawk` startup never
  ...
  try {
    ...
    deps.stderr.write(`Failed to start nighthawk vis: ${msg}\n`);
    return deps.exit(1);
  }
  ...
  deps.stdout.write(`nighthawk vis is running at ${server.url}\n`);
  ...
  return {
    ...
    startVisServer:
      ...
      (async (opts) => {
        ...
        const { startVisServer } = await import('@nighthawk/vis-server/start');
