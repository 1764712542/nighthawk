...
import { isNighthawkV2Enabled } from '#/cli/experimental-v2';
...
export interface DoctorDeps {
  ...
  readonly configRpc?: NighthawkConfigRpc;
...
export function registerDoctorCommand(parent: Command, deps?: Partial<DoctorDeps>): void {
  const doctor = parent
    .command('doctor')
    .description('Validate NightHawk configuration files.')
    .action(async () => {
...
function resolveDeps(deps: Partial<DoctorDeps> | DoctorDeps | undefined): ResolvedDoctorDeps {
  ...
  const getConfigRpc = (): NighthawkConfigRpc => {
    configRpc ??= createNighthawkConfigRpc();
    return configRpc;
  };
    ...
    validateConfigToml:
      ...
      (async (text, filePath) => {
        if (isNighthawkV2Enabled()) {
          // Default v2 route (same engine gate as `nighthawk -p`): validate with
...
function formatFailure(results: readonly CheckResult[], issueCount: number): string {
  return [
    `NightHawk doctor found ${String(issueCount)} ${issueCount === 1 ? 'issue' : 'issues'}.`,
    '',
    ...formatResults(results),
    '',
  ].join('\n');
...
function findValidationIssues(error: unknown): readonly NighthawkConfigValidationIssue[] | undefined {
...
function isValidationIssueArray(value: unknown): value is readonly NighthawkConfigValidationIssue[] {
  return Array.isArray(value) && value.every(isValidationIssue);
}

function isValidationIssue(value: unknown): value is NighthawkConfigValidationIssue {
