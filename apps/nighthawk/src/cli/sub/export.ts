...
export function registerExportCommand(parent: Command, deps?: Partial<ExportDeps>): void {
  parent
    ...
    .option(
      ...
      'Skip bundling the active global diagnostic log (~/.nighthawk/logs/nighthawk.log, not rotated .1 files). By default the global log is included.',
    )
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps & {
  ...
  const getHarness = (): NighthawkHarness => {
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    // Same engine gate as `nighthawk -p` / the TUI: the SDK's v2-backed harness by
    // default, the legacy agent-core harness when NIGHTHAWK_LEGACY_FLAG is set.
    harness ??= (isNighthawkV2Enabled() ? createNighthawkHarnessV2 : createNighthawkHarness)({
