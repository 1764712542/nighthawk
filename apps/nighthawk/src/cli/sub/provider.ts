/**
 * `nighthawk provider` sub-command — non-interactive provider management.
 *
 * Mirrors the TUI `/provider` flow (apps/nighthawk/src/tui/commands/provider.ts)
 ...
 * (apps/nighthawk/src/tui/utils/refresh-providers.ts) groups by URL, retries
...
import { createNighthawkHostIdentity, createNighthawkUserAgent } from '#/cli/version';
import { fetchCatalogOrBuiltIn } from '#/utils/catalog-fetch';

import { isNighthawkV2Enabled } from '../experimental-v2';
...
export interface ProviderDeps {
  readonly getHarness: () => NighthawkHarness;
...
export async function handleProviderAdd(
  ...
  if (apiKey === undefined) {
    deps.stderr.write(
      'Missing API key. Pass --api-key <key> or set NIGHTHAWK_REGISTRY_API_KEY.\n',
    );
    deps.exit(1);
  }
  ...
  try {
    entries = await fetchCustomRegistry(source, { userAgent: createNighthawkUserAgent() });
  } catch (error) {
  ...
  // `core-impl.ts removeNighthawkProvider`), so calling it inside the apply loop
...
export async function handleCatalogAdd(
  ...
  if (opts.defaultModel !== undefined && !models.some((m) => m.id === opts.defaultModel)) {
    deps.stderr.write(
      `Model "${opts.defaultModel}" is not in provider "${providerId}". Run "nighthawk provider catalog list ${providerId}" to see available ids.\n`,
    );
    deps.exit(1);
  }
  ...
  // `core-impl.ts removeNighthawkProvider`). Without this, re-importing an
...
async function loadCatalogOrExit(deps: ProviderDeps, url: string): Promise<Catalog> {
  try {
    const loaded = await fetchCatalogOrBuiltIn(url, { userAgent: createNighthawkUserAgent() });
...
export function registerProviderCommand(parent: Command, deps?: Partial<ProviderDeps>): void {
  ...
  provider
    ...
    .option('--api-key <key>', 'Registry API key. Falls back to NIGHTHAWK_REGISTRY_API_KEY.')
  ...
  catalog
    ...
    .option('--api-key <key>', 'API key for the provider. Falls back to NIGHTHAWK_REGISTRY_API_KEY.')
...
function resolveDeps(overrides: Partial<ProviderDeps> = {}): ResolvedProviderDeps {
  let harness: NighthawkHarness | undefined;
  ...
  return {
    getHarness:
      ...
      (() => {
        ...
        // NIGHTHAWK_LEGACY_FLAG is set.
        harness ??= (isNighthawkV2Enabled() ? createNighthawkHarnessV2 : createNighthawkHarness)({ identity });
        return harness;
      }),
...
function resolveApiKey(flag: string | undefined, env: NodeJS.ProcessEnv): string | undefined {
  ...
  const fromEnv = env['NIGHTHAWK_REGISTRY_API_KEY'];
  if (typeof fromEnv === 'string' && fromEnv.length > 0) return fromEnv;
  return undefined;
}

function asManaged(config: NighthawkConfig): ManagedNighthawkConfigShape {
  return config as unknown as ManagedNighthawkConfigShape;
}

function providerSourceLabel(provider: NighthawkConfig['providers'][string]): string {
