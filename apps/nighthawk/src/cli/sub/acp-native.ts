/**
 * Native `nighthawk acp` implementation.
 *
 ...
 * clients can drive a nighthawk session on the default engine.
 *
 * Wire-up mirrors `nighthawk acp` for the parts that are host-independent:
 ...
 *  - `NIGHTHAWK_HOME` (if set) is forwarded into `authMethods[0].env` so the
 ...
 *
 * `@nighthawk/acp-server` (and its `agent-core-v2` engine) is loaded via a
 * lazy dynamic import so parsing the CLI does not initialize the ACP engine —
 * mirroring the `nighthawk server run` v2 routing in `#/cli/sub/server/run.ts`.
 */
...
export function registerNativeAcpCommand(parent: Command): void {
  parent
    ...
    .action(async (opts: { login?: boolean; region?: string }) => {
      ...
      try {
        const { runAcpServer } = await import('@nighthawk/acp-server');
        await runAcpServer({
          homeDir: getDataDir(),
