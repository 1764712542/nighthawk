/**
 * `nighthawk web rotate-token` — generate a new persistent server token.
 *
 * Rewrites `<NIGHTHAWK_HOME>/server.token` (0600, atomic). The previous token
...
import { getLiveServerInstance, rotateServerToken } from '@nighthawk/kap-server';
