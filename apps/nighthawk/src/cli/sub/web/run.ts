/**
 * `nighthawk web` — run the local server in the foreground and open the web UI.
 *
...
import { createServerLogger, startServer, type ServerLogger } from '@nighthawk/kap-server';
import { shutdownTelemetry, track } from '@nighthawk/telemetry';
...
export function buildWebCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      '--web-title <title>',
      'Set a custom browser tab title for this web UI instance (default: "<workspace dir> | NightHawk").',
    )
...
function formatReadyLine(
  ...
  const notice = dangerousBypassAuth
    ...
    : '';
  return `${notice}NightHawk server: ${buildOpenableUrl(origin, token)}\n`;
}
...
/**
 * `nighthawk web` — runs the local server in-process, attached to the current
...
async function runServerInProcess(
  ...
  const v2 = await startServer({
    ...
    hostIdentity: {
      ...createNighthawkHostIdentity(version),
 ...
 * (`NIGHTHAWK_DEV_SERVER=1`, set by the repo's `dev:server` / `dev:kap-server*`
...
export function serverWebAssetsDir(
  ...
  if (env['NIGHTHAWK_DEV_SERVER'] === '1' && !existsSync(join(dir, 'index.html'))) {
    return undefined;
  }
  return dir;
}
