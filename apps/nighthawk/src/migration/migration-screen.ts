 ...
 * conventions in `apps/nighthawk/src/tui/components/dialogs/choice-picker.ts`.
 *
 ...
 */
import { Container, matchesKey, Key, truncateToWidth, type Focusable } from '@nighthawk/pi-tui';
...
} from '@nighthawk/migration-legacy';
...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `nighthawk migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `nighthawk migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "nighthawk migrate".'));
      lines.push('');
      lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to nighthawk'));
    ...
    if (r !== undefined) {
      ...
      if (r.notices.detectedPlugins.length > 0) {
        lines.push(
          chalk.hex(colors.warning)(
            `  ⚠ ${r.notices.detectedPlugins.length} legacy CLI plugins — not yet supported for migration`,
          ),
        );
      }
      // OAuth credentials are deliberately not migrated (refresh tokens cannot
      // safely be held by two installs at once). nighthawk's normal auth flow
      ...
      if (sum.config.wroteSiblingDueToConflict) {
        ...
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ config.toml could not be parsed — see config.migrated-from-legacy-cli.toml',
          ),
        );
      ...
      lines.push('');
      lines.push(
        chalk.hex(colors.textMuted)(' Old data kept at ~/.kimi/ — the legacy CLI still works.'),
      );
    }
    lines.push('');
    lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to nighthawk'));
  ...
  private renderProgress(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(colors.primary).bold(' Migrating from the legacy CLI'),
      '',
    ];
  ...
  private renderAsk(width: number): string[] {
    ...
    const lines: string[] = [
      chalk.hex(colors.primary)('─'.repeat(width)),
      chalk.hex(colors.primary).bold(' Migrate from the legacy CLI'),
      '',
    ];
    if (this.phase === 'ask1') {
      lines.push(chalk.hex(colors.text)(' Found an existing legacy CLI installation:'));
...
function stepFor(phase: Phase, plan: MigrationPlan): StepDef {
  if (phase === 'ask1') {
    return {
      title: 'Migrate this data to nighthawk?',
      options: [
