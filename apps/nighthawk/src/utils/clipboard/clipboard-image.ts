 ...
 *
 * nighthawk-core's LLM pipeline only accepts PNG/JPEG/GIF/WebP, and the
...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
  const tmpFile = join(tmpdir(), `nighthawk-wsl-clip-${randomUUID()}.png`);
  try {
    ...
    const psScript = [
      ...
      '$path = $env:NIGHTHAWK_WSL_CLIPBOARD_IMAGE_PATH',
    ...
    const result = runCommand('powershell.exe', ['-NoProfile', '-Command', psScript], {
      ...
      env: { ...process.env, NIGHTHAWK_WSL_CLIPBOARD_IMAGE_PATH: winPath },
    });
