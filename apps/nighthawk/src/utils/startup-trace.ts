...
//
// Debug-only startup phase tracer, enabled with NIGHTHAWK_STARTUP_TRACE=1.
// Each call appends one `<elapsed-ms> <label>` line to the trace file
// (default /tmp/nighthawk-startup-trace.log, override with
// NIGHTHAWK_STARTUP_TRACE_LOG=<path>), so a slow or BLOCKING startup phase
...
const enabled = process.env['NIGHTHAWK_STARTUP_TRACE'] !== undefined && process.env['NIGHTHAWK_STARTUP_TRACE'] !== '';
const logPath = process.env['NIGHTHAWK_STARTUP_TRACE_LOG'] ?? '/tmp/nighthawk-startup-trace.log';
