...
import { NIGHTHAWK_BUILD_INFO } from '#/cli/build-info';
import {
...
function currentTarget(): string {
  return NIGHTHAWK_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  const cacheDirEnv = optionalEnvValue(env, 'NIGHTHAWK_CACHE_DIR');
  if (cacheDirEnv !== null) return cacheDirEnv;

  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'nighthawk');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'nighthawk')
      : pathWin32.join(home, 'AppData', 'Local', 'nighthawk', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'nighthawk');
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? NIGHTHAWK_BUILD_INFO.version ?? 'dev');
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.nighthawk-native-entry.cjs');
  ensureFile(
    entryPath,
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = NIGHTHAWK_BUILD_INFO.version ?? 'dev';
