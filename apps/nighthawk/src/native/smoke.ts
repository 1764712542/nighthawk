...
import { getSearchWorkerRuntimeState } from '@nighthawk/kap-server/search-worker-runtime';
...
const smokePackages = ['@mariozechner/clipboard', '@nighthawk/pi-tui'];
...
export function runNativeAssetSmokeIfRequested(): boolean {
  if (process.env['NIGHTHAWK_NATIVE_ASSET_SMOKE'] !== '1') return false;
  void runSmoke().then(
    () => process.exit(0),
    (error: unknown) => {
