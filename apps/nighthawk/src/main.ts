 ...
 * the process itself. It reports here whether a headless (`nighthawk -p`) run
...
/** `nighthawk migrate`: launch the migration screen only, then exit. */
...
/** A neutral CLIOptions value — `nighthawk migrate` never opens a chat session. */
    ...
    (opts) => {
      void handleMainCommand(opts, version)
        .then(async (outcome) => {
          ...
          // from the run can't wedge a completed `nighthawk -p` until an external
        ...
        .catch(async (error: unknown) => {
          ...
          // 0 and `process.exit(1)` never runs — headless (`nighthawk -p`) failures
          ...
          process.stderr.write(`See log: ${resolveGlobalLogPath(resolveNighthawkHome())}\n`);
          process.exit(1);
        });
    },
    () => {
