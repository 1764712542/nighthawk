 ...
 *     `nighthawk-file://<id>` image part — the engine's prompt intake materializes
 ...
 *   - video placeholders expand to a bare `nighthawk-file://<id>` video part:
...
import type { PromptPart, Session } from '@nighthawk/nighthawk-sdk';
import {
  buildDaemonFileUrl,
...
export function extractMediaAttachments(
  ...
  while ((match = PLACEHOLDER_REGEX.exec(text)) !== null) {
    ...
    if (attachment.kind === 'video') {
      // The paste was uploaded to the daemon file store in the background:
      // reference it by a bare `nighthawk-file://` url — the engine's prompt
      ...
      if (attachment.fileId !== undefined) {
        ...
        // (v2): reference them by a bare `nighthawk-file://` url — the engine's
...
export function refreshExpiringImageFileRefs(
  ...
  const next = parts.map((part) => {
    ...
    if (attachmentId === undefined || !part.imageUrl.url.startsWith('nighthawk-file://')) return part;
...
export function makeExtractionResendable(
  ...
  for (const part of extraction.parts) {
    ...
    if (snapshot === undefined || !part.imageUrl.url.startsWith('nighthawk-file://')) {
      parts.push(part);
      continue;
    }
    parts.push({
...
function imagePartMatchesAttachment(
  ...
  if (url.startsWith('nighthawk-file://')) {
 ...
 * `nighthawk-file://` url — the engine's prompt intake materializes the session
...
function originalImageTempDir(): string {
  return join(tmpdir(), 'nighthawk-original-images');
}
