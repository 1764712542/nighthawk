 ...
 * `ImageAttachment.original`) and video placeholders to `nighthawk-file://`
 * daemon references (the paste was uploaded to the daemon file store in
 * the background, exactly like an uploaded image).
 *
 * Scope is per-`NighthawkTUI` instance. Reloads (`/new`, `/clear`,
...
export interface ImageAttachment {
   ...
   * (v2 engine only). Submit-time expansion then emits a `nighthawk-file://`
...
export interface VideoAttachment {
   ...
   * time. Submit-time expansion emits a `nighthawk-file://` video reference;
