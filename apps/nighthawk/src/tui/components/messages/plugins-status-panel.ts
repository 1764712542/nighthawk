import type { PluginInfo, PluginSummary } from '@nighthawk/nighthawk-sdk';
...
export function buildPluginsInfoLines(input: PluginsInfoPanelInput): readonly string[] {
  ...
  const trustLine = (() => {
    ...
    if (label === 'official') {
      return `${muted('Trust:')}  ${success(OFFICIAL_BADGE)} ${muted('(built-in, first-party maintained)')}`;
    }
    if (label === 'curated') {
      return `${muted('Trust:')}  ${primary(CURATED_BADGE)} ${muted('(reviewed, upstream-maintained)')}`;
    }
