...
import { Container, Spacer, Text, truncateToWidth, visibleWidth } from '@nighthawk/pi-tui';
import type { Component, TUI } from '@nighthawk/pi-tui';
...
import type { TokenUsage } from '@nighthawk/nighthawk-sdk';
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `NighthawkTUI` calls `setPlanInfo` with
  ...
  // when NighthawkTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `NighthawkTUI` fetches
  ...
  // ── Subagent API (called by NighthawkTUI event routing) ───────────────
