...
import { capabilitiesForModel } from '@nighthawk/nighthawk-oauth';
import type {
...
export function promptApiKey(
  host: SlashCommandHost,
  platformName: string,
  subtitleLines: readonly string[] = ['Your key will be saved to ~/.nighthawk/config.toml'],
): Promise<string | undefined> {
  return new Promise((resolve) => {
...
): Promise<{ model: ManagedNighthawkModelInfo; thinking: ThinkingEffort } | undefined> {
