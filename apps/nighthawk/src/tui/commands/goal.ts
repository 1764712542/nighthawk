import { ErrorCodes, isNighthawkError, type PermissionMode } from '@nighthawk/nighthawk-sdk';
...
async function startGoal(
  ...
  try {
    ...
    if (isNighthawkError(error) && error.code === ErrorCodes.GOAL_ALREADY_EXISTS) {
      host.showError(
...
async function pauseGoal(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (isNighthawkError(error) && error.code === ErrorCodes.GOAL_NOT_FOUND) {
