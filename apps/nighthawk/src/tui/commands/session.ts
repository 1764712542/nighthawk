...
import { LLM_NOT_SET_MESSAGE, NO_ACTIVE_SESSION_MESSAGE } from '../constant/nighthawk-tui';
...
function forkResumeCommand(workDir: string, forkId: string): string {
  ...
  return `${changeDir} && nighthawk --resume ${quoteShellArg(forkId)}`;
}
...
export async function handleExportMdCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  try {
    ...
    const defaultName = `nighthawk-export-${shortId}-${timestamp}.md`;
