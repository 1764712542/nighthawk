...
export function renderDanceWelcomeHeader(
  ...
  const rightRow0 = truncateToWidth(
    rainbowText('Welcome to NightHawk!', palette, phase + 2, true),
    textWidth,
    '…',
  );

  const lines = logo.map((row, i) =>
    rainbowText(row.padEnd(logoWidth), palette, phase + i * 3),
  );
  if (lines.length > 0) lines[0] += gap + rightRow0;
  if (lines.length > 1) lines[1] += gap + rightRow1;
  return lines;
}
