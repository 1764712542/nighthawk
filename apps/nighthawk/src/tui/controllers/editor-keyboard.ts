...
import type { FileMeta, NighthawkHarness, Session } from '@nighthawk/nighthawk-sdk';
import { compressImageForModel } from '@nighthawk/nighthawk-sdk';
...
export interface EditorKeyboardHost {
  ...
  /**
   * The host's harness (NighthawkTUI always has one). Its `imageLimits` drives
   * paste-time image compression; hosts without one fall back to the
   * env/built-in default.
   */
  harness?: NighthawkHarness | undefined;
...
export class EditorKeyboardController {
  ...
  private async finishClipboardImagePaste(
    ...
    // expansion emits a `nighthawk-file://` reference instead of inline base64.
