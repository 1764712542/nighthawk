...
import type { AppState, NighthawkTUIOptions } from '../types';
...
export class AuthFlowController {
  ...
  private buildRefreshHost(): RefreshProviderHost {
    ...
    const userAgent = createNighthawkUserAgent();
    ...
    let staged: NighthawkConfig | undefined;
    const requireStaged = (): NighthawkConfig => {
      if (staged === undefined) {
    ...
    return {
      ...
      setConfig: async (patch) => {
        ...
        // clone), so the Partial-shaped patch is a full NighthawkConfig overlay.
        staged = { ...requireStaged(), ...patch } as NighthawkConfig;
