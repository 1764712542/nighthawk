...
function validBundle(): Record<string, string> {
  return {
    'manifest.json': JSON.stringify({ sessionId: 'session_orig', nighthawkVersion: '0.20.2', workspaceDir: '/home/u/proj', title: 'imported demo' }),
    ...
    'logs/nighthawk.log': '2026-06-01T00:00:00.000Z INFO  hello  k=v\n',
  };
}
...
describe('import-store', () => {
  ...
  it('imports a valid bundle and lists it with manifest metadata', async () => {
    ...
    expect((await stat(join(dir, 'logs', 'nighthawk.log'))).isFile()).toBe(true);
