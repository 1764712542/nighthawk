...
import { NIGHTHAWK_HOME } from '../config';
import { discoverLogFiles, readLogs } from '../lib/log-reader';
import { readSessionDetail } from '../lib/session-store';

const SESSION_LOG_REL = ['logs', 'nighthawk.log'] as const;
const GLOBAL_LOG_REL = ['logs', 'global', 'nighthawk.log'] as const;
const HOME_GLOBAL_LOG_REL = ['logs', 'nighthawk.log'] as const;

export function logsRoute(home: string = NIGHTHAWK_HOME): Hono {
  const r = new Hono();
  r.get('/:id/logs', async (c) => {
    ...
    // it is captured under the session dir (logs/global/nighthawk.log); for a
    // live local session it lives at <NIGHTHAWK_HOME>/logs/nighthawk.log
    ...
    // Either log may have rotated (nighthawk.log.1, .2, …); discover the active
