...
import { hostForUrl, resolveHost, resolveNighthawkHome, resolvePort, resolveVisAuthToken } from './config';
import type { WebAsset } from './lib/web-asset';

export interface StartVisServerOptions {
  /** Sessions home. Defaults to env NIGHTHAWK_HOME, else ~/.nighthawk. */
...
export async function startVisServer(
  ...
  const homeDir = opts.homeDir ?? resolveNighthawkHome();
