    ...
    >
      ...
      <Link to={`/sessions/${session.sessionId}`} className="block px-3 py-2 pr-10">
        ...
        <div className="mt-1 flex items-center gap-3 font-mono text-[10.5px] text-fg-2">
          ...
          {session.imported && session.importMeta?.manifest?.nighthawkVersion ? (
            <span className="tabular text-fg-3" title="nighthawk version that produced this bundle">
              v{session.importMeta.manifest.nighthawkVersion}
            </span>
          ) : null}
          {session.health !== 'ok' ? (
