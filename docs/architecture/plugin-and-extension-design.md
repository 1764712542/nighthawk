...
Markdown + YAML frontmatter（`name` / `description` / `whenToUse` / `type` / `disableModelInvocation`）。多层 root（项目 > 用户 > plugin > builtin），按需激活、注入带 origin 标记的上下文块。详见[专项文档](../interview/skill-system.md)。

### 2. MCP —— 能力扩展

任何 MCP server（stdio / sse / http 传输）经 `McpConnectionManager` 接入，工具以 `mcp__<server>__<tool>` 命名空间注册。`needs-auth` 状态机引导 OAuth。详见[专项文档](../interview/mcp.md)。
