...
- Fix several known issues and make various refinements. See the [changelog on GitHub](https://github.com/nighthawk-dev/nighthawk/blob/main/apps/nighthawk/CHANGELOG.md) for more technical entries.
...
- Make several refinements and internal improvements. See the [changelog on GitHub](https://github.com/nighthawk-dev/nighthawk/blob/main/apps/nighthawk/CHANGELOG.md) for more technical entries.
...
- web: Generate session titles with AI (experimental). Off by default — set `NIGHTHAWK_EXPERIMENTAL_AUTO_SESSION_TITLE=1` (or the master flag `NIGHTHAWK_EXPERIMENTAL_FLAG=1`) to turn it on.
...
- Upgrade the experimental subagent model setting to a model pool: the `[secondary_model]` section can now hold a set of candidate models with descriptions, and the main agent picks from them per spawn based on the task.
  ...
  Set `NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL=1` (or the master flag `NIGHTHAWK_EXPERIMENTAL_FLAG=1`) before starting NightHawk to enable it.
  ...
  - Declare a named pool with a one-line scenario description for each alias — the descriptions are what the main agent sees when choosing:
    ...
    "nighthawk/nighthawk-highspeed" = "Fast and cheap — good for daily refactoring, code explanation, and small edits."
    "nighthawk/k3" = "Strong at complex reasoning and deep debugging — pick it for hard problems."
    ```

  See the [subagent model pool docs](https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html#subagent-model-pool) for details.
- Add an experimental fullscreen TUI mode. Set the `NIGHTHAWK_TUI_FULL_SCREEN=1` environment variable to enable it.
...
- The NightHawk Computer Use plugin now supports Windows x64 — install it from `/plugins`.
- Show a cache-expiry reminder when resuming or sending after a long idle. Set [`cache_expiry_hint`](https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html#tui-toml) to `false` to disable it.
...
- Show browser extension links and activation steps after installing NightHawk WebBridge.
...
- Fix `nighthawk -p` exiting before background tasks and subagents finish.
...
- Add NightHawk Computer Use and NightHawk WebBridge as built-in official marketplace entries in the v2 CLI. Installing from `/plugins` sets up the latest managed runtime and plugin together, reports incomplete manual steps, and supports retrying interrupted setup.
...
- Rename the partner plugin marketplace tab to Curated and clarify that it contains third-party plugins from NightHawk partners.
...
- Run the CLI surfaces (interactive TUI, `nighthawk -p`, `nighthawk acp`, `nighthawk export`, `nighthawk provider`) on the agent-core-v2 engine by default. Set `NIGHTHAWK_LEGACY_FLAG=1` to fall back to the legacy engine.

## 0.32.0 (2026-08-04)

### Features

- Add four hook events: `TurnStarted`, `UserPromptQueued`, `TaskStarted`, and `SessionHeartbeat`. Configure them under `[[hooks]]` in `config.toml` — see [Hooks](https://nighthawk-dev.github.io/nighthawk/en/customization/hooks.html) for details.

### Polish

- Rename two `[loop_control]` keys: `max_retries_per_step` → `max_attempts_per_step` and `max_steps_per_run` → `max_steps_per_turn`; the old keys stop working with a rename warning at startup — see [loop_control](https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html#loop-control).
- Add a `[token_counting]` config section: when a provider doesn't report token usage, switch the context-size display to local estimates — see [token_counting](https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html#token-counting).
...
- Fix sporadic "model is not configured" errors when starting nighthawk web, caused by the background provider-model refresh transiently clearing the model catalog while the first session was being created.
...
- Plugins can contribute system prompt instructions through `systemPrompt` or `systemPromptPath` in `nighthawk.plugin.json`.
...
- Show a quota note after installing official plugins that bill against plan quota (such as NightHawk Datasource).
...
- web: Support defining agents in Markdown files, declaring system prompt, name, description, and tool permissions. [Details](https://nighthawk-dev.github.io/nighthawk/en/customization/agents.html#agent-file-format)
- web: Permanently override the main agent's system prompt with SYSTEM.md. [Details](https://nighthawk-dev.github.io/nighthawk/en/customization/agents.html#overriding-the-main-agent-s-system-prompt-with-system-md)
- web: Globally enable or disable tools across all sessions via config.toml. [Details](https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html#tools)
...
- Remove references to the non-existent `nighthawk resume` command from the scheduled-task tool descriptions.
...
- Run web servers foreground-only end to end: the /web slash command now always starts a new server, and the `nighthawk web kill` / `nighthawk web ps` subcommands are removed — foreground servers stop with Ctrl+C. `nighthawk server kill` remains as a deprecated fallback that only stops servers started by a version before 0.28.0.
...
- **Breaking:**
  - The `nighthawk server` command tree is deprecated; use `nighthawk web` instead.
  - `nighthawk web` now runs in the foreground of the current terminal and opens the browser; stop it with Ctrl+C.
...
- Using an API key for NightHawk coding models now also fetches the latest model list automatically.
...
- Fix `/export-debug-zip` and `nighthawk export` overwriting the previous ZIP on repeated runs for the same session; the default filename now includes a timestamp.
...
- Fix NightHawk-provider models routed through the Anthropic protocol incorrectly showing reasoning effort options.
...
- Replay empty thinking content verbatim instead of substituting a placeholder space on Anthropic-compatible and NightHawk preserved-thinking endpoints.
- Keep legacy migrations idempotent across multiple legacy homes and report damaged or unmapped sessions instead of silently skipping them.
...
- Add a builtin `/check-nighthawk-docs` skill that automatically answers NightHawk product questions with official-docs sources.

### Polish

- Align `nighthawk -p` behavior across engines: `print_background_mode` and `print_max_turns` now apply, and `/goal` runs stay alive until the goal finishes.
- `nighthawk -p` now stays alive by default while background tasks are pending, with no effective wait or turn limit, and feeds each completion back to the agent. Set `print_background_mode = "exit"` or `"drain"` to restore the old exit-after-one-turn behavior.
- `nighthawk -p` background tasks and subagents no longer time out by default (interactive mode is unchanged); restore limits with `[background] bash_task_timeout_s` or `[subagent] timeout_ms`.
- Subagent timeout now defaults to 2 hours everywhere; override with `[subagent] timeout_ms` or `NIGHTHAWK_SUBAGENT_TIMEOUT_MS`.
...
- `nighthawk web` now logs failed requests and key operations so daemon issues are easier to diagnose.
...
- Fix NightHawk sessions getting stuck when preserved-thinking history contains an empty reasoning step.
- Fix built-in tools being unavailable when the model provider becomes ready after the session starts.
- Fix Thinking effort routing: non-NightHawk providers now preserve configured values, while NightHawk models validate runtime selections and fall back safely during model resolution.
...
- Send the nighthawk-cli User-Agent on provider registry (api.json) and model catalog fetches, so registries can identify the client version.
...
- `nighthawk web` now runs on the reworked agent engine by default.
...
- Add a `subagent.timeout_ms` config option (or the `NIGHTHAWK_SUBAGENT_TIMEOUT_MS` env var) to control how long a single subagent may run before timing out; the default is raised from 30 minutes to 2 hours.
- Add a print-mode background policy: set `[background].print_background_mode = "steer"` to keep `nighthawk -p` alive across background-task completions, so the main agent can be steered into follow-up turns.
...
- Keep `nighthawk -p` runs alive after a turn ends while a goal is still active or a cron task is pending, so goal continuations and cron fires run their turns instead of being cut off when the main turn finishes.
...
- Fix `nighthawk -p` runs exiting with code 0 when a turn fails.
...
- Fix `nighthawk -p` abandoning background subagents that start late or run long, so their results reach the main agent.
...
- Preserve prior turns' thinking by default on the Anthropic provider (Claude and NightHawk's Anthropic-compatible mode), matching the NightHawk default. Disable with `[thinking] keep = "off"` or `NIGHTHAWK_MODEL_THINKING_KEEP=off`.
...
- web: Show the NightHawk icon and clearer titles in desktop notifications.
...
- Keep prior reasoning across turns for NightHawk models by default when Thinking is on. Set `[thinking] keep = "off"` to disable.

## 0.22.3 (2026-07-04)

### Bug Fixes

- Wait for background subagents to finish and respond to their results before exiting in `nighthawk -p`, instead of ending the turn early.
...
- Add `--dangerous-bypass-auth` and `--keep-alive` flags to `nighthawk server run`, so the server can run without a token on trusted networks and stay alive past the idle timeout.
...
- Fix `nighthawk upgrade` failing on Windows with a spawn error when installing the new version.
...
- Enrich PATH from the user's login shell at startup, so shell commands find user-installed tools (e.g. Homebrew's `gh`) even when nighthawk was launched without the full profile PATH.
...
- In `nighthawk -p` runs, wait for background subagents to finish before exiting when `background.keep_alive_on_exit` is enabled. Set `keep_alive_on_exit = true` to let concurrent background subagents complete.
...
- Force-exit headless runs (`nighthawk -p`) so a stray ref'd handle left over from the run can't keep a completed run alive until an external timeout, and bound prompt cleanup so a wedged shutdown step can't hang shutdown.
...
- Support the Anthropic-compatible protocol for NightHawk, including video input.
...
- Add `NIGHTHAWK_CUSTOM_HEADERS` for custom outbound LLM request headers, and send the `User-Agent` header to non-NightHawk providers. Set `NIGHTHAWK_CUSTOM_HEADERS` to newline-separated `Name: Value` lines.
...
- Route NightHawk models on the Anthropic-compatible protocol through the beta Messages API.
...
- Plugins now support declaring lifecycle hooks in `nighthawk.plugin.json` to run scripts at specific stages. See [Hooks in Plugins](../customization/plugins.md#hooks-in-plugins).
- `/feedback` now supports attaching diagnostic logs and codebase context.
- Add the `nighthawk update` command, equivalent to `nighthawk upgrade`, for upgrading to the latest version.
- `nighthawk web` adds the `--allowed-host <host>` option to add a specified Host to the DNS-rebinding allowlist; 403 errors now explain how to allow it via `--allowed-host` or `NIGHTHAWK_ALLOWED_HOSTS`, e.g. `nighthawk web --allowed-host example.com`.

### Bug Fixes

- Fix nighthawk server failing to start on Windows after the first run.
...
- Add a `--host` CLI option so `nighthawk web --host` can expose the server to the internet, with hardened token authentication, rate limiting, and other security measures.
- Render LaTeX display math (`$$…$$`) in the web UI.

### Bug Fixes

- Fix a startup crash on Linux caused by an unhandled native clipboard error.
- Fix `nighthawk web` and `/web` failing to start the background server daemon on Windows with `spawn EFTYPE` when the CLI is installed via npm/pnpm or run from source. The official single-binary install script was not affected.
...
- Redesign `/plugins` as a single tabbed panel: **Installed** (manage installed plugins — toggle, remove, MCP, details, reload), **Official** (NightHawk-maintained marketplace plugins), **Third-party** (marketplace plugins from other publishers), and **Custom** (install straight from a GitHub URL, zip URL, or local path). Use `Tab` / `Shift-Tab` to switch tabs.
...
- Added the ability to add extra workspace directories:
  ...
  - Use `nighthawk --add-dir <path>` to add them on startup.
...
- Fix the `nighthawk web` command failing to start in the background.
...
- Add NightHawk Web mode, which you can start with `nighthawk web` or `/web` in the CLI, and continue sessions in a browser chat interface.
...
- Add a built-in `nighthawk vis` command that launches the session visualizer in your browser, pointed at your local sessions. Supports `--port`/`--host`, `--no-open`, and `nighthawk vis <sessionId>` deep-links.
...
- Repair mismatched JSON Schema types emitted by Xcode 26.5 MCP server for NightHawk compatibility.
...
- Fix NightHawk Datasource to use the matching OAuth credentials and service endpoint for the active NightHawk environment.
...
- Add custom color themes. Define your own palette as a JSON file in `~/.nighthawk/themes/`, or generate one with the built-in `/custom-theme` skill command.
...
- Load NightHawk-specific user Skills and global agent instructions from `NIGHTHAWK_HOME` when it is set.

## 0.11.0 (2026-06-05)

### Features

- Add experimental sub-skill discovery gated by the `NIGHTHAWK_EXPERIMENTAL_SUB_SKILL` environment variable. Ships the `sub-skill` builtin bundle (`sub-skill.review`, `sub-skill.consolidate`) for inventorying and consolidating skills into hierarchical groups.
- Add the following environment variables:

  - `NIGHTHAWK_MODEL_TEMPERATURE`, `NIGHTHAWK_MODEL_TOP_P` — sampling parameters applied globally to any `nighthawk` provider (not tied to `NIGHTHAWK_MODEL_NAME`).
  - `NIGHTHAWK_MODEL_THINKING_KEEP` — NightHawk preserved-thinking passthrough (`thinking.keep`), injected only while Thinking is on.
...
- Add the built-in `update-config` skill — you can now have NightHawk edit its own config files.
...
- Add a doctor command for validating NightHawk configuration files.
...
- Set the CLI process title to nighthawk during startup.
...
- Add experimental goal mode for longer tasks that need more than one turn. Turn it on with `NIGHTHAWK_EXPERIMENTAL_GOAL_COMMAND=1` before you start NightHawk.

  Use `/goal <objective>` in the TUI when you want NightHawk to keep working on one task across turns. For example:

  ```text
  /goal Fix the failing checkout test
  ```

  NightHawk shows the goal in the TUI and keeps progress visible while it works. Use `/goal status`, `/goal pause`, `/goal resume`, `/goal cancel`, and `/goal replace <objective>` to manage the goal. This feature is still experimental. Try it and tell us what would make it more useful.
- Add `nighthawk provider` CLI subcommand with `add`, `remove`, `list`, and `catalog list` / `catalog add` actions, so providers from a custom registry (api.json) or the public models.dev catalog can be imported and managed without launching the TUI.
...
- Add a `nighthawk upgrade` command for manually checking and upgrade NightHawk CLI.
...
- Clarify NightHawk Platform API key login labels and prompt details.
...
- Add a `NIGHTHAWK_MODEL_*` environment-variable channel that lets you run NightHawk against a specific model (provider type, base URL, API key, context size, capabilities, and thinking settings) without editing `config.toml`.
- Install plugins directly from GitHub repository URLs, and surface each install's origin and trust level (official, curated, third-party) in the plugin manager.
...
- Restrict plugin trust badges to official plugin CDN URL patterns.
...
- Enhance `nighthawk export` to include more diagnostic information in the manifest.
...
- Exit promptly when the controlling terminal goes away. The TUI now handles `SIGHUP` / `SIGTERM` and stdout/stderr `EIO` / `EPIPE` / `ENOTCONN` errors, preventing leftover `nighthawk` processes that pin a CPU core after the parent shell or multiplexer dies unexpectedly.
...
- Migrate user skills from `~/.kimi/skills/` to `~/.nighthawk/skills/` during the first-launch migration; existing target skills are kept.
...
- Correct the `X-Msh-Platform` header value to `nighthawk_cli`.
...
- Let NightHawk requests use the remaining context window for completion tokens by default while keeping explicit environment limits as hard caps.
...
- When no models are configured, `/model` and the welcome panel now point users to `/login` (for NightHawk) and `/connect` (for other providers).
