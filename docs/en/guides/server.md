...
NightHawk CLI ships with a built-in local server: running `nighthawk web` starts a foreground process that mounts three things at once — the web UI in your browser, a REST API (`/api/v1`), and a WebSocket event stream (`/api/v1/ws`). The web UI lets you use NightHawk in a browser; the REST and WebSocket APIs are for scripts and third-party tools, letting you create sessions, submit prompts, and follow execution from code — all reading and writing the same session data as the TUI and the web UI.

> Make sure NightHawk CLI is installed and ready to use first — either logged in via `/login` (in the TUI, or `nighthawk login`), or with a provider configured in `config.toml`. The server shares the CLI's login state and configuration, so no separate credential is needed for it.
...
nighthawk web --no-open       # run the server only, don't open the browser
nighthawk web --port 58628    # pick a specific bind port
```

The server binds to `127.0.0.1:58627` by default (loopback only). If the port is taken it automatically retries with the next one, so multiple instances can coexist on the same machine; each instance registers under `~/.nighthawk/server/instances/`. The startup banner prints the access URL and the plaintext token:
...
The server runs in the foreground; press `Ctrl-C` for a clean shutdown. For the full option list such as `--host` and `--log-level`, see the [nighthawk command reference](../reference/nighthawk-command.md#nighthawk-web).

## Authentication

Every `/api/*` endpoint requires a bearer token (any request carrying this string is treated as authorized). The token is generated on the first server boot, persisted at `~/.nighthawk/server.token` (file mode 0600), and reused across restarts.
...
- **WebSocket**: clients that can set headers use `Authorization: Bearer`; clients that cannot (such as browsers) pass the subprotocol (a protocol name declared during the WebSocket handshake) `nighthawk.bearer.<token>` instead.

If the token leaks, run `nighthawk web rotate-token`: the new token is written to `server.token` immediately, the old one stops working at once, and running instances pick up the new token without a restart.

If you bind the server to a non-loopback address (`--host`), also set the `NIGHTHAWK_PASSWORD` environment variable as a parallel credential; the server then rate-limits authentication failures automatically.

::: danger
`--dangerous-bypass-auth` disables authentication entirely — anyone who can reach the port can control your sessions, file system, and shell. Only use it on trusted networks or behind your own authenticating proxy. See the [nighthawk command reference](../reference/nighthawk-command.md#nighthawk-web).
:::
...
const ws = new WebSocket('ws://127.0.0.1:58627/api/v1/ws', [
  `nighthawk.bearer.${process.env.TOKEN}`,
]);
...
- [nighthawk command](../reference/nighthawk-command.md#nighthawk-web) — all `nighthawk web` command-line options
