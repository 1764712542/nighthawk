...
[Model Context Protocol（MCP）](https://modelcontextprotocol.io/) 是一个开放协议，让模型可以安全地调用外部进程或服务暴露的工具——例如读取 GitHub issues、查询数据库、操作本地文件系统。NightHawk CLI 作为 MCP client 接入这些外部工具，并把它们与内置工具（`Read`、`Bash`、`Grep` 等）一起暴露给 Agent 使用，行为上没有差异。

## 接入方式

NightHawk CLI 支持三种 MCP server 接入方式：
...
- **用户级**：`~/.nighthawk/mcp.json`（或 `$NIGHTHAWK_HOME/mcp.json`），跨项目共享
- **项目级**：工作目录下的 `.nighthawk/mcp.json`，只对当前仓库生效
...
当 NightHawk 在不受信任的文件夹中发现项目级 MCP server 时，工作区信任提示会显示每个 server 的传输方式和启动目标。提示默认选中 `Don't trust`；请先移动到 `Trust this folder`，核对列出的命令与参数或远程 URL 后，再确认信任。信任文件夹后，该工作区的项目级 MCP server 才会启用。
...
连接超时和单次工具调用超时的默认值都不必逐个 server 设置：`config.toml` 的 `[mcp] startup_timeout_ms` / `[mcp] tool_timeout_ms` 或环境变量 `NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS` / `NIGHTHAWK_MCP_TOOL_TIMEOUT_MS` 可以调整全局默认值，优先级为 server 字段 > 环境变量 > `config.toml` > 内置默认。详见 [配置文件](../configuration/config-files.md#mcp)。

HTTP 与 SSE server 支持通过 `headers` 或 `bearerTokenEnvVar` 提供静态凭证。需要认证时，运行 `/mcp-config login <server-name>` 完成浏览器授权。
...
::: warning 注意
项目级 `.nighthawk/mcp.json` 中的 stdio 条目会在会话启动时执行本地命令，只在你信任的仓库里启用。
:::
