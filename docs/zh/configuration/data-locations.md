...
NightHawk CLI 把所有运行时数据——配置文件、会话历史、登录凭据、诊断日志——集中存放在 `~/.nighthawk/` 下。本页帮你搞清楚每类数据在哪里、用来做什么，以及需要时怎么清理或搬迁。

## 数据根目录

默认数据根是 `~/.nighthawk/`，在不同平台的实际路径：

- macOS：`/Users/<name>/.nighthawk`
- Linux：`/home/<name>/.nighthawk`
- Windows：`C:\Users\<name>\.nighthawk`

如果你需要把数据目录挪到别处（比如用多个独立环境隔离不同项目的配置），设置 `NIGHTHAWK_HOME` 即可：
...
**通用 `.agents` 资源**仍放在真实 OS home 下，以便跨工具共享。例如，用户级通用 Skills 仍位于 `~/.agents/skills/`，而 NightHawk 专属用户级 Skills 会随 `NIGHTHAWK_HOME` 移动到 `$NIGHTHAWK_HOME/skills/`。
:::

## 目录结构

```
$NIGHTHAWK_HOME  （默认 ~/.nighthawk）
├── config.toml             # 用户配置
├── tui.toml                # 终端界面偏好（含自动更新开关）
├── AGENTS.md               # 全局 NightHawk 专属 Agent 指令（可选）
├── mcp.json                # 用户级 MCP server 声明（可选）
├── skills/                 # NightHawk 专属用户级 Skills（可选）
├── plugins/
...
- **`AGENTS.md`**：全局 NightHawk 专属 Agent 指令。该文件会随 `NIGHTHAWK_HOME` 移动；跨工具通用指令仍可放在 `~/.agents/AGENTS.md`。
...
- **`logs/nighthawk.log`**：该会话的诊断日志，只有发生诊断事件时才存在。
- **`tasks/`**：后台任务持久化——`tasks/<task_id>.json` 保存状态/pid/退出码，`tasks/<task_id>/output.log` 保存输出。
- **`cron/`**：定时任务持久化，用 `nighthawk --session` 恢复会话时重新加载到调度器。详见[定时任务](../reference/tools.md#定时任务)。
...
- **`logs/nighthawk.log`**（全局）：记录启动、登录、导出等跨会话事件。
- **`<sessionDir>/logs/nighthawk.log`**（会话级）：记录单个会话内的诊断事件。

报 bug 时，优先用 `nighthawk export` 导出相关会话（详见 [nighthawk 命令](../reference/nighthawk-command.md)）；会话日志默认包含在导出包里。不想分享全局日志时加 `--no-include-global-log`。
...
删除数据根目录（`~/.nighthawk/` 或 `NIGHTHAWK_HOME` 指定路径）可清除所有运行时数据。只需清理部分内容时：

| 需求 | 操作 |
| --- | --- |
| 重置配置 | 删除 `~/.nighthawk/config.toml` |
| 重置终端界面偏好 | 删除 `~/.nighthawk/tui.toml` |
| 清理所有会话 | 删除 `~/.nighthawk/sessions/` 和 `session_index.jsonl` |
| 清理诊断日志 | 删除 `~/.nighthawk/logs/` |
...
| 清空 NightHawk 专属用户级 Skills | 删除 `$NIGHTHAWK_HOME/skills/`（默认为 `~/.nighthawk/skills/`） |

## 下一步

- [配置文件](./config-files.md) — `config.toml` 各字段的完整说明
- [环境变量](./env-vars.md) — `NIGHTHAWK_HOME` 等路径变量的详细用法
