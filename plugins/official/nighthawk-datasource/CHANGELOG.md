...
- Retry once with a credential refreshed by the NightHawk host when the backend rejects the previous access token during rotation.
...
- Use OAuth credentials and datasource endpoints that match the active NightHawk environment.
