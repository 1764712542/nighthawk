#!/usr/bin/env node
// Stdio MCP server for nighthawk-datasource.
//
// Speaks newline-delimited JSON-RPC 2.0 on stdin/stdout per the MCP "stdio"
// transport. Implements the minimal surface the NightHawk host calls:
//   - initialize
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Get the current API documentation for one data source before calling a specific API. For a simple lookup, choose exactly one specialized source; do not inspect fallback or comparison sources unless the user explicitly asks for a cross-source comparison.',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
  ...
  switch (method) {
    case 'initialize':
      return {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'nighthawk-datasource', version: VERSION },
      };
    case 'ping':
      return {};
    case 'tools/list':
...
async function runTool(params) {
  ...
  try {
    ...
    const response = await callDatasourceTool(handler.method, built, trace);
...
function appendTrace(text, trace) {
  ...
  return `${text}\n\n[nighthawk-datasource] ${parts.join(' · ')}`;
}

function resolveNighthawkHome() {
  const explicit = process.env.NIGHTHAWK_HOME?.trim();
  return explicit && explicit.length > 0 ? explicit : path.join(homedir(), '.nighthawk');
}

function datasourceApiUrl() {
  const explicit = process.env.NIGHTHAWK_DATASOURCE_API_URL?.trim();
  if (explicit !== undefined && explicit.length > 0) return explicit;
  return `${nighthawkBaseUrl()}/tools`;
}

function nighthawkBaseUrl() {
  return (process.env.NIGHTHAWK_BASE_URL ?? DEFAULT_NIGHTHAWK_BASE_URL).replace(/\/+$/, '');
}

function nighthawkOAuthHost() {
  return normalizeEndpoint(
    process.env.NIGHTHAWK_OAUTH_HOST ??
      process.env.NIGHTHAWK_OAUTH_HOST ??
      DEFAULT_NIGHTHAWK_OAUTH_HOST,
  );
}
...
function resolveNighthawkCredentialName() {
  const oauthHost = nighthawkOAuthHost();
  const baseUrl = nighthawkBaseUrl();
  if (
    oauthHost === normalizeEndpoint(DEFAULT_NIGHTHAWK_OAUTH_HOST) &&
    baseUrl === DEFAULT_NIGHTHAWK_BASE_URL
  ) {
    return 'nighthawk';
  }

  // Keep this in sync with packages/oauth/src/managed-nighthawk.ts.
  const digest = createHash('sha256')
    .update(JSON.stringify({ oauthHost, baseUrl }))
    .digest('hex')
    .slice(0, 16);
  return `nighthawk-env-${digest}`;
}

async function loadAccessToken() {
  const nighthawkHome = resolveNighthawkHome();
  const credentialsFile = path.join(
    nighthawkHome,
    'credentials',
    `${resolveNighthawkCredentialName()}.json`,
  );
  let parsed;
  try {
    ...
    if (isNotFound(err)) {
      throw new Error(
        `NightHawk credentials file not found: ${credentialsFile}\nRun /login in NightHawk first.`,
      );
    }
    if (err instanceof SyntaxError) {
      throw new Error(`Failed to parse NightHawk credentials file: ${err.message}`);
    }
    throw err;
  }

  if (!isRecord(parsed)) {
    throw new Error(`Invalid NightHawk credentials file: ${credentialsFile}`);
  }
  const token = typeof parsed.access_token === 'string' ? parsed.access_token : '';
  if (token.length === 0) {
    throw new Error('NightHawk credentials do not contain access_token. Run /login again.');
  }
  return { nighthawkHome, token };
}

async function callDatasourceTool(method, params, trace = {}) {
  const { nighthawkHome, token: initialToken } = await loadAccessToken();
  ...
  try {
    const request = async (accessToken) => {
      const response = await fetch(API_URL, {
        method: 'POST',
        headers: await buildHeaders(nighthawkHome, accessToken, toolCallId),
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error('NightHawk access_token was rejected. Run /login again and retry.');
      }
...
async function buildHeaders(nighthawkHome, token, toolCallId) {
  return {
    Authorization: `Bearer ${token}`,
    'Content-Type': 'application/json',
    'X-Msh-Tool-Call-Id': toolCallId,
    'X-Msh-Platform': asciiHeader(process.env.NIGHTHAWK_MSH_PLATFORM ?? 'nighthawk-cli'),
    'X-Msh-Version': asciiHeader(process.env.NIGHTHAWK_MSH_VERSION ?? VERSION),
    'X-Msh-Device-Name': asciiHeader(process.env.NIGHTHAWK_MSH_DEVICE_NAME ?? hostname()),
    'X-Msh-Device-Model': asciiHeader(process.env.NIGHTHAWK_MSH_DEVICE_MODEL ?? deviceModel()),
    'X-Msh-Os-Version': asciiHeader(process.env.NIGHTHAWK_MSH_OS_VERSION ?? release()),
    'X-Msh-Device-Id': asciiHeader(process.env.NIGHTHAWK_MSH_DEVICE_ID ?? (await createDeviceId(nighthawkHome))),
    'User-Agent': `nighthawk-datasource/${VERSION}`,
  };
}

async function createDeviceId(nighthawkHome) {
  const deviceIdPath = path.join(nighthawkHome, 'device_id');
  try {
  ...
  const id = randomUUID();
  try {
    await mkdir(nighthawkHome, { recursive: true, mode: 0o700 });
