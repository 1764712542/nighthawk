...
const NIGHTHAWK_FILE_SCHEME = 'nighthawk-file://';

/** The daemon upload reference behind a `nighthawk-file://<fileId>` url. */
export interface DaemonFileRef {
  readonly fileId: string;
}

/**
 * Parse a `nighthawk-file://<fileId>` url — the mirror of the engine's
...
export function parseDaemonFileRef(url: string): DaemonFileRef | undefined {
  if (!url.startsWith(NIGHTHAWK_FILE_SCHEME)) return undefined;
  const rest = url.slice(NIGHTHAWK_FILE_SCHEME.length);
  const queryAt = rest.indexOf('?');
  const fileId = queryAt === -1 ? rest : rest.slice(0, queryAt);
  return fileId.length > 0 ? { fileId } : undefined;
}

/** The daemon upload id behind a `nighthawk-file://<fileId>` url. */
 ...
 * from the part type, the file id from the `nighthawk-file://` url. This is the
