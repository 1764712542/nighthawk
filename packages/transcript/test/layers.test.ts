...
describe('groupMessagesIntoSnapshot (cold path)', () => {
  ...
  it('maps persisted nighthawk-file media refs to attachments', () => {
    const snapshot = groupMessagesIntoSnapshot([
      {
        role: 'user',
        content: [
          {
            type: 'video_url',
            videoUrl: { url: 'nighthawk-file://file_1' },
      ...
      },
      {
        ...
        content: [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'nighthawk-file://file_2?path=%2Fcache%2Fshot.png' },
          } as HistoryContentPart,
        ],
        toolCalls: [],
        origin: { kind: 'user' },
    ...
    const snapshot = groupMessagesIntoSnapshot([
      {
        ...
        content: [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'nighthawk-file://file_4?path=%2Fcache%2Fshot.png' },
          } as HistoryContentPart,
        ],
        toolCalls: [],
  ...
  it('keeps a nighthawk-file ref as a nameless attachment and inline tags as user text', () => {
    const snapshot = snapshotOf(
      { type: 'text', text: 'open <image path="/tmp/other.png"></image> please' },
      {
        type: 'image_url',
        imageUrl: { url: 'nighthawk-file://file_3' },
      } as HistoryContentPart,
    );
  ...
  it('keeps a legacy tag+ref pair as prompt text plus the ref-derived attachment', () => {
    const snapshot = snapshotOf(
      ...
      {
        type: 'image_url',
        imageUrl: { url: 'nighthawk-file://file_5?path=%2Fcache%2Fshot.png' },
      } as HistoryContentPart,
    );
