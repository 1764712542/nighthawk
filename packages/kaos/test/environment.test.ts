 ...
 *   - Windows resolves Git Bash via `NIGHTHAWK_SHELL_PATH`, `git.exe` on PATH
...
describe('detectEnvironment', () => {
  ...
  it('uses NIGHTHAWK_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { NIGHTHAWK_SHELL_PATH: 'D:\\custom\\bash.exe' },
