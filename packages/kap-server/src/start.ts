...
export interface ServerStartOptions {
   ...
   * reads (e.g. an embedded server pinning `NIGHTHAWK_REGION_MARKER=off`)
   ...
   * Defaults to the `NIGHTHAWK_PLUGIN_MARKETPLACE_URL` env var, then the
   * production catalog.
   */
   ...
   * and frozen at boot; omit to let the UI fall back to `<workspace dir> | NightHawk`.
   */
   ...
   * (User-Agent + `X-Msh-*` via `createNighthawkDefaultHeaders`), and the session
  ...
  /**
   * Directory of the built NightHawk web UI (`dist-web`). When set, `GET /` and the
   ...
   * endpoint unintentionally; the CLI's `nighthawk web` host passes true.
   */
...
export async function startServer(opts: ServerStartOptions): Promise<RunningServer> {
  ...
  const { app: core } = bootstrap(
    {
      ...
      args: {
        requestHeaders: createNighthawkDefaultHeaders({ homeDir, ...opts.hostIdentity }),
  ...
  if (exposureClass !== 'loopback') {
    ...
    if (!passwordConfigured) {
      logger.warn(
        { host, exposureClass },
        'binding non-loopback host with token-only auth (no NIGHTHAWK_PASSWORD) — the bearer token printed in the startup banner is the only credential protecting this server',
      );
    }
  }
  ...
  async function registerOpenApi(): Promise<void> {
    ...
    await app.register(swagger, {
      openapi: {
        info: {
          title: 'NightHawk Server API',
          description:
            'REST API for the NightHawk local server. All JSON responses are wrapped in a uniform envelope `{ code, msg, data, request_id }`.',
  ...
  await registerApiV1Routes(app, core, {
    ...
    pluginMarketplaceIsDefault:
      ...
      (process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'] === undefined ||
        process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_FROM_DEV_SERVER'] === '1'),
    onShutdown: () => {
 ...
 * busy port may be a sibling nighthawk instance. The `port + 1` walk then serves
