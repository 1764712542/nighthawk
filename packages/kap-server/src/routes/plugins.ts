...
> = {
  'nighthawk-cu': { capabilityId: 'nighthawk-cu', wiringPluginIds: ['nighthawk-cu', 'nighthawk-cu-win'] },
  'nighthawk-cu-win': { capabilityId: 'nighthawk-cu', wiringPluginIds: ['nighthawk-cu', 'nighthawk-cu-win'] },
  'nighthawk-webbridge': { capabilityId: 'nighthawk-webbridge', wiringPluginIds: ['nighthawk-webbridge'] },
};

function orderedWiringPluginIds(ids: readonly string[]): readonly string[] {
  if (process.platform === 'win32' && process.arch === 'x64' && ids.includes('nighthawk-cu-win')) {
    return ['nighthawk-cu-win', ...ids.filter((id) => id !== 'nighthawk-cu-win')];
  }
  return ids;
}
