...
export function registerSessionExportRoute(
  ...
  core: Scope,
  options: { readonly hostIdentity: NighthawkHostIdentity },
): void {
  ...
  const route = defineRoute(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        tempDir = await mkdtemp(join(tmpdir(), `nighthawk-session-export-${safeSessionId}-`));
        if (aborted) {
          await cleanup();
          return;
        }
        ...
        const sent = response
          ...
          .header(
            'content-disposition',
            `attachment; filename="nighthawk-session-${safeSessionId}.zip"`,
          )
