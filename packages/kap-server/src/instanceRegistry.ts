...
import { resolveNighthawkHome } from '@nighthawk/agent-core-v2';
import { ulid } from 'ulid';

/** Default cadence for refreshing `heartbeat_at`. */
export const HEARTBEAT_INTERVAL_MS = 15_000;

export const DEFAULT_SERVER_DIR = join(resolveNighthawkHome(), 'server');
...
export interface InstanceRegistryOptions {
  /** Directory holding `<serverId>.json` files. Defaults to `<NIGHTHAWK_HOME>/server/instances`. */
...
/** Resolve the instances directory for a given home (or the default nighthawk home). */
 ...
 * talk to (e.g. the CLI's `server ps/kill` and the `nighthawk web` spawner).
 */
