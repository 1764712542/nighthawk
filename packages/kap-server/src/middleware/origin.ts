...
const CORS_ALLOW_HEADERS = 'Content-Type, Authorization, X-NightHawk-Client-Id, X-NightHawk-Client-Name, X-NightHawk-Client-Version, X-NightHawk-Client-Ui-Mode';
...
/**
 * Parse `NIGHTHAWK_CORS_ORIGINS` into an allowlist.
 *
...
export function parseCorsOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['NIGHTHAWK_CORS_ORIGINS'];
  if (raw === undefined) {
    return [];
  }
  return raw
