...
import { ISessionApprovalService, ensureMainAgent, getLiveSessionById } from '@nighthawk/agent-core-v2';
...
describe('server-v2 /api/v1/sessions/{sid}/approvals', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-approvals-'));
