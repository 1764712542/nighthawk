...
describe('formatHostErrorMessage', () => {
  it('includes the rejected host and allow guidance', () => {
    expect(formatHostErrorMessage('APP.Example.com:443')).toBe(
      "Invalid Host header: app.example.com; allow this host with NIGHTHAWK_ALLOWED_HOSTS=app.example.com or 'nighthawk web --allowed-host app.example.com'.",
    );
  });
});
...
describe('parseAllowedHosts', () => {
  it('splits, trims, and drops empties', () => {
    expect(parseAllowedHosts({ NIGHTHAWK_ALLOWED_HOSTS: ' a, .b.example.com, ' })).toEqual([
      'a',
      '.b.example.com',
    ]);
  });
...
describe('isHostCheckDisabled', () => {
  it('is true when set to "1"', () => {
    expect(isHostCheckDisabled({ NIGHTHAWK_DISABLE_HOST_CHECK: '1' })).toBe(true);
  });
...
describe('createHostCheck (onRequest hook)', () => {
  ...
  it('rejects a disallowed Host with the 40301 envelope', async () => {
    ...
    expect(body['msg']).toBe(
      "Invalid Host header: evil.example.test; allow this host with NIGHTHAWK_ALLOWED_HOSTS=evil.example.test or 'nighthawk web --allowed-host evil.example.test'.",
    );
...
describe('startServer allowedHosts — env + option merge', () => {
  const ENV_KEY = 'NIGHTHAWK_ALLOWED_HOSTS';
  ...
  it('appends opts.allowedHosts to NIGHTHAWK_ALLOWED_HOSTS instead of replacing it', async () => {
    process.env[ENV_KEY] = 'env-only.example.com';
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-host-merge-'));
