...
} from '@nighthawk/agent-core-v2/app/kosongConfig/modelsDevUpstream';
...
describe('server-v2 /api/v1 catalog browse + import endpoints', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-catalog-'));
  ...
  it('never touches the global default pointers on import', async () => {
    ...
    expect(status).toBe(201);
    const config = await readConfigToml();
    expect(config['default_provider']).toBe('nighthawk');
