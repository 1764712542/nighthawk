...
import { Emitter } from '@nighthawk/agent-core-v2/_base/event';
...
describe('SessionEventBroadcaster', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-broadcaster-test-'));
    sessions = new Map();
    eventBus = new FakeEventBus();
  ...
  it.each(['prompt.steered', 'prompt.queued'])(
    ...
    async (type) => {
      ...
      main.bus.emit(
        agentEvent(type, {
          ...
          content: [
            ...
            {
              type: 'image_url',
              imageUrl: { url: 'nighthawk-file://f_img1?path=%2Fabs%2Fsession%2Fmedia%2Ff_img1.png' },
            },
          ],
        }),
      );
      ...
      expect(JSON.stringify(live!.payload)).not.toContain('nighthawk-file://');
  ...
  it('getSnapshotState returns the live subagent roster until the next main turn starts', async () => {
    ...
    main.bus.emit(
      agentEvent('subagent.spawned', {
        subagentId: 'agent-1',
        subagentName: 'nighthawk-subagent',
  ...
  describe('global fan-out to unsubscribed connections', () => {
    ...
    it('fans out event.plugin.changed and event.capability.changed to global targets', async () => {
      ...
      eventBus.emit({
        ...
        payload: {
          capability_id: 'nighthawk-webbridge',
    ...
    it('drops malformed event.capability.changed payloads', async () => {
      ...
      eventBus.emit({
        type: 'event.capability.changed',
        payload: { capability_id: 'nighthawk-cu' },
      });

      eventBus.emit({
        type: 'event.capability.changed',
        payload: { capability_id: 'nighthawk-cu', install: { running: false } },
      });
  ...
  it('replays only the allowlisted agent events while keeping the global sequence', async () => {
    ...
    const dir2 = await mkdtemp(join(tmpdir(), 'nighthawk-broadcaster-test-'));
