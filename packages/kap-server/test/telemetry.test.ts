...
describe('server telemetry', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-telemetry-'));
  });
  ...
  async function bootCore(
    ...
    const resolvedEnv = env ?? {
      ...process.env,
      NIGHTHAWK_DISABLE_TELEMETRY: undefined,
    };
    if (toml !== undefined) {
  ...
  it('attaches the cloud appender by default and persists the device id', async () => {
    ...
    expect(readNighthawkDeviceId(home as string)).not.toBeNull();
  ...
  it.each(['1', 'true', 't', 'yes', 'y', ' TRUE '])(
    'keeps the null appender when NIGHTHAWK_DISABLE_TELEMETRY=%s',
    async (value) => {
      const app = await bootCore(undefined, {
        ...process.env,
        NIGHTHAWK_DISABLE_TELEMETRY: value,
      });
      const telemetry = await initializeServerTelemetry(app, home as string);
      expect(telemetry.appender).toBeUndefined();
      expect(readNighthawkDeviceId(home as string)).toBeNull();
      await shutdownServerTelemetry(telemetry);
    },
  );
});
