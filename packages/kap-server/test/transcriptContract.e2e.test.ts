...
async function subscribeTranscript(server: RunningServer, sid: string): Promise<TranscriptChannel> {
  const ws = new WebSocket(`ws://127.0.0.1:${server.port}/api/v1/ws`, [`nighthawk.bearer.${bearerToken(server)}`]);
  const frames: any[] = [];
  const ops: any[] = [];
  let resetFrame: any;
  ws.on('message', (data) => {
...
describe('transcript contract e2e', () => {
  ...
  async function boot(routes: readonly LlmRoute[]): Promise<void> {
    ...
    home = await mkdtemp(join(tmpdir(), 'nighthawk-transcript-contract-'));
