...
describe('parseCorsOrigins', () => {
  it('splits, trims, and drops empties', () => {
    expect(
      parseCorsOrigins({
        NIGHTHAWK_CORS_ORIGINS: ' https://a.example.com, https://b.example.com, ',
      }),
...
describe('createOriginHook (onRequest hook)', () => {
  ...
  it('reflects Access-Control-Request-Headers in Allow-Headers for an allowed origin', async () => {
    const res = await app.inject({
      ...
      headers: {
        ...
        'access-control-request-headers': 'x-request-id, x-legacy-client-version, authorization',
      },
    });
    expect(res.statusCode).toBe(204);
    expect(res.headers['access-control-allow-origin']).toBe('https://foo.example.com');
    expect(res.headers['access-control-allow-headers']).toBe(
      'x-request-id, x-legacy-client-version, authorization',
    );
  });
  ...
  it('falls back to CORS_ALLOW_HEADERS for non-preflight responses', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.headers['access-control-allow-headers']).toBe(
      'Content-Type, Authorization, X-NightHawk-Client-Id, X-NightHawk-Client-Name, X-NightHawk-Client-Version, X-NightHawk-Client-Ui-Mode',
    );
  });
});
