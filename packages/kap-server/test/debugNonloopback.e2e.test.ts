...
beforeEach(() => {
  prevPassword = process.env['NIGHTHAWK_PASSWORD'];
});
...
afterEach(async () => {
  ...
  if (prevPassword === undefined) {
    delete process.env['NIGHTHAWK_PASSWORD'];
  } else {
    process.env['NIGHTHAWK_PASSWORD'] = prevPassword;
  }
});

async function tmpHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-v2-debug-loopback-'));
...
describe('debug endpoints are not exposed on a non-loopback bind', () => {
  it('returns 404 for /api/v1/debug/* on a 0.0.0.0 bind even when requested', async () => {
    process.env['NIGHTHAWK_PASSWORD'] = 'test-pw';
