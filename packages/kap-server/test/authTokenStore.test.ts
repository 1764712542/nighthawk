...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'nighthawk-server-v2-auth-token-'));
});
...
describe('password', () => {
  it('resolvePasswordHash returns undefined when env is unset or empty', async () => {
    ...
    expect(await resolvePasswordHash({ NIGHTHAWK_PASSWORD: '' })).toBeUndefined();
  });

  it('hashes a set password with bcrypt and verifies correctly', async () => {
    const passwordHash = await resolvePasswordHash({
      NIGHTHAWK_PASSWORD: 'correct-horse-battery-staple',
    });
