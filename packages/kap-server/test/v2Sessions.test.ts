...
import {
  ...
  type FsPullRequest,
  IGitService,
} from '@nighthawk/agent-core-v2/app/git/git';
...
describe('server /api/v2/sessions', () => {
  ...
  beforeEach(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-sessions-list-'));
...
describe('server /api/v2/sessions batch archive/restore', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-sessions-batch-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
