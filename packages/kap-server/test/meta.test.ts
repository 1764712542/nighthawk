...
import { getFeatureRecipes } from '@nighthawk/agent-core-v2/features/featureRegistry';
...
describe('/api/v1/meta experimental_flags', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_TOOL_SELECT', undefined);
  });
  ...
  async function boot(toml?: string): Promise<string> {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-meta-'));
    if (toml !== undefined) {
  ...
  it('reflects a flag enabled via its NIGHTHAWK_EXPERIMENTAL_* env var', async () => {
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_TOOL_SELECT', '1');
    const base = await boot();
    const flags = await getMetaFlags(base);
    expect(flags['tool-select']).toBe(true);
  });
...
describe('/api/v1/meta web_title', () => {
  ...
  async function bootWithWebTitle(
    ...
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-meta-title-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
...
describe('/api/v1/meta features', () => {
  ...
  async function boot(): Promise<string> {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-meta-features-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
