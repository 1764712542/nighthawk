...
const MANAGED_TOML = [
  ...
  '',
  '[models."managed:nighthawk/kimi-k2"]',
...
describe('server-v2 /api/v1 provider write endpoints', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-provider-write-'));
    process.env['NIGHTHAWK_MODEL_CATALOG_REFRESH_ON_START'] = '0';
    process.env['NIGHTHAWK_MODEL_CATALOG_REFRESH_INTERVAL_MS'] = '0';
  });
  ...
  afterEach(async () => {
    ...
    delete process.env['NIGHTHAWK_MODEL_CATALOG_REFRESH_ON_START'];
    delete process.env['NIGHTHAWK_MODEL_CATALOG_REFRESH_INTERVAL_MS'];
  });
  ...
  it('rejects a duplicate provider id with 40921', async () => {
    ...
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['nighthawk', 'openai']);
  });
  ...
  it('accepts a Unicode provider id (Chinese + space)', async () => {
    ...
    expect(status).toBe(201);
    expect(body.code).toBe(0);
    expect(body.data.id).toBe('测试 NightHawk');

    const onDisk = await readConfigToml();
    expect(onDisk['providers']).toMatchObject({ '测试 NightHawk': { type: 'openai' } });
    expect(onDisk['models']).toMatchObject({
      '测试 NightHawk/gpt-4.1': { provider: '测试 NightHawk', model: 'gpt-4.1' },
    });
  });
  ...
  it('deletes a provider and its model aliases, keeping unrelated defaults', async () => {
    ...
    const onDisk = await readConfigToml();
    expect(onDisk['providers']).toEqual({ nighthawk: { type: 'nighthawk', api_key: 'sk-test' } });
    expect(onDisk['models']).toEqual({
      k2: { provider: 'nighthawk', model: 'kimi-k2', max_context_size: 131072 },
    });
    expect(onDisk['default_model']).toBe('k2');

    const providers = await getJson<{ items: Array<{ id: string }> }>('/api/v1/providers');
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['nighthawk']);
  ...
  it('rejects deleting an OAuth-managed provider with 40003', async () => {
    ...
    const { body } = await deleteJson<unknown>('/api/v1/providers/managed%3Anighthawk');
    expect(body?.code).toBe(40003);
    expect(body?.msg).toContain('/oauth/logout');

    const providers = await getJson<{ items: Array<{ id: string }> }>('/api/v1/providers');
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['managed:nighthawk']);
  });
  ...
  it('replaces a provider, keeping the stored api_key and rebuilding its aliases', async () => {
    ...
    const onDisk = await readConfigToml();
    expect(onDisk['providers']).toEqual({
      nighthawk: { type: 'nighthawk', api_key: 'sk-test' },
      openai: {
        type: 'openai',
        api_key: 'sk-openai',
        base_url: 'https://api.openai.example/v1',
  ...
  it('rejects replacing an OAuth-managed provider with 40003', async () => {
    ...
    const { body } = await putJson<unknown>(
      '/api/v1/providers/managed%3Anighthawk',
      REPLACE_BODY,
    );
    expect(body.code).toBe(40003);
    expect(body.msg).toContain('/oauth/logout');
    ...
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['managed:nighthawk']);
    const models = await getJson<{ items: Array<{ model: string }> }>('/api/v1/models');
    expect(models.body.data.items.map((m) => m.model)).toEqual(['managed:nighthawk/kimi-k2']);
  });
