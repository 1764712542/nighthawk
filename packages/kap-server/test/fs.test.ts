...
import { IModelCatalog, IWorkspaceInstanceManager } from '@nighthawk/agent-core-v2';
import { HostFileSystem } from '@nighthawk/agent-core-v2/os/backends/node-local/hostFsService';
...
describe('server-v2 /api/v1 fs routes', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-home-'));
    work = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-work-'));
  ...
  it('fs:read uses the selected non-local runtime and mapped workspace root', async () => {
    ...
    const remote = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-remote-'));
  ...
  it('rejects reads and downloads that escape the workspace through a symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-outside-'));
    try {
  ...
  it('serves fs actions when the session cwd itself goes through a symlink', async () => {
    const link = join(tmpdir(), `nighthawk-server-v2-fs-cwd-link-${process.pid}`);
  ...
  it('fs:suggest merges candidates across roots with relative paths for the primary root only', async () => {
    const extra = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-extra-'));
    try {
