...
describe('server-v2 /api/v1 prompts', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-prompts-'));
  ...
  it('submits a bundled skill prompt through the skills field', async () => {
    ...
    const submitted = await call<PromptItemWire>('POST', `/api/v1/sessions/${id}/prompts`, {
      ...
      skills: [{ name: 'update-config' }, { name: 'check-nighthawk-docs' }],
    });
    expect(submitted.body.code).toBe(0);
    expect(submitted.body.data.prompt_id).toMatch(/^msg_/);
    ...
    expect(bundled?.origin).toMatchObject({
      kind: 'user',
      skillActivations: [{ skillName: 'update-config' }, { skillName: 'check-nighthawk-docs' }],
    });
  ...
  it('carries an uploaded video into the prompt as an internal nighthawk-file reference', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
  ...
  it('carries a compressed uploaded image into the prompt as an internal nighthawk-file reference', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
    ...
    expect(JSON.stringify(content)).not.toContain('nighthawk-file://');
  ...
  it('carries an uncompressed uploaded image into the prompt as an internal nighthawk-file reference', async () => {
    const id = await createSession(home as string);
    await createMainAgent(id);
  ...
  it('accepts a stored session-media reference after the transient upload is deleted', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(replayedMessage!.content).toContainEqual({
        type: 'image_url',
        imageUrl: {
          url: `nighthawk-file://${uploaded.id}`,
          id: uploaded.id,
        },
      });
    });
    ...
    try {
      ...
      await vi.waitFor(() => {
        ...
        expect(message!.content).toContainEqual({
          type: 'image_url',
          imageUrl: { url: `nighthawk-file://${uploaded.id}` },
        });
      });
