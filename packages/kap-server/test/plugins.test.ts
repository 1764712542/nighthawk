...
const CATALOG = {
  ...
  plugins: [
    ...
    },
    {
      ...
      source: 'https://cdn.example.test/nighthawk-webbridge.zip',
    },
    {
      id: 'nighthawk-cu',
      displayName: 'NightHawk Computer Use',
      source: 'https://cdn.example.test/nighthawk-cu.zip',
    },
    {
...
describe('server-v2 /api/v1 plugins', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-plugins-'));
  ...
  async function makePluginDir(id: string, version: string): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), `nighthawk-test-plugin-${id}-`));
  ...
  it('fans out event.plugin.changed over WS on install and remove', async () => {
    const ws = new WebSocket(`${base.replace('http', 'ws')}/api/v1/ws`, [
      `nighthawk.bearer.${bearerToken(server!)}`,
    ]);
    const types: string[] = [];
    try {
  ...
  it('maps client-fixable install input errors to 4xx, never 50001', async () => {
    ...
    const noManifest = await mkdtemp(join(tmpdir(), 'nighthawk-no-manifest-'));
  ...
  it('serves the marketplace catalog merged with live install state', async () => {
    ...
    expect(before.body.data.entries.map((e) => [e.id, e.tier])).toEqual([
      ...
      ['nighthawk-webbridge', 'third-party'],
      ['nighthawk-cu', 'third-party'],
    ...
    expect(
      before.body.data.entries.find((e) => e.id === 'nighthawk-webbridge')?.capabilityId,
    ).toBeUndefined();
  ...
  it('treats the dev marketplace server as the default catalog', async () => {
    await server?.close();
    vi.stubEnv('NIGHTHAWK_PLUGIN_MARKETPLACE_URL', CATALOG_URL);
    vi.stubEnv('NIGHTHAWK_PLUGIN_MARKETPLACE_FROM_DEV_SERVER', '1');
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
    ...
    expect(body.data.entries.find((e) => e.id === 'nighthawk-webbridge')?.capabilityId).toBe(
      'nighthawk-webbridge',
    );
    ...
    if (!cuSupported) {
      expect(after0.body.data.entries.find((e) => e.id === 'nighthawk-cu')).toBeUndefined();
      return;
    }

    const winSource = await makePluginDir('nighthawk-cu-win', '0.5.4');
    await call('POST', '/api/v1/plugins', { source: winSource });
    const after = await call<{
      entries: { id: string; capabilityId?: string; installed?: { version?: string } }[];
    }>('GET', '/api/v1/plugins/marketplace');
    const cu = after.body.data.entries.find((e) => e.id === 'nighthawk-cu');
    expect(cu?.capabilityId).toBe('nighthawk-cu');
    expect(cu?.installed?.version).toBe('0.5.4');

    const staleSource = await makePluginDir('nighthawk-cu', '0.1.0');
    ...
    expect(both.body.data.entries.find((e) => e.id === 'nighthawk-cu')?.installed?.version).toBe(
      expected,
    );
  });
  ...
  it('reads a local marketplace catalog from disk (plain path or file://)', async () => {
    await server?.close();
    const catalogDir = await mkdtemp(join(tmpdir(), 'nighthawk-local-catalog-'));
  ...
  it('falls back to the source-checkout catalog when the remote is unreachable', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string | URL, init?: RequestInit) => {
        ...
        if (url === 'https://cdn.jsdelivr.net/gh/nighthawk-dev/nighthawk@main/plugins/cdn/marketplace.json') {
          throw new Error('offline');
        }
        return realFetch(url as never, init);
      }),
    );
    vi.stubEnv('NIGHTHAWK_PLUGIN_MARKETPLACE_URL', undefined as unknown as string);
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
    ...
    expect(body.code).toBe(0);
    const datasource = body.data.entries.find((e) => e.id === 'nighthawk-datasource');
    expect(datasource?.source.startsWith('http')).toBe(false);
    expect(datasource?.source.endsWith(join('plugins', 'official', 'nighthawk-datasource'))).toBe(true);
    const webbridge = body.data.entries.find((e) => e.id === 'nighthawk-webbridge');
    expect(webbridge?.capabilityId).toBe('nighthawk-webbridge');
    const cuSupported = process.platform === 'darwin' || (process.platform === 'win32' && process.arch === 'x64');
    const cu = body.data.entries.find((e) => e.id === 'nighthawk-cu');
    if (!cuSupported) {
    ...
    expect(cu?.capabilityId).toBe('nighthawk-cu');
    expect(cu?.source).toBe('capability:nighthawk-cu');
    expect(cu?.displayName).toBe('NightHawk Computer Use');

    const cuSource = await makePluginDir('nighthawk-cu', '0.5.8');
    await call('POST', '/api/v1/plugins', { source: cuSource });
    const after = await call<{
      entries: { id: string; installed?: { version?: string; enabled: boolean } }[];
    }>('GET', '/api/v1/plugins/marketplace');
    expect(after.body.data.entries.find((e) => e.id === 'nighthawk-cu')?.installed).toEqual({
      version: '0.5.8',
      enabled: true,
    });
  });
  ...
  it('expands ~ in local catalog paths like the CLI loader', async () => {
    await server?.close();
    const fakeHome = await mkdtemp(join(tmpdir(), 'nighthawk-tilde-home-'));
