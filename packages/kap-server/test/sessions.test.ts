...
import { TurnStarted } from '@nighthawk/agent-core-v2/agent/loop/turnEvents';
import { sessionWarningsResponseSchema } from '@nighthawk/agent-core-v2/app/sessionLegacy/sessionProtocol';
...
describe('server-v2 /api/v1/sessions', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-sessions-'));
  ...
  it('downloads a ZIP with the supplied Web log and cleans up its temporary directory', async () => {
    ...
    expect(res.headers.get('content-disposition')).toBe(
      `attachment; filename="nighthawk-session-${id}.zip"`,
    );
    ...
    const manifest = JSON.parse(entries.get('manifest.json')?.toString('utf8') ?? 'null') as {
      sessionId: string;
      nighthawkVersion: string;
      desktopVersion?: string;
      webLogPath?: string;
    };
    expect(entries.get('logs/nighthawk-web.jsonl')?.toString('utf8')).toBe(webLog);
    expect(manifest).toMatchObject({
      sessionId: id,
      nighthawkVersion: TEST_HOST_IDENTITY.version,
      webLogPath: 'logs/nighthawk-web.jsonl',
    });
  ...
  it('bundles the on-disk desktop app log when the desktop flag is set', async () => {
    ...
    await writeFile(
      join(home as string, 'logs', 'nighthawk-desktop.log'),
    ...
    expect(entries.get('logs/nighthawk-desktop.log')?.toString('utf8')).toBe(
      '2026-07-27T00:00:00.000Z INFO  [renderer] hello\n',
    );
    expect(manifest.desktopLogPath).toBe('logs/nighthawk-desktop.log');
    expect(manifest.nighthawkVersion).toBe(TEST_HOST_IDENTITY.version);
  ...
  it('generates and persists a title through the public REST path', async () => {
    ...
    const oauth: IOAuthServiceType = {
      ...
      status: async () => ({ loggedIn: true, provider: 'managed:nighthawk' }),
...
async function listExportTempDirs(sessionId: string): Promise<string[]> {
  const prefix = `nighthawk-session-export-${sessionId}-`;
...
describe('server-v2 /api/v1/sessions status context window', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-status-'));
    await writeFile(
...
describe('server-v2 /api/v1/sessions (minidb read model)', () => {
  ...
  const READ_MODEL_ENV = 'NIGHTHAWK_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL';
  ...
  beforeEach(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-sessions-rm-'));
