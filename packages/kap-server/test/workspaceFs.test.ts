...
describe('server-v2 /api/v1 fs folder picker', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-'));
    instancesDir = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fs-instances-'));
...
describe('server-v2 /api/v1 fs:mkdir', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fsmkdir-'));
    instancesDir = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fsmkdir-instances-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
...
describe('server-v2 /api/v1 fs:content', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fscontent-'));
    instancesDir = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-fscontent-instances-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
