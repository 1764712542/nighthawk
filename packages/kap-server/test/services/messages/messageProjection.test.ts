...
describe('toProtocolMessage', () => {
  ...
  it('projects a daemon-ref image part to a session_media source', () => {
    const msg: ContextMessage = {
      ...
      content: [
        ...
        { type: 'image_url', imageUrl: { url: 'nighthawk-file://file_9?path=%2Fcache%2Fpic.png' } },
      ],
      toolCalls: [],
    };
  ...
  it('projects a nighthawk-file video reference to a structured file source without leaking the path', () => {
    const msg: ContextMessage = {
      role: 'user',
      content: [
        { type: 'video_url', videoUrl: { url: 'nighthawk-file://file_9?path=%2Fcache%2Fclip.mp4' } },
      ],
      toolCalls: [],
    };
