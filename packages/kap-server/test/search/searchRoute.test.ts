...
process.env['NIGHTHAWK_EXPERIMENTAL_SEARCH_WORKER'] = '1';

import { ISessionIndex, type SessionSummary } from '@nighthawk/agent-core-v2';
...
describe('server-v2 /api/v1/search', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-search-'));
...
describe('server-v2 session routes with the global search DB unavailable', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-search-down-'));
