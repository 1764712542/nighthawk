...
import { MiniDb } from '@nighthawk/minidb';
import { TranscriptStore, type TranscriptOperation } from '@nighthawk/transcript';
...
describe('GlobalSearchService', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-kap-search-'));
  });
  ...
  describe('lifecycle drain correctness (plan 13)', () => {
    ...
    it('drainGlobalSearchDisposals also waits for disposals registered while it was draining (review #21)', async () => {
      ...
      const homeB = await mkdtemp(join(tmpdir(), 'nighthawk-kap-search-drain-'));
      try {
...
describe('search worker host (stage 4)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-kap-search-worker-'));
  });
...
describe('search lifecycle diagnostics (stage 5)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-kap-search-lifecycle-'));
  });
...
describe('baseline: synthetic corpus', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-kap-search-baseline-'));
  });
