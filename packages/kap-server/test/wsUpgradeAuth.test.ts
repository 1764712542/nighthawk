...
describe('WS upgrade auth', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-ws-upgrade-auth-'));
  ...
  describe('/api/v1/ws', () => {
    ...
    it('accepts a valid bearer subprotocol and echoes it', async () => {
      const { ws, firstFrame } = await openConn(url(), {
        protocols: [`nighthawk.bearer.${TOKEN}`],
      });
      sockets.push(ws);
      expect(ws.protocol).toBe(`nighthawk.bearer.${TOKEN}`);
    ...
    it('rejects a wrong bearer token', async () => {
      await expectRejected(url(), { protocols: ['nighthawk.bearer.wrong'] });
    });
  ...
  it('rejects upgrades to a non-WS path', async () => {
    ...
    await expectRejected(badUrl, { protocols: [`nighthawk.bearer.${TOKEN}`] });
  });
});
