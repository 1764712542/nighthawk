...
describe('WsConnectionV1 global target registration', () => {
  ...
  it('opts only nighthawk-inspect connections into the event.di.* debug feed on client_hello', async () => {
    const socket = new FakeSocket();
    ...
    socket.emit(
      'message',
      JSON.stringify({ type: 'client_hello', id: 'h1', payload: { client_id: 'nighthawk-web' } }),
    );
    ...
    socket.emit(
      ...
      JSON.stringify({
        type: 'client_hello',
        id: 'h2',
        payload: { client_id: 'nighthawk-inspect' },
      }),
    );
