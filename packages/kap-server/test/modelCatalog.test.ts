...
describe('server-v2 /api/v1 model/provider catalog', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-model-catalog-'));
  ...
  it('lists providers and returns a single provider by id', async () => {
    ...
    const single = await getJson<unknown>('/api/v1/providers/nighthawk');
  ...
  it('refreshes OAuth provider models through POST /providers:refresh_oauth', async () => {
    const refreshOAuthProviderModels = vi.fn(async () => ({
      changed: [
        { provider_id: 'managed:nighthawk', provider_name: 'NightHawk', added: 1, removed: 0 },
      ],
      unchanged: [],
      failed: [],
    }));
  ...
  it('refreshes all provider models through POST /providers:refresh', async () => {
    const refreshProviderModels = vi.fn(async () => ({
      changed: [
        { provider_id: 'managed:nighthawk', provider_name: 'NightHawk', added: 2, removed: 1 },
      ],
      unchanged: ['nighthawk-cn'],
      failed: [],
    }));
  ...
  it('refreshes a single provider through POST /providers/{id}:refresh', async () => {
    ...
    const { status, body } = await postJson('/api/v1/providers/managed%3Anighthawk:refresh', {});
    expect(status).toBe(200);
    expect(body.code).toBe(0);
    expect(refreshProviderModels).toHaveBeenCalledWith({ providerId: 'managed:nighthawk' });
  });
