...
describe('nighthawkUserInfoUrl', () => {
  it('follows the NIGHTHAWK_BASE_URL override', () => {
    vi.stubEnv('NIGHTHAWK_BASE_URL', 'https://gw.example.com/');
    expect(nighthawkUserInfoUrl()).toBe('https://gw.example.com/me');
  });
});
...
describe('fetchManagedUserInfo', () => {
  ...
  it('falls back to the local profile hint on an empty 404 body', async () => {
    ...
    expect(result.message).toBe('Profile endpoint not available. Try NightHawk For Coding.');
  });
