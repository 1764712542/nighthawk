...
function makeTmpDir(): string {
  const dir = join(
    tmpdir(),
    `nighthawk-storage-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe('FileTokenStorage', () => {
  ...
  it('returns undefined when no token exists', async () => {
    expect(await storage.load('nighthawk')).toBeUndefined();
  });

  it('round-trips a token via save/load', async () => {
    const token = sampleToken();
    await storage.save('nighthawk', token);
    const loaded = await storage.load('nighthawk');
  ...
  it('persists tokens in snake_case JSON (Python-compatible)', async () => {
    const token = sampleToken();
    await storage.save('nighthawk', token);
    const raw = readFileSync(join(dir, 'nighthawk.json'), 'utf-8');
  ...
  it.skipIf(process.platform === 'win32')('writes the credentials file with mode 0600', async () => {
    await storage.save('nighthawk', sampleToken());
    const stat = statSync(join(dir, 'nighthawk.json'));
  ...
  it('remove() deletes the file; load() then returns undefined', async () => {
    await storage.save('nighthawk', sampleToken());
    await storage.remove('nighthawk');
    expect(await storage.load('nighthawk')).toBeUndefined();
  });
  ...
  it('save() overwrites an existing token atomically', async () => {
    await storage.save('nighthawk', sampleToken({ accessToken: 'first' }));
    await storage.save('nighthawk', sampleToken({ accessToken: 'second' }));
    const loaded = await storage.load('nighthawk');
    expect(loaded?.accessToken).toBe('second');
  });

  it('load() returns undefined on corrupt JSON (does not throw)', async () => {
    const file = join(dir, 'nighthawk.json');
  ...
  it('load() tolerates missing numeric fields by defaulting to 0', async () => {
    ...
    chmodSync(file, 0o600);
    const token = await storage.load('nighthawk');
  ...
  it('list() returns all stored token names', async () => {
    ...
    expect(names.toSorted()).toEqual(['nighthawk', 'other-provider']);
  });

  it('list() ignores non-JSON files in the credentials dir', async () => {
    await storage.save('nighthawk', sampleToken());
    writeFileSync(join(dir, 'nighthawk.lock'), 'lock', 'utf-8');
    writeFileSync(join(dir, 'readme.txt'), 'readme', 'utf-8');
    const names = await storage.list();
    expect(names).toEqual(['nighthawk']);
  });
  ...
  it.skipIf(process.platform === 'win32')('creates the credentials dir with mode 0700 if missing', async () => {
    ...
    await s.save('nighthawk', sampleToken());
  ...
  it('save() leaves no *.tmp.* sibling once the rename completes', async () => {
    ...
    const tmps = entries.filter((name) => name.startsWith('nighthawk.json.tmp.'));
    expect(tmps).toEqual([]);
    expect(entries).toContain('nighthawk.json');
  });
