...
import type { ManagedNighthawkConfigShape, ManagedNighthawkModelAlias } from './managed-nighthawk';

export type { ManagedNighthawkConfigShape };
 ...
 * mirrors these literal values. `nighthawk` is included because the api.json schema
 ...
 * by editing `~/.nighthawk/config.toml`.
 */
 ...
 *
 * `userAgent` identifies the host product (e.g. `nighthawk-cli/1.2.3`); when
 ...
 * provider object via `ManagedNighthawkProviderConfig`'s index signature so the
...
export function applyCustomRegistryProvider(
  ...
  for (const [modelKey, model] of Object.entries(entry.models)) {
    ...
    const remoteAlias: ManagedNighthawkModelAlias = {
