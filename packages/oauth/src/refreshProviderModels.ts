...
import {
  ...
  type ManagedNighthawkModelAlias,
  type ManagedNighthawkOAuthRef,
} from './managed-nighthawk';
import { isManagedNighthawkBaseUrl } from './managed-usage';
import {
 ...
 * {@link ManagedNighthawkConfigShape} (the oauth package's own minimal config shape)
 * rather than the SDK's full `NighthawkConfig`, so this module has no dependency on
 * `agent-core` / the SDK and can be reused by both the CLI and the daemon.
 */
export interface RefreshProviderHost {
  getConfig(): Promise<ManagedNighthawkConfigShape>;
  removeProvider(providerId: string): Promise<ManagedNighthawkConfigShape>;
  setConfig(patch: ManagedNighthawkConfigShape): Promise<ManagedNighthawkConfigShape>;
  resolveOAuthToken(providerName: string, oauthRef?: ManagedNighthawkOAuthRef): Promise<string>;
  /**
   * Product User-Agent sent on custom-registry (api.json) fetches, e.g.
   * `nighthawk-cli/1.2.3`. When omitted the fetch falls back to the runtime
...
interface ProviderView {
  ...
  readonly oauth?: ManagedNighthawkOAuthRef;
  readonly source?: unknown;
  readonly env?: unknown;
}

/**
 * Mirrors the runtime credential resolution for `type: 'nighthawk'` providers
 * (`providerApiKey` in agent-core's provider-manager): the inline `apiKey`
 * wins, with `env.NIGHTHAWK_API_KEY` as the documented config-file fallback.
 */
function resolveProviderApiKey(provider: ProviderView): string | undefined {
  ...
  if (isRecord(provider.env)) {
    const fromEnv = provider.env['NIGHTHAWK_API_KEY'];
...
function readModel(
  config: ManagedNighthawkConfigShape,
  alias: string,
): ManagedNighthawkModelAlias | undefined {
  const model = config.models?.[alias];
  if (model === undefined) return undefined;
  return model as ManagedNighthawkModelAlias;
}
...
function providerAliasKeys(config: ManagedNighthawkConfigShape, providerId: string): Set<string> {
  const keys = new Set<string>();
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    if ((raw as ManagedNighthawkModelAlias).provider === providerId) keys.add(alias);
  }
  return keys;
}
...
function generatedProviderAliasKeys(
  ...
  const keys = new Set<string>();
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    const model = raw as ManagedNighthawkModelAlias;
...
interface ProviderModelSnapshot {
  ...
  readonly model: ManagedNighthawkModelAlias;
}
...
function providerModelsEqual(
  config: ManagedNighthawkConfigShape,
  nextConfig: ManagedNighthawkConfigShape,
  providerId: string,
  aliasKeys: ReadonlySet<string>,
): boolean {
  return (
...
function providerConfigSnapshot(config: ManagedNighthawkConfigShape, providerId: string): string {
...
): Record<string, ManagedNighthawkModelAlias> {
  const preserved: Record<string, ManagedNighthawkModelAlias> = {};
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    const model = raw as ManagedNighthawkModelAlias;
...
function restoreProviderAliases(
  config: ManagedNighthawkConfigShape,
  aliases: Record<string, ManagedNighthawkModelAlias>,
): void {
...
function clampDanglingDefault(config: ManagedNighthawkConfigShape): void {
 ...
 *
 *  1. Managed NightHawk (OAuth) — `GET /models` against the runtime endpoint.
 *  2. Open platforms (nighthawk-cn, nighthawk-ai, …) — platform catalog fetch.
 *  2.5. Managed-endpoint API-key providers — hand-written `type: 'nighthawk'`
 *     providers (including a hand-written `managed:nighthawk` without an oauth
 *     ref) whose baseUrl is exactly the managed NightHawk endpoint; refreshed
...
export async function refreshProviderModels(
  ...
  // ---------------------------------------------------------------------------
  // 1. Managed NightHawk (OAuth)
  // ---------------------------------------------------------------------------
  const managedProvider = readProvider(config, NIGHTHAWK_PROVIDER_NAME);
  const managedWanted = targetId === undefined || targetId === NIGHTHAWK_PROVIDER_NAME;
  if (
    managedWanted &&
    managedProvider !== undefined &&
    managedProvider.type === 'nighthawk' &&
    ...
    try {
      ...
      const accessToken = await host.resolveOAuthToken(NIGHTHAWK_PROVIDER_NAME, auth.oauthRef);
      ...
      if (models.length > 0) {
        ...
        applyManagedNighthawkConfig(next, {
          models,
          baseUrl: auth.baseUrl,
        ...
        restoreProviderAliases(
          next,
          preserveUserProviderAliases(config, NIGHTHAWK_PROVIDER_NAME, refreshedAliasKeys),
        );
        ...
        if (providerModelsEqual(config, next, NIGHTHAWK_PROVIDER_NAME, refreshedAliasKeys)) {
          unchanged.push(NIGHTHAWK_PROVIDER_NAME);
        } else {
          ...
          await host.removeProvider(NIGHTHAWK_PROVIDER_NAME);
  ...
  // handles a hand-written `managed:nighthawk` that carries an API key instead
  ...
  // ---------------------------------------------------------------------------
  // 2. Open Platforms (nighthawk-cn, nighthawk-ai, …)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // A hand-written `type: 'nighthawk'` provider whose baseUrl is exactly the managed
  // NightHawk endpoint, carrying an API key (inline or via `env.NIGHTHAWK_API_KEY`)
  ...
  for (const providerId of Object.keys(config.providers)) {
    ...
    try {
      ...
      // A hand-written `managed:nighthawk` shares the OAuth branch's
      // `nighthawk/` alias prefix so the two shapes merge cleanly if the user
      // later logs in via OAuth; ordinary providers use their own id.
      const aliasPrefix =
        providerId === NIGHTHAWK_PROVIDER_NAME ? `${NIGHTHAWK_PLATFORM_ID}/` : `${providerId}/`;
      const next = structuredClone(config);
  ...
  >();
  for (const providerId of Object.keys(config.providers)) {
    if (providerId === NIGHTHAWK_PROVIDER_NAME) continue;
    if (isOpenPlatformId(providerId)) continue;
    const provider = readProvider(config, providerId);
    if (provider === undefined) continue;
