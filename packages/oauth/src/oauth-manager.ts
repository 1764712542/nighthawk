...
export interface OAuthManagerOptions {
   ...
   *
   * **Production callers MUST pass this explicitly** (NighthawkCoreClient /
   ...
   * falls back to `process.env.NIGHTHAWK_HOME` so multi-process test
   * harnesses don't need to thread the dir through every fixture. In
   * production the fallback is inert. Windows platforms and
   * `process.env.NIGHTHAWK_DISABLE_OAUTH_LOCK === '1'` always skip; the
...
export class OAuthManager {
  ...
  constructor(options: OAuthManagerOptions) {
    ...
    // The `NIGHTHAWK_HOME` fallback MUST stay test-only so production
    ...
    const envConfigDir =
      process.env['NODE_ENV'] === 'test' ? process.env['NIGHTHAWK_HOME'] : undefined;
  ...
  private resolveLockTarget(): string | undefined {
    ...
    if (process.env['NIGHTHAWK_DISABLE_OAUTH_LOCK'] === '1') return undefined;
