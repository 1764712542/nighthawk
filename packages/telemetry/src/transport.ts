...
export interface AsyncTransportOptions {
  ...
  /** Static endpoint, or a resolver invoked per flush so an in-process
      reconfiguration takes effect without rebuilding the transport. When the
      resolved endpoint is empty or `undefined`, events are dropped silently —
      telemetry stays off unless a host configures an endpoint. */
...
export class AsyncTransport {
  ...
  async send(events: readonly EnrichedTelemetryEvent[], signal?: AbortSignal): Promise<void> {
    ...
    try {
      for (let attempt = 0; attempt <= this.retryBackoffsMs.length; attempt++) {
        try {
          await this.sendHttp(endpoint, payload, signal);
          return;
        } catch (error) {
  ...
  async retryDiskEvents(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      try {
        await this.sendHttp(endpoint, payload);
        unlinkSync(path);
      } catch (error) {
  ...
  private async sendHttp(
    ...
    const response = await this.post(endpoint, payload, headers, signal);
    if (response.status === 401 && headers['Authorization'] !== undefined) {
      delete headers['Authorization'];
      const retry = await this.post(endpoint, payload, headers, signal);
