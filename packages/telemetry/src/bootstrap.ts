...
export const TELEMETRY_DISABLE_ENV = 'NIGHTHAWK_DISABLE_TELEMETRY';
...
export interface TelemetryBootstrapOptions {
  ...
  /**
   * Endpoint supplied by the composition root — this package stays
   * dependency-free and owns no default endpoint. A resolver is invoked per
   * flush so an in-process reconfiguration takes effect without
   * re-initialization. When the resolved endpoint is empty or `undefined`,
   * events are dropped silently: telemetry stays off unless a host configures
   * an endpoint.
   */
