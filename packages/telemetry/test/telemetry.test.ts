...
async function tempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-telemetry-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('AsyncTransport', () => {
  ...
  it('drops events silently when no endpoint is configured', async () => {
    const fetchImpl = vi.fn(async (_url: string | URL, _init?: RequestInit) =>
      Promise.resolve(new Response('', { status: 200 })),
    );
    ...
    expect(existsSync(join(homeDir, 'telemetry'))).toBe(false);
  });
...
describe('telemetry bootstrap', () => {
  it('matches the NIGHTHAWK_DISABLE_TELEMETRY true-value semantics', () => {
    expect(isTelemetryDisabledByEnv({ NIGHTHAWK_DISABLE_TELEMETRY: '1' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ NIGHTHAWK_DISABLE_TELEMETRY: 'yes' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ NIGHTHAWK_DISABLE_TELEMETRY: '0' })).toBe(false);
    expect(isTelemetryDisabledByEnv({ NIGHTHAWK_DISABLE_TELEMETRY: 'false' })).toBe(false);
  });

  it('disables the singleton without attaching a sink when opted out', async () => {
    const fetchImpl = vi.fn(async () => new Response('', { status: 200 }));
    vi.stubGlobal('fetch', fetchImpl);
    const saved = process.env['NIGHTHAWK_DISABLE_TELEMETRY'];
    try {
      process.env['NIGHTHAWK_DISABLE_TELEMETRY'] = 'true';
      initializeTelemetry({
        homeDir: await tempHome(),
        deviceId: 'dev',
        appName: 'nighthawk-cli',
      ...
      if (saved === undefined) delete process.env['NIGHTHAWK_DISABLE_TELEMETRY'];
      else process.env['NIGHTHAWK_DISABLE_TELEMETRY'] = saved;
    }
