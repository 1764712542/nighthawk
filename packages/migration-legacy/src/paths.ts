...
export const migratedMarker = (src: string): string => join(src, '.migrated-to-nighthawk');

// Target (~/.nighthawk/) paths
...
export const skipMarker = (tgt: string): string => join(tgt, '.skip-migration-from-legacy-cli');
...
export const siblingConfigToml = (tgt: string): string =>
  join(tgt, 'config.migrated-from-legacy-cli.toml');
export const siblingTuiToml = (tgt: string): string =>
  join(tgt, 'tui.migrated-from-legacy-cli.toml');
export const siblingMcpJson = (tgt: string): string =>
  join(tgt, 'mcp.migrated-from-legacy-cli.json');
