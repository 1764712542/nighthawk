...
/**
 * Copy the user's legacy skills tree (~/.kimi/skills/) into nighthawk's
 * default user skills root (~/.nighthawk/skills/). Granularity is one
...
export async function migrateSkillsStep(input: SkillsStepInput): Promise<SkillsStepResult> {
  ...
  for (const name of entries) {
    ...
    // in it — touching it earlier would fail when ~/.nighthawk/skills is
