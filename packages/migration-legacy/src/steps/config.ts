...
import { FLAG_DEFINITIONS } from '@nighthawk/agent-core/flags/registry';
...
// nighthawk's tui.toml `theme` enum (mirrors apps/nighthawk TuiThemeSchema).
...
// The config.toml top-level keys nighthawk understands, derived from the live
// NighthawkConfigSchema so the set tracks nighthawk automatically. `raw` is internal
// — never migrate it. `providers` / `models` / `hooks` are filtered per-entry,
// not via this set.
const SUPPORTED_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(
  Object.keys(NighthawkConfigSchema.shape)
...
export interface ConfigStepResult {
   ...
   * content that landed in `config.migrated-from-legacy-cli.toml` instead of
...
/** True when the kimi-cli provider entry validates against nighthawk's schema. */
...
/** True when the kimi-cli model entry validates against nighthawk's schema. */
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 2) Models — keep only those nighthawk's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries nighthawk's HookDefSchema accepts. kimi-cli
  //     and nighthawk share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field nighthawk does not know).
  ...
  // 4) Build the migrated top-level — only keys nighthawk's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // kimi-cli's `default_yolo` maps to nighthawk's `default_permission_mode`.
  ...
  // 4b) Drop any supported top-level key whose VALUE nighthawk's config
  ...
  for (;;) {
    const result = NighthawkConfigSchema.safeParse(transformTomlData(migratedTop));
  ...
  // In sibling mode the source hooks land in `config.migrated-from-legacy-cli.toml`,
