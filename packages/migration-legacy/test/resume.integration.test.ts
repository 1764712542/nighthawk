 ...
 *     all from `@nighthawk/agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `getReadyAgent()`), from
 *     `@nighthawk/agent-core`; `localKaos` from `@nighthawk/kaos`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@nighthawk/agent-core/session/store/index';
import { Session, type SDKSessionRPC } from '@nighthawk/agent-core';
import { LocalKaos } from '@nighthawk/kaos';
