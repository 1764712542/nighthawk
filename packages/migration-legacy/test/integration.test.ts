...
const MARKER_PATH = join(SOURCE_HOME, '.migrated-to-nighthawk');
...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not write a completed marker when legacy session data remains unreadable', async () => {
    ...
    try {
      ...
      await expect(readFile(join(src, '.migrated-to-nighthawk'), 'utf-8')).rejects.toThrow();
    } finally {
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'nighthawk.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in nighthawk.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('nighthawk.json');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });
