 ...
 * Run: pnpm --filter @nighthawk/migration-legacy test -- marker.test.ts
 */
...
describe('marker', () => {
  ...
  it('suppresses migration for both targets when appending a run to a legacy marker', async () => {
    ...
    await writeFile(
      join(dir, '.migrated-to-nighthawk'),
      JSON.stringify({
        version: 1,
  ...
  it('readMarker returns undefined when file is corrupt', async () => {
    await writeFile(join(dir, '.migrated-to-nighthawk'), 'not-json', 'utf-8');
    expect(await readMarker(dir)).toBeUndefined();
  });
  ...
  it('suppresses migration when an old marker has no target path', async () => {
    await writeFile(join(dir, '.migrated-to-nighthawk'), '{}', 'utf-8');
  ...
  it('suppresses migration when Windows drive letters differ only by case', async () => {
    await writeFile(
      join(dir, '.migrated-to-nighthawk'),
      JSON.stringify({ target_path: 'C:\\Users\\Example\\.nighthawk' }),
      'utf-8',
    );
    ...
    expect(
      shouldSuppressMigration({
        sourceHome: dir,
        targetHome: 'c:\\Users\\Example\\.nighthawk',
      }),
    ).toBe(true);
  });
});
