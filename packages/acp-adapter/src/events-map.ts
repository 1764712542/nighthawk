...
/**
 * Heuristic map from a NightHawk tool's `name` to ACP {@link ToolKind}.
 *
...
/**
 * Translate the nighthawk TodoList display block into an ACP `plan`
 * session update.
 *
 ...
 *   - We default `priority` to `'medium'` because the nighthawk
 ...
 *
 * The nighthawk TodoList tool publishes both a structured display
 ...
 * notification. The NightHawk adapter sits at the SDK layer, beneath the
 * TUI slash-command registry (`apps/nighthawk/src/tui/commands/`),
