...
//      legacy-cli `acp/server.py:77-96`.
//
 ...
 *
 * Optional inputs:
 *  - `env`: extra env vars forwarded to the spawned `nighthawk login`
 *    subprocess (e.g. `{ NIGHTHAWK_HOME: '/tmp/sandbox' }` for tests).
...
export function buildTerminalAuthMethod(
  ...
  const method: AuthMethod = {
    id: 'login',
    type: 'terminal',
    name: 'Login with NightHawk account',
    ...
    // `--login` flag on `nighthawk acp` pivots into the login flow before
    // touching stdio.
    args: ['--login'],
    env: { ...env },
  };
  if (opts.legacyCommand !== undefined && opts.legacyCommand.length > 0) {
    (method as AuthMethod & { _meta: { 'terminal-auth': unknown } })._meta = {
      'terminal-auth': {
        ...
        // `<command> login` runs the top-level `nighthawk login` subcommand,
        // skipping the `acp` subprocess entirely. Same behaviour the
        // `legacy-cli` Python reference advertises.
        args: ['login'],
        env: { ...env },
      },
    };
