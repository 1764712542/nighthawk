...
/**
 * Inline auth gate — moved out of `NighthawkAuthFacade.hasUsableToken()` so
 ...
 */
async function harnessIsAuthed(harness: NighthawkHarness): Promise<boolean> {
...
async function hasUsableConfiguredDefaultModel(harness: NighthawkHarness): Promise<boolean> {
  if (typeof harness.getConfig !== 'function') return false;
  let config: NighthawkConfig;
  try {
...
function providerForAlias(config: NighthawkConfig, alias: ModelAlias): ProviderConfig | undefined {
...
function providerHasNonOAuthCredentials(provider: ProviderConfig): boolean {
  ...
  switch (provider.type) {
    ...
    case 'nighthawk':
      return hasProviderValue(provider, 'NIGHTHAWK_API_KEY');
    case 'google-genai':
 ...
 * into {@link NighthawkHarness}; refuses methods that are not yet wired with a
...
export class AcpServer implements Agent {
  ...
  constructor(
    ...
    opts?: {
       ...
       * exploration of `~/.nighthawk/skills/`.
       */
  ...
  async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    // `docs/{zh,en}/reference/nighthawk-acp.md` and the changeset for the
   ...
   * {@link NighthawkHarness.listSessions} (optionally filtered by `cwd` —
   ...
   * the nighthawk app's registry. Phase 11 keeps it as a no-op stub.
   */
  async extMethod(
    method: string,
   ...
   * `NighthawkHarness` shapes that only stub `auth.status` + `createSession`.
 ...
 * {@link NighthawkHarness.close} so an editor terminating the agent process
...
/**
 * Project a NightHawk SDK {@link SessionSummary} into the ACP
