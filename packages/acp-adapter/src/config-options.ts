 ...
 * reference's `_expand_llm_models` (`legacy-cli/src/kimi_cli/acp/server.py:441-468`)
 ...
 * capable model (e.g. `nighthawkr`) to a non-thinking one (e.g.
 * `nighthawk-plain`) causes the next `config_option_update` to omit the
