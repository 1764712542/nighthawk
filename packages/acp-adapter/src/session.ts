...
/**
 * Adapter-side wrapper around a {@link Session} from the NightHawk node SDK.
 *
...
export class AcpSession {
   ...
   * `~/.nighthawk/skills/` and incurred permission prompts. Defaults
  ...
  constructor(
     ...
     */
    private readonly harness?: NighthawkHarness,
    /**
   ...
   *
   * ACP allows model identifiers like `"nighthawk-k2,thinking"` where the
   * `,thinking` suffix signals "always-thinking" mode (mirrors the
   * Python ref's `_ModelIDConv.from_acp_model_id` at
   * `legacy-cli/src/kimi_cli/acp/server.py:425-433`). Phase 15 decoupled
   ...
   *
   * Wire semantics:
   *  - `'nighthawk-v2'`           → setModel('nighthawk-v2'); requested thinking
   *    effort unchanged (the engine re-resolves it against the new
   *    model; see below).
   *  - `'nighthawk-v2,thinking'`  → setModel('nighthawk-v2') + setThinking(<default
   ...
   * injecting the `<nighthawk-skill-loaded>` block — see
...
/**
 * Map a NightHawk SDK error (raw `Error`, `NighthawkError`, or `NighthawkErrorPayload`)
 ...
 *
 * The legacy-cli Python reference performs the same mapping at
 * `legacy-cli/src/kimi_cli/acp/session.py:218-247`; this is the TS port.
 */
 ...
 * `apps/nighthawk/src/tui/commands/parse.ts` and `resolve.ts` to
...
/**
 * Inspect a {@link NighthawkErrorPayload} (as carried on `turn.ended`
 ...
 *  - `auth.login_required` — NightHawk Platform / OAuth login flow needed.
 *  - `provider.auth_error` — the downstream provider rejected the
 *    request with a 401 (the node SDK lifts these into `NighthawkError`
 *    at `nighthawk-model-provider.ts:99-103`).
 */
 ...
 *  - A `NighthawkError` instance with a recognized `code` field.
