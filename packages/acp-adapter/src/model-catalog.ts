 ...
 *
 * Used to live inside `@nighthawk/nighthawk-sdk` as
 * `NighthawkHarness.listAvailableModels()`; moved here so the SDK keeps a
 ...
 *      allow-list (mirrors `legacy-cli/src/kimi_cli/llm.py:derive_model_capabilities`).
 *
...
import { effectiveModelAlias, type ProviderType } from '@nighthawk/agent-core';
import type { NighthawkHarness, ModelAlias } from '@nighthawk/nighthawk-sdk';
 ...
 */
const TOGGLEABLE_THINKING_MODELS = new Set(['nighthawk', 'nighthawk']);
...
export async function listModelsFromHarness(
  ...
  let config: Awaited<ReturnType<NighthawkHarness['getConfig']>>;
  try {
 ...
 * `effectiveModelAlias` only applies to non-NightHawk providers, and then only to
 ...
 * while managed NightHawk models and clearly non-Claude names keep only their
