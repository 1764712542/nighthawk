...
function makeHarnessWithModels(
  entries: ReadonlyArray<{
    ...
    protocol?: 'anthropic';
    providerType?: 'anthropic' | 'nighthawk' | 'openai';
    supportEfforts?: readonly string[];
    defaultEffort?: string;
  }>,
): { harness: NighthawkHarness; getConfig: ReturnType<typeof vi.fn> } {
  ...
  return { harness: { getConfig } as unknown as NighthawkHarness, getConfig };
}
...
describe('buildModelOption', () => {
  ...
  it('treats `currentValue` as the bare base model id — Phase 15 keeps the snapshot suffix-free', () => {
    const models: readonly AcpModelEntry[] = [
      { id: 'nighthawk-v2', name: 'NightHawk v2', thinkingSupported: true, supportEfforts: [], defaultThinkingEffort: 'on' },
    ];

    const option = buildModelOption(models, 'nighthawk-v2');
    if (option.type !== 'select') {
      throw new Error('expected a SessionConfigSelect option');
    }
    expect(option.currentValue).toBe('nighthawk-v2');
    expect(option.options.map((o) => ('value' in o ? o.value : ''))).toEqual(['nighthawk-v2']);
  });
...
describe('buildSessionConfigOptions', () => {
  it('composes [model, thinking, mode] when current model supports thinking and calls getConfig exactly once', async () => {
    // `nighthawk` is on the toggleable allow-list so its derived
    // thinkingSupported is true even without explicit capabilities.
    const { harness, getConfig } = makeHarnessWithModels([
      { id: 'nighthawkr', model: 'nighthawk', displayName: 'NightHawkr' },
    ]);

    const result = await buildSessionConfigOptions(harness, 'nighthawkr', 'off', 'default');
    ...
    if (result[0]!.type === 'select') {
      expect(result[0]!.currentValue).toBe('nighthawkr');
    }
  ...
  it('hides the thinking control for an unknown model on a NightHawk provider using the Anthropic protocol', async () => {
    const { harness } = makeHarnessWithModels([
      {
        id: 'custom',
        model: 'custom-anthropic-model',
  ...
  it('omits the thinking toggle when current model is non-thinking-supported', async () => {
    const { harness } = makeHarnessWithModels([
      { id: 'nighthawkr', model: 'nighthawk', displayName: 'NightHawkr' },
      { id: 'nighthawk-plain', model: 'qwen-2.5-coder', displayName: 'NightHawk Plain' },
    ]);

    const result = await buildSessionConfigOptions(harness, 'nighthawk-plain', 'off', 'default');
  ...
  it('reflects the thinking toggle currentValue from the explicit argument', async () => {
    ...
    const result = await buildSessionConfigOptions(harness, 'nighthawkr', 'on', 'default');
    ...
    const { harness } = makeHarnessWithModels([
      {
        ...
        displayName: 'NightHawk Deep',
        capabilities: ['thinking', 'always_thinking'],
      },
    ]);

    const result = await buildSessionConfigOptions(harness, 'nighthawk-deep', 'off', 'default');
  ...
  it('handles missing getConfig (partial-stub harness) by suppressing the toggle and shipping an empty model picker', async () => {
    const harness = {} as unknown as NighthawkHarness;
