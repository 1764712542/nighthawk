...
import { NighthawkError, ErrorCodes, type Event, type NighthawkHarness, type Session } from '@nighthawk/nighthawk-sdk';
...
function makeHarness(opts: {
  ...
  return {
    ...
    // these to assemble configOptions. `nighthawkr` opts in to thinking
    // via `capabilities: ['thinking']`; `nighthawk-plain` stays off.
    getConfig: async () => ({
      providers: {},
      defaultModel: 'nighthawkr',
      models: makeModelsMap([
        { id: 'nighthawkr', name: 'NightHawkr', thinkingSupported: true },
        { id: 'nighthawk-plain', name: 'NightHawk Plain', thinkingSupported: false },
      ]),
    }),
...
describe('AcpServer.resumeSession', () => {
  ...
  it('returns configOptions matching the resumed session model + mode + thinking', async () => {
    ...
    // Resume state reports nighthawk-plain (thinking unsupported) so we can
    ...
    expect(modelOpt!.currentValue).toBe('nighthawkr');
  ...
  it('maps SDK session.not_found error to invalidParams (-32602)', async () => {
    const harness = makeHarness({
      hasUsableToken: true,
      resumeError: new NighthawkError(ErrorCodes.SESSION_NOT_FOUND, 'Session "ghost" was not found'),
    });
    const { agentStream, clientStream } = makeInMemoryStreamPair();
    new AgentSideConnection((c) => new AcpServer(harness, c), agentStream);
    const clientConn = new ClientSideConnection((_a) => new CapturingClient(), clientStream);
