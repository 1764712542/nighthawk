...
import type { NighthawkConfig, NighthawkHarness, Session } from '@nighthawk/nighthawk-sdk';
...
function makeHarnessWithToken(hasToken: boolean): NighthawkHarness {
  return {
    auth: {
...
function configuredModelConfig(provider: NighthawkConfig['providers'][string]): NighthawkConfig {
  return {
...
function makeHarnessWithConfig(config: NighthawkConfig, hasToken = false): {
  harness: NighthawkHarness;
  ...
  it('rejects mixed api_key and OAuth provider config without a token', async () => {
    const { harness, createCalls } = makeHarnessWithConfig(
      configuredModelConfig({
        type: 'nighthawk',
        apiKey: 'sk-test',
        oauth: { storage: 'file', key: 'nighthawk' },
      }),
    );
...
describe('AcpServer.authenticate', () => {
  ...
  it('returns void when config credentials are already usable', async () => {
    const { harness } = makeHarnessWithConfig(
      configuredModelConfig({ type: 'nighthawk', apiKey: 'sk-nighthawk' }),
    );
    const { agentStream, clientStream } = makeInMemoryStreamPair();
