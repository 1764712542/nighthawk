/**
 * Test stubs for `NighthawkHarness` interactions that used to live as
...
export const AUTHED_STATUS = {
  providers: [{ providerName: 'nighthawk', hasToken: true }],
} as const;

/** Stub `auth.status()` payload for an unauthenticated harness. */
export const UNAUTHED_STATUS = {
  providers: [{ providerName: 'nighthawk', hasToken: false }],
} as const;
