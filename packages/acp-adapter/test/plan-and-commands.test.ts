...
import type { Event, NighthawkHarness, Session } from '@nighthawk/nighthawk-sdk';
 ...
 *     lives in `apps/nighthawk` and is intentionally out-of-scope for
 *     the adapter; see STATUS for the registry-gap note).
 *   - `plan` session_update derived from the nighthawk TodoList
 *     `display.kind === 'todo_list'` payload attached to a
 *     `tool.call.started` event.
 *
 * Status mapping is the nighthawk TodoStatus → ACP PlanEntryStatus
