...
function makeHarness(handle: FakeSessionHandle): NighthawkHarness {
  return {
    ...
    getConfig: async () => ({
      ...
      models: makeModelsMap([{ id: 'nighthawkr', name: 'NightHawkr', thinkingSupported: false }]),
    }),
...
describe('AcpServer session/unstable_setSessionModel', () => {
  it('forwards modelId to Session.setModel exactly once + emits one config_option_update', async () => {
    ...
    await client.unstable_setSessionModel({ sessionId, modelId: 'nighthawk-v2-something' });

    expect(handle.setModelCalls).toEqual(['nighthawk-v2-something']);
    const updates = capturing.notifications.filter(
      (n) => n.sessionId === sessionId && n.update.sessionUpdate === 'config_option_update',
    );
    expect(updates).toHaveLength(1);
    ...
    if (modelOpt && modelOpt.type === 'select') {
      expect(modelOpt.currentValue).toBe('nighthawk-v2-something');
    }
  });
  ...
  it('splits a `,thinking` suffix into a bare setModel + setThinking(<model default>) call; snapshot model carries the base id', async () => {
    ...
    const harness = {
      ...
      getConfig: async () => ({
        providers: {},
        defaultModel: 'nighthawk-v2-something',
        models: makeModelsMap([
          { id: 'nighthawk-v2-something', name: 'NightHawk v2 something', thinkingSupported: true },
        ]),
      }),
    } as unknown as NighthawkHarness;
    const { client, capturing, sessionId } = await openSession(harness);
    ...
    await client.unstable_setSessionModel({
      sessionId,
      modelId: 'nighthawk-v2-something,thinking',
    });
  ...
  it('rejects unknown sessionId with invalid_params and does not call setModel or emit notifications', async () => {
    ...
    await expect(
      client.unstable_setSessionModel({ sessionId: 'sess-unknown', modelId: 'nighthawk-v2' }),
    ).rejects.toMatchObject({ code: -32602 });
