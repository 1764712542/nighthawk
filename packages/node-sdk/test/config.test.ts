...
import { createNighthawkConfigRpc, createNighthawkHarness, NighthawkError } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-config-'));
  tempDirs.push(dir);
  return dir;
}

const COMPLETE_TOML = `
default_model = "nighthawk"
...
describe('SDK config TOML', () => {
  it('resolves config paths through the config RPC wrapper', async () => {
    ...
    const rpc = createNighthawkConfigRpc();
...
`,
      ...
      details: {
        validationIssues: [
          {
            path: ['models', 'nighthawk', 'maxContextSize'],
          },
        ],
      },
    });
  ...
  it('parses the documented config shape and keeps TUI-only fields in raw', () => {
    ...
    expect(config.defaultModel).toBe('nighthawk');
    ...
    const provider = config.providers['nighthawk'];
    ...
    expect(config.models?.['nighthawk']).toMatchObject({
      provider: 'nighthawk',
      model: 'nighthawk',
  ...
  it('writes typed fields in snake_case and preserves unknown raw sections', async () => {
    ...
    expect(text).toContain('default_model = "nighthawk"');
    ...
    expect(text).toContain('display_name = "NightHawk for Coding"');
...
describe('NighthawkHarness config API', () => {
  it('loads default config when missing and deep-merges setConfig patches from disk', async () => {
    ...
    expect(config.providers['nighthawk']).toMatchObject({
      type: 'nighthawk',
      baseUrl: 'https://api.kimi.com/coding/v1',
      apiKey: 'sk-updated',
      env: { GOOGLE_CLOUD_PROJECT: 'project-1' },
    });
    expect(config.services?.nighthawkSearch?.apiKey).toBe('sk-search-updated');
    expect(config.raw?.['theme']).toBe('dark');
  ...
  it('does not write invalid config patches', async () => {
    ...
    await expect(setInvalidConfig).rejects.toBeInstanceOf(NighthawkError);
    ...
    expect(features).toEqual([
      {
        ...
        surface: 'core',
        env: 'NIGHTHAWK_EXPERIMENTAL_TOOL_SELECT',
      ...
      },
      {
        ...
        surface: 'core',
        env: 'NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL',
        defaultEnabled: false,
        enabled: false,
        source: 'default',
      },
  ...
  it('reloads an active session without closing the SDK session wrapper', async () => {
    ...
    await expect(session.getStatus()).resolves.toMatchObject({ model: 'nighthawk' });
  });
