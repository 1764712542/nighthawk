...
import { createNighthawkHarness, log } from '#/index';
...
const LOG_ENV_KEYS = [
  'NIGHTHAWK_LOG_LEVEL',
  'NIGHTHAWK_LOG_GLOBAL_MAX_BYTES',
  'NIGHTHAWK_LOG_GLOBAL_FILES',
  'NIGHTHAWK_LOG_SESSION_MAX_BYTES',
  'NIGHTHAWK_LOG_SESSION_FILES',
] as const;

beforeEach(async () => {
  process.env['NIGHTHAWK_LOG_LEVEL'] = 'info';
  await __resetRootLoggerForTest();
});

afterEach(async () => {
  await __resetRootLoggerForTest();
  process.env['NIGHTHAWK_LOG_LEVEL'] = 'off';
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    const homeDir = await makeTempDir('nighthawk-log-home-');
    const workDir = await makeTempDir('nighthawk-log-work-');

    const harness = createNighthawkHarness({ identity: TEST_IDENTITY, homeDir });
    ...
    const globalPath = join(homeDir, 'logs', 'nighthawk.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'nighthawk.log');

    // Trigger an export — this flushes both global and session via NighthawkCore
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/nighthawk.log')).toBe(true);
    expect(entries.has('logs/global/nighthawk.log')).toBe(false);
    expect(entries.get('logs/nighthawk.log')!.toString('utf-8')).toContain(
      'session export marker',
    );
    expect(result.manifest.sessionLogPath).toBe('logs/nighthawk.log');
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/nighthawk.log');
  ...
  it('default export works when no session log file exists', async () => {
    ...
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/nighthawk.log')).toBe(false);
  ...
  it('default export includes rotated session log files without requiring active nighthawk.log', async () => {
    const env = snapshotLogEnv();
    process.env['NIGHTHAWK_LOG_LEVEL'] = 'warn';
    process.env['NIGHTHAWK_LOG_SESSION_MAX_BYTES'] = '1024';
    process.env['NIGHTHAWK_LOG_SESSION_FILES'] = '2';
    try {
      const homeDir = await makeTempDir('nighthawk-log-home-');
      const workDir = await makeTempDir('nighthawk-log-work-');
      const harness = createNighthawkHarness({ identity: TEST_IDENTITY, homeDir });
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/nighthawk.log' || entry.startsWith('logs/nighthawk.log.'),
      );
      expect(sessionLogEntries.length).toBeGreaterThan(0);
      expect(sessionLogEntries).toContain('logs/nighthawk.log.1');
      expect(entries.has('logs/global/nighthawk.log')).toBe(false);
      if (entries.has('logs/nighthawk.log')) {
        expect(result.manifest.sessionLogPath).toBe('logs/nighthawk.log');
      } else {
        expect(result.manifest.sessionLogPath).toBeUndefined();
      }
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/nighthawk.log')).toBe(true);
    expect(result.manifest.globalLogPath).toBe('logs/global/nighthawk.log');
    // Global log carries entries that don't have a sessionId routed to a sink.
    expect(entries.get('logs/global/nighthawk.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });

  it('--include-global-log bundles the active root global log path', async () => {
    const firstHome = await makeTempDir('nighthawk-log-home-a-');
    const secondHome = await makeTempDir('nighthawk-log-home-b-');
    const workDir = await makeTempDir('nighthawk-log-work-');
    const first = createNighthawkHarness({ identity: TEST_IDENTITY, homeDir: firstHome });
    const firstSession = await first.createSession({ id: 'ses_first_global_export', workDir });
    const second = createNighthawkHarness({ identity: TEST_IDENTITY, homeDir: secondHome });
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/nighthawk.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'nighthawk.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/nighthawk.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/nighthawk.log')!.toString('utf-8');
  ...
  it('multiple NighthawkHarness constructions in the same process do not throw', async () => {
    const homeDir = await makeTempDir('nighthawk-log-home-');
    expect(() => createNighthawkHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createNighthawkHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => createNighthawkHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
  });
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'nighthawk.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'nighthawk.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    try {
      ...
      expect(logsDir).not.toContain('nighthawk.log');
    } finally {
      restoreLogEnv(env);
    }
  });

  it('NighthawkHarness.close() flushes the global log', async () => {
    const homeDir = await makeTempDir('nighthawk-log-home-');
    const harness = createNighthawkHarness({ identity: TEST_IDENTITY, homeDir });
