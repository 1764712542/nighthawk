...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-export-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('exportSessionDirectory', () => {
  ...
  it('uses a timestamped default output path when outputPath is omitted', async () => {
    ...
    expect(basename(result.zipPath)).toMatch(/^nighthawk-debug-session_-\d{8}-\d{6}\.zip$/);
...
describe('NighthawkHarness.exportSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    await expect(missingExport).rejects.toBeInstanceOf(NighthawkError);
