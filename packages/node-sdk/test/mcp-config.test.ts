 ...
 * Wiring: real NighthawkHarness/Core/filesystem and stdio transport; only the OAuth
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-mcp-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('MCP OAuth facade (host-controlled browser flow)', () => {
  it('merges global and plugin MCP entries without writing plugin entries to mcp.json', async () => {
    ...
    await writeFile(
      join(pluginDir, 'nighthawk.plugin.json'),
      JSON.stringify({
        name: 'status-plugin',
        mcpServers: {
...
function oauthHarness(rpc: OAuthRpc): NighthawkHarness {
  return new NighthawkHarness(rpc, {
    homeDir: '/tmp/nighthawk-sdk-mcp-oauth-home',
    configPath: '/tmp/nighthawk-sdk-mcp-oauth-home/config.toml',
    auth: {} as never,
