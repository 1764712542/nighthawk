/**
 * Scenario: NighthawkHarness session creation and resume transport behavior.
...
import { createNighthawkHarness, NighthawkHarness } from '#/index';
import type { NighthawkError } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-create-'));
  tempDirs.push(dir);
  return dir;
}

async function writeTestModelConfig(homeDir: string, modelName = 'nighthawk-test-model'): Promise<void> {
  await writeFile(
...
async function writeReviewerAgent(workDir: string): Promise<void> {
  const agentDir = join(workDir, '.nighthawk', 'agents');
...
describe('NighthawkHarness.createSession transport link', () => {
...
`,
  ...
  it('rebinds an active session when resumeSession receives a new Kaos', async () => {
    ...
    const harness = new NighthawkHarness(rpc, {
      homeDir: '/tmp/home',
...
function coreSessionIds(harness: NighthawkHarness): readonly string[] {
  const core = (
