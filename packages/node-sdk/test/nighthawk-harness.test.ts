...
import { createNighthawkHarness, ImageLimits, NighthawkHarness, SDKRpcClientBase } from '#/index';
...
/**
 * The recursive RPC surface NighthawkHarness touches for the tests below: kept
...
function makeHarnessWithRpc(rpc: SDKRpcClientBase): NighthawkHarness {
...
describe('NighthawkHarness capability facade', () => {
  const ready = {
  ...
  it('routes capability calls through the global channel with no session', async () => {
    ...
    expect((await harness.getCapability('nighthawk-webbridge')).state).toBe('ready');
    await harness.installCapability('nighthawk-webbridge');
    expect(calls).toEqual(['list', 'get:nighthawk-webbridge', 'install:nighthawk-webbridge']);
  });
  ...
  it('reports the capability surface as unavailable on v1', async () => {
    ...
    await expect(harness.installCapability('nighthawk-cu')).rejects.toThrow(/requires v2/);
  });
});

describe('NighthawkHarness imageLimits', () => {
  it('exposes the in-process core [image] limits loaded from config.toml', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-harness-'));
...
`,
  ...
  it('a hand-built harness returns the injected ImageLimits as-is', () => {
    ...
    const harness = new NighthawkHarness(new StubRpc(), {
      homeDir: '/tmp/home',
      configPath: '/tmp/config.toml',
      auth: { status: async () => ({ providers: [] }) } as never,
      telemetry: recordingTelemetry([]),
...
describe('NighthawkHarness v1 file ops', () => {
