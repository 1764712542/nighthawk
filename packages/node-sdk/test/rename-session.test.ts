...
import { createNighthawkHarness, NighthawkError, type Event } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-rename-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('NighthawkHarness.renameSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    try {
      ...
      await expect(missingRename).rejects.toBeInstanceOf(NighthawkError);
