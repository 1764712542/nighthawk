...
import { createNighthawkHarness, ErrorCodes, type Event, type NighthawkError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  it('sets plan mode through manualEnterPlan and clears the active plan file', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-plan-work-');
  ...
  it('rejects setTowerMode on the v1 engine with not_implemented', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-tower-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-tower-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('prepares the plans directory without creating plan files on repeated toggles', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-plan-toggle-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-plan-toggle-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-compact-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('returns current session usage totals', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-usage-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-usage-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('resumes a persisted session and restores runtime plan mode from wire history', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-resume-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-resume-legacy-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-resume-legacy-plan-work-');
    await writeTestConfig(homeDir);
    const createdHarness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
    try {
  ...
  it('forks a session, drops goal state, and returns an active fork session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-fork-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-fork-work-');
    await writeTestConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an empty resume id', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-resume-empty-home-');
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
