...
import { createNighthawkHarness, SDKRpcClientV2 } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-list-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('NighthawkHarness.listSessions', () => {
...
describe('SDKRpcClientV2.listSessionsPage', () => {
  it('pages through the listing with keyset cursors (read model off)', async () => {
    ...
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL', '0');
    const homeDir = await makeTempDir();
    const workDir = await makeTempDir();
  ...
  it('drains follow-up pages when the mapping drops entries (read model on)', async () => {
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_FLAG', '0');
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_PERSISTENCE_MINIDB_READMODEL', '1');
    const homeDir = await makeTempDir();
    const workDir = await makeTempDir();
    const client = new SDKRpcClientV2({ homeDir, identity: TEST_IDENTITY });
