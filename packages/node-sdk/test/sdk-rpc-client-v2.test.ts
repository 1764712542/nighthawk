 ...
 * engine (klient memory transport) instead of the v1 NighthawkCore RPC pair.
 ...
 * foldAgentWireReplay).
 * Wiring: real v2 engine bootstrapped on a temp NIGHTHAWK_HOME; remote provider calls are stubbed.
...
import { McpOAuthService as McpOAuthServiceV2 } from '@nighthawk/agent-core-v2/mcpCore/oauth/service';
...
vi.mock('@nighthawk/agent-core-v2/_base/execEnv/environmentProbe', async (importOriginal) => {
  const actual = await importOriginal<
    typeof import('@nighthawk/agent-core-v2/_base/execEnv/environmentProbe')
  >();
  return {
    ...actual,
...
async function makeHarness(): Promise<{ harness: NighthawkHarness; homeDir: string }> {
  const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-'));
  tempDirs.push(homeDir);
  return { harness: createNighthawkHarnessV2({ homeDir, identity: TEST_IDENTITY }), homeDir };
}
...
describe('SDKRpcClientV2 (agent-core-v2 wiring)', () => {
  it('exposes the validated runtime binding through Session', async () => {
    ...
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-work-'));
  ...
  it('reports global MCP authorization without probing when verify is false', async () => {
    ...
    const harness = createNighthawkHarnessV2({ homeDir, identity: TEST_IDENTITY });
  ...
  it('restates engine MCP management Error2s as NighthawkError, undeclared codes as internal', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-'));
    tempDirs.push(homeDir);
    ...
    try {
      ...
      try {
        ...
        expect(isNighthawkError(oauthError)).toBe(true);
        expect(oauthError).toMatchObject({
          code: 'mcp.oauth_failed',
          message: 'OAuth flow timed out',
          details: { flowId: 'flow-1' },
        });
        expect(toNighthawkErrorPayload(oauthError)).toMatchObject({
          code: 'mcp.oauth_failed',
          message: 'OAuth flow timed out',
        });
        ...
        // undeclared NighthawkError code the serializer would reject.
        ...
        expect(isNighthawkError(unknownError)).toBe(true);
        expect(unknownError).toMatchObject({
          code: ErrorCodes.INTERNAL,
          message: 'from a newer engine',
        });
        expect(toNighthawkErrorPayload(unknownError)).toMatchObject({
          code: ErrorCodes.INTERNAL,
          message: 'from a newer engine',
        });
      } finally {
  ...
  it('seeds the host request headers (User-Agent + X-Msh-*) into the engine', async () => {
    ...
    try {
      ...
      expect(headers['User-Agent']).toBe(`nighthawk-cli/${TEST_IDENTITY.version}`);
      expect(headers['X-Msh-Platform']).toBe('nighthawk_cli');
  ...
  it('emits one complete metadata event when a generated title is applied', async () => {
    ...
    const titleOAuthRef = resolveNighthawkOAuthRef({ baseUrl: titleBaseUrl });
    ...
    await new FileTokenStorage(join(homeDir, 'credentials')).save(
      resolveNighthawkTokenStorageName({ oauthKey: titleOAuthRef.key }),
      {
...
`,
    ...
    try {
      ...
      await expect(
        harness.auth.getCachedAccessToken('managed:nighthawk', {
...
`,
  ...
  it('serves listWorkspaceSkills through the engineAccessor escape hatch', async () => {
    ...
    await writeSkill(join(workDir, '.nighthawk', 'skills', 'demo-project-skill'), 'demo-project-skill');
    try {
  ...
  it('honors skillDirs (explicit dirs) over default user / project discovery', async () => {
    ...
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-work-'));
    tempDirs.push(workDir);
    const explicitBase = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-explicit-'));
    ...
    const harness = createNighthawkHarnessV2({
      homeDir,
  ...
  it('serves setTowerMode and getStatus towerMode through the agent scope tower service', async () => {
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_TOWER', '1');
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-work-'));
    tempDirs.push(workDir);
  ...
  it('rejects setTowerMode when the tower feature is unavailable', async () => {
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_FLAG', '0');
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-work-'));
    tempDirs.push(workDir);
...
describe('SDKRpcClientV2 workspace trust', () => {
  it('reports an untrusted workspace with the project MCP servers it gates', async () => {
    ...
    await writeFile(
      join(workDir, '.nighthawk', 'mcp.json'),
  ...
  it('trustWorkspace flips the state and persists the marker in the nighthawk home', async () => {
    const { harness, homeDir } = await makeHarness();
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-work-'));
    tempDirs.push(workDir);
    try {
      ...
      // The trust marker lives in the nighthawk home, never in the checkout.
...
describe('foldAgentWireReplay', () => {
  it('folds a journal into v1 replay records and the tool store', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-fold-'));
    tempDirs.push(dir);
...
describe('SDKRpcClientV2 engine telemetry', () => {
  it('forwards engine-side events to the host-supplied telemetry client', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-tel-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-tel-work-'));
    tempDirs.push(workDir);
  ...
  it('honors telemetry = false for engine-side events', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-tel-off-'));
    tempDirs.push(homeDir);
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-v2-tel-off-work-'));
    tempDirs.push(workDir);
