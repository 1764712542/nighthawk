...
import { createNighthawkHarness, type NighthawkError, type Event } from '#/index';
...
describe('Session.cancel', () => {
  it('cancels an active streaming turn and emits turn_ended(cancelled)', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-cancel-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-cancel-work-');
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-cancel-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-cancel-compact-work-');
    await writeFakeModelConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
...
describe('NighthawkHarness.forkSession', () => {
  it('rejects while the source session has an active turn', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-fork-active-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-fork-active-work-');
    await writeFakeModelConfig(homeDir);
    const harness = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
