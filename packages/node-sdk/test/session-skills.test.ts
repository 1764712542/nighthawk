 ...
 * Responsibilities: list workspace/session skills and activate a session skill through NighthawkHarness.
...
import type * as KosongModule from '@nighthawk/kosong';
...
vi.mock('@nighthawk/kosong', async (importOriginal) => {
...
describe('Session skills', () => {
  it('submits multiple skills with a prompt as one grouped turn (v2 engine)', async () => {
    ...
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-skills-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-skills-work-');
  ...
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.nighthawk/skills/review/SKILL.md')).toBe(true);
  ...
  it('activates a skill through core and emits the public skill event', async () => {
    ...
    try {
      ...
      const skillDir = normalizeWorkDir(await realpath(join(workDir, '.nighthawk', 'skills', 'review')));
      await expect(
        waitForAgentWireEvent(
          homeDir,
          session.id,
        ...
        input: [
          {
            ...
            text: [
              ...
              '',
              `<nighthawk-skill-loaded name="review" trigger="user-slash" source="project" dir="${skillDir}" args="src/app.ts">`,
              'Review the requested file.',
              '',
  ...
  it('resolves user brand skills from NIGHTHAWK_HOME, not the OS home', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-skills-home-');
    const processHome = await makeTempDir(tempDirs, 'nighthawk-sdk-skills-process-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-skills-work-');
    ...
    const harness = createNighthawkHarness({ identity: TEST_IDENTITY });
...
describe('NighthawkHarness workspace skills', () => {
  it('returns project skills when no session exists', async () => {
    const homeDir = await makeTempDir(tempDirs, 'nighthawk-sdk-workspace-skills-home-');
    const workDir = await makeTempDir(tempDirs, 'nighthawk-sdk-workspace-skills-work-');
