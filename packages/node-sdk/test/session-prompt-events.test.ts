...
import { NIGHTHAWK_PLATFORM } from '@nighthawk/nighthawk-oauth';
import type * as KosongModule from '@nighthawk/kosong';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { createNighthawkHarness, type Event, type NighthawkHarness } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-sdk-prompt-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('Session.prompt events', () => {
  ...
  it('emits mapped turn events through Session.onEvent', async () => {
    ...
    try {
      ...
      expect(fakeProviderState.calls[0]?.systemPrompt).toContain('You are NightHawk CLI');
...
async function configureFakeProvider(harness: NighthawkHarness): Promise<void> {
