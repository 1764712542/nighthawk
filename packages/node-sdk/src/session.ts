...
/**
 * The capability surface (built-in product capabilities: nighthawk-cu,
 * nighthawk-webbridge) exists only on the v2 engine — v1 has no capability
...
export class Session {
   ...
   * session) reaches a terminal state. Used by `nighthawk -p` after the main agent's
  ...
  /**
   * Used by `nighthawk -p` after the main agent's turn ends with `reason ===
   ...
   * bounded session lifetime (e.g. `nighthawk -p`) poll this to decide whether
  ...
  async runCommand(name: string, args?: string): Promise<void> {
    ...
    if (commandName.length === 0) {
      throw new NighthawkError(ErrorCodes.REQUEST_INVALID, 'Command name cannot be empty');
    }
  ...
  private ensureOpen(): void {
    if (this.closed) {
      throw new NighthawkError(ErrorCodes.SESSION_CLOSED, 'Session is closed');
    }
  }

  private requireSummary(): SessionSummary {
    if (this.summary === undefined) {
      throw new NighthawkError(ErrorCodes.SESSION_STATE_INVALID, 'Session summary is unavailable');
    }
    return this.summary;
  }
}
...
function normalizeRequiredString(
  ...
  if (normalized.length === 0) {
    throw new NighthawkError(code, message);
  }
  return normalized;
}
