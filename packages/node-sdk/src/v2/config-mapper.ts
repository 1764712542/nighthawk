 ...
 * `inspect().userValue` / `diagnostics()`) onto the v1 `NighthawkConfig` /
 * `ConfigDiagnostics` shapes the SDK contract returns.
 *
 * Why a mapping layer exists: v1 loads config.toml as ONE zod-validated
 * document (`NighthawkConfigSchema`), while v2 registers one config section per
 ...
 */
import type { ConfigDiagnostics, NighthawkConfig } from '#/types';

/**
 * Every top-level `NighthawkConfig` field except `raw` (a v1 write-path
 * implementation detail with no v2 counterpart). Each entry is both the v1
 * field name and the v2 config domain name.
 */
const NIGHTHAWK_CONFIG_DOMAINS = [
  'providers',
  'defaultProvider',
  'defaultModel',
  'models',
 ...
 */
export function resolvedConfigToNighthawkConfig(resolved: Record<string, unknown>): NighthawkConfig {
  const config: Record<string, unknown> = {};
  for (const domain of NIGHTHAWK_CONFIG_DOMAINS) {
    const value = resolved[domain];
    if (value !== undefined) {
      config[domain] = value;
    }
  }
  return config as NighthawkConfig;
}
...
/** The writes needed to reproduce v1 `removeNighthawkProvider` semantics. */
...
/**
 * Apply the v1 remove-provider cascade to a whole `NighthawkConfig` in memory (no
 ...
 * own refresh path.
 */
export function removeProviderFromConfig(config: NighthawkConfig, providerId: string): NighthawkConfig {
  ...
  return {
    ...config,
    providers: plan.providers as NighthawkConfig['providers'],
    models: plan.models as NighthawkConfig['models'],
    ...
    secondaryModel:
      plan.secondaryModel === null
        ? undefined
        : ((plan.secondaryModel ?? config.secondaryModel) as NighthawkConfig['secondaryModel']),
  };
}
