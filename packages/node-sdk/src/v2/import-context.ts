 ...
 * history.
 */
import { ErrorCodes, NighthawkError } from '@nighthawk/agent-core';
import type { ContextMessage } from '@nighthawk/agent-core-v2';
import { estimateTokensForMessages } from '@nighthawk/agent-core-v2/kosong/contract/tokens';
...
export function buildImportContextMessage(content: string, source: string): ContextMessage {
  if (content.trim().length === 0) {
    throw new NighthawkError(ErrorCodes.REQUEST_INVALID, 'Imported context cannot be empty', {
      details: { reason: 'import_content_empty' },
    });
  }
  const normalizedSource = source.trim();
  if (normalizedSource.length === 0) {
    throw new NighthawkError(ErrorCodes.REQUEST_INVALID, 'Imported context source cannot be empty', {
