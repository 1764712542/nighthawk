...
async function main(): Promise<void> {
  const explicitHomeDir = process.env['NIGHTHAWK_SDK_AUTH_SMOKE_HOME'];
  const explicitWorkDir = process.env['NIGHTHAWK_SDK_AUTH_SMOKE_WORK_DIR'];
  const homeDir = explicitHomeDir ?? (await mkdtemp(join(tmpdir(), 'nighthawk-sdk-auth-smoke-home-')));
  const workDir = explicitWorkDir ?? (await mkdtemp(join(tmpdir(), 'nighthawk-sdk-auth-smoke-work-')));
  const keepToken = shouldKeepToken(explicitHomeDir !== undefined);
  const forceLogin = process.env['NIGHTHAWK_SDK_AUTH_SMOKE_FORCE_LOGIN'] === '1';
  const prompt =
    process.env['NIGHTHAWK_SDK_AUTH_SMOKE_PROMPT'] ?? 'Reply with exactly: NightHawk SDK auth smoke ok';
  const harness = createNighthawkHarness({ homeDir, identity: smokeIdentityFromEnv() });
  ...
  try {
    if (forceLogin) {
      await harness.auth.logout(MANAGED_NIGHTHAWK_PROVIDER);
    ...
    const status = await harness.auth.status(MANAGED_NIGHTHAWK_PROVIDER);
    const usage = await harness.auth.getManagedUsage(MANAGED_NIGHTHAWK_PROVIDER);
    ...
    if (config.providers[MANAGED_NIGHTHAWK_PROVIDER]?.oauth?.key !== 'oauth/nighthawk') {
    ...
    if (!keepToken) {
      await harness.auth.logout(MANAGED_NIGHTHAWK_PROVIDER).catch(() => {});
    }
    await harness.close();
...
function printUsage(usage: Awaited<ReturnType<NighthawkHarness['auth']['getManagedUsage']>>): void {
  if (usage.kind === 'error') {
...
function shouldKeepToken(hasExplicitHomeDir: boolean): boolean {
  const value = process.env['NIGHTHAWK_SDK_AUTH_SMOKE_KEEP_TOKEN'];
