...
import { type NighthawkHarness, type Session, type Event } from '@nighthawk/nighthawk-sdk';

export function smokeIdentityFromEnv(): NighthawkHostIdentity {
  const version = process.env['NIGHTHAWK_SMOKE_VERSION'];
  if (version === undefined || version.trim().length === 0) {
    throw new Error('NIGHTHAWK_SMOKE_VERSION is required for NightHawk SDK smoke examples.');
  }
  return {
...
export async function createConfiguredSession(harness: NighthawkHarness): Promise<Session> {
