...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/node-sdk/examples/nighthawk-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     NightHawk home dir; defaults to NIGHTHAWK_HOME or ~/.nighthawk
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveNighthawkHome(options.homeDir);
  const harness = createNighthawkHarness({
    identity: { productName: 'nighthawk-cli', version: 'log-marker', platform: 'nighthawk_cli' },
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'nighthawk.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that NIGHTHAWK_LOG_LEVEL is not "off" and that the session id exists in this NIGHTHAWK_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
