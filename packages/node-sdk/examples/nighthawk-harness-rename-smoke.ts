...
import type { Event } from '@nighthawk/nighthawk-sdk';

import { smokeIdentityFromEnv } from './runtime-smoke-helpers';

async function main(): Promise<void> {
  const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-harness-rename-home-'));
  const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-harness-rename-work-'));
