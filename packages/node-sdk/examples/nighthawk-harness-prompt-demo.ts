import { createNighthawkHarness, type Event, type Session } from '@nighthawk/nighthawk-sdk';

import { smokeIdentityFromEnv } from './runtime-smoke-helpers';

const PROMPT =
  process.env['NIGHTHAWK_SDK_PROMPT'] ??
  'Introduce yourself in two concise sentences and mention the current working directory.';

async function main(): Promise<void> {
  const workDir = process.cwd();
  const harness = createNighthawkHarness({ identity: smokeIdentityFromEnv() });
