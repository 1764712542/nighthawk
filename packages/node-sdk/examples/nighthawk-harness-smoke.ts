...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-harness-home-'));
  ...
  try {
    ...
    const model = config.defaultModel ?? 'nighthawk/nighthawk';
