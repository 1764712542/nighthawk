...
import { createNighthawkHarness, log } from '@nighthawk/nighthawk-sdk';

const SESSION_LOG = 'logs/nighthawk.log';
const GLOBAL_LOG = 'logs/global/nighthawk.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const TEST_HOME = join(homedir(), '.nighthawk-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'nighthawk.log' || file.startsWith('nighthawk.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('nighthawk.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('nighthawk.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['NIGHTHAWK_HOME'] = TEST_HOME;
  process.env['NIGHTHAWK_LOG_LEVEL'] = 'warn';
  process.env['NIGHTHAWK_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['NIGHTHAWK_LOG_SESSION_FILES'] = '2';
  process.env['NIGHTHAWK_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['NIGHTHAWK_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = createNighthawkHarness({
    identity: { productName: 'nighthawk-cli', version: '0.1.1', platform: 'nighthawk_cli' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        'Open these files:',
        '  - logs/nighthawk.log',
        '  - logs/nighthawk.log.1',
        `  - ${summary.sessionDir}/logs/nighthawk.log`,
        `  - ${summary.sessionDir}/logs/nighthawk.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - nighthawk.log.1 exists',
        '  - nighthawk.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/nighthawk.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/nighthawk.log',
        '  - global rotated logs such as ~/.nighthawk-test/logs/nighthawk.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
