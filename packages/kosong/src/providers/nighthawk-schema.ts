...
// Child-schema positions that this NightHawk normalizer knows how to walk. This is
// also the source of truth for child-schema keywords that imply the parent
// schema's type. It is not a list of keywords that NightHawk accepts on the wire.
 ...
 * NightHawk tool compatibility.
 *
 * NightHawk's tool validator rejects some valid JSON Schema shapes when nested
 ...
 */
export function normalizeNighthawkToolSchema(schema: Record<string, unknown>): Record<string, unknown> {
  return ensureNighthawkPropertyTypes(derefJsonSchema(schema));
}

function ensureNighthawkPropertyTypes(schema: Record<string, unknown>): Record<string, unknown> {
...
function normalizeProperty(node: unknown): void {
  ...
  if (!hasOwn(node, 'type') && !hasAnyKey(node, TYPE_COMPLETION_SKIP_KEYS)) {
    ...
    // type: 'object' alongside string enum values). NightHawk rejects these
...
function inferTypeFromValues(values: unknown[]): JsonSchemaType {
  ...
  throw new Error('Mixed JSON Schema enum or const types are not supported by NightHawk tool schemas.');
}
