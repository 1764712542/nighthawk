...
export interface NighthawkOptions {
...
export interface GenerationKwargs {
  /**
   * Legacy completion-budget alias. The NightHawk NightHawk API still accepts
...
const NIGHTHAWK_TOOL_CALL_ID_POLICY: ToolCallIdPolicy = {
...
function convertMessage(
  ...
  // those definitions mid-conversation (`messages[].tools` in the NightHawk
...
function convertTool(tool: Tool): OpenAIToolParam {
  if (tool.name.startsWith('$')) {
    // NightHawk builtin functions start with `$`
    return {
...
/**
 * Extract usage from a streaming chunk. NightHawk may place usage in
...
export function extractUsageFromChunk(
  ...
  // choices[0].usage (NightHawk proprietary)
...
class NighthawkStreamedMessage implements StreamedMessage {
  ...
  private async *_convertStreamResponse(
    ...
    try {
      ...
      throw convertOpenAIError(error, classifyNighthawkQuotaError);
    }
  }
}
export class NighthawkChatProvider implements ChatProvider {
  readonly name: string = 'nighthawk';
  ...
  private _files: NighthawkFiles | undefined;
  ...
  /**
   * File upload client for NightHawk/NightHawk.
   *
   ...
   * messages.
   */
  get files(): NighthawkFiles {
    this._files ??= new NighthawkFiles({
      apiKey: this._apiKey,
      baseUrl: this._baseUrl,
      defaultHeaders: this._defaultHeaders,
  ...
  async generate(
    ...
    // The nighthawk provider never pins an explicit reasoning key, so the dialect
    // always resolves to one of the known wire keys.
    const reasoningKey = this._reasoningKeyDialect.outboundKey() as ReasoningKey;
    const normalizedHistory = normalizeToolCallIdsForProvider(history, NIGHTHAWK_TOOL_CALL_ID_POLICY);
    ...
    // Normalize the legacy `max_tokens` alias to NightHawk's preferred
    // `max_completion_tokens`. When both are set, `max_completion_tokens`
    // wins (confirmed against the live NightHawk API). When neither is
    ...
    try {
      ...
      // Use type assertion via unknown because we pass the NightHawk-proprietary
      ...
      const { data, response } = await client.chat.completions
        ...
        .withResponse();
      return new NighthawkStreamedMessage(
        data as unknown as
  ...
  withThinking(effort: ThinkingEffort): NighthawkChatProvider {
    ...
    // NIGHTHAWK_MODEL_THINKING_KEEP path applies keep after withThinking and merges
  ...
  withGenerationKwargs(kwargs: GenerationKwargs): NighthawkChatProvider {
  ...
  withExtraBody(extraBody: ExtraBody): NighthawkChatProvider {
    const oldExtra = this._generationKwargs.extra_body ?? {};
  ...
  private _createClient(auth: ProviderRequestAuth | undefined): OpenAI {
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAI({
          apiKey: requireProviderApiKey('NighthawkChatProvider', a, this._apiKey),
          baseURL: this._baseUrl,
          defaultHeaders,
        });
      },
  ...
  private _withGenerationKwargs(kwargs: GenerationKwargs): NighthawkChatProvider {
    const clone = this._clone();
    clone._generationKwargs = { ...clone._generationKwargs, ...kwargs };
    return clone;
  }

  private _clone(): NighthawkChatProvider {
    const clone = Object.assign(
      Object.create(Object.getPrototypeOf(this) as object) as NighthawkChatProvider,
      this,
    );
    clone._generationKwargs = { ...this._generationKwargs };
    // Do not share the memoized NighthawkFiles instance with the clone; let it be
    ...
    // construction; instead build a new NighthawkChatProvider when a real new
    // client is required.
