...
export interface AnthropicOptions {
  ...
  nighthawkThinking?: boolean | undefined;
  /**
   ...
   * keeps the base classification. A NightHawk provider routed over this
   * transport passes `classifyNighthawkQuotaError` here so a quota-exhausted 429
...
function convertMessage(message: Message, model: string): MessageParam {
  ...
  for (const part of message.content) {
    if (part.type === 'text') {
      ...
      // field. Anthropic-compatible backends (e.g. NightHawk) stream thinking with
...
export class AnthropicChatProvider implements ChatProvider {
  ...
  private readonly _nighthawkThinking: boolean;
  ...
  constructor(options: AnthropicOptions) {
    ...
    this._nighthawkThinking = options.nighthawkThinking ?? false;
  ...
  async generate(
    ...
    const messages = mergeConsecutiveUserMessages(
      normalizeToolCallIdsForProvider(
        // Message-level tool declarations are a NightHawk wire feature; here the
  ...
  withThinking(effort: ThinkingEffort): AnthropicChatProvider {
    const profile = resolveThinkingProfile(
      ...
      this._nighthawkThinking ? true : this._adaptiveThinking,
    );
    ...
    } else if (this._nighthawkThinking) {
  ...
  withThinkingKeep(keep: string): AnthropicChatProvider {
    ...
    // `[thinking] keep` to an off-value (or NIGHTHAWK_MODEL_THINKING_KEEP=off) is the
