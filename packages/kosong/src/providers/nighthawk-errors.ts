...
// Structured error `type`/`code` value that means the NightHawk account's
...
const NIGHTHAWK_QUOTA_EXHAUSTED_ERROR_CODES = new Set(['exceeded_current_quota_error']);
...
// Grounded in observed NightHawk bodies: "You exceeded your current token
...
const NIGHTHAWK_QUOTA_EXHAUSTED_MESSAGE_PATTERNS = [
...
/**
 * Classify a raw provider failure as NightHawk's quota/balance-exhausted 429,
 * or answer `undefined` to keep the base classification. This is the NightHawk
 * vendor's error knowledge, kept out of the shared OpenAI conversion: the
 * NightHawk provider (and the NightHawk files client) passes it to
 ...
 */
export function classifyNighthawkQuotaError(
  error: unknown,
  ...
  const structuredHit = collectErrorCodes(error).some((code) =>
    NIGHTHAWK_QUOTA_EXHAUSTED_ERROR_CODES.has(code),
  );
  const lowerMessage = message.toLowerCase();
  const wordingHit = NIGHTHAWK_QUOTA_EXHAUSTED_MESSAGE_PATTERNS.some((pattern) =>
