...
import { NighthawkChatProvider, type NighthawkOptions } from './nighthawk';
...
export type ProviderConfig =
  ...
  | ({ type: 'nighthawk' } & NighthawkOptions)
...
export function createProvider(config: ProviderConfig): ChatProvider {
  switch (config.type) {
    ...
    case 'nighthawk':
      return new NighthawkChatProvider(config);
    case 'google-genai':
 ...
 * Unknown / uncatalogued models (and the NightHawk wire, whose capabilities come
