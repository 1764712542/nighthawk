...
export interface NighthawkUploadOptions {
  auth?: ProviderRequestAuth;
  signal?: AbortSignal;
}

export interface NighthawkFilesOptions {
  apiKey?: string;
  baseUrl: string;
...
/**
 * NightHawk-specific file upload client.
 *
 * Wraps the underlying OpenAI-compatible `files.create` API to upload videos
 * to NightHawk's file service and return them as {@link VideoURLPart} values
 * suitable for use in chat messages.
 *
 * A `NighthawkFiles` instance is typically obtained from
 * {@link NighthawkChatProvider.files}.
 */
export class NighthawkFiles {
  ...
  constructor(options: NighthawkFilesOptions) {
    ...
    if (typeof input === 'string') {
      ...
      if (mimeType === undefined || !mimeType.startsWith('video/')) {
        throw new ChatProviderError(
          `NighthawkFiles.uploadVideo: file extension does not indicate a video type: ${filename}`,
        );
      }
    ...
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAIClient({
          apiKey: requireProviderApiKey('NighthawkFiles.uploadVideo', a, this._apiKey),
