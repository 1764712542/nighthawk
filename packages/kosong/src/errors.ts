...
export class APIStatusError extends ChatProviderError {
   ...
   * (NightHawk/KFC only), or `null` when the error response did not carry one.
 ...
 *
 * Observed shapes: NightHawk returns `error.type =
...
// providers (Anthropic, OpenAI, NightHawk/NightHawk, Gemini) — image COUNT/SIZE
...
const IMAGE_FORMAT_STATUS_MESSAGE_PATTERNS = [
  // Unsupported format — OpenAI / NightHawk "unsupported image …".
...
const REQUEST_TOO_LARGE_MESSAGE_PATTERNS = [
  // NightHawk / NightHawk: "Request exceeds the maximum size".
...
const THINKING_EFFORT_CONFIG_DOCS_URL =
  'https://1764712542.github.io/nighthawk/en/configuration/config-files.html#thinking';
...
The provider rejected the configured thinking effort. Non-NightHawk providers receive effort strings without client-side mapping; choose an effort supported by the selected model. For NightHawk models, check support_efforts and default_effort. See ${THINKING_EFFORT_CONFIG_DOCS_URL}`;
}
 ...
 * (NightHawk/KFC only). Returns `null` when the header is absent or empty.
 */
...
// `tool_call_id` / `role 'tool'` / `tool_calls`: NightHawk / NightHawk as a
...
const TOOL_EXCHANGE_ADJACENCY_MESSAGE_PATTERNS = [
  ...
  // OpenAI-compatible (NightHawk / NightHawk): a `tool` message references a
...
const STRUCTURAL_REQUEST_MESSAGE_PATTERNS = [
  ...
  // NightHawk / NightHawk rejects a message whose serialized form carries nothing —
