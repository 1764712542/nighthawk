...
export interface StreamedMessage {
   ...
   * (NightHawk/KFC only). Available as soon as the response headers arrive —
 ...
 *
 * Each provider implementation (NightHawk, OpenAI, Anthropic, Google GenAI, etc.)
...
export interface ChatProvider {
  /** Short identifier for the provider backend (e.g. `"nighthawk"`, `"anthropic"`). */
  readonly name: string;
  /** Model name passed to the upstream API (e.g. `"nighthawk-v1-auto"`). */
   ...
   * original. See `NighthawkChatProvider._clone()` for the rationale.
   */
