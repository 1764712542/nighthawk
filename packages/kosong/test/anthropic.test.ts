...
describe('Anthropic model profile matching', () => {
  ...
  it.each(['k3', 'nighthawk', 'glm-5.2', 'deepseek-v4-pro', 'Example Compatible Model'])(
...
describe('AnthropicChatProvider', () => {
  ...
  describe('message conversion', () => {
    ...
    it('video url content passes a non-data URL through as a url source', async () => {
      ...
      // Non-data video references (nighthawk `ms://` file ids carried over from a
      // nighthawk turn, or http URLs) are emitted as url-source video blocks — the
      // nighthawk anthropic endpoint resolves them server-side, exactly like image
      // url sources.
      expect(body['messages']).toEqual([
        {
          role: 'user',
    ...
    it('thinking without signature is preserved (no signature field)', async () => {
      ...
      // (e.g. NightHawk) reject a tool-call turn whose thinking is missing
    ...
    it('unsigned thinking is preserved before a tool_use block', async () => {
      ...
      // thinking made NightHawk reject the *next* request with
  ...
  describe('with thinking', () => {
    ...
    it('NightHawk thinking mode sends concrete effort without budget conversion', async () => {
      const provider = new AnthropicChatProvider({
        model: 'nighthawk',
        apiKey: 'test-key',
        defaultMaxTokens: 1024,
        stream: false,
        nighthawkThinking: true,
      }).withThinking('max');
      const body = await captureRequestBody(provider, '', [], thinkHistory);
    ...
    it('NightHawk thinking mode passes concrete efforts through and omits only on', async () => {
      const provider = new AnthropicChatProvider({
        model: 'nighthawk',
        apiKey: 'test-key',
        defaultMaxTokens: 1024,
    ...
    it('NightHawk thinking mode keeps thinking off clean', async () => {
      const provider = new AnthropicChatProvider({
        model: 'nighthawk',
        apiKey: 'test-key',
        defaultMaxTokens: 1024,
    ...
    it('thinkingEffort reads back NightHawk concrete efforts and boolean on', () => {
      const provider = new AnthropicChatProvider({
        model: 'nighthawk',
        apiKey: 'test-key',
        defaultMaxTokens: 1024,
