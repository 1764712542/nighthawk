...
import { NighthawkFiles } from '#/providers/nighthawk-files';
import { APIError as OpenAIAPIError } from 'openai';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

function createProvider(): NighthawkChatProvider {
...
describe('NighthawkFiles', () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nighthawk-files-test-'));
  });
  ...
  describe('exposure on NighthawkChatProvider', () => {
    it('exposes files property returning a NighthawkFiles instance', () => {
      const provider = createProvider();
      const files = provider.files;
      expect(files).toBeInstanceOf(NighthawkFiles);
