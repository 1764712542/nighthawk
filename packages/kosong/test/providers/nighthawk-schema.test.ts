import { derefJsonSchema, normalizeNighthawkToolSchema } from '#/providers/nighthawk-schema';
...
describe('normalizeNighthawkToolSchema', () => {
  it.each([
    {
      name: 'string enum',
    ...
    ({ property, expectedType }) => {
      ...
      const result = normalizeNighthawkToolSchema(schema);
  ...
  it('leaves mixed enum types with explicit type untouched to surface provider error', () => {
    ...
    expect(() => normalizeNighthawkToolSchema(schema)).not.toThrow();
    const result = normalizeNighthawkToolSchema(schema);
    expect(result).toEqual({
      type: 'object',
      properties: {
  ...
  it('fails fast for mixed enum types instead of emitting an unsupported NightHawk type array', () => {
    const schema = {
      type: 'object',
      properties: {
    ...
    expect(() => normalizeNighthawkToolSchema(schema)).toThrow(
  ...
  it('does not default the root schema itself to string', () => {
    expect(normalizeNighthawkToolSchema({})).toEqual({});
  });
