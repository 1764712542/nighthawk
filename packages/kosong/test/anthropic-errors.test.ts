...
describe('convertAnthropicError: quota-exhausted 429 via the convertError hook', () => {
  ...
  it('classifies the NightHawk quota body as quota-exhausted through the hook', () => {
    const result = convertAnthropicError(quota429(), classifyNighthawkQuotaError);
  ...
  it('the provider threads options.convertError to its generate catch', async () => {
    const provider = new AnthropicChatProvider({
      ...
      defaultMaxTokens: 1024,
      stream: false,
      convertError: classifyNighthawkQuotaError,
    });
