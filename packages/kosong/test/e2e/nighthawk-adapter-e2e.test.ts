...
function createProvider(baseUrl: string): NighthawkChatProvider {
...
function makeChunk(
  ...
  const chunk: Record<string, unknown> = {
    id: opts?.id ?? 'chatcmpl-nighthawk-1',
...
describe('e2e: nighthawk adapter', () => {
  it('sends NightHawk chat-completion requests, streams reasoning + tool-call deltas, and preserves usage', async () => {
    await withHarness(async (harness) => {
      ...
      expect(result.id).toBe('chatcmpl-nighthawk-1');
