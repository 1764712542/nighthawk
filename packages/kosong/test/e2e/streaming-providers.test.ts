...
describe('integration: streaming provider contracts', () => {
  describe('synthetic NightHawk-style SSE streaming', () => {
    it('multi-chunk text + tool_call + tool_call_part + usage', async () => {
      ...
      const stream = buildStream(parts, { id: 'nighthawk-resp-1', usage });
      const provider = new SingleStreamProvider(stream, 'nighthawk');
      ...
      expect(result.id).toBe('nighthawk-resp-1');
    });
    ...
    it('reasoning_content chunks interleave with text', async () => {
      ...
      const stream = buildStream(parts, { id: 'nighthawk-think-1' });
      const provider = new SingleStreamProvider(stream, 'nighthawk');
