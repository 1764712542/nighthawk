...
import { extractUsageFromChunk, NighthawkChatProvider } from '#/providers/nighthawk';
import { classifyNighthawkQuotaError } from '#/providers/nighthawk-errors';
...
function createProvider(stream: boolean = false): NighthawkChatProvider {
  return new NighthawkChatProvider({
...
function getGenerationState(provider: NighthawkChatProvider): NighthawkGenerationState {
  return Reflect.get(provider, '_generationKwargs') as NighthawkGenerationState;
}

/** Capture the request body sent to OpenAI by mocking the client. */
async function captureRequestBody(
  provider: NighthawkChatProvider,
  systemPrompt: string,
  tools: Tool[],
  history: Message[],
...
async function captureNighthawkMessages(
  history: Message[],
  configure?: (provider: NighthawkChatProvider) => NighthawkChatProvider,
  ...
  let provider = new NighthawkChatProvider({
    model: 'kimi-k2',
    apiKey: 'test-key',
    stream: false,
  ...
  if (captured === undefined) {
    throw new Error('Expected NightHawk provider to send a request.');
  }
...
describe('NighthawkChatProvider', () => {
  describe('message conversion', () => {
    ...
    it('adds NightHawk-only types to JetBrains-like enum-only tool parameters without mutation', async () => {
      const provider = createProvider();
    ...
    it('dereferences draft-7 definitions and normalizes referenced enum-only schemas for NightHawk', async () => {
      const provider = createProvider();
    ...
    it('assistant with reasoning content', async () => {
      ...
      // Snapshot of the expected wire format.
      // NightHawk injects ThinkPart as `reasoning_content` field on the
      // assistant message (NightHawk API extension).
      expect(body['messages']).toEqual([
        { role: 'user', content: 'What is 2+2?' },
        {
          role: 'assistant',
    ...
    it('sends empty reasoning_content for an assistant tool call when preserved thinking is active', async () => {
      ...
      const messages = await captureNighthawkMessages(history, (provider) =>
  ...
  describe('generation kwargs', () => {
    ...
    it('passes constructor generation kwargs into the request body', async () => {
      const provider = new NighthawkChatProvider({
        model: 'kimi-k2-turbo-preview',
        apiKey: 'test-key',
        stream: false,
  ...
  describe('provider properties', () => {
    it('has correct name and model', () => {
      const provider = createProvider();
      expect(provider.name).toBe('nighthawk');
    ...
    it('throws during generation when no constructor or request API key is provided', async () => {
      ...
      const saved = process.env['NIGHTHAWK_API_KEY'];
      delete process.env['NIGHTHAWK_API_KEY'];
      try {
        const provider = new NighthawkChatProvider({ model: 'test' });
        await expect(provider.generate('', [], [])).rejects.toThrow(/options\.auth\.apiKey/);
      } finally {
        if (saved !== undefined) {
          process.env['NIGHTHAWK_API_KEY'] = saved;
        }
      }
    });
    ...
    it('withThinking returns a new instance', () => {
      ...
      expect(newProvider).toBeInstanceOf(NighthawkChatProvider);
  ...
  describe('clone client sharing', () => {
    ...
    function getInternalClient(provider: NighthawkChatProvider): unknown {
...
describe('extractUsageFromChunk', () => {
  ...
  it('extracts choices[0].usage (NightHawk proprietary)', () => {
    const chunk = {
...
describe('extractUsage', () => {
  ...
  it('extracts usage with NightHawk cached_tokens', () => {
    const usage = extractUsage({
      prompt_tokens: 100,
...
describe('classifyNighthawkQuotaError', () => {
  const QUOTA_MESSAGE =
  ...
  it('classifies a structured exceeded_current_quota_error body as quota-exhausted', () => {
    const error = classifyNighthawkQuotaError(
  ...
  it.each([QUOTA_MESSAGE, TOKEN_QUOTA_MESSAGE])(
    ...
    (message) => {
      expect(classifyNighthawkQuotaError(quota429(message))).toBeInstanceOf(
  ...
  it.each(['Too many requests', 'your token quota per minute was exceeded'])(
    ...
    (message) => {
      expect(classifyNighthawkQuotaError(quota429(message))).toBeUndefined();
    },
  );

  it('answers undefined for non-429 and non-SDK shapes', () => {
    expect(
      classifyNighthawkQuotaError(new OpenAIAPIError(403, undefined, QUOTA_MESSAGE, new Headers())),
    ).toBeUndefined();
    expect(classifyNighthawkQuotaError(new Error(QUOTA_MESSAGE))).toBeUndefined();
    expect(classifyNighthawkQuotaError(undefined)).toBeUndefined();
  });
  ...
  it('classifies the Anthropic SDK error shape (body nested under .error)', () => {
    ...
    const error = classifyNighthawkQuotaError(source);
    expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
    expect(isRetryableGenerateError(error)).toBe(false);
  });
});
