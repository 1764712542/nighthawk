...
describe('normalizeAPIStatusError', () => {
  ...
  it.each([
    // NightHawk / NightHawk 413 observed in the field when accumulated media pushed
...
describe('isToolExchangeAdjacencyError', () => {
  ...
  // The exact OpenAI-compatible (NightHawk / NightHawk) message observed in the field
  ...
  it('matches the OpenAI/NightHawk tool_call_id-not-found 400', () => {
    expect(
...
describe('isRecoverableRequestStructureError', () => {
  ...
  it('matches the NightHawk/NightHawk vacuous-message rejection', () => {
    expect(
      isRecoverableRequestStructureError(
        new APIStatusError(
          400,
...
describe('isImageFormatError', () => {
  it('matches documented provider image format/data rejections', () => {
    ...
    // NightHawk/NightHawk (from the NightHawk error reference)
    expect(
      isImageFormatError(
  ...
  it('does not match image count/size/support errors that stripping media cannot fix', () => {
    ...
    // format/data errors: Anthropic's per-image size cap, NightHawk's
...
describe('normalizeAPIStatusError: 429 stays vendor-neutral', () => {
  ...
  // classification lives with the vendor (`classifyNighthawkQuotaError`, the
