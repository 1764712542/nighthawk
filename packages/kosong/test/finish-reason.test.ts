...
import { NighthawkChatProvider } from '#/providers/nighthawk';
...
describe('normalizeOpenAIFinishReason (NightHawk + OpenAILegacy shared helper)', () => {
  // Covers A for NightHawk + OpenAILegacy.
...
function makeNighthawkStream(rawFinish: string | null | undefined): AsyncIterable<unknown> {
  // NightHawk / Chat Completions stream: emit one content chunk + a terminal
...
// The NightHawk provider consumes `APIPromise.withResponse()` to read the
// `x-trace-id` response header, so its mocked client must expose that method.
function makeNighthawkChatClient(response: unknown) {
  return {
    chat: {
      completions: {
...
function createNighthawkProvider(response: unknown, stream: boolean): NighthawkChatProvider {
  return new NighthawkChatProvider({
    model: 'kimi-k2-turbo-preview',
    stream,
    clientFactory: () => makeNighthawkChatClient(response) as never,
  });
}
...
describe('NighthawkChatProvider finish reason (stream, table coverage)', () => {
  // A + B coverage for NightHawk.
  it.each<[string, FinishReason, string]>([
    ...
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createNighthawkProvider(makeNighthawkStream(raw), true);
  ...
  it('returns null finishReason when stream never emits finish_reason', async () => {
    ...
    const provider = createNighthawkProvider(makeAsyncIterable(chunks), true);
  ...
  // C coverage for NightHawk.
  it('captures finish_reason from a non-stream response', async () => {
    const provider = createNighthawkProvider(
      {
        id: 'chatcmpl-ns',
        choices: [
          {
  ...
  // D (non-stream) coverage for NightHawk.
...
describe('OpenAILegacyChatProvider finish reason (stream + non-stream)', () => {
  ...
  it.each<[string, FinishReason, string]>([
    ...
    'raw stream finish_reason %j maps to %j (raw=%j)',
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createOpenAILegacyProvider(makeNighthawkStream(raw), true);
