...
describe('getModelCapability: nighthawk', () => {
  it('does not infer capabilities from NightHawk model names', () => {
    for (const model of [
      'nighthawk',
      'nighthawk',
      'kimi-k2-turbo-preview',
      'kimi-k2.5',
      'nighthawk-thinking-preview',
    ]) {
      expect(getModelCapability('nighthawk', model)).toEqual(UNKNOWN_CAPABILITY);
    }
  });

  it('unknown NightHawk model → UNKNOWN_CAPABILITY (no throw)', () => {
    expect(getModelCapability('nighthawk', 'some-fake-model')).toEqual(UNKNOWN_CAPABILITY);
  });
});
