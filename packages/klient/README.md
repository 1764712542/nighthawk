...
import { createKlient } from '@nighthawk/klient/memory';   // or '/ipc'
...
|---|---|---|
| `@nighthawk/klient/ipc` | `{ socketPath, token? }` | same socket |
| `@nighthawk/klient/memory` | `{ scope }` (a bootstrapped engine app scope) | direct emitter/bus subscription |
...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live suites
  and their client harness (skip unless `NIGHTHAWK_SERVER_URL` is set; the v1
...
readings through a real prompt (requires `NIGHTHAWK_EXAMPLE_MODEL` +
`NIGHTHAWK_EXAMPLE_API_KEY`).
