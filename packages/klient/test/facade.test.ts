...
describe('facade routing', () => {
  ...
  it('forwards the login region option through the wire contract', async () => {
    ...
    await klient.global.auth.startLogin('managed:nighthawk', { region: 'global' });
    expect(channel.calls[0]).toMatchObject({
      service: 'oauthService',
      method: 'startLogin',
      args: ['managed:nighthawk', { region: 'global' }],
    });
  });
  ...
  it('routes capability calls through the registered app service contract', async () => {
    ...
    channel.result = status;
    await expect(klient.global.capabilities.get('nighthawk-cu')).resolves.toEqual(status);
    await expect(klient.global.capabilities.install('nighthawk-cu')).resolves.toEqual(status);

    expect(channel.calls).toEqual([
      { scope: {}, service: 'capabilityService', method: 'listCapabilities', args: [] },
      { scope: {}, service: 'capabilityService', method: 'getCapability', args: ['nighthawk-cu'] },
      { scope: {}, service: 'capabilityService', method: 'installCapability', args: ['nighthawk-cu'] },
    ]);
  });
