 ...
 * `process.env.NIGHTHAWK_SERVER_URL ?? http://127.0.0.1:58627`.
 *
...
const BASE_URL = process.env['NIGHTHAWK_SERVER_URL'] ?? 'http://127.0.0.1:58627';
...
describe('DaemonClient session action helpers', () => {
  ...
  it('model catalog helpers call the catalog and action-suffix routes', async () => {
    ...
    const calls: FetchCall[] = [];
    const model = testModel({ model: 'nighthawk/nighthawk' });
    const provider = testProvider({ id: 'nighthawk', models: [model.model] });
    const client = new DaemonClient({
      baseUrl: 'http://server.example.test',
    ...
    await expect(client.getProvider('nighthawk')).resolves.toEqual(provider);

    log('fetch calls', calls);
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      ['GET', 'http://server.example.test/api/v1/auth'],
      ['GET', 'http://server.example.test/api/v1/models'],
      ['POST', 'http://server.example.test/api/v1/models/nighthawk%2Fnighthawk:set_default'],
      ['GET', 'http://server.example.test/api/v1/providers'],
      ['GET', 'http://server.example.test/api/v1/providers/nighthawk'],
    ]);
