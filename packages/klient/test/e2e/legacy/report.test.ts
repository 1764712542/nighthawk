...
describe('server-e2e report', () => {
  ...
  it('records createCaseLogger entries under the active case', () => {
    const reportDir = tmpReportDir();
    resetReportDir(reportDir);
    const previousReportDir = process.env['NIGHTHAWK_SERVER_E2E_REPORT_DIR'];
    process.env['NIGHTHAWK_SERVER_E2E_REPORT_DIR'] = reportDir;
    try {
      ...
      if (previousReportDir === undefined) {
        delete process.env['NIGHTHAWK_SERVER_E2E_REPORT_DIR'];
      } else {
        process.env['NIGHTHAWK_SERVER_E2E_REPORT_DIR'] = previousReportDir;
      }
    }
  });
