 ...
 * NightHawk (protocol `openai` + provider `type: nighthawk` — the trait-composition
...
const M_KIMI = 'matrix-nighthawk';
const M_ANTHROPIC = 'matrix-anthropic';
const M_GOOGLE = 'matrix-google';

const NIGHTHAWK_PROVIDER = 'matrix-nighthawk-provider';
...
beforeAll(async () => {
  ...
  await klient.global.kosong.addProvider(NIGHTHAWK_PROVIDER, {
    type: 'nighthawk',
  ...
  // M_KIMI needs a `provider` reference to NIGHTHAWK_PROVIDER so the engine
  // resolves nighthawk provider traits (uploadVideo). The facade's addProvider()
...
describe('image blocks with invalid data', () => {
  it('a data-URL image with an unaccepted mime is replaced at prompt ingestion on EVERY provider (l2)', async () => {
    ...
    const cases = [
      ...
      { label: 'bmp-nighthawk', model: M_KIMI, reply: OK_OPENAI },
  ...
  it('nighthawk (composed): same media-strip fallback as plain openai (l3 + engine fallback)', async () => {
    const ctx = await newCase(M_KIMI, 'nighthawk-image-base64');
    resetMock(queueScript(jsonError(400, 'Invalid image data'), OK_OPENAI));
...
// ---------------------------------------------------------------------------
// Daemon file references (nighthawk-file://): engine-side resolution before the
// provider wire.
// ---------------------------------------------------------------------------

describe('daemon file references (nighthawk-file://)', () => {
  it('a nighthawk-file image reference reaches the provider as a data URL, never verbatim', async () => {
    ...
    // order, so image nighthawk-file refs leaked to the provider unchanged and
    ...
    const cases = [
      { label: 'nighthawkfile-image-openai', model: M_OPENAI_VISION, reply: OK_OPENAI },
      { label: 'nighthawkfile-image-nighthawk', model: M_KIMI, reply: OK_OPENAI },
    ] as const;
    for (const { label, model, reply } of cases) {
      ...
      resetMock(queueScript(reply));
      await promptAndWait(ctx, [
        { type: 'image_url', imageUrl: { url: `nighthawk-file://${meta.id}` } },
        { type: 'text', text: 'what is this?' },
      ]);
      expect(requests, label).toHaveLength(1);
      expect(JSON.stringify(requests[0]?.json), label).not.toContain('nighthawk-file://');
...
describe('video blocks', () => {
  ...
  it('nighthawk (composed): ReadMediaFile on a video uploads via the files API (uploadVideo trait)', async () => {
    const ctx = await newCase(M_KIMI, 'nighthawk-video-upload');
    ...
    // The NighthawkFiles client POSTs multipart form data to {baseUrl}/files.
...
describe('tool exchange structure', () => {
  ...
  it('media tool result: plain openai extracts text + appends a media user message; nighthawk keeps parts in place', async () => {
    ...
    // Composed nighthawk: trait mode hands shaping to the trait — the image part
    // stays inside the tool message content; no extra user message appears.
    const nighthawkWire = await runMediaToolResultCase(M_KIMI, 'media-result-nighthawk');
    const nighthawkTool = nighthawkWire.find((message) => message['role'] === 'tool');
    const nighthawkContent = nighthawkTool?.['content'] as { type: string }[];
    expect(Array.isArray(nighthawkContent)).toBe(true);
    expect(nighthawkContent.some((part) => part.type === 'image_url')).toBe(true);
    expect(nighthawkWire.at(-1)?.['role']).toBe('tool');
  }, 60_000);
