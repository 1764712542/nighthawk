 ...
 *
 * Env: NIGHTHAWK_MODEL_NAME is unset for the run (it would pin `defaultModel` and
...
import { IKosongConfigService } from '@nighthawk/agent-core-v2/app/kosongConfig/kosongConfig';
import { type Klient } from '@nighthawk/klient';
...
const pinnedModelEnv = process.env['NIGHTHAWK_MODEL_NAME'];
delete process.env['NIGHTHAWK_MODEL_NAME'];
try {
  await main();
} catch (error) {
  console.error(error);
  ...
  if (pinnedModelEnv !== undefined) process.env['NIGHTHAWK_MODEL_NAME'] = pinnedModelEnv;
}
