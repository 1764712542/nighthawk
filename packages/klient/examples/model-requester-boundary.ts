 ...
 * NightHawk home (`NIGHTHAWK_HOME` or `~/.nighthawk`), resolves `IModelCatalog`
 ...
 *   NIGHTHAWK_BOUNDARY_MODELS  — comma-separated model ids to ping (default: all)
 *   NIGHTHAWK_BOUNDARY_SKIP_LIVE — set to `1` to skip part 1 (no real API calls)
 */
...
import { isError2 } from '@nighthawk/agent-core-v2/_base/errors/errors';
...
} from '@nighthawk/agent-core-v2/kosong/contract/errors';
import type { ToolCall } from '@nighthawk/agent-core-v2/kosong/contract/message';
...
async function probeRealConfig(): Promise<void> {
  ...
  try {
    ...
    const filter = process.env['NIGHTHAWK_BOUNDARY_MODELS']?.split(',').map((s) => s.trim());
...
async function main(): Promise<void> {
  if (process.env['NIGHTHAWK_BOUNDARY_SKIP_LIVE'] !== '1') {
