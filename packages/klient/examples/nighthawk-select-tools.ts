 ...
 * nighthawk-type providers, based on the real providers/auth in
 * `~/.nighthawk/config.toml`.
 *
 ...
 *
 * The wire fact that makes this nighthawk-specific: only the nighthawk vendor trait
 * serializes `message.tools` into the outbound request
 * (`nighthawkOpenAITrait.convertMessage`); every other base SKIPS
 * tool-declaration-only messages entirely. So the whole mechanism can only
 * work on `(nighthawk, openai)` — this example proves that on a stub, then
 * behaviorally tests each real nighthawk model with a simulated "nighthawk computer
 * use" tool pool.
 *
 * Part A — deterministic, local stub (OpenAI Chat Completions wire):
 *   1. nighthawk providerType: the tool-declaration message reaches the wire with
 *      an embedded `tools` array.
 *   2. no providerType (plain openai): the same message is dropped — the
 *      boundary non-nighthawk providers cannot cross.
 ...
 *
 * Part B — live, per nighthawk model from the real config: a two-step flow that
 ...
 *     --import ../../build/register-raw-text-loader.mjs examples/nighthawk-select-tools.ts
 *
 * Env:
 *   NIGHTHAWK_HOME              — default `~/.nighthawk`
 *   NIGHTHAWK_SELECT_TOOLS_MODELS    — comma-separated model ids for the live parts (default: all nighthawk-type)
 *   NIGHTHAWK_SELECT_TOOLS_SKIP_LIVE — set to `1` to skip part B (no real API calls)
 *   NIGHTHAWK_SELECT_TOOLS_TAP       — set to `1` to run part C instead of B: route
...
import { bootstrap, logSeed, resolveLoggingConfig } from '@nighthawk/agent-core-v2';
import { IConfigService } from '@nighthawk/agent-core-v2/app/config/config';
import { renderLoadableToolsAnnouncement } from '@nighthawk/agent-core-v2/agent/toolSelect/dynamicTools';
import { UNKNOWN_CAPABILITY } from '@nighthawk/agent-core-v2/kosong/contract/capability';
import type { Message } from '@nighthawk/agent-core-v2/kosong/contract/message';
import type { Tool } from '@nighthawk/agent-core-v2/kosong/contract/tool';
import type { AuthProvider, Model } from '@nighthawk/agent-core-v2/kosong/model/catalog';
import { IModelCatalog } from '@nighthawk/agent-core-v2/kosong/model/catalog';
import type {
  ModelRequestInput,
  ModelRequester,
} from '@nighthawk/agent-core-v2/kosong/model/modelRequester';
import { ModelRequesterImpl } from '@nighthawk/agent-core-v2/kosong/model/modelRequesterImpl';
import { IProtocolAdapterRegistry } from '@nighthawk/agent-core-v2/kosong/protocol/protocol';
import { ProtocolAdapterRegistry } from '@nighthawk/agent-core-v2/kosong/provider/protocolAdapterRegistry';
...
// ---------------------------------------------------------------------------
// The simulated "nighthawk computer use" MCP tool pool.
// ---------------------------------------------------------------------------
...
async function probeWireEncoding(): Promise<void> {
  ...
  try {
    // 1) nighthawk providerType: the declaration message reaches the wire, with the
    // loaded schema embedded in the message — and the deferred top-level tool
    // does NOT duplicate into the wire tools[].
    await collect(makeRequester('nighthawk'), input);
    const nighthawkBody = lastBody;
    const declaration = nighthawkBody?.messages?.find((m) => Array.isArray(m.tools));
    assert(declaration !== undefined, 'nighthawk: a wire message carries the loaded tool schemas');
    assert(
      declaration.tools?.some((t) => t.function?.name === 'computer_screenshot') === true,
      'nighthawk: declaration embeds computer_screenshot',
    );
    assert(
      nighthawkBody?.tools?.some((t) => t.function?.name === 'select_tools') === true,
      'nighthawk: top-level tools[] keeps select_tools',
    );
    assert(
      nighthawkBody?.tools?.some((t) => t.function?.name === 'computer_screenshot') !== true,
      'nighthawk: deferred tool stays OUT of top-level tools[]',
    );
    console.log(
      `[ok] nighthawk: declaration on wire (role=${String(declaration.role)}), ` +
        `top-level tools=[${(nighthawkBody?.tools ?? []).map((t) => t.function?.name ?? '?').join(', ')}]`,
    );

    // 2) plain openai: the same declaration message is dropped outright — the
    // dynamic-tool schema can never reach a non-nighthawk wire. (3 wire messages =
    ...
    console.log(
      '[ok] openai: declaration message dropped — dynamic tools cannot cross a non-nighthawk wire',
    );
  } finally {
...
// ---------------------------------------------------------------------------
// Part B — live two-step select_tools flow per real nighthawk model.
// ---------------------------------------------------------------------------
...
async function probeLiveNighthawkProviders(): Promise<void> {
  const homeDir = process.env['NIGHTHAWK_HOME'] ?? join(homedir(), '.nighthawk');
  console.log(`\n=== part B: live select_tools flow on real nighthawk providers (${homeDir}) ===`);
  ...
  try {
    ...
    const filter = process.env['NIGHTHAWK_SELECT_TOOLS_MODELS']?.split(',').map((s) => s.trim());
    ...
    const targets = models.filter((m) => {
      ...
      try {
        return catalog.get(m.model).providerType === 'nighthawk';
      } catch {
        return false;
      }
    });
    assert(targets.length > 0, 'at least one nighthawk-type model configured');
...
async function main(): Promise<void> {
  ...
  if (process.env['NIGHTHAWK_SELECT_TOOLS_TAP'] === '1') {
    await probeTappedContext();
  } else if (process.env['NIGHTHAWK_SELECT_TOOLS_SKIP_LIVE'] !== '1') {
    await probeLiveNighthawkProviders();
  }
  console.log('\nselect-tools: OK');
}
