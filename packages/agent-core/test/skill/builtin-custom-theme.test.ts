...
describe('builtin skill: custom-theme', () => {
  ...
  it('pins the docs token reference and points users at NIGHTHAWK_HOME/themes and /theme', () => {
    ...
    expect(content).toContain('<NIGHTHAWK_HOME>/themes');
  ...
  it('registers through registerBuiltinSkills but stays out of the model skill listing', () => {
    ...
    expect(registry.getNighthawkSkillsDescription()).toContain('custom-theme');
