...
import { APIStatusError, type Message, type ToolCall } from '@nighthawk/kosong';
...
const SUBAGENT_TIMEOUT_ENV = 'NIGHTHAWK_SUBAGENT_TIMEOUT_MS';
...
describe('SessionSubagentHost', () => {
  ...
  it('resumes an idle child agent by id', async () => {
    ...
    const session = fakeSession(parent.agent, child.agent, {
      'agent-0': {
        homedir: '/tmp/nighthawk-session/agents/agent-0',
        type: 'sub',
        parentAgentId: 'main',
      },
    });
  ...
  describe('secondary model binding', () => {
    const secondaryFlags = () =>
      new FlagResolver({ NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL: '1' });
    const LONG_SUMMARY =
    ...
    const withSecondaryModels = (config?: NighthawkConfig): NighthawkConfig => ({
      providers: { 'test-provider': { type: 'nighthawk', apiKey: 'test-key' } },
      ...config,
      models: {
        'cheap-model': {
...
describe('Session resume permission parent chain', () => {
  it('restores subagent live-derived permission when metadata lists the child first', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nighthawk-permission-chain-'));
    tempDirs.push(dir);
...
describe('Session.createAgent', () => {
  ...
  it('renders profiles with the current directory listing and merged AGENTS.md files', async () => {
    ...
    const kaos = createFakeKaos({
      ...
      readText: vi.fn(async (path: string) => {
        ...
        if (path === `${workDir}/.nighthawk/AGENTS.md`) return 'brand instructions';
    ...
    expect(created.agent.config.systemPrompt).toContain(
      '<!-- From: /repo/packages/app/.nighthawk/AGENTS.md -->',
    );
  ...
  it('uses the nighthawk home for global branded AGENTS.md files', async () => {
    const realHome = '/real-home';
    const nighthawkHome = '/nighthawk-home';
    const workDir = '/repo/packages/app';
    const kaos = createFakeKaos({
      gethome: () => realHome,
      mkdir: vi.fn(async () => {}),
      ...
      stat: vi.fn(async (path: string) => {
        ...
        if ([`${nighthawkHome}/AGENTS.md`, `${realHome}/.nighthawk/AGENTS.md`].includes(path)) {
          return stat('file');
        }
        throw new Error(`ENOENT ${path}`);
      }),
      ...
      readText: vi.fn(async (path: string) => {
        if (path === `${nighthawkHome}/AGENTS.md`) return 'nighthawk home instructions';
        if (path === `${realHome}/.nighthawk/AGENTS.md`) return 'stale real-home instructions';
        throw new Error(`ENOENT ${path}`);
      }),
    });
    const session = new Session({
      id: 'test-nighthawk-home-agents-md',
      kaos: kaos.withCwd(workDir),
      homedir: '/tmp/nighthawk-session',
    ...
    expect(created.agent.config.systemPrompt).toContain('nighthawk home instructions');
  ...
  it('allocates the next unused generated agent id', async () => {
    ...
    expect(session.metadata.agents['agent-1']).toMatchObject({
      homedir: '/tmp/nighthawk-session/agents/agent-1',
      type: 'sub',
    });
  });
...
function testAgentCatalog(): SessionAgentProfileCatalog {
  ...
  return new SessionAgentProfileCatalog({
    workDir: '/nonexistent-nighthawk-test-workdir',
    brandHomeDir: '/nonexistent-nighthawk-test-brandhome',
    osHomeDir: '/nonexistent-nighthawk-test-oshome',
  });
}
