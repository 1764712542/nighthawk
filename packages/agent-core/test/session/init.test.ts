...
import type { ProviderConfig, ToolCall } from '@nighthawk/kosong';
import type { Kaos, StatResult } from '@nighthawk/kaos';
...
describe('Session.init', () => {
  it('subscribes to MCP credential events before app-level registration', async () => {
    const mcpOAuth = new McpOAuthService({ nighthawkHomeDir: await makeTempDir() });
...
describe('Session secondary-model live config', () => {
  const SECONDARY_BASE_CONFIG: NighthawkConfig = {
    providers: {
  ...
  const SECONDARY_POINTER_CONFIG: NighthawkConfig = {
    ...SECONDARY_BASE_CONFIG,
    secondaryModel: { model: MOCK_PROVIDER.model },
  };
  const SECONDARY_PATCHED_CONFIG: NighthawkConfig = {
    ...SECONDARY_BASE_CONFIG,
    models: {
  ...
  async function makeSession(config?: NighthawkConfig): Promise<Session> {
    const workDir = await makeTempDir();
    const sessionDir = await makeTempDir();
    return new Session({
      ...
      experimentalFlags: new FlagResolver({
        NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL: '1',
      }),
      config,
    });
  }
  ...
  it('live-applies the recipe to the session snapshot and live agents', async () => {
    ...
    try {
      ...
      expect(session.nighthawkConfig?.secondaryModel).toEqual({ model: MOCK_PROVIDER.model });
      expect(agent.nighthawkConfig?.secondaryModel).toEqual({ model: MOCK_PROVIDER.model });
      // A pointer-only recipe synthesizes no derived entry.
      expect(session.nighthawkConfig?.models?.['__secondary__']).toBeUndefined();
      ...
      expect(second.nighthawkConfig?.secondaryModel).toEqual({ model: MOCK_PROVIDER.model });
    } finally {
      await session.close();
    }
  });
  ...
  it('replaces a patched runtime snapshot with a pointer-only runtime snapshot', async () => {
    ...
    try {
      ...
      const derived = session.nighthawkConfig?.models?.['__secondary__'];
  ...
  it('keeps unrelated session settings when applying a secondary-model config', async () => {
    ...
    try {
      ...
      expect(session.nighthawkConfig?.loopControl?.maxStepsPerTurn).toBe(7);
    } finally {
      await session.close();
    }
  });
  ...
  it('rejects a dangling pointer with the wrapped secondary-model error', async () => {
    ...
    try {
      ...
      expect(session.nighthawkConfig?.secondaryModel).toBeUndefined();
    } finally {
      await session.close();
    }
  });
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-core-init-'));
  tempDirs.push(dir);
  return dir;
}
