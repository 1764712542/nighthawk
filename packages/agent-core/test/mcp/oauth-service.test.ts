 ...
 *
 * Run with `pnpm --filter @nighthawk/agent-core exec vitest run test/mcp/oauth-service.test.ts`.
 */
...
function makeFixture(): Fixture {
  ...
  const storeDir = `${tmpdir()}/nighthawk-mcp-oauth-service-${process.pid}-${Math.random().toString(36).slice(2)}`;
