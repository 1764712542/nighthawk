 ...
 * @nighthawk/agent-core exec vitest run test/mcp/connection-manager.test.ts`.
 */
...
import type { ProviderConfig } from '@nighthawk/kosong';
...
describe('McpConnectionManager', () => {
  ...
  it('reconnect throws a coded NighthawkError when the server name is unknown', async () => {
    const cm = new McpConnectionManager();
    try {
      await expect(cm.reconnect('nope')).rejects.toBeInstanceOf(NighthawkError);
  ...
  it('reconnect rejects disabled servers without connecting them', async () => {
    ...
    try {
      ...
      await expect(reconnect).rejects.toBeInstanceOf(NighthawkError);
  ...
  it('marks an explicitly OAuth HTTP server as needs-auth when non-auth headers accompany a 401', async () => {
    ...
    const storeDir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-cm-'));
  ...
  it('flips SSE servers into needs-auth when the server returns 401 and no static token is set', async () => {
    ...
    await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
    const port = (server.address() as HttpAddress).port;
    const storeDir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-sse-cm-'));
    const oauthService = new McpOAuthService({ store: new JsonFileStore(storeDir) });
    const cm = new McpConnectionManager({ oauthService });
    try {
      await cm.connectAll({
  ...
  it('flips cached OAuth credentials that require reauth into needs-auth', async () => {
    ...
    const storeDir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-cached-'));
    const oauthService = new McpOAuthService({ store: new JsonFileStore(storeDir) });
    ...
    try {
      await cm.connectAll({
        crashy: {
          ...
          env: { NIGHTHAWK_TEST_MCP_EXIT_AFTER_MS: '500', NIGHTHAWK_TEST_MCP_STDERR: 'fatal: out of memory' },
    ...
    try {
      await cm.connectAll({
        nope: {
          ...
          env: { NIGHTHAWK_TEST_MCP_STDERR: 'fatal: missing API token NIGHTHAWK_X' },
          startupTimeoutMs: 4_000,
        },
      });
      const entry = cm.get('nope');
      expect(entry?.status).toBe('failed');
      expect(entry?.error).toContain('fatal: missing API token NIGHTHAWK_X');
    } finally {
      await cm.shutdown();
    }
  }, 10000);
...
describe('McpConnectionManager sources and config-aware reconnect', () => {
  ...
  it('reconnect re-resolves the config through the resolver instead of the snapshot', async () => {
    ...
    const cm = new McpConnectionManager({
      configResolver: async () => ({
        ...
        origin: '/home/user/.nighthawk/mcp.json',
        mutable: true,
      }),
    });
    try {
  ...
  it('flips connected HTTP servers to needs-auth when a mid-session failure looks like a 401', async () => {
    ...
    const storeDir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-close-'));
    const oauthService = new McpOAuthService({ store: new JsonFileStore(storeDir) });
    const cm = new McpConnectionManager({ oauthService });
    try {
      await cm.connectAll({
...
describe('Session MCP startup', () => {
  it('stores default MCP OAuth credentials under the configured NightHawk home', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-oauth-home-'));
    const processHome = join(tmp, 'process-home');
    const nighthawkHome = join(tmp, 'nighthawk-home');
    ...
    const session = new Session({
      ...
      nighthawkHomeDir: nighthawkHome,
      rpc: sessionRpc(),
    });
    ...
    try {
      ...
      await expect(
        readFile(
          join(nighthawkHome, 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
      ).resolves.toContain('session-token');
      await expect(
        readFile(
          join(processHome, '.nighthawk', 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
  ...
  it('does not block main agent creation on slow MCP startup', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-startup-'));
  ...
  it('starts stdio MCP servers in the session cwd when config.cwd is omitted', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-cwd-'));
    const session = new Session({
      id: 'test-mcp-cwd',
      kaos: testKaos.withCwd(tmp),
      homedir: join(tmp, 'session'),
  ...
  it("times out tool calls using the Session's global MCP config", async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-global-timeout-'));
    const session = new Session({
      ...
      mcpConfig: {
        servers: {
          slowTool: {
            ...
            env: { NIGHTHAWK_TEST_MCP_TOOL_DELAY_MS: '300' },
          },
        },
      },
    });
  ...
  it('waits for initial MCP startup before the first prompt reaches the model', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-prompt-'));
      ...
      mcpConfig: {
        servers: {
          slow: {
            ...
            command: process.execPath,
            args: [stdioFixture],
            env: { NIGHTHAWK_TEST_MCP_START_DELAY_MS: '250' },
            startupTimeoutMs: 2_000,
          },
        },
      },
  ...
  it('emits tool.list.updated(mcp.disconnected) when reconnect drops the live tools', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-session-mcp-reconnect-'));
    const events: SessionRpcEvent[] = [];
    const session = new Session({
      id: 'test-mcp-mixed',
      kaos: testKaos.withCwd(tmp),
