 ...
 *
 * Run with `pnpm --filter @nighthawk/agent-core exec vitest run test/mcp/registry.test.ts`.
 */
...
async function makeNighthawkHome(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'nighthawk-mcp-registry-home-'));
}
...
async function makePlugin(
  ...
  const root = await mkdtemp(join(tmpdir(), `nighthawk-mcp-registry-plugin-`));
  await writeJson(join(root, 'nighthawk.plugin.json'), { name, mcpServers });
...
describe('McpServerRegistry', () => {
  it('lists user-level global entries without a cwd', async () => {
    ...
    const registry = new McpServerRegistry({
      ...
      plugins: new PluginManager({ nighthawkHomeDir: home }),
    });
  ...
  it('merges project layers when a cwd is given, tracking origin and mutability', async () => {
    ...
    const project = await realpath(await mkdtemp(join(tmpdir(), 'nighthawk-mcp-registry-proj-')));
    ...
    await writeJson(join(sub, '.nighthawk', 'mcp.json'), {
    ...
    expect(byName.get('localOnly')).toMatchObject({
      mutable: false,
      origin: join(sub, '.nighthawk', 'mcp.json'),
    });
  });
  ...
  it('exposes plugin servers as read-only entries with their effective config', async () => {
    ...
    const registry = new McpServerRegistry({
      ...
      store: new GlobalMcpConfigStore(home),
      plugins,
      managedPluginEnv: () => ({ NIGHTHAWK_BASE_URL: 'https://managed.example.com' }),
    });
    ...
    expect(env).toMatchObject({
      ...
      NIGHTHAWK_BASE_URL: 'https://managed.example.com',
    });
