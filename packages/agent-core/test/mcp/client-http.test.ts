...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as NighthawkError).code).toBe(ErrorCodes.CONFIG_INVALID);
    return;
  }
...
describe('buildMcpHttpHeaders', () => {
  ...
  it('passes through configured static headers', () => {
    expect(
      buildMcpHttpHeaders(
        { transport: 'http', url: 'https://x', headers: { 'X-Tenant': 'nighthawk' } },
        () => undefined,
      ),
    ).toEqual({ 'X-Tenant': 'nighthawk' });
  });
  ...
  it('throws NighthawkError(config.invalid) when a configured bearer token env var is empty or missing', () => {
    expectConfigInvalid(() =>
      buildMcpHttpHeaders(
