...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-loader-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user, project-root, and project-local paths', async () => {
    ...
    const paths = await resolveMcpJsonPaths({ cwd, homeDir: '/home/user/.nighthawk' });

    expect(paths.user).toBe('/home/user/.nighthawk/mcp.json');
    expect(paths.projectRoot).toBe(join(repoRoot, '.mcp.json'));
    expect(paths.project).toBe(join(cwd, '.nighthawk', 'mcp.json'));
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.nighthawk', 'mcp.json'), {
      mcpServers: {
  ...
  it('keeps a server named __proto__ as an own key across layers', async () => {
    ...
    await mkdir(join(cwd, '.nighthawk'), { recursive: true });
    await writeFile(
      join(home, 'mcp.json'),
    ...
    expect(detailed.origins['__proto__']).toBe(join(cwd, '.nighthawk', 'mcp.json'));
  });

  it('loads root .mcp.json from the repo root and lets project-local .nighthawk/mcp.json override it', async () => {
    const home = makeTempDir();
    const repoRoot = makeTempDir();
    const cwd = join(repoRoot, 'packages', 'agent-core');
    await mkdir(join(repoRoot, '.git'), { recursive: true });
  ...
  it('throws NighthawkError(config.invalid) on invalid JSON', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeFile(join(home, 'mcp.json'), '{not json}', 'utf-8');
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toBeInstanceOf(NighthawkError);
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toMatchObject({
      code: ErrorCodes.CONFIG_INVALID,
    });
  });

  it('throws NighthawkError(config.invalid) on schema violation (unknown transport)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('throws NighthawkError(config.invalid) on schema violation (missing required field)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('honors NIGHTHAWK_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
