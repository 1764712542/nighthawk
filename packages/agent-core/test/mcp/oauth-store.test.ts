...
describe('JsonFileStore', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-store-'));
  });
  afterEach(async () => {
...
describe('MCP OAuth credential identity', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-identity-'));
  });
  afterEach(async () => {
    await rm(dir, { recursive: true, force: true });
  });
...
describe('McpOAuthClientProvider.invalidateStaleRegistration', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nighthawk-mcp-oauth-stale-'));
  });
  afterEach(async () => {
    await rm(dir, { recursive: true, force: true });
  });
