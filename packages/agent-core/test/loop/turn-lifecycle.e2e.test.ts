...
import { inputTotal } from '@nighthawk/kosong';
...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws NighthawkError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as NighthawkError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as NighthawkError).details).toEqual({ maxSteps: 2 });
