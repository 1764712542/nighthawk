...
async function makeNighthawkHome(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), 'nighthawk-home-'));
}
...
describe('PluginManager', () => {
  it('install() adds a plugin and load() rehydrates it from disk', async () => {
    ...
    let manager = new PluginManager({ nighthawkHomeDir: home });
    await manager.load();
    ...
    manager = new PluginManager({ nighthawkHomeDir: home });
    await manager.load();
    expect(manager.list()).toHaveLength(1);
  ...
  it('install() accepts a .nighthawk-plugin manifest', async () => {
    const home = await makeNighthawkHome();
    const root = await mkdtemp(path.join(tmpdir(), 'nighthawk-plugin-'));
    await mkdir(path.join(root, '.nighthawk-plugin'), { recursive: true });
    await mkdir(path.join(root, 'skills'), { recursive: true });
    await writeFile(
      path.join(root, '.nighthawk-plugin', 'plugin.json'),
      JSON.stringify({
        name: 'superpowers',
        skills: './skills/',
    ...
    expect(record.manifestKind).toBe('nighthawk-plugin-dir');
    ...
    expect(manager.pluginSkillRoots()).toContainEqual({
      ...
      source: 'extra',
      plugin: { id: 'superpowers', instructions: 'Use NightHawk tools.' },
    });
  });
  ...
  it('install() copies a symlinked plugin root into the managed plugins dir', async () => {
    ...
    const reloaded = new PluginManager({ nighthawkHomeDir: home });
    await reloaded.load();
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'nighthawk.plugin.json'),
      '{ not json',
      'utf8',
    );
    ...
    expect(manager.enabledMcpServers()).toEqual(
      expect.objectContaining({
        'plugin-demo:finance': expect.objectContaining({
          command: 'finance-mcp',
          cwd: managedRoot,
          env: expect.objectContaining({ NIGHTHAWK_HOME: home, NIGHTHAWK_PLUGIN_ROOT: managedRoot }),
        }),
  ...
  it('install() supports zip URL', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'plugin/nighthawk.plugin.json',
  ...
  it('install() from zip-url overwrites existing zip-url plugin', async () => {
    ...
    const zipBuffer1 = await createZipBuffer([
      { name: 'plugin/nighthawk.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '1.0.0' }) },
    ]);
    ...
    const zipBuffer2 = await createZipBuffer([
      { name: 'plugin/nighthawk.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '2.0.0' }) },
    ]);
  ...
  it('install() from github URL resolves latest release and records github metadata', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-abc/nighthawk.plugin.json',
  ...
  it('install() from /tree/<tag-shaped-ref> downloads via short form, not refs/heads/ (P1 regression)', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'obra-superpowers-v5.1.0/nighthawk.plugin.json',
  ...
  it('install() from github /tree/<branch> bypasses the GitHub API', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-main/nighthawk.plugin.json',
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    const cdnZip = await createZipBuffer([
      { name: 'pkg/nighthawk.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.0.0' }) },
    ]);
    ...
    const ghZip = await createZipBuffer([
      { name: 'pkg/nighthawk.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.1.0' }) },
    ]);
    using _ = mockGithubFetch({
      releaseTag: 'v5.1.0',
      tarball: ghZip,
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        timeout: 10,
        cwd: installedRoot,
        env: { NIGHTHAWK_HOME: home, NIGHTHAWK_PLUGIN_ROOT: installedRoot },
      },
    ]);
  });
