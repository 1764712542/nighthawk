...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'nighthawk-services-test-'));
  prevHome = process.env['NIGHTHAWK_HOME'];
  process.env['NIGHTHAWK_HOME'] = tmpHome;
});
...
describe('CoreProcessService direct construction', () => {
  ...
  it('rpc round-trip through createRPC reaches NighthawkCore (getCoreInfo smoke)', async () => {
    const { eventService, approvalService, questionService, logService, workspaceRegistry } = makePeers();
    const core = new CoreProcessService(
      {},
      makeEnv(tmpHome),
  ...
  it('default-wires a resolveOAuthTokenProvider when caller omits one', () => {
    ...
    const tokenProvider = resolver('managed:nighthawk');
  ...
  it('threads identity into the default resolver so refreshes carry X-Msh-Platform', async () => {
    ...
    writeFileSync(
      join(credentialsDir, 'nighthawk.json'),
      JSON.stringify({
  ...
  it('default-wires nighthawkRequestHeaders from identity when caller omits headers', () => {
    const headers = CoreProcessService._defaultNighthawkRequestHeaders(
      tmpHome,
      { productName: 'nighthawk-cli', version: '9.9.9', platform: 'nighthawk_cli' },
    );
    expect(headers).toBeDefined();
    expect(headers!['User-Agent']).toMatch(/^nighthawk-cli\/9\.9\.9/);
  ...
  it('returns undefined headers when no identity is provided (back-compat)', () => {
    const headers = CoreProcessService._defaultNighthawkRequestHeaders(tmpHome);
    expect(headers).toBeUndefined();
  });

  it('caller-supplied nighthawkRequestHeaders win over identity-derived defaults', () => {
    const explicit = { 'User-Agent': 'override/1.0' };
    const picked =
      explicit ?? CoreProcessService._defaultNighthawkRequestHeaders(
        tmpHome,
        { productName: 'nighthawk-cli', version: '9.9.9', platform: 'nighthawk_cli' },
      );
