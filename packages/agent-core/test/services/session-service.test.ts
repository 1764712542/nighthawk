...
import { emptySessionUsage, type Event, type Session } from '@nighthawk/protocol';
...
describe('SessionService.create', () => {
  ...
  it('passes model through to the agent_config when supplied', async () => {
    await svc.create({
      metadata: { cwd: '/tmp/x' },
      agent_config: { model: 'nighthawk-v1-128k' },
    });
...
describe('SessionService.update', () => {
  ...
  it('forwards agent_config.model through IPromptService.applyAgentState (source="meta")', async () => {
    await svc.update(created.id, { agent_config: { model: 'nighthawk/k9' } });
    expect(promptStub.calls).toEqual([
      { sid: created.id, patch: { model: 'nighthawk/k9' }, source: 'meta', promptId: undefined },
    ]);
  });
  ...
  it('combines model + runtime controls into a single applyAgentState call', async () => {
    await svc.update(created.id, {
      agent_config: { model: 'nighthawk/k9', plan_mode: false },
    });
    expect(promptStub.calls).toHaveLength(1);
    expect(promptStub.calls[0]?.patch).toEqual({ model: 'nighthawk/k9', plan_mode: false });
