...
import type { Event } from '@nighthawk/protocol';
...
describe('WorkspaceRegistryService', () => {
  ...
  beforeEach(async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-ws-home-'));
  ...
  async function makeProjectRoot(label: string): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), `nighthawk-ws-${label}-`));
    tempRoots.push(root);
  ...
  it('keeps a derived bucket visible even when its root no longer exists on disk', async () => {
    ...
    const goneRoot = normalizeWorkDir(join(tmpdir(), 'nighthawk-ws-gone-never-created'));
  ...
  it('registers a derived workspace under the symlink bucket key, not the realpath', async () => {
    ...
    const linkParent = await mkdtemp(join(tmpdir(), 'nighthawk-ws-link-'));
...
describe('touchWorkspaceRegistry', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-ws-touch-home-'));
    tempRoots = [];
  });
  ...
  async function makeProjectRoot(label: string): Promise<string> {
    const root = await mkdtemp(join(tmpdir(), `nighthawk-ws-touch-${label}-`));
    tempRoots.push(root);
    return normalizeWorkDir(await realpath(root));
  }
