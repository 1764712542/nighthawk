...
const MOCK_PROVIDER = { type: 'nighthawk', apiKey: 'test-key', model: 'mock-model' } as const satisfies ProviderConfig;
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-goal-session-'));
  tempDirs.push(dir);
  return dir;
}
