...
function requiredFlagEnv(id: string): string {
  ...
  return `NIGHTHAWK_EXPERIMENTAL_${id.toUpperCase()}`;
}
...
function experimentalFeatureEnabled(core: NighthawkCore, id: string): boolean | undefined {
  return core.getExperimentalFeatures().find((feature) => feature.id === id)?.enabled;
}

function setCoreKaos(core: NighthawkCore, kaos: Promise<Kaos>): void {
...
describe('NighthawkCore runtime config', () => {
  let tmp: string;
  ...
  it.skip('logs all enabled experimental flags once on core startup', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-runtime-'));
    ...
    void new NighthawkCore(async () => ({}) as never, { homeDir });
...
`,
    ...
    const first = new NighthawkCore(async () => ({}) as never, { homeDir: firstHome });
    const second = new NighthawkCore(async () => ({}) as never, { homeDir: secondHome });
  ...
  it.skip('updates the scoped experimental resolver after setNighthawkConfig', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-runtime-'));
    const homeDir = join(tmp, 'home');
    await mkdir(homeDir, { recursive: true });
    await writeFile(
...
`,
    ...
    const core = new NighthawkCore(async () => ({}) as never, { homeDir });
    expect(experimentalFeatureEnabled(core, 'micro_compaction')).toBe(false);

    await core.setNighthawkConfig({
      experimental: {
        'micro_compaction': true,
      },
    });
...
`,
    ...
    const core = new NighthawkCore(coreRpc, { homeDir });
  ...
  it('live-applies the complete persisted secondary recipe', async () => {
    ...
    vi.stubEnv('NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL', '1');
    ...
    const config = core.sessions.get(created.id)?.getReadyAgent('main')?.nighthawkConfig;
  ...
  // Regression for https://github.com/nighthawk-dev/nighthawk/issues/988: during
  ...
  it('reads workspace local.toml through persistenceKaos during createSession', async () => {
    ...
    await mkdir(join(workDir, '.nighthawk'), { recursive: true });
    await mkdir(sharedDir, { recursive: true });
    await writeFile(
      join(workDir, '.nighthawk', 'local.toml'),
  ...
  it('uses the shared OAuth resolver for NightHawk service tokens', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-runtime-'));
    const homeDir = join(tmp, 'home');
    const workDir = join(tmp, 'work');
    await mkdir(homeDir, { recursive: true });
...
oauth = { storage = "file", key = "oauth/custom-nighthawk" }
...
`,
    ...
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    const core = new NighthawkCore(coreRpc, {
      homeDir,
      nighthawkRequestHeaders: {
        'User-Agent': 'nighthawk-cli/0.0.0-test',
    ...
    expect(resolveOAuthTokenProvider).toHaveBeenCalledWith('managed:nighthawk', {
      storage: 'file',
      key: 'oauth/custom-nighthawk',
    });
    expect(session?.options.toolServices?.webSearcher).toBeDefined();

    await session!.options.toolServices?.webSearcher!.search('nighthawk');
  ...
  it('enables NightHawk web services from NIGHTHAWK_WEB_* env vars without a services config section', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-runtime-'));
    const homeDir = join(tmp, 'home');
    const workDir = join(tmp, 'work');
    await mkdir(homeDir, { recursive: true });
    ...
    vi.stubEnv('NIGHTHAWK_WEB_SEARCH_BASE_URL', 'https://search-env.example/v1');
    vi.stubEnv('NIGHTHAWK_WEB_SEARCH_API_KEY', 'env-search-key');
    vi.stubEnv('NIGHTHAWK_WEB_FETCH_BASE_URL', 'https://fetch-env.example/v1');
    vi.stubEnv('NIGHTHAWK_WEB_FETCH_API_KEY', 'env-fetch-key');
    ...
    await webSearcher!.search('nighthawk');
  ...
  it('keeps persisted credentials off an env-selected NightHawk search endpoint', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-runtime-'));
    const homeDir = join(tmp, 'home');
    const workDir = join(tmp, 'work');
    await mkdir(homeDir, { recursive: true });
...
`,
    ...
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    const core = new NighthawkCore(coreRpc, { homeDir, resolveOAuthTokenProvider });
    const rpc = await sdkRpc({
      emitEvent: vi.fn(),
      requestApproval: vi.fn(async (): Promise<ApprovalResponse> => ({ decision: 'rejected' })),
      requestQuestion: vi.fn(async () => null),
...
`,
  ...
  it('supports multiple project local and caller additionalDirs', async () => {
    ...
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    void new NighthawkCore(coreRpc, { homeDir });
    const rpc = await sdkRpc({
      emitEvent: vi.fn(),
      requestApproval: vi.fn(async (): Promise<ApprovalResponse> => ({ decision: 'rejected' })),
      requestQuestion: vi.fn(async () => null),
  ...
  it('adds an additional dir through the session RPC', async () => {
    ...
    const localToml = await readFile(join(workDir, '.nighthawk', 'local.toml'), 'utf-8');
    const session = core.sessions.get(created.id);
    const mainAgent = session?.getReadyAgent('main');
    ...
    expect(result).toMatchObject({
      ...
      projectRoot: workDir,
      configPath: join(workDir, '.nighthawk', 'local.toml'),
      persisted: true,
    });
  ...
  it('adds a session-only additional dir without writing local.toml', async () => {
    ...
    await expect(readFile(join(workDir, '.nighthawk', 'local.toml'), 'utf-8')).rejects.toThrow();
  });
  ...
  it('rejects createSession when shell runtime initialization fails', async () => {
    ...
    setCoreKaos(
      core,
      rejectedKaos(
        new NighthawkError(ErrorCodes.SHELL_GIT_BASH_NOT_FOUND, 'Git Bash missing'),
      ),
    );
...
describe('NighthawkCore print-mode defaults', () => {
  let tmp: string;
  ...
  async function createCorePair(homeDir: string, uiMode?: string) {
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    const core = new NighthawkCore(coreRpc, { homeDir, uiMode });
    const rpc = await sdkRpc({
      emitEvent: vi.fn(),
      requestApproval: vi.fn(async (): Promise<ApprovalResponse> => ({ decision: 'rejected' })),
      requestQuestion: vi.fn(async () => null),
  ...
  async function setupDirs(configToml: string): Promise<{ homeDir: string; workDir: string }> {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-print-defaults-'));
    const homeDir = join(tmp, 'home');
    const workDir = join(tmp, 'work');
    await mkdir(homeDir, { recursive: true });
    await mkdir(workDir, { recursive: true });
  ...
  it('applies print-mode subagent/loop defaults to sessions when uiMode is print', async () => {
    ...
    expect(main?.nighthawkConfig?.subagent?.timeoutMs).toBe(0);
    expect(main?.nighthawkConfig?.background?.bashTaskTimeoutS).toBe(0);
    expect(main?.nighthawkConfig?.loopControl?.maxStepsPerTurn).toBe(0);

    // The raw user config is left untouched so config reads/writes still
    // round-trip the user's file values.
    const raw = await core.getNighthawkConfig();
...
`);
    ...
    const main = core.sessions.get(created.id)?.getReadyAgent('main');
    expect(main?.nighthawkConfig?.subagent?.timeoutMs).toBe(5000);
    expect(main?.nighthawkConfig?.loopControl?.maxStepsPerTurn).toBe(7);
  });
  ...
  it('does not apply print-mode defaults outside print mode', async () => {
    ...
    const main = core.sessions.get(created.id)?.getReadyAgent('main');
    expect(main?.nighthawkConfig?.subagent).toBeUndefined();
    expect(main?.nighthawkConfig?.loopControl).toBeUndefined();
  });
...
async function writeSessionStartPlugin(root: string, skillBody: string): Promise<void> {
  ...
  await writeFile(
    join(root, 'nighthawk.plugin.json'),
    JSON.stringify({
      name: 'demo',
      version: '1.0.0',
      skills: ['./skills'],
...
function pluginSessionStartReminders(core: NighthawkCore, sessionId: string): string[] {
