import { classifyNighthawkQuotaError } from '@nighthawk/kosong';
import { describe, expect, it } from 'vitest';

import type { NighthawkConfig, ModelAlias } from '../../src/config';
...
function resolveRuntimeProvider(input: {
  ...
  const manager = new ProviderManager({
    config: input.config,
    nighthawkRequestHeaders: input.nighthawkRequestHeaders,
...
const BASE_CONFIG: NighthawkConfig = {
...
const TEST_NIGHTHAWK_HEADERS = {
  'User-Agent': 'nighthawk-cli/0.0.0-test',
  'X-Msh-Platform': 'nighthawk_cli',
...
describe('resolveRuntimeProvider model metadata', () => {
  it('uses config model metadata as the source of truth', () => {
    ...
    expect(resolved.provider.model).toBe('nighthawk');
  });
  ...
  it('uses config NightHawk capabilities without requiring an api key during OAuth setup', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('does not infer NightHawk capabilities from the provider model name', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        models: {
...
describe('resolveRuntimeProvider maxOutputSize forwarding', () => {
  ...
  it('prefers alias.baseUrl over the provider base URL on the nighthawk, google-genai, and openai_responses wires', () => {
    const config = {
      ...BASE_CONFIG,
      providers: {
        ...BASE_CONFIG.providers,
        nighthawk: { type: 'nighthawk', apiKey: 'sk-nighthawk', baseUrl: 'https://nighthawk.example.test/v1' } as const,
        google: {
          type: 'google-genai',
          apiKey: 'sk-google',
    ...
    expect(resolveRuntimeProvider({ config, model: 'nighthawk/tenant' }).provider).toMatchObject({
      type: 'nighthawk',
      baseUrl: 'https://tenant.example.test/v1',
    });
...
describe('resolveRuntimeProvider NightHawk request headers', () => {
  it('does not set defaultHeaders when no nighthawkRequestHeaders or customHeaders exist', () => {
  ...
  it('uses only customHeaders when nighthawkRequestHeaders are missing', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('passes nighthawkRequestHeaders through to NightHawk provider defaultHeaders', () => {
    const resolved = resolveRuntimeProvider({
      config: BASE_CONFIG,
      nighthawkRequestHeaders: TEST_NIGHTHAWK_HEADERS,
    });

    expect(resolved.provider).toMatchObject({
      type: 'nighthawk',
      defaultHeaders: TEST_NIGHTHAWK_HEADERS,
    });
  });

  it('passes the prompt cache key to NightHawk generation kwargs', () => {
    const resolved = resolveRuntimeProvider({
      config: BASE_CONFIG,
  ...
  it('lets provider customHeaders override nighthawkRequestHeaders', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('applies only the User-Agent from nighthawkRequestHeaders to non-NightHawk providers', () => {
    const resolved = resolveRuntimeProvider({
      config: {
    ...
    expect(resolved.provider).toMatchObject({
      ...
      apiKey: 'sk-openai',
      defaultHeaders: {
        'User-Agent': TEST_NIGHTHAWK_HEADERS['User-Agent'],
      },
    });
    // Device identity headers (`X-Msh-*`) stay NightHawk-only — they must not leak
...
describe('ProviderManager prompt cache key', () => {
  it('applies a prompt cache key to NightHawk providers', () => {
    const manager = new ProviderManager({
      config: BASE_CONFIG,
      promptCacheKey: 'session-test',
    });
    const resolved = manager.resolveProviderConfig('nighthawk/nighthawk');
  ...
  it('applies a prompt cache key to OpenAI providers (chat completions + responses)', () => {
    for (const type of ['openai', 'openai_responses'] as const) {
      ...
      // Same session-affinity intent as the NightHawk branch above: every request
  ...
  it('reads the current config when constructed with a function', () => {
    let sharedConfig: NighthawkConfig = { providers: {} };
    const manager = new ProviderManager({
      config: () => sharedConfig,
      promptCacheKey: 'session-test',
    });
...
describe('ProviderManager OAuth auth', () => {
  function oauthConfig(): NighthawkConfig {
    return {
      ...BASE_CONFIG,
      providers: {
        'managed:nighthawk': {
  ...
  it('preserves non-NightHawk token fetch failures instead of guessing their category', async () => {
    ...
    const resolveAuth = manager.resolveAuth('nighthawk/nighthawk');
  ...
  it('keeps explicit login-required token failures as login-required errors', async () => {
    const manager = new ProviderManager({
      ...
      resolveOAuthTokenProvider: () => ({
        async getAccessToken() {
          throw new NighthawkError(ErrorCodes.AUTH_LOGIN_REQUIRED, 'not logged in');
        },
      }),
    });
...
describe('per-model protocol routing', () => {
  it('routes a protocol:anthropic model on a nighthawk provider through the anthropic transport with the REST base stripped of /v1', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        models: {
          'nighthawk/nighthawk': {
            ...BASE_CONFIG.models!['nighthawk/nighthawk']!,
            protocol: 'anthropic',
          },
        },
      },
    ...
    expect(resolved.providerName).toBe('managed:nighthawk');
    expect(resolved.provider).toMatchObject({
      type: 'anthropic',
      model: 'nighthawk',
      baseUrl: 'https://api.example',
    });
    // NightHawk over the Anthropic transport keeps its vendor error classifier —
    // a NightHawk quota 429 must fail fast on this route too.
    expect(
      (resolved.provider as { convertError?: (error: unknown) => unknown }).convertError,
    ).toBe(classifyNighthawkQuotaError);
  });
  ...
  it('does not strip the baseUrl of a provider that is itself typed anthropic', () => {
    ...
    // A plain anthropic provider carries no NightHawk vendor classifier.
    expect(
      (resolved.provider as { convertError?: (error: unknown) => unknown }).convertError,
    ).toBeUndefined();
  });
...
describe('resolveRuntimeProvider model overrides', () => {
  it('keeps supportEfforts out of the nighthawk provider config', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        models: {
    ...
    expect(resolved.provider).toMatchObject({ type: 'nighthawk' });
