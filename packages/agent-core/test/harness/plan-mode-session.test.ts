...
import { createRPC, NighthawkCore, type CoreAPI, type SDKAPI } from '../../src';
...
describe('plan-mode bootstrap from config.defaultPlanMode', () => {
  ...
  beforeEach(async () => {
    tmp = await mkdtemp(join(tmpdir(), 'nighthawk-plan-mode-'));
  ...
  async function createTestRpc() {
    ...
    void new NighthawkCore(coreRpc, { homeDir, configPath });
    return sdkRpc({
      emitEvent: vi.fn(),
