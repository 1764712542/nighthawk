...
beforeEach(async () => {
  workDir = await mkdtemp(join(tmpdir(), 'nighthawk-agent-profile-'));
});
...
describe('agent profile loader', () => {
  it('loads YAML profiles, inherits templates, and renders with runtime context', async () => {
    const systemPath = await write(
      ...
      [
        'os={{ NIGHTHAWK_OS }}',
        'cwd={{ NIGHTHAWK_WORK_DIR }}',
        'listing={{ NIGHTHAWK_WORK_DIR_LS }}',
        'agents={{ NIGHTHAWK_AGENTS_MD }}',
        'skills={{ NIGHTHAWK_SKILLS }}',
        'parent={{ parentOnly }}',
        'child={{ childOnly }}',
        'role={{ ROLE_ADDITIONAL }}',
        '{% if NIGHTHAWK_OS == "macOS" %}nunjucks-ok{% endif %}',
      ].join('\n'),
    );
    await write(
      'agent.yaml',
...
describe('default agent profiles', () => {
  it('links bundled subagents and keeps role-specific tool sets observable', () => {
    ...
    expect(DEFAULT_AGENT_PROFILES['agent']?.subagents?.['security-audit']).toBe(
      DEFAULT_AGENT_PROFILES['security-audit'],
    );
    expect(DEFAULT_AGENT_PROFILES['agent']?.subagents?.['pentest']).toBe(
      DEFAULT_AGENT_PROFILES['pentest'],
    );
    expect(DEFAULT_AGENT_PROFILES['agent']?.subagents?.['review']).toBe(
      DEFAULT_AGENT_PROFILES['review'],
    );
    ...
    expect(DEFAULT_AGENT_PROFILES['agent']?.tools).toEqual(
      expect.arrayContaining(['SecurityScan', 'SecretScan', 'TaintTrace', 'DepAudit']),
    );
    expect(DEFAULT_AGENT_PROFILES['security-audit']?.tools).toEqual(
      expect.arrayContaining(['SecurityScan', 'SecretScan', 'TaintTrace', 'DepAudit']),
    );
    expect(DEFAULT_AGENT_PROFILES['security-audit']?.tools).not.toContain('Write');
    expect(DEFAULT_AGENT_PROFILES['pentest']?.tools).toEqual(
      expect.arrayContaining(['Bash', 'SecurityScan', 'SecretScan']),
    );
    expect(DEFAULT_AGENT_PROFILES['pentest']?.tools).not.toContain('Write');
    expect(DEFAULT_AGENT_PROFILES['review']?.tools).not.toContain('Write');
  });
  ...
  it('renders the bundled default prompt from the current runtime context', () => {
    ...
    expect(first).toContain('You are NightHawk CLI');
