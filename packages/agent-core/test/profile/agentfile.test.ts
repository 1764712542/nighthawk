...
async function makeTempDir(prefix = 'nighthawk-agentfile-'): Promise<string> {
...
describe('Session agentfile wiring', () => {
  ...
  it('binds a file-defined main profile selected by profileName', async () => {
    ...
    const brandHome = await makeTempDir();
    const osHome = await makeTempDir();
    await mkdir(join(workDir, '.nighthawk', 'agents'), { recursive: true });
    await writeFile(
      join(workDir, '.nighthawk', 'agents', 'reviewer.md'),
