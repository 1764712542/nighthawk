...
describe('AskUserQuestionTool', () => {
  ...
  it('preserves foreground error semantics when background questions are disabled', async () => {
    const { tool } = makeTool({
      ...
      requestQuestion: async () => {
        throw new NighthawkError(ErrorCodes.NOT_IMPLEMENTED, 'Client does not support questions');
      },
    });
  ...
  it('resolves question rpc error responses as dismissed answers', async () => {
    const { tool } = makeTool({
      requestQuestion: async () => {
        throw new NighthawkError(ErrorCodes.INTERNAL, 'JSON-RPC question error response');
      },
    });
