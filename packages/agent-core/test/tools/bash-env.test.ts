...
import type { Environment, KaosProcess } from '@nighthawk/kaos';
...
describe('BashTool noninteractive env semantics', () => {
  ...
  it('lets kaos-level env override BashTool env and observes in-place updates', async () => {
    ...
    expect(firstEnv['NIGHTHAWK_ENV']).toBe('initial');

    sessionEnv.NIGHTHAWK_ENV = 'updated';
    await executeTool(tool, {
      turnId: '0',
    ...
    expect(secondEnv['NIGHTHAWK_ENV']).toBe('updated');
  });
});
