...
describe('originalImageCacheDir', () => {
  it('defaults to a nighthawk cache directory under the OS temp dir', () => {
    const dir = originalImageCacheDir();
    expect(dir.startsWith(tmpdir())).toBe(true);
    expect(dir).toContain('nighthawk');
  });
});

describe('sessionMediaOriginalsDir', () => {
  it('nests the originals dir inside the session dir', () => {
    expect(sessionMediaOriginalsDir('/home/u/.nighthawk/sessions/ws/abc')).toBe(
      '/home/u/.nighthawk/sessions/ws/abc/media-originals',
    );
  });
});
