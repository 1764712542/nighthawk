...
import { NighthawkWebSearchProvider } from '../../src/tools/providers/nighthawk-web-search';
  ...
  it('classifies authentication failures', async () => {
    const provider: WebSearchProvider = {
      search: vi
        .fn()
        .mockRejectedValue(
          new Error('NightHawk search request failed: HTTP 401 (auth/unauthorized).'),
        ),
    };
    const tool = new WebSearchTool(provider);
    const result = await executeTool(tool, {
...
describe('NighthawkWebSearchProvider', () => {
  it('maps site_name to siteName and does not forward page content', async () => {
    ...
    const provider = new NighthawkWebSearchProvider({
      apiKey: 'k',
