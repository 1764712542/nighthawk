...
import { NighthawkFetchURLProvider } from '../../../src/tools/providers/nighthawk-fetch-url';
...
describe('NighthawkFetchURLProvider auth fallback', () => {
  it('falls back to the configured API key when the token provider has no token', async () => {
    ...
    // the host should call nighthawk with the static api key when oauth
    ...
    const provider = new NighthawkFetchURLProvider({
...
describe('NighthawkFetchURLProvider content kind', () => {
