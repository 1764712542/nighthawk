...
describe('SkillTool metadata and schema', () => {
  ...
  it('references the nighthawk-skill-loaded block in the tool description', () => {
    const tool = skillTool(registry());

    // A skill loaded earlier surfaces a <nighthawk-skill-loaded> block; the description
    // must steer the model to follow it rather than re-invoking the tool.
    expect(tool.description).toContain('nighthawk-skill-loaded');
  });
});
...
describe('SkillTool execution', () => {
  ...
  it('records inline skill content as a loaded skill message', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toContain(
      '<nighthawk-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="message text">\nbody of commit\n\nARGUMENTS: message text\n</nighthawk-skill-loaded>',
    );
  ...
  it('keeps plugin instructions adjacent to model-invoked skill content', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toContain(
      '<nighthawk-skill-loaded name="brainstorming" trigger="model-tool" source="extra" dir="/skills/brainstorming" args="">\n' +
        '<nighthawk-plugin-instructions plugin="superpowers">\n' +
        'Use AskUserQuestion for clarifying questions.\n' +
        '</nighthawk-plugin-instructions>\n\nbrainstorm body\n' +
        '</nighthawk-skill-loaded>',
    );
  });
  ...
  it('expands skill body placeholders for model-invoked inline skills', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toContain(
      '<nighthawk-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="-m &quot;fix login&quot;">\nFlag: -m\nCommit message: fix login\nRaw: -m "fix login"\n</nighthawk-skill-loaded>',
    );
  ...
  it('expands session id from the skill registry for model-invoked skills', async () => {
    const methods = skillToolMethods();
    const tool = skillTool(
      registry([skill('session-aware', {}, 'Session: ${NIGHTHAWK_SESSION_ID}')], {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toContain(
      '<nighthawk-skill-loaded name="session-aware" trigger="model-tool" source="user" dir="/skills/session-aware" args="">\nSession: ses_model_skill\n</nighthawk-skill-loaded>',
    );
  });
  ...
  it('escapes skill name and args in the wrapper boundaries', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toContain(
      '<nighthawk-skill-loaded name="a&amp;b" trigger="model-tool" source="user" dir="/skills/a&amp;b" args="&lt;raw &quot;value&quot;&gt;">\nbody of a&b\n\nARGUMENTS: &lt;raw "value"&gt;\n</nighthawk-skill-loaded>',
    );
    expect(methods.recordSkillActivation).toHaveBeenCalledTimes(1);
  });
