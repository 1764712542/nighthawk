 ...
 * boundary. Run with `pnpm --filter @nighthawk/agent-core exec vitest run
...
import { ErrorCodes, NighthawkError } from '../../src/errors';
import {
  NighthawkConfigSchema,
...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'nighthawk-core-config-'));
  tempDirs.push(dir);
  return dir;
}

function expectNighthawkErrorCode(fn: () => unknown, code: string): void {
  try {
    fn();
  } catch (error) {
    expect(error).toBeInstanceOf(NighthawkError);
    expect((error as NighthawkError).code).toBe(code);
    return;
  }
...
[providers."managed:nighthawk".env]
...
describe('harness config TOML loader', () => {
  it('parses the current config.toml shape through explicit field mappings', () => {
    ...
    expect(config.providers['managed:nighthawk']).toMatchObject({
      type: 'nighthawk',
    ...
    expect(config.services?.nighthawkSearch?.customHeaders).toEqual({ 'X-Search': '1' });
    expect(config.services?.nighthawkFetch?.apiKey).toBe('sk-fetch');
...
api_key = ""
oauth = { storage = "file", key = "oauth/nighthawk-env-1234", oauth_host = "https://auth.dev.example.test" }
...
`;
    const config = parseConfigString(toml, configPath);
    expect(config.providers['managed:nighthawk']?.oauth).toEqual({
      storage: 'file',
      key: 'oauth/nighthawk-env-1234',
      oauthHost: 'https://auth.dev.example.test',
    });
    expect(config.services?.nighthawkSearch?.oauth?.oauthHost).toBe('https://auth.dev.example.test');

    await writeConfigFile(configPath, config);
    const text = await readFile(configPath, 'utf-8');
    expect(text).toContain('oauth_host = "https://auth.dev.example.test"');
    const roundTripped = parseConfigString(text, configPath);
    expect(roundTripped.providers['managed:nighthawk']?.oauth?.oauthHost).toBe(
...
`;
  ...
  it('loads defaults for absent files and writes typed fields without dropping raw sections', async () => {
    ...
    const text = await readFile(configPath, 'utf-8');
    expect(text).toContain('default_model = "nighthawk/nighthawk"');
  ...
  it('creates a parseable default config scaffold without changing runtime defaults', async () => {
    ...
    const text = await readFile(configPath, 'utf-8');
    expect(text).toContain('Runtime settings for NightHawk.');
  ...
  it('rejects invalid TOML and invalid schema with NighthawkError(config.invalid)', () => {
    expectNighthawkErrorCode(
...
describe('harness config schema and patch merge', () => {
  it('accepts the empty public config and requires model context size in full configs', () => {
    expect(NighthawkConfigSchema.parse({})).toEqual({ providers: {} });
    expect(() =>
      validateConfig({
        providers: {
  ...
  it('accepts the Node.js timer upper boundary for MCP timeouts', () => {
    expect(
      NighthawkConfigSchema.safeParse({
        mcp: {
  ...
  it('deep-merges validated patches while preserving existing typed and raw data', () => {
    ...
    expect(merged.providers['managed:nighthawk']).toMatchObject({
      type: 'nighthawk',
      baseUrl: 'https://api.kimi.com/coding/v1',
      apiKey: 'sk-patched',
      env: { GOOGLE_CLOUD_PROJECT: 'project-1' },
    });
    expect(merged.models?.['nighthawk/nighthawk']).toMatchObject({
      provider: 'managed:nighthawk',
      model: 'nighthawk',
      maxContextSize: 262144,
      capabilities: ['tool_use'],
...
`);
  ...
  it('accepts maxOutputSize on a model alias and round-trips it', () => {
    const parsed = NighthawkConfigSchema.parse({
...
describe('config path env override', () => {
  it('uses NIGHTHAWK_HOME when no explicit homeDir is supplied', () => {
    const saved = process.env['NIGHTHAWK_HOME'];
    try {
      process.env['NIGHTHAWK_HOME'] = '/tmp/nighthawk-from-env';

      expect(resolveNighthawkHome()).toBe('/tmp/nighthawk-from-env');
      expect(resolveNighthawkHome('/tmp/nighthawk-explicit')).toBe('/tmp/nighthawk-explicit');
      expect(resolveConfigPath({})).toBe('/tmp/nighthawk-from-env/config.toml');
      expect(resolveConfigPath({ configPath: '/tmp/custom.toml' })).toBe('/tmp/custom.toml');
    } finally {
      if (saved === undefined) delete process.env['NIGHTHAWK_HOME'];
      else process.env['NIGHTHAWK_HOME'] = saved;
    }
  });
});
...
describe('config value env override helpers', () => {
  ...
  it('resolves env before config before default', () => {
    expect(
      resolveConfigValue({
        env: { NIGHTHAWK_TEST_FLAG: '0' },
        envKey: 'NIGHTHAWK_TEST_FLAG',
        configValue: true,
        defaultValue: true,
  ...
  it('ignores invalid env values', () => {
    expect(
      resolveConfigValue({
        env: { NIGHTHAWK_TEST_FLAG: 'invalid' },
        envKey: 'NIGHTHAWK_TEST_FLAG',
        configValue: false,
        defaultValue: true,
        parseEnv: parseBooleanEnv,
...
`;
  ...
  it('reports a fileError and defaults on invalid TOML syntax', async () => {
    ...
    expect(result.fileError).toBeInstanceOf(NighthawkError);
...
`);
    ...
    expect(result.config.providers['nighthawk']).toMatchObject({ type: 'nighthawk', apiKey: 'sk-good' });
...
`);
    ...
    expect(result.config.providers['nighthawk']).toMatchObject({ type: 'nighthawk' });
    expect(result.fileWarnings).toHaveLength(1);
...
`);
    ...
    expect(result.config.providers['nighthawk']).toBeDefined();
...
`);
  ...
  it('applies NIGHTHAWK_MODEL_* env overrides on top of a salvaged config', async () => {
    const configPath = await writeTempConfig(`${VALID_TOML}
[loop_control]
max_steps_per_turn = "nope"
`);
    const result = loadRuntimeConfigSafe(configPath, {
      ...
      NIGHTHAWK_MODEL_API_KEY: 'sk-env',
      NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE: '262144',
    });
    expect(result.envWarnings).toEqual([]);
    expect(result.config.models?.['__nighthawk_env_model__']).toBeDefined();
    expect(result.config.providers['nighthawk']).toBeDefined();
    expect(result.fileWarnings).toHaveLength(1);
  });

  it('skips NIGHTHAWK_MODEL_* overrides with an env warning instead of throwing', async () => {
    const configPath = await writeTempConfig(VALID_TOML);
    const result = loadRuntimeConfigSafe(configPath, {
      NIGHTHAWK_MODEL_NAME: 'env-model',
    });
    ...
    expect(result.envWarnings[0]).toContain('NIGHTHAWK_MODEL');
...
`);
    try {
      ...
      expect(error).toBeInstanceOf(NighthawkError);
      expect((error as NighthawkError).message).toContain('fix it first');
      expect((error as NighthawkError).message).toContain('nighthawk doctor');
      expect((error as NighthawkError).message).not.toContain('invalid_type');
    }
...
describe('model overrides TOML', () => {
  it('parses nested model overrides from snake_case TOML', () => {
    const config = parseConfigString(`
[models."nighthawk/kimi-k2"]
provider = "managed:nighthawk"
model = "kimi-k2"
max_context_size = 262144
support_efforts = ["low", "high", "max"]

[models."nighthawk/kimi-k2".overrides]
support_efforts = ["low", "high"]
default_effort = "high"
`);

    expect(config.models?.['nighthawk/kimi-k2']?.overrides).toEqual({
...
`);
    ...
    const overrides = models['nighthawk/kimi-k2']?.['overrides'] as Record<string, unknown>;
