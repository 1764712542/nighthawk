/**
 * Scenario: the unified MCP management plane on NighthawkCore — the registry-backed
 ...
 *
 * Run with `pnpm --filter @nighthawk/agent-core exec vitest run test/rpc/mcp-rpc.test.ts`.
 */
...
async function makePlugin(name: string, mcpServers: Record<string, unknown>): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), `nighthawk-mcp-rpc-plugin-`));
...
async function makeCore(seed?: (home: string) => Promise<void>): Promise<CoreFixture> {
  const tmp = await realpath(await mkdtemp(join(tmpdir(), 'nighthawk-mcp-rpc-')));
  ...
  const core = new NighthawkCore(coreRpc, { homeDir: home });
  cores.push(core);
...
function coreOAuth(core: NighthawkCore): McpOAuthService {
  return (core as unknown as { mcpOAuth: McpOAuthService }).mcpOAuth;
}

describe('NighthawkCore unified MCP management plane', () => {
