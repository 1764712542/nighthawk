...
function makeCore(home: string): NighthawkCore {
  return new NighthawkCore(async () => ({}) as never, { homeDir: home });
}
...
describe('NighthawkCore degraded config loading', () => {
  it('reports no diagnostics for a valid config', async () => {
    const core = makeCore(await makeHome(VALID_TOML));
    const config = await core.getNighthawkConfig({});
    expect(config.providers['nighthawk']).toBeDefined();
...
`);
    ...
    const write = core.setNighthawkConfig({ thinking: { enabled: true } });
    await expect(write).rejects.toThrow(/fix it first/i);
    await expect(write).rejects.toThrow(/nighthawk doctor/);
  ...
  it('keeps the last good config when the file breaks mid-run', async () => {
    ...
    const kept = await core.getNighthawkConfig({ reload: true });
    expect(kept.providers['nighthawk']).toBeDefined();
    ...
    const adopted = await core.getNighthawkConfig({ reload: true });
    expect(adopted.thinking?.enabled).toBe(true);
    await expect(core.getConfigDiagnostics({})).resolves.toEqual({ warnings: [] });
  });
});

describe('NighthawkCore imageLimits scoping', () => {
...
`,
    ...
    await coreB.getNighthawkConfig({ reload: true });
...
`,
    ...
    await core.getNighthawkConfig({ reload: true });
