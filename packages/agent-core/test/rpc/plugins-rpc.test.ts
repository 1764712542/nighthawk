...
import { NighthawkCore } from '../../src/rpc/core-impl';

describe('NighthawkCore plugin RPCs', () => {
  it('install → list → setEnabled → remove round trip', async () => {
    ...
    await writeFile(
      path.join(pluginRoot, 'nighthawk.plugin.json'),
      JSON.stringify({ name: 'demo', version: '1.0.0' }),
      'utf8',
    );

    const core = new NighthawkCore(async () => ({}) as never, { homeDir: home });
  ...
  it('inspects global and plugin MCP servers through the v1 app catalog', async () => {
    ...
    const oauth = new McpOAuthService({ nighthawkHomeDir: home });
    await oauth
  ...
  it('injects persisted managed NightHawk environment into the datasource plugin MCP server', async () => {
    const previousBaseUrl = process.env['NIGHTHAWK_BASE_URL'];
    const previousCodeOAuthHost = process.env['NIGHTHAWK_OAUTH_HOST'];
    const previousOAuthHost = process.env['NIGHTHAWK_OAUTH_HOST'];
    delete process.env['NIGHTHAWK_BASE_URL'];
...
`,
      ...
      await writeFile(
        path.join(pluginRoot, 'nighthawk.plugin.json'),
        JSON.stringify({
          name: 'nighthawk-datasource',
          mcpServers: {
            data: { command: 'node', args: ['./bin/nighthawk-datasource.mjs'] },
          },
        }),
        'utf8',
      );
      ...
      expect(mcpConfig.servers['plugin-nighthawk-datasource:data']?.env).toEqual(
        expect.objectContaining({
          NIGHTHAWK_BASE_URL: 'https://api.dev.example.test/coding/v1',
          NIGHTHAWK_OAUTH_HOST: 'https://auth.dev.example.test',
        }),
      );
    } finally {
      restoreEnv('NIGHTHAWK_BASE_URL', previousBaseUrl);
      restoreEnv('NIGHTHAWK_OAUTH_HOST', previousCodeOAuthHost);
      restoreEnv('NIGHTHAWK_OAUTH_HOST', previousOAuthHost);
    }
  });
