...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as NighthawkError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(NighthawkError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown NighthawkError across the wire preserving code and details', async () => {
    ...
    await connectRemote({
      async prompt(payload) {
        if (payload.input === '') {
          throw new NighthawkError(ErrorCodes.REQUEST_PROMPT_INPUT_EMPTY, 'Prompt input cannot be empty', {
    ...
    expect(received).toBeInstanceOf(NighthawkError);
    const error = received as NighthawkError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new NighthawkError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as NighthawkError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new NighthawkError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to NighthawkError(internal)', async () => {
    interface RemoteSide {
    ...
    await expect(remoteProxy.misbehave({})).rejects.toBeInstanceOf(NighthawkError);
  });
});
