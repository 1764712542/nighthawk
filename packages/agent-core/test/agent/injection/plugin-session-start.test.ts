    ...
    skills: {
      registry: {
        ...
        renderSkillPrompt: (skill) => {
          ...
          return `<nighthawk-plugin-instructions plugin="${plugin.id}">\n${instructions}\n</nighthawk-plugin-instructions>\n\n${skill.content}`;
        },
      },
    },
    log: {
...
describe('PluginSessionStartInjector', () => {
  it('injects one <plugin_session_start> block per declared sessionStart on first call', async () => {
    ...
    expect(text).toContain('<nighthawk-plugin-instructions plugin="superpowers">');
  ...
  it('does not hard-code Superpowers guidance when the skill has no plugin instructions', async () => {
    ...
    expect(text).not.toContain('<nighthawk-plugin-instructions plugin="superpowers">');
