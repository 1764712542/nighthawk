...
// than mocking it.
import type { ContentPart, Message } from '@nighthawk/kosong';
...
const PROVIDER = { type: 'nighthawk', apiKey: 'test-key', model: 'nighthawk' } as const;
const CAPS = {
  image_in: true,
  video_in: true,
  audio_in: false,
...
describe('compaction — probe tests (high-risk scenarios)', () => {
  ...
  it.skip('does not clear recent tool results when projecting a shrunk suffix under an active micro-compaction cutoff', () => {
    ...
    // flag explicitly rather than inheriting the ambient NIGHTHAWK_EXPERIMENTAL
    ...
    const ctx = testAgent({
      experimentalFlags: new FlagResolver(
        { NIGHTHAWK_EXPERIMENTAL_MICRO_COMPACTION: '1' },
        FLAG_DEFINITIONS,
      ),
    });
    ctx.configure({ provider: PROVIDER, modelCapabilities: CAPS });
