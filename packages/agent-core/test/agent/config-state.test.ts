...
import { emptyUsage } from '@nighthawk/kosong';
...
describe('ConfigState model capabilities', () => {
  ...
  it('computes provider and model capabilities from ProviderManager metadata', () => {
    ...
    config.update({ modelAlias: 'nighthawk/nighthawk' });

    expect(config.model).toBe('nighthawk/nighthawk');
    expect(config.providerConfig.model).toBe('nighthawk');
  ...
  it('does not infer NightHawk capabilities from the provider catalogue', () => {
    ...
    config.update({ modelAlias: 'nighthawk' });
  ...
  it('clamps the LLM completion cap to 128k for openai-compatible providers', async () => {
    ...
    // non-NightHawk chat-completions providers.
...
describe('ConfigState thinking clamp for always-thinking models', () => {
  function alwaysThinkingAgent() {
    ...
    // `agent.nighthawkConfig.models`, so the same config must back both the
    // ProviderManager (provider resolution) and the agent's nighthawkConfig (the
    // clamp's model lookup).
    const config: NighthawkConfig = {
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'test-key' } },
      models: {
        'nighthawk/deep': {
          provider: 'nighthawk',
  ...
  it('clamps thinkingEffort off to the model default effort', () => {
    ...
    ctx.agent.config.update({ modelAlias: 'nighthawk/deep', thinkingEffort: 'off' });
  ...
  it('keeps thinking off working for toggleable models', () => {
    const ctx = alwaysThinkingAgent();
    ctx.agent.config.update({ modelAlias: 'nighthawk/toggle', thinkingEffort: 'off' });
  ...
  it('re-clamps a stale off when switching onto an always-thinking model', () => {
    ...
    ctx.agent.config.update({ modelAlias: 'nighthawk/deep' });
    expect(ctx.agent.config.thinkingEffort).toBe('on');
  });

  it('falls back to the target default when a model switch carries an unsupported effort', () => {
    const ctx = alwaysThinkingAgent();
    ctx.agent.config.update({ modelAlias: 'nighthawk/ultra', thinkingEffort: 'ultra' });

    ctx.agent.config.update({ modelAlias: 'nighthawk/standard' });
  ...
  it('projects an inherited concrete effort to on when switching to a boolean model', () => {
    ...
    ctx.agent.config.update({ modelAlias: 'nighthawk/toggle' });
  ...
  it('rejects an unsupported effort explicitly set on the current NightHawk model', () => {
    ...
    expect(() => {
      ...
      'Thinking effort "ultra" is not supported by model "nighthawk/standard"',
    );
  });
});

describe('ConfigState.provider applies global NIGHTHAWK_MODEL_* request config', () => {
  function nighthawkAgent() {
    const config: NighthawkConfig = {
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'test-key' } },
      models: {
        'nighthawk': {
  ...
  // the agent's nighthawkConfig (where ConfigState reads thinking.keep).
  function nighthawkAgentWithThinkingKeep(keep: string | undefined) {
    const config: NighthawkConfig = {
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'test-key' } },
      models: {
        'nighthawk': {
  ...
  it('injects NIGHTHAWK_MODEL_TEMPERATURE into config.provider (the provider compaction also uses)', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_TEMPERATURE', '0.3');
    try {
      const ctx = nighthawkAgent();
      ctx.agent.config.update({ modelAlias: 'nighthawk' });
  ...
  it('injects NIGHTHAWK_MODEL_THINKING_KEEP into config.provider when thinking is on (so compaction keeps it)', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_KEEP', 'all');
    try {
      const ctx = nighthawkAgent();
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'high' });
  ...
  it('does NOT inject thinking.keep into config.provider when thinking is off', () => {
    ...
    try {
      const ctx = nighthawkAgent();
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'off' });
  ...
  it('injects thinking.keep="all" into config.provider by default (no env, no config)', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_KEEP', '');
    try {
      const ctx = nighthawkAgent();
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'high' });
  ...
  it('config thinking.keep="off" disables keep by default', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_KEEP', '');
    try {
      const ctx = nighthawkAgentWithThinkingKeep('off');
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'high' });
  ...
  it('env off-value overrides config thinking.keep="all"', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_KEEP', 'off');
    try {
      const ctx = nighthawkAgentWithThinkingKeep('all');
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'high' });
  ...
  it('keeps the forced NightHawk effort synchronized between state and provider', () => {
  ...
  it('reports the forced effort for an env-synthesized boolean NightHawk model', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_NAME', 'nighthawk');
    vi.stubEnv('NIGHTHAWK_MODEL_API_KEY', 'test-key');
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_EFFORT', 'max');
    try {
  ...
  it('applies the NightHawk force through an Anthropic protocol override', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_EFFORT', 'max');
    try {
      const config: NighthawkConfig = {
        providers: { nighthawk: { type: 'nighthawk', apiKey: 'test-key' } },
      ...
      ctx.agent.config.update({
        modelAlias: 'nighthawk-anthropic',
        thinkingEffort: 'high',
      });
  ...
  it('does not carry the NightHawk force into a non-NightHawk model switch', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_EFFORT', 'max');
    try {
      const config: NighthawkConfig = {
        providers: {
  ...
  it('does NOT inject NIGHTHAWK_MODEL_THINKING_EFFORT into config.provider when thinking is off', () => {
    vi.stubEnv('NIGHTHAWK_MODEL_THINKING_EFFORT', 'max');
    try {
      const ctx = nighthawkAgent();
      ctx.agent.config.update({ modelAlias: 'nighthawk', thinkingEffort: 'off' });
...
describe('ConfigState.provider memo (reasoning dialect survives turns)', () => {
  function nighthawkAgentWithTwoModels() {
    const config: NighthawkConfig = {
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'test-key' } },
      models: {
        'nighthawk': { provider: 'nighthawk', model: 'nighthawk', maxContextSize: 128_000 },
        'nighthawk-2': { provider: 'nighthawk', model: 'nighthawk-2', maxContextSize: 128_000 },
      },
    };
    return testAgent({
      initialConfig: config,
  ...
  it('shares the reasoning-dialect cell across repeated accesses with unchanged config', () => {
    const ctx = nighthawkAgentWithTwoModels();
    ctx.agent.config.update({ modelAlias: 'nighthawk' });
  ...
  it('rebuilds the base provider (fresh dialect cell) when the resolved config changes', () => {
    ...
    ctx.agent.config.update({ modelAlias: 'nighthawk-2' });
