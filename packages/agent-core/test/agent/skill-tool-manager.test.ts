...
describe('ToolManager SkillTool registration', () => {
        ...
        content: [
          {
            ...
            text: [
              ...
              '',
              '<nighthawk-skill-loaded name="review" trigger="model-tool" source="user" dir="/skills/review" args="">',
              'body of review',
  ...
  it('exposes session skills after the main agent is created', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-core-skill-tool-refresh-'));
    try {
      const homeDir = join(tmp, 'home');
      const workDir = join(tmp, 'work');
      const skillDir = join(workDir, '.nighthawk', 'skills', 'review');
