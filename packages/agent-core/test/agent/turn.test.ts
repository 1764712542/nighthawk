 ...
 * Run with: pnpm --filter @nighthawk/agent-core test -- turn.test.ts
 */
...
describe('Agent turn flow', () => {
  it('degrades older history media and retries when the provider rejects the request body as too large', async () => {
    ...
    ctx.configure({
      provider: { type: 'nighthawk', apiKey: 'test-key', model: 'nighthawk' },
      modelCapabilities: {
        image_in: true,
        video_in: false,
        audio_in: false,
  ...
  it('tracks turn_ended telemetry with protocol props', async () => {
    ...
    expect(started).toEqual({
      event: 'turn_started',
      properties: expect.objectContaining({ mode: 'agent', provider_type: 'nighthawk', protocol: 'nighthawk', thinking_effort: 'off' }),
    });
  ...
  it('emits a friendly model.not_configured error when no model is configured', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] turn.ended               { "turnId": 0, "reason": "failed", "error": { "code": "model.not_configured", "message": "LLM not set, send \\"/login\\" to login", "name": "NighthawkError", "details": { "turnId": 0 }, "retryable": false } }
    `);
    expect(ctx.newEvents()).toMatchInlineSnapshot(
      `[emit] error   { "code": "model.not_configured", "message": "LLM not set, send \\"/login\\" to login", "name": "NighthawkError", "details": { "turnId": 0 }, "retryable": false }`,
    );
  });
  ...
  it('cancels while waiting for a Stop hook', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-stop-hook-'));
  ...
  it('cancels while waiting for a PreToolUse hook inside permission evaluation', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-pre-tool-hook-'));
    const marker = join(dir, 'started');
    const script = [
      "const fs=require('node:fs');",
      `fs.writeFileSync(${JSON.stringify(marker)}, 'started');`,
  ...
  it('classifies OAuth resolver connection failures as provider connection errors without retrying', async () => {
    ...
    const oauthOptions = oauthAgentOptions(async (options) => {
      ...
      throw new NighthawkError(
        ...
        'OAuth provider "managed:nighthawk" failed to fetch an access token: fetch failed',
      );
    });
  ...
  describe('loop control env overrides', () => {
    ...
    it('honors NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN over config in agent turns', async () => {
      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', '1');
      const ctx = testAgent({
        initialConfig: {
          providers: {},
    ...
    it('prefers NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN over config and ignores invalid values', () => {
      expect(resolveMaxStepsPerTurn(100)).toBe(100);
      expect(resolveMaxStepsPerTurn()).toBeUndefined();

      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', '5');
      ...
      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', '0');
      expect(resolveMaxStepsPerTurn(100)).toBe(0);

      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', 'abc');
      expect(resolveMaxStepsPerTurn(100)).toBe(100);
      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', '-3');
      expect(resolveMaxStepsPerTurn(100)).toBe(100);
      vi.stubEnv('NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN', '1.5');
      expect(resolveMaxStepsPerTurn()).toBeUndefined();
    });

    it('prefers NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP over config and ignores invalid values', () => {
      expect(resolveMaxRetriesPerStep(10)).toBe(10);
      expect(resolveMaxRetriesPerStep()).toBeUndefined();

      vi.stubEnv('NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP', '3');
      expect(resolveMaxRetriesPerStep(10)).toBe(3);
      expect(resolveMaxRetriesPerStep()).toBe(3);

      vi.stubEnv('NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP', '0');
      expect(resolveMaxRetriesPerStep(10)).toBe(0);

      vi.stubEnv('NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP', 'not-a-number');
      expect(resolveMaxRetriesPerStep(10)).toBe(10);
      vi.stubEnv('NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP', '-1');
      expect(resolveMaxRetriesPerStep(10)).toBe(10);
    });
  });
