...
import { type Environment, type Kaos, type KaosProcess } from '@nighthawk/kaos';
...
export interface TestAgentOptions {
  ...
  readonly initialConfig?: NighthawkConfig;
...
export class AgentTestContext {
  ...
  private nighthawkConfig: NighthawkConfig;

  constructor(options: TestAgentOptions = {}) {
    this.options = options;
    this.emitter.on('error', () => {});
    this.nighthawkConfig = options.initialConfig ?? emptyConfig();
  ...
  async expectResumeMatches(): Promise<void> {
    const resumed = testAgent({
      ...
      initialConfig: this.nighthawkConfig,
...
function providerConfigForAlias(provider: ProviderConfig): NighthawkConfig['providers'][string] {
  return {
    type: provider.type,
