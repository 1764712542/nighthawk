...
describe('CronManager', () => {
  ...
  describe('listTaskSnapshots', () => {
    it('returns every task with its recurring flag and post-jitter next fire', () => {
      ...
      // for pending work (e.g. `nighthawk -p`) never wait on a task that can
  ...
  describe('isStale', () => {
    ...
    it('NIGHTHAWK_CRON_NO_STALE=1 disables stale judgment for recurring', () => {
      vi.stubEnv('NIGHTHAWK_CRON_NO_STALE', '1');
      const { agent } = createAgentStub();
      const harness = createClocks();
      const manager = new CronManager(agent, {
        clocks: harness.clocks,
