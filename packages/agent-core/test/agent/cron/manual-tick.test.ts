 ...
 * `NIGHTHAWK_CRON_MANUAL_TICK=1` env disables the auto-tick interval and,
...
describe('CronManager — P1.8 manual tick + SIGUSR1', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('NIGHTHAWK_CRON_NO_JITTER', '1');
  });
  ...
  describe('NIGHTHAWK_CRON_MANUAL_TICK=1', () => {
    it('does not install setInterval; tick() must be called manually', async () => {
      vi.stubEnv('NIGHTHAWK_CRON_MANUAL_TICK', '1');
  ...
  describe('without NIGHTHAWK_CRON_MANUAL_TICK', () => {
  ...
  describe('SIGUSR1', () => {
    // SIGUSR1 binding is opt-in via NIGHTHAWK_CRON_MANUAL_TICK=1 so that
    ...
    it('logs swallowed tick() throws to stderr when NIGHTHAWK_CRON_DEBUG=1', async () => {
      if (process.platform === 'win32') return;
      vi.stubEnv('NIGHTHAWK_CRON_DEBUG', '1');
    ...
    it('does not write to stderr on tick() throw when NIGHTHAWK_CRON_DEBUG is unset', async () => {
      if (process.platform === 'win32') return;
      // NIGHTHAWK_CRON_DEBUG intentionally NOT set in this test.
    ...
    it('stop() removes the SIGUSR1 listener (no leak)', async () => {
      ...
      // Constructor auto-starts, which binds SIGUSR1 under NIGHTHAWK_CRON_MANUAL_TICK=1.
      const manager = new CronManager(stub.agent, { pollIntervalMs: null });
    ...
    it('does not bind when NIGHTHAWK_CRON_MANUAL_TICK is unset', async () => {
      if (process.platform === 'win32') return;
