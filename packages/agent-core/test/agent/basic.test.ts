import type { ContentPart, ToolCall } from '@nighthawk/kosong';
...
describe('prompt-attached video resolution', () => {
  ...
  // Stubs the NightHawk files endpoint. `status` drives the ladder: 200 issues a
  ...
  function nighthawkProvider(baseUrl: string) {
    return { type: 'nighthawk', apiKey: 'test-key', model: 'mock-model', baseUrl } as const;
  }
  ...
  it('uploads a local file:// video and sends the issued ms:// reference to the model', async () => {
    ...
    try {
      const ctx = testAgent();
      ctx.configure({ provider: nighthawkProvider(stub.url), modelCapabilities: VIDEO_CAPS });
  ...
  it('ends the turn as cancelled when the user aborts mid-upload, appending nothing', async () => {
    ...
    try {
      const ctx = testAgent();
      ctx.configure({
        provider: nighthawkProvider(`http://127.0.0.1:${String(port)}`),
  ...
  it('degrades to a <video path> tag when the model lacks video_in (no upload)', async () => {
    ...
    try {
      const ctx = testAgent();
      ctx.configure({
        provider: nighthawkProvider(stub.url),
