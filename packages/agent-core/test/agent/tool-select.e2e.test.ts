...
import type { ToolCall } from '@nighthawk/kosong';
...
const DISCLOSURE_PROVIDER = { type: 'nighthawk', apiKey: 'test-key', model: 'select-capable-model' } as const;
...
const INLINE_PROVIDER = { type: 'nighthawk', apiKey: 'test-key', model: 'inline-model' } as const;
...
/** Empty env so an ambient NIGHTHAWK_EXPERIMENTAL_FLAG cannot force flags on. */
