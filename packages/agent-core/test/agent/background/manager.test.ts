...
describe('BackgroundManager', () => {
  ...
  describe('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS env override', () => {
    afterEach(() => {
      vi.unstubAllEnvs();
    });

    it('caps registration from the env even without config', () => {
      vi.stubEnv('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS', '1');
    ...
    it('prefers the env value over config and ignores invalid values', () => {
      ...
      vi.stubEnv('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS', '2');
      ...
      vi.stubEnv('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS', '0');
      expect(resolveMaxRunningTasks(4)).toBe(4);

      vi.stubEnv('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS', 'abc');
      expect(resolveMaxRunningTasks(4)).toBe(4);
      vi.stubEnv('NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS', '-2');
      expect(resolveMaxRunningTasks()).toBeUndefined();
    });
  ...
  it('persists graceful process shutdown as killed when stop was requested', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'nighthawk-bg-stop-race-'));
    try {
  ...
  it('getTask on an unknown id does not create persisted state', async () => {
    const sessionDir = await mkdtemp(join(tmpdir(), 'nighthawk-bg-mgr-missing-'));
    try {
