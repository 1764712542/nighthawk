...
import { ErrorCodes, NighthawkError, makeErrorPayload } from '#/errors';
import { log } from '#/logging/logger';
import type { Logger } from '#/logging/types';
import type { AgentAPI, AgentEvent, NighthawkConfig, SDKAgentRPC, UsageStatus } from '#/rpc';
import { generate, type ChatProvider } from '@nighthawk/kosong';
...
export class Agent {
   ...
   * binding descriptions, ...). Mutable via {@link updateNighthawkConfig} so the
  ...
  /**
   * Print-mode (`nighthawk -p`) only: when true and the agent ends a turn while
  ...
  constructor(options: AgentOptions) {
    ...
    this.nighthawkConfig = options.config;
  ...
  get llm(): KosongLLM {
    ...
    const loopControl = this.nighthawkConfig?.loopControl;
  ...
  updateNighthawkConfig(config: NighthawkConfig | undefined): void {
    this.nighthawkConfig = config;
    if (this.config.hasProvider) {
  ...
  get rpcMethods(): PromisableMethods<AgentAPI> {
    return {
      ...
      activateSkill: (payload) => {
        if (this.skills === null) {
          throw new NighthawkError(ErrorCodes.SKILL_NOT_FOUND, `Skill "${payload.name}" was not found`);
        }
