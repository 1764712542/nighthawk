...
import { NighthawkChatProvider, type ChatProvider, type Message, type Tool } from '@nighthawk/kosong';

import { parseFloatEnv } from '#/config/resolve';
import { resolveThinkingKeep } from '#/config/nighthawk-env-params';
...
export class LlmRequestRecorder {
  ...
  record(input: {
    ...
    // Mirror the ConfigState.provider pipeline for NightHawk-only request params:
    // env sampling overrides and the preserved-thinking keep passthrough
    // reach the wire only for NightHawk providers, resolved by the same exported
    // helpers used at construction. thinkingEffort needs no mirroring — the
    // NightHawk provider derives it from the request body's thinking payload, so
    // env effort overrides are already reflected in the read value.
    const isNighthawkProvider = provider instanceof NighthawkChatProvider;
    this.agent.records.logRecord({
      type: 'llm.request',
      ...
      thinkingKeep: isNighthawkProvider
        ? resolveThinkingKeep(
            process.env,
            this.agent.nighthawkConfig?.thinking?.keep,
            provider.thinkingEffort ?? 'off',
          )
        : undefined,
      temperature: isNighthawkProvider
        ? parseFloatEnv(process.env['NIGHTHAWK_MODEL_TEMPERATURE'], 'NIGHTHAWK_MODEL_TEMPERATURE')
        : undefined,
      topP: isNighthawkProvider
        ? parseFloatEnv(process.env['NIGHTHAWK_MODEL_TOP_P'], 'NIGHTHAWK_MODEL_TOP_P')
        : undefined,
      maxTokens: provider.maxCompletionTokens,
      betaApi:
        modelAlias === undefined
          ? undefined
          : this.agent.nighthawkConfig?.models?.[modelAlias]?.betaApi,
