import type { ContentPart, ThinkingEffort, TokenUsage } from '@nighthawk/kosong';
...
export interface AgentRecordEvents {
  ...
  'llm.request': {
    ...
    modelAlias?: string;
    /**
     * Provider-effective thinking effort — for NightHawk providers this is derived
     * from the request body's thinking payload, so env overrides
     * (`NIGHTHAWK_MODEL_THINKING_EFFORT`) are already reflected.
     */
    thinkingEffort?: ThinkingEffort;
    /**
     * NightHawk preserved-thinking passthrough (`thinking.keep`) in effect for
     * this request — resolved from env, config, and the default, none of
     * which are otherwise recorded.
     */
    thinkingKeep?: string;
    /** Effective env-driven sampling overrides (NightHawk provider only). */
    temperature?: number;
    topP?: number;
    /**
