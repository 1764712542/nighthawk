...
export class TurnFlow {
  ...
  private async runStepLoop(turnId: number, signal: AbortSignal): Promise<LoopTurnStopReason> {
    ...
    while (true) {
      ...
      const loopControl = this.agent.nighthawkConfig?.loopControl;
      ...
      try {
        const result = await runTurn({
          ...
          hooks: {
            ...
            shouldContinueAfterStop: async (ctx) => {
              ...
              // Print-mode drain: when `nighthawk -p` ends a turn while background
            ...
            finalizeToolResult: async (ctx) => {
              ...
              void this.agent.hooks?.fireAndForgetTrigger(event, {
                ...
                inputData: {
                  ...
                  error: isError === true ? toNighthawkErrorPayload(toolOutputText(output)) : undefined,
        ...
        const isContextOverflow =
          ...
          (isNighthawkError(error) && error.code === ErrorCodes.CONTEXT_OVERFLOW);
        ...
        if (isMaxStepsExceededError(error)) {
          this.agent.log.warn('turn hit max steps', {
            ...
            limit: isNighthawkError(error) ? error.details?.['maxSteps'] : undefined,
          });
        } else {
...
const MAX_STEPS_PER_TURN_ENV = 'NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN';
const MAX_RETRIES_PER_STEP_ENV = 'NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP';

/**
 * Resolve the effective per-turn step cap. Precedence:
 * `NIGHTHAWK_LOOP_MAX_STEPS_PER_TURN` (non-negative integer) → config
 ...
 * `NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP` (non-negative integer) → config
...
function summarizeTurnError(error: unknown, turnId: number): NighthawkErrorPayload {
  const payload = toNighthawkErrorPayload(error);
...
function providerFilteredPayload(turnId: number): NighthawkErrorPayload {
  return {
...
function classifyApiError(error: unknown, summary: NighthawkErrorPayload): ApiErrorClassification {
...
function summaryStatusCode(summary: NighthawkErrorPayload): number | undefined {
  const statusCode = summary.details?.['statusCode'];
  return typeof statusCode === 'number' ? statusCode : undefined;
}

function isApiConnectionError(error: unknown, summary: NighthawkErrorPayload): boolean {
  return error instanceof APIConnectionError || summary.name === 'APIConnectionError';
}

function isApiTimeoutError(error: unknown, summary: NighthawkErrorPayload): boolean {
  return (
...
function isApiEmptyResponseError(error: unknown, summary: NighthawkErrorPayload): boolean {
