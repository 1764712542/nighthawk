...
const MAX_RUNNING_TASKS_ENV = 'NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS';

/**
 * Resolve the effective background-task concurrency cap. Precedence:
 * `NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS` (positive integer) → config
...
export class BackgroundManager {
  ...
  private assertCanRegister(startedInBackground: boolean): void {
    const maxRunningTasks = resolveMaxRunningTasks(
      this.agent.nighthawkConfig?.background?.maxRunningTasks,
    );
   ...
   *
   * Used by print-mode (`nighthawk -p`) turn draining to hold a turn open while
