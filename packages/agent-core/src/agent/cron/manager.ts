 ...
 */
import type { ContentPart } from '@nighthawk/kosong';
 ...
 * `NIGHTHAWK_CRON_NO_STALE=1` (bench / acceptance tests).
 *
 ...
 * applications (e.g. the `nighthawk -p` flow deciding whether pending work
...
export interface CronManagerOptions {
   ...
   * `resolveClockSources(process.env.NIGHTHAWK_CRON_CLOCK)` so production
   * picks up `NIGHTHAWK_CRON_CLOCK=file:...` automatically.
   ...
   * (1000ms unless `NIGHTHAWK_CRON_MANUAL_TICK=1`, which forces `null` here
...
export class CronManager {
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['NIGHTHAWK_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => process.env['NIGHTHAWK_DISABLE_CRON'] === '1',
      onFire: (task, ctx) => {
      ...
      // P1.8: `NIGHTHAWK_CRON_MANUAL_TICK=1` forces the scheduler into
      ...
      pollIntervalMs:
        process.env['NIGHTHAWK_CRON_MANUAL_TICK'] === '1'
          ? null
          : opts.pollIntervalMs,
    });
   ...
   * Stale judgment.
   *
   *   - `NIGHTHAWK_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (process.env['NIGHTHAWK_CRON_NO_STALE'] === '1') return false;
   ...
   *
   * Gated on `NIGHTHAWK_CRON_MANUAL_TICK=1` for two reasons:
   *
   ...
   * Set `NIGHTHAWK_CRON_DEBUG=1` to surface the swallowed error to stderr —
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (process.env['NIGHTHAWK_CRON_MANUAL_TICK'] !== '1') return;
    if (this.sigusr1Handler !== null) return;
    const handler: NodeJS.SignalsListener = () => {
      try {
        this.tick();
      } catch (error) {
        if (process.env['NIGHTHAWK_CRON_DEBUG'] === '1') {
