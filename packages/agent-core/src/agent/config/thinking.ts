import type { ThinkingEffort } from '@nighthawk/kosong';
...
export function supportsThinkingEffort(
  ...
  nighthawkProtocol: boolean,
): boolean {
  if (!nighthawkProtocol || effort === 'off') return true;
  const effective = model === undefined ? undefined : effectiveModelAlias(model);
...
export function resolveThinkingEffort(
  ...
  return normalizeThinkingEffortForModel(effort, effectiveModel, nighthawkProtocol);
}
