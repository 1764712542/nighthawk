/**
 * Error codes for NightHawk Core's public error protocol.
 *
 * `ErrorCodes` is the source of truth for every code NightHawk Core may emit.
...
export type NighthawkErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

export interface NighthawkErrorInfo {
...
export const NIGHTHAWK_ERROR_INFO = {
  'config.invalid': {
  ...
  'plugin.load_failed': {
    ...
    retryable: true,
    public: true,
    action: 'Fix the installed.json file under $NIGHTHAWK_HOME/plugins/ and run /plugins reload.',
  },
  ...
  'shell.git_bash_not_found': {
    ...
    retryable: false,
    public: true,
    action: 'Install Git for Windows from https://gitforwindows.org/ or set NIGHTHAWK_SHELL_PATH to a bash.exe.',
  },
...
} as const satisfies Record<NighthawkErrorCode, NighthawkErrorInfo>;
 ...
 * branch on this before indexing `NIGHTHAWK_ERROR_INFO` with an untrusted code.
 */
export function isNighthawkErrorCode(code: unknown): code is NighthawkErrorCode {
  return typeof code === 'string' && Object.hasOwn(NIGHTHAWK_ERROR_INFO, code);
}
