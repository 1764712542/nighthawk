# `@nighthawk/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@nighthawk/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
1. **Zero runtime dependencies** — the container is ~600 LoC of plain
   ...
   subsystem.
2. **Familiar shape** — most nighthawk contributors have seen VSCode's
...
Test files import from the subpath `@nighthawk/agent-core/di/test`
...
├── test.ts                      ← subpath barrel (`@nighthawk/agent-core/di/test`)
