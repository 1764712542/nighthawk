 ...
 * (`skills` ↔ `agents`, `.nighthawk` ↔ `.agents`).
 *
...
// Relative to brandHomeDir, which already IS the brand data dir (~/.nighthawk
// or $NIGHTHAWK_HOME) — no '.nighthawk' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['agents'] as const;
const USER_GENERIC_DIRS = ['.agents/agents'] as const;
const PROJECT_BRAND_DIRS = ['.nighthawk/agents'] as const;
