...
import type { NighthawkConfig, SDKSessionRPC } from '#/rpc';
...
export interface SessionOptions {
  ...
  /**
   * Print-mode (`nighthawk -p`) only: hold the main turn open while background
...
export interface SessionSkillConfig {
  ...
  /** Brand data dir (NIGHTHAWK_HOME); user brand skills live under `<brandHomeDir>/skills`. */
 ...
 * discovery layout: user brand dir `<nighthawkHomeDir>/agents` and
 * `~/.agents/agents`, project `.nighthawk/agents` and `.agents/agents`, plus
...
const BACKGROUND_KEEP_ALIVE_ON_EXIT_ENV = 'NIGHTHAWK_BACKGROUND_KEEP_ALIVE_ON_EXIT';
...
export class Session {
   ...
   */
  private runtimeConfig: NighthawkConfig | undefined;

  /** The session's current config snapshot (see {@link Session.runtimeConfig}). */
  get nighthawkConfig(): NighthawkConfig | undefined {
  ...
  constructor(public readonly options: SessionOptions) {
    ...
    this.mcp = new McpConnectionManager({
      oauthService:
        options.mcpOAuthService ?? new McpOAuthService({ nighthawkHomeDir: options.nighthawkHomeDir }),
    ...
    this.agentCatalog =
      ...
      new SessionAgentProfileCatalog({
        ...
        brandHomeDir: options.nighthawkHomeDir ?? join(homedir(), '.nighthawk'),
   ...
   * terminal state before a `nighthawk -p` (print) run exits.
   *
  ...
  /**
   * Resolve the effective print-mode (`nighthawk -p`) background-task policy.
   *
  ...
  /**
   * Decide what the `nighthawk -p` driver should do after the main agent's turn ends
  ...
  async ensureAgentResumed(id: string): Promise<Agent> {
    ...
    if (this.metadata.agents[id] === undefined) {
      throw new NighthawkError(ErrorCodes.AGENT_NOT_FOUND, `Agent "${id}" was not found`);
    }
  ...
  private async bootstrapAgentProfile(
    ...
    const context = await prepareSystemPromptContext(
      ...
      this.options.nighthawkHomeDir,
      { additionalDirs: this.additionalDirs },
    );
    const subagentNames = Object.keys(this.agentCatalog.delegatableSubagents(profile.name));
    agent.useProfile(profile, context, this.options.nighthawkHomeDir, subagentNames);
   ...
   * agent's `nighthawkConfig` (tool descriptions, loop control) all read the
   ...
   */
  setSecondaryModelConfig(config: NighthawkConfig): void {
  ...
  private computeSecondaryModelWarnings(): SessionWarning[] {
    ...
    const warnings: SessionWarning[] = [];
    const secondary = resolveSecondaryModel(this.nighthawkConfig, this.experimentalFlags);
    if (secondary?.model !== undefined) {
      const boundAlias =
  ...
  async generateAgentsMd(): Promise<void> {
    ...
    try {
      ...
      const agentsMd = await loadAgentsMd(mainAgent.kaos, this.options.nighthawkHomeDir);
  ...
  private async loadSkills(): Promise<void> {
    const roots = await resolveSkillRoots({
      paths: {
        ...
        brandHomeDir: this.options.skills?.brandHomeDir ?? this.options.nighthawkHomeDir,
  ...
  *readyAgents(): Iterable<Agent> {
  ...
  private async resumePersistedAgent(
    ...
    if (meta === undefined) {
      throw new NighthawkError(ErrorCodes.SESSION_STATE_INVALID, `Session agent "${id}" is missing`);
    }
  ...
  private restoreAgentProfileHandle(
    ...
    agent.setActiveProfile(profile, this.options.nighthawkHomeDir);
  }
