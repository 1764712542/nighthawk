 ...
 * `NIGHTHAWK_SUBAGENT_TIMEOUT_MS` (integer ms) → `configMs` →
...
export class SessionSubagentHost {
  ...
  private async configureChild(
    ...
    const context = await prepareSystemPromptContext(
      ...
      this.session.options.nighthawkHomeDir,
    ...
    child.useProfile(profile, context, this.session.options.nighthawkHomeDir, subagentNames);
   ...
   * Bash, nested background agents) settle — the print-mode (`nighthawk -p`)
