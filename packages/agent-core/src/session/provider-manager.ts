...
import type { ProviderConfig as KosongProviderConfig, ModelCapability, ProviderRequestAuth } from '@nighthawk/kosong';
import {
  APIStatusError,
...
import { parseNighthawkCustomHeaders } from '@nighthawk/nighthawk-oauth';
import {
  effectiveModelAlias,
...
import { ErrorCodes, isNighthawkError, NighthawkError } from '../errors';
...
interface ProviderManagerOptions {
  readonly config: NighthawkConfig | (() => NighthawkConfig);
...
export class ProviderManager implements ModelProvider {
  ...
  private get config(): NighthawkConfig {
  ...
  resolveProviderConfig(model: string): ResolvedRuntimeProvider {
    ...
    const provider = toKosongProviderConfig(
      ...
      alias.protocol,
      alias.baseUrl,
      this.options.nighthawkRequestHeaders,
  ...
  resolveAuth(
    ...
    const loginRequired = (cause?: unknown): NighthawkError =>
      new NighthawkError(
    ...
    const fetchAuth = async (force: boolean): Promise<ProviderRequestAuth> => {
      ...
      try {
        ...
        if (!isNighthawkError(error) || error.code !== ErrorCodes.AUTH_LOGIN_REQUIRED) {
...
function toKosongProviderConfig(
  ...
  nighthawkRequestHeaders: Record<string, string> | undefined,
  ...
  const envCustomHeaders = parseNighthawkCustomHeaders();
  switch (effectiveType) {
    case 'anthropic': {
      ...
      return {
        ...
        // NightHawk routed over the Anthropic transport keeps its vendor error
        // classification: a NightHawk quota-exhausted 429 must fail fast here
        // exactly as it does on the NightHawk OpenAI transport.
        ...(provider.type === 'nighthawk'
          ? { nighthawkThinking: true, convertError: classifyNighthawkQuotaError }
          : {}),
        ...
        // When a NightHawk provider is routed through the Anthropic transport
        // (`protocol: 'anthropic'`), upstream is the managed NightHawk endpoint,
        // so align its full outbound identity headers (User-Agent + X-Msh-*)
        // with the NightHawk OpenAI transport. Plain Anthropic providers only
        // receive the unified `User-Agent` (no `X-Msh-*` device identity),
        // matching the other non-NightHawk transports. Provider `customHeaders`
        // still win on conflict.
        ...defaultHeadersField(
          provider.type === 'nighthawk' && modelProtocol === 'anthropic'
            ? { ...envCustomHeaders, ...nighthawkRequestHeaders, ...provider.customHeaders }
            : { ...envCustomHeaders, ...nighthawkUserAgentHeader(nighthawkRequestHeaders), ...provider.customHeaders },
        ),
      };
    }
    case 'openai':
      return {
        ...
        // generate time, matching the `nighthawk` branch below.
        generationKwargs: { prompt_cache_key: promptCacheKey },
        ...defaultHeadersField({
          ...envCustomHeaders,
          ...nighthawkUserAgentHeader(nighthawkRequestHeaders),
    ...
    case 'nighthawk':
      return {
        type: 'nighthawk',
        model,
        baseUrl: modelBaseUrl ?? providerValue(provider.baseUrl, provider.env, 'NIGHTHAWK_BASE_URL'),
        apiKey: providerApiKey(provider),
        generationKwargs: { prompt_cache_key: promptCacheKey },
        ...defaultHeadersField({
          ...envCustomHeaders,
...
// Extract just the `User-Agent` from the NightHawk identity headers so non-NightHawk
// providers (OpenAI, Anthropic, Google, Vertex) also identify as
// `nighthawk-cli/<version>` without leaking the `X-Msh-*` device identity
// headers to third-party endpoints. The full `nighthawkRequestHeaders` set stays
// reserved for the NightHawk transport (and the NightHawk-routed Anthropic transport),
// where upstream is the managed NightHawk endpoint.
function nighthawkUserAgentHeader(
  nighthawkRequestHeaders: Record<string, string> | undefined,
): Record<string, string> {
  const userAgent = nighthawkRequestHeaders?.['User-Agent'];
...
function providerApiKey(provider: ProviderConfig): string | undefined {
  switch (provider.type) {
    ...
    case 'nighthawk':
      return providerValue(provider.apiKey, provider.env, 'NIGHTHAWK_API_KEY');
    case 'google-genai':
