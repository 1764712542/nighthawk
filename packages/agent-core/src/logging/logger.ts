...
const ROOT_SYMBOL = Symbol.for('nighthawk.logger.root');
const SESSION_LOG_ID = Symbol('nighthawk.logger.sessionLogId');
...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'nighthawk.log'),
 ...
 *
 *   import { log } from 'nighthawk-sdk';
 *   log.info('nighthawk starting', { version });
 *
 ...
 * importing `log` at module load (before `NighthawkHarness` configures the root)
