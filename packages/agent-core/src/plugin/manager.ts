...
// Hidden NightHawk CLI subcommand that re-enters as a Node interpreter.
// Used as fallback when an MCP server declares `"command": "node"` but the
// user is running a single-binary NightHawk build that doesn't have `node` on PATH.
const NIGHTHAWK_NODE_FALLBACK_SUBCOMMAND = '__plugin_run_node';
...
export class PluginManager {
  private readonly nighthawkHomeDir: string;
  private records = new Map<string, PluginRecord>();

  constructor(options: PluginManagerOptions) {
    this.nighthawkHomeDir = options.nighthawkHomeDir;
  }

  async load(): Promise<void> {
    const file = await readInstalled(this.nighthawkHomeDir);
  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    if (resolved.kind === 'local-path') {
      ...
      normalizedRoot = await copyPluginToManagedRoot(this.nighthawkHomeDir, id, sourceRoot);
      ...
      const tmpDir = await mkdtemp(path.join(tmpdir(), 'nighthawk-plugin-zip-'));
      try {
        ...
        id = normalizePluginId(parsed.manifest.name);
        normalizedRoot = await copyPluginToManagedRoot(this.nighthawkHomeDir, id, detectedRoot);
        parsed = await parseManifest(normalizedRoot);
      } finally {
   ...
   * passes `managedEnv` — the host-managed NightHawk env for stdio servers. This is
    ...
    for (const record of this.records.values()) {
      ...
      for (const hook of record.manifest.hooks ?? []) {
        out.push({
          ...hook,
          cwd: record.root,
          env: { NIGHTHAWK_HOME: this.nighthawkHomeDir, NIGHTHAWK_PLUGIN_ROOT: record.root },
        });
      }
    }
    return out;
  ...
  private async persist(): Promise<void> {
    ...
    await writeInstalled(this.nighthawkHomeDir, { version: 1, plugins: installed });
  }
...
async function copyPluginToManagedRoot(
  ...
  const managedRoot = path.join(nighthawkHomeDir, 'plugins', 'managed', id);
...
function withPluginMcpRuntime(
  ...
  const env = {
    ...config.env,
    NIGHTHAWK_HOME: nighthawkHomeDir,
    NIGHTHAWK_PLUGIN_ROOT: pluginRoot,
  };

  if (config.command === 'node' && isNighthawkNativeBinary()) {
    return {
      ...config,
      command: process.execPath,
      args: [NIGHTHAWK_NODE_FALLBACK_SUBCOMMAND, ...(config.args ?? [])],
...
function isNighthawkNativeBinary(): boolean {
