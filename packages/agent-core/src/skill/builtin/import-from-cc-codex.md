...
Help them migrate selected local Claude Code and Codex assets into NightHawk.
...
- Do **not** migrate `.agents` content. NightHawk already supports `.agents`
...
- Only write under NightHawk targets:
  - User-global: `$NIGHTHAWK_HOME` if set, otherwise `~/.nighthawk`.
  - Project instructions/skills: `<project root>/.nighthawk`, where the project
    root is the nearest parent directory containing `.git`; if no `.git` exists,
    use the current working directory.
  - Project-local MCP: `<cwd>/.nighthawk/mcp.json`, because NightHawk reads the
    current working directory's NightHawk-specific MCP file, not every project-root
    `.nighthawk/mcp.json` from subdirectories.
- Preserve existing NightHawk files. Never overwrite existing skills or replace an
...
Resolve paths explicitly; `~` is the real OS home, and NightHawk home follows
`$NIGHTHAWK_HOME` before `~/.nighthawk`.
...
NightHawk-readable, and project-root `.mcp.json` is already read by NightHawk as a
...
- `$NIGHTHAWK_HOME/AGENTS.md`, or `~/.nighthawk/AGENTS.md` if the env var is not
  set.

Map project-level instruction sources to:

- `<project root>/.nighthawk/AGENTS.md`
...
- `$NIGHTHAWK_HOME/skills/`, or `~/.nighthawk/skills/` if the env var is not set.

Map project-level skill sources to:

- `<project root>/.nighthawk/skills/`
...
- Read a bundle's `SKILL.md` enough to verify that directory skills have
  frontmatter with non-empty `name` and `description`, because NightHawk requires
...
- If two source entries would write the same target path, keep the first one in
  ...
  3. user Claude
  4. user Codex
- Warn when a source skill uses Claude/Codex-specific fields or syntax that NightHawk
...
For the preview, collect MCP candidates and normalize them into NightHawk's MCP shape
when possible:
...
- Map Codex fields to NightHawk fields:
  - `command` -> `command`
  - `args` -> `args`
  - `env` -> `env`
  - `cwd` -> `cwd`
...
- Do not import project-root `.mcp.json`; NightHawk already reads it.

For each MCP candidate, choose the target scope in the preview:

- User-level source -> user-global MCP target (`$NIGHTHAWK_HOME/mcp.json` or
  `~/.nighthawk/mcp.json`).
- Project-level source -> project-local NightHawk MCP target (`<cwd>/.nighthawk/mcp.json`). If `<cwd>` is not the project root, call this out in the preview so the user understands when NightHawk will load it.
...
- collision policy: keep existing NightHawk entries on name conflict,
...
- Tell the user to start a new session (for example `/new`) or restart NightHawk
