...
Help the user design, write, and apply a custom color theme for the nighthawk TUI. A theme is a single JSON file; the TUI ships with `dark`, `light`, and `auto`, and any file the user adds becomes selectable alongside them.
...
- **Never assume the data directory is `~/.nighthawk`.** Always resolve `$NIGHTHAWK_HOME` first with the Bash command below.
...
The nighthawk runtime resolves the data directory as `NIGHTHAWK_HOME` first, falling back to `~/.nighthawk`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `NIGHTHAWK_HOME` is set and fall back to `~/.nighthawk` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<NIGHTHAWK_HOME>` means that resolved data root — **never assume `~/.nighthawk`**. Theme files live at `<NIGHTHAWK_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.

## What a theme is

- A theme lives at `<NIGHTHAWK_HOME>/themes/<name>.json`.
...
```
https://nighthawk-dev.github.io/nighthawk/en/customization/themes.html
```
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$NIGHTHAWK_HOME"`; if empty, use `echo "$HOME/.nighthawk"`. Use `<root>/themes` for every subsequent step.
   - If tweaking an existing custom theme, **Read** `<NIGHTHAWK_HOME>/themes/<name>.json` first — never overwrite a theme you haven't read.
   - Starting fresh: build a `colors` object from the token table. You can `ls <NIGHTHAWK_HOME>/themes/` and Read one of the user's existing themes as a reference for the format.
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<NIGHTHAWK_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
