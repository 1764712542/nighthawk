...
The nighthawk runtime resolves the user-global directory as `NIGHTHAWK_HOME`
first, falling back to `~/.nighthawk`. Before touching the user-global file,
resolve the actual directory with Bash so you don't read or write the wrong
one. Check whether `NIGHTHAWK_HOME` is set and fall back to `~/.nighthawk`
when it is empty:
...
rest of this skill, `<NIGHTHAWK_HOME>` means that resolved data root —
**never assume `~/.nighthawk`**.

- User-global: `<NIGHTHAWK_HOME>/mcp.json`. Use for servers you want
  everywhere.
...
- Project-local: `<cwd>/.nighthawk/mcp.json`. Use for NightHawk-specific
...
or the `NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS` / `NIGHTHAWK_MCP_TOOL_TIMEOUT_MS` env vars;
...
1. **Pick a scope.** Infer it from the user's words when you can
   ...
   current directory / NightHawk-specific / `.nighthawk` → project-local). When
...
3. **Write and tell them how to reload MCP servers.** Preserve unrelated
   ...
   restart `nighthawk` for the change to take effect.
