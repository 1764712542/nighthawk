...
Help the user inspect, change, and validate nighthawk's configuration files. The files are **TOML** with **snake_case** keys.

## The two config files

nighthawk has two TOML config files, both under `<NIGHTHAWK_HOME>/`, both snake_case, but with different ownership — decide which one the user means before doing anything.

The runtime resolves the data directory as `NIGHTHAWK_HOME` first, falling back to `~/.nighthawk`. Before doing anything, resolve the actual directory with Bash so you don't write to the wrong place. Check whether `NIGHTHAWK_HOME` is set and fall back to `~/.nighthawk` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<NIGHTHAWK_HOME>` means that resolved root — **never assume `~/.nighthawk`**.
...
```
https://nighthawk-dev.github.io/nighthawk/en/configuration/config-files.html
```
...
- Location: `<NIGHTHAWK_HOME>/config.toml` or `<NIGHTHAWK_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <NIGHTHAWK_HOME>/config.toml`).
...
5. **Validate the candidate** (see Capability 3, via `nighthawk doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`nighthawk doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs nighthawk's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `nighthawk doctor config <config-new.toml path>`
- changed `tui.toml` → `nighthawk doctor tui <tui-new.toml path>`
...
Then do two checks `nighthawk doctor` can't:
...
> To also check whether the currently **active** config is OK overall, run `nighthawk doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
...
Note: `/reload` is available **only when idle** — if a reply is streaming, press Esc / Ctrl-C to stop first. `nighthawk doctor` already validated the schema before the overwrite, so reload should apply cleanly; if it still errors, follow the message to fix it or recover from the most recent timestamped backup. If you don't want to reload now, the **next new session** picks it up automatically.

## Capability 5: fix a deprecated key or env-var warning

NightHawk reports configuration deprecations as warnings — in the TUI startup notices and pushed to clients as the `event.config.warning` event. There are two shapes, handled differently:
...
- **Deprecated environment variable** — e.g. `Environment variable NIGHTHAWK_LOOP_MAX_RETRIES_PER_STEP is deprecated; use NIGHTHAWK_LOOP_MAX_ATTEMPTS_PER_STEP instead.` The old variable still works, but this is **not** fixable by editing `config.toml`/`tui.toml` — tell the user to rename the variable where they set it (shell profile, CI environment, launcher script). Do not add anything to the config files for this.
