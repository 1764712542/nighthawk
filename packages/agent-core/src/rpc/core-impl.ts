...
import { NighthawkFetchURLProvider } from '#/tools/providers/nighthawk-fetch-url';
import { NighthawkWebSearchProvider } from '#/tools/providers/nighthawk-web-search';
...
import {
  ...
  migrateProviderTypeKimiToNighthawk,
...
import type {
  ...
  ReloadPluginsResult,
  RemoveNighthawkProviderPayload,
  RemovePluginPayload,
...
import type { SessionWarning } from '@nighthawk/protocol';
import { proxyWithExtraPayload } from './types';
import { KaosShellNotFoundError, LocalKaos, type Kaos } from '@nighthawk/kaos';
import type { ToolServices } from '../tools/support/services';

const NIGHTHAWK_PROVIDER_NAME = 'managed:nighthawk';
const NIGHTHAWK_BASE_URL_ENV = 'NIGHTHAWK_BASE_URL';
const NIGHTHAWK_OAUTH_HOST_ENV = 'NIGHTHAWK_OAUTH_HOST';
const WEB_SEARCH_BASE_URL_ENV = 'NIGHTHAWK_WEB_SEARCH_BASE_URL';
const WEB_SEARCH_API_KEY_ENV = 'NIGHTHAWK_WEB_SEARCH_API_KEY';
const WEB_FETCH_BASE_URL_ENV = 'NIGHTHAWK_WEB_FETCH_BASE_URL';
const WEB_FETCH_API_KEY_ENV = 'NIGHTHAWK_WEB_FETCH_API_KEY';
...
export interface NighthawkCoreOptions {
  readonly homeDir?: string | undefined;
  readonly configPath?: string | undefined;
  readonly runtime?: ToolServices | undefined;
  readonly nighthawkRequestHeaders?: Record<string, string> | undefined;
  ...
  /**
   * Host UI mode (`'print'` for `nighthawk -p`, `'cli'` for the TUI, ...). When
...
export class NighthawkCore implements PromisableMethods<CoreAPI> {
  ...
  private config: NighthawkConfig;
  ...
  private readonly nighthawkRequestHeaders: Record<string, string> | undefined;
  ...
  /** `true` when the host runs `nighthawk -p` (v1 print mode); see `withPrintModeDefaults`. */
  ...
  constructor(
    ...
    this.nighthawkRequestHeaders = options.nighthawkRequestHeaders;
    ...
    migrateProviderTypeKimiToNighthawk(this.configPath, this.homeDir);
    ...
    this.mcpOAuth = new McpOAuthService({ nighthawkHomeDir: this.homeDir });
    this.plugins = new PluginManager({ nighthawkHomeDir: this.homeDir });
    this.mcpRegistry = new McpServerRegistry({
      homeDir: this.homeDir,
      store: this.globalMcpConfig,
      plugins: this.plugins,
      managedPluginEnv: () => this.managedNighthawkEnvForPlugins(),
    });
  ...
  async createSessionWithOverrides(
    ...
    // Read the workspace local config (`.nighthawk/local.toml`) through the
  ...
  async resumeSessionWithOverrides(
    ...
    // Read `.nighthawk/local.toml` through the persistence (local) kaos, not the
  ...
  async getNighthawkConfig(input?: GetNighthawkConfigPayload): Promise<NighthawkConfig> {
    if (input?.reload) {
  ...
  async setNighthawkConfig(input: SetNighthawkConfigPayload): Promise<NighthawkConfig> {
    const config = mergeConfigPatch(this.readConfigForWrite(), input);
    await writeConfigFile(this.configPath, config);
    return this.reloadRuntimeConfig();
  }

  async removeNighthawkProvider(input: RemoveNighthawkProviderPayload): Promise<NighthawkConfig> {
  ...
  private async syncPluginMcpServersInSessions(): Promise<void> {
    const names = new Set<string>(
      this.plugins
        .mcpServerEntries({ managedEnv: this.managedNighthawkEnvForPlugins() })
  ...
  async completeMcpServerAuth(
    ...
    if (active === undefined) {
      throw new NighthawkError(ErrorCodes.REQUEST_INVALID, `Unknown MCP OAuth flow: ${flowId}`);
    }
    try {
  ...
  private async resolveMcpTestTarget(
    ...
    if (matches.length === 0) {
      throw new NighthawkError(ErrorCodes.MCP_SERVER_NOT_FOUND, `MCP server "${name}" was not found`);
    }
  ...
  setPentestMode({ sessionId, ...payload }: SessionAgentPayload<SetPentestModePayload>) {
    return this.sessionApi(sessionId).setPentestMode(payload);
  }
  ...
  async reloadPlugins(_: EmptyPayload): Promise<ReloadPluginsResult> {
    ...
    try {
      ...
      throw new NighthawkError(
        ...
        { cause: error, details: { nighthawkHomeDir: this.homeDir } },
      );
    }
  ...
  private assertPluginsLoaded(): void {
    ...
    throw new NighthawkError(
      ...
      { cause: this.pluginsLoadError, details: { nighthawkHomeDir: this.homeDir } },
    );
  }

  private async resolveRuntime(config: NighthawkConfig): Promise<ToolServices> {
    if (this.runtime !== undefined) return this.runtime;
    const runtime = await createRuntimeConfig({
      config,
      nighthawkRequestHeaders: this.nighthawkRequestHeaders,
  ...
  private getKaos(): Promise<Kaos> {
    this.kaos ??= LocalKaos.create().catch((error: unknown) => {
      if (error instanceof KaosShellNotFoundError) {
        throw new NighthawkError(ErrorCodes.SHELL_GIT_BASH_NOT_FOUND, error.message);
      }
      throw error;
    });
    return this.kaos;
  ...
  private resolveSessionSkillConfig(config: NighthawkConfig): SessionSkillConfig {
  ...
  private mergePluginMcpConfig(base: SessionMcpConfig | undefined): SessionMcpConfig | undefined {
    ...
    // (runtime rename, env/cwd constraints, managed NightHawk env); disabled ones
  ...
  private async resolveMcpRegistryEntry(
    ...
    try {
      ...
      if (error instanceof NighthawkError && error.code === ErrorCodes.MCP_SERVER_NOT_FOUND) {
        return undefined;
      }
      throw error;
    }
  ...
  private managedNighthawkEnvForPlugins(): Record<string, string> {
    const provider = this.config.providers[NIGHTHAWK_PROVIDER_NAME];
    const envBaseUrl = process.env[NIGHTHAWK_BASE_URL_ENV];
    const envOAuthHost = process.env[NIGHTHAWK_OAUTH_HOST_ENV] ?? process.env[NIGHTHAWK_OAUTH_HOST_ENV];
    ...
    if (baseUrl !== undefined) env[NIGHTHAWK_BASE_URL_ENV] = baseUrl;
    if (oauthHost !== undefined) env[NIGHTHAWK_OAUTH_HOST_ENV] = oauthHost;
    return env;
  }
  ...
  private requireSession(sessionId: string): Session {
    const session = this.sessions.get(sessionId);
    if (session === undefined) {
      throw new NighthawkError(ErrorCodes.SESSION_NOT_FOUND, `Session "${sessionId}" was not found`, {
        details: { sessionId },
      });
    }
    return session;
  ...
  private reloadProviderManager(): NighthawkConfig {
    return this.reloadRuntimeConfig();
  }

  private readConfigForWrite(): NighthawkConfig {
    return readConfigFileForUpdate(this.configPath);
  }

  private reloadRuntimeConfig(): NighthawkConfig {
    const loaded = loadRuntimeConfigSafe(this.configPath);
  ...
  private setRuntimeConfig(config: NighthawkConfig): NighthawkConfig {
    this.config = config;
  ...
  /**
   * Config bound to a newly created/resumed session. In print mode (`nighthawk -p`,
   * v1) the print-mode defaults are merged in; explicit user config wins. The
   * raw `this.config` is left untouched so `getNighthawkConfig` and config writes
   * still round-trip the user's file values.
   */
  private withPrintModeDefaults(config: NighthawkConfig): NighthawkConfig {
  ...
  private async refreshSessionRuntimeConfig(
    ...
    for (const model of candidates) {
      try {
        ...
        if (
          aliasMissing &&
          error instanceof NighthawkError &&
...
/**
 * Resolve `NIGHTHAWK_WEB_SEARCH_*` / `NIGHTHAWK_WEB_FETCH_*` without mixing credentials
...
function withServiceEnv(
  service: NighthawkServiceConfig | undefined,
  baseUrlEnv: string,
  apiKeyEnv: string,
): NighthawkServiceConfig | undefined {
...
function serviceCredentials(
  service: NighthawkServiceConfig,
  ...
  return {
    ...
    tokenProvider:
      service.oauth !== undefined
        ? resolveOAuthTokenProvider?.(NIGHTHAWK_PROVIDER_NAME, service.oauth)
        : undefined,
...
function telemetryErrorReason(error: unknown): string {
  if (error instanceof NighthawkError) return error.code;
...
function clientTelemetryProperties(client: ClientTelemetryInfo | undefined): TelemetryProperties {
  ...
  // producer in `nighthawk-harness.ts`. Other session events also inherit these as
