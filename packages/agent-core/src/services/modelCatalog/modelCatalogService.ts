...
import type { NighthawkConfig, ModelAlias, ProviderConfig, ProviderType } from '../../config';
import type {
  ModelCatalogItem,
  ProviderCatalogItem,
...
export class ModelCatalogService
  ...
  async setDefaultModel(modelId: string): Promise<SetDefaultModelResponse> {
    ...
    const updated = await this.core.rpc.setNighthawkConfig({ defaultModel: modelId });
  ...
  private _providerTypeOf(config: NighthawkConfig, alias: ModelAlias): ProviderType | undefined {
  ...
  private _buildRefreshHost(): RefreshProviderHost {
    return {
      ...
      removeProvider: (providerId) => this.core.rpc.removeNighthawkProvider({ providerId }),
      setConfig: (patch) => this.core.rpc.setNighthawkConfig(patch as Record<string, unknown>),
      resolveOAuthToken: (providerName, oauthRef) =>
        this._resolveOAuthToken(providerName, oauthRef),
      userAgent: this.core.nighthawkRequestHeaders?.['User-Agent'],
    };
  }
  ...
  private async _readConfig(): Promise<NighthawkConfig> {
    return this.core.rpc.getNighthawkConfig({ reload: true });
  }
...
function hasConfiguredApiKey(provider: ProviderConfig): boolean {
  ...
  switch (provider.type) {
    ...
    case 'nighthawk':
      return nonEmpty(provider.env?.['NIGHTHAWK_API_KEY']) !== undefined;
    case 'google-genai':
