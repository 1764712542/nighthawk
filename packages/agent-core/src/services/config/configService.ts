...
import type { NighthawkConfig, ProviderConfig } from '../../config';
import type { ConfigResponse, PatchConfigRequest } from '@nighthawk/protocol';
...
export class ConfigService extends Disposable implements IConfigService {
  ...
  async get(): Promise<ConfigResponse> {
    const config = await this.core.rpc.getNighthawkConfig({ reload: true });
  ...
  async set(patch: PatchConfigRequest): Promise<ConfigResponse> {
    ...
    const updated = await this.core.rpc.setNighthawkConfig(camelPatch);
...
function toConfigResponse(config: NighthawkConfig): ConfigResponse {
