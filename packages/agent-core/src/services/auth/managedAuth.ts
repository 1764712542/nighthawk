...
import type { NighthawkConfig, OAuthRef } from '../../config';
...
type ServicesManagedConfig = NighthawkConfig & ManagedNighthawkConfigShape;

type ServicesAuthLoginOptions = Omit<NighthawkOAuthLoginOptions, 'provisionConfig'>;
...
class ServicesManagedAuthFacade implements ServicesAuthFacade {
  private readonly toolkit: NighthawkOAuthToolkit<ServicesManagedConfig>;

  constructor(
    private readonly options: Pick<IEnvironmentService, 'homeDir' | 'configPath'>,
    identity?: NighthawkHostIdentity,
  ) {
    this.toolkit = new NighthawkOAuthToolkit<ServicesManagedConfig>({
  ...
  private runtimeOAuthRef(
    ...
    if ((providerName ?? NIGHTHAWK_PROVIDER_NAME) !== NIGHTHAWK_PROVIDER_NAME) {
      return oauthRef;
    }
    const auth = this.resolveManagedAuth(providerName);
    return resolveNighthawkRuntimeAuth({
