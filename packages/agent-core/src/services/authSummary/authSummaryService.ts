...
import type { NighthawkConfig } from '../../config';
import type { AuthSummary } from '@nighthawk/protocol';
...
/** Wire name of the OAuth-managed provider (`@nighthawk/nighthawk-oauth`'s `NIGHTHAWK_PROVIDER_NAME`). */
const MANAGED_PROVIDER_NAME = 'managed:nighthawk';
...
export class AuthSummaryService
  ...
  private async _readConfig(): Promise<NighthawkConfig> {
    // `reload: true` forces NighthawkCore to re-read `config.toml` from disk
    ...
    // NighthawkCore's `this.config` only refreshes when something explicitly
