 ...
 * role — routes `ApprovalRequest`s coming out of `NighthawkCore` to a waiter
 ...
 *      sees coming off `NighthawkCore.requestApproval(...)`. See
 ...
 * the adapter (NighthawkCore → BridgeClientAPI.requestApproval →
...
import type {} from '@nighthawk/protocol'; // type-only marker — keep protocol dep referenced
...
export interface IApprovalService {
  ...
  /**
   * Called by the adapter when NighthawkCore needs user approval. Resolves with the
