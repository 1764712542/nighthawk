 ...
 *   2. `new NighthawkCore(coreRpc, options)` — the core is constructed with the
 ...
 *
 * Lifecycle:
 *   - `ready()` resolves when both the `NighthawkCore` plugin/config load AND the
 ...
 *     method dispatch throws before reaching `NighthawkCore`, then walks the
 *     `Disposable` child stack. `NighthawkCore` itself has no `dispose()` today —
...
import type { CoreRPC, NighthawkCoreOptions } from '../../rpc';
...
export interface CoreProcessServiceOptions extends NighthawkCoreOptions {
  /**
   * Host identity (product name + version). When set and
   * `nighthawkRequestHeaders` is omitted, the adapter default-wires
   * `createNighthawkDefaultHeaders({ homeDir, ...identity })` into NighthawkCore so
   * upstream sees `User-Agent: <product>/<version>` + `X-Msh-Platform: …`.
   * Without this, the managed NightHawk-for-Coding endpoint rejects requests
   ...
   *
   * Callers can still pass explicit `nighthawkRequestHeaders` (or `appVersion`)
...
export interface ICoreProcessService {
  ...
  /**
   * The telemetry client the host wired into `NighthawkCore` (noop when the host
  ...
  /**
   * Resolves once `NighthawkCore` is fully constructed and the SDK side of the
   ...
   * "core process disposed" error before reaching `NighthawkCore`. Idempotent.
   */
  dispose(): void;
}
