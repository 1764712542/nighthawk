...
import { createRPC, NighthawkCore } from '../../rpc';
...
export class CoreProcessService extends Disposable implements ICoreProcessService {
  ...
  public readonly nighthawkRequestHeaders: Record<string, string> | undefined;
  ...
  /**
   * The in-process `NighthawkCore` instance. Kept private so daemon-side code can't
   * grab it and bypass the peer-service indirection.
   */
  private readonly _core: NighthawkCore;
   ...
   * marker today; once `NighthawkCore.pluginsReady` is publicly exposed we can
   * combine them here.
   */
  ...
  constructor(
    ...
    //    function NighthawkCore receives, `sdkRpc` is the one we satisfy.
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();

    // Default-wire the OAuth token resolver. Without this, NighthawkCore's
    ...
    // home + config paths NighthawkCore will use, and handing its
    ...
    // Default-wire the NightHawk request headers (User-Agent + X-Msh-* device
    // identity). Without this, NighthawkCore's outbound fetch carries the
    // default Node fetch User-Agent and the managed NightHawk-for-Coding
    // endpoint rejects with 40340 ("only available for Coding Agents
    // such as NightHawk CLI, Claude Code, …"). Mirrors what `SDKRpcClient`
    // does for the in-process TUI path (node-sdk's sdk-rpc-client.ts).
    // Caller-supplied `nighthawkRequestHeaders` always wins; absent that, we
    ...
    this.nighthawkRequestHeaders =
      options.nighthawkRequestHeaders ??
      CoreProcessService._defaultNighthawkRequestHeaders(env.homeDir, options.identity);
    ...
    // Default-wire the workspace-id resolver. Without it, NighthawkCore's session
    ...
    // 2. Construct the core. NighthawkCore's ctor wires itself into `coreRpc` and
    //    exposes `this.sdk: Promise<SDKRPC>` for the reverse direction.
    this._core = new NighthawkCore(coreRpc, {
      ...options,
      homeDir: env.homeDir,
    ...
    //    happens inside NighthawkCore's ctor and self-heals (the worker captures
  ...
  override dispose(): void {
    ...
    // NighthawkCore does not currently expose a dispose() — when it does, we'll
    ...
    // before they reach NighthawkCore.
    super.dispose();
  }
   ...
   * paths NighthawkCore resolves internally. Mirrors `SDKRpcClient`'s default in
   ...
   * carry the same `X-Msh-*` device headers as `_defaultNighthawkRequestHeaders`.
   *
  ...
  static _defaultOAuthTokenResolver(
    homeDir: string,
    configPath: string,
    identity?: NighthawkHostIdentity,
  ...
  /**
   * Build the default `nighthawkRequestHeaders` from `options.identity` so the
   * outbound `User-Agent` + device-identity headers identify this process
   * as a real Coding Agent host (e.g. `nighthawk-cli/<ver>`). Without
   * these, the managed NightHawk-for-Coding endpoint rejects with 40340.
   *
   * Returns `undefined` when no identity is provided — preserves the
   * pre-fix contract for hosts that pass headers explicitly via
   * `options.nighthawkRequestHeaders` (or for legacy callers / tests that
   * don't talk to the managed endpoint at all).
   *
   * `homeDir` resolution matches NighthawkCore's so the per-device id (minted
   * + cached at `<homeDir>/device_id` on first call) lives in the same
   * root as everything else NighthawkCore touches.
   *
   * Exposed as `static` so tests can assert the wiring without booting
   * the service.
   */
  static _defaultNighthawkRequestHeaders(
    homeDir: string,
    identity?: NighthawkHostIdentity,
