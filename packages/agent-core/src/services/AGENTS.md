...
In-process service layer for the nighthawk server, now hosted inside
`@nighthawk/agent-core` (merged from the former `@nighthawk/services`
...
from `index.ts` so importing `@nighthawk/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@nighthawk/agent-core` loads the package barrel, whose impl
