 ...
 *    state under `<NIGHTHAWK_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.nighthawk`).
...
import { OAuthTokenTransaction } from '@nighthawk/nighthawk-oauth';
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("nighthawk (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `nighthawk (${options.serverName})`;
