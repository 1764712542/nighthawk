 ...
 *
 * One instance per process (NighthawkCore shares it with every Session). The
...
export interface McpOAuthServiceOptions {
  /** Storage backend; overrides `nighthawkHomeDir` when supplied. */
  readonly store?: JsonFileStore;
  /** Resolved NightHawk home; credentials default to `<nighthawkHomeDir>/credentials/mcp/`. */
...
export class McpOAuthService {
  ...
  constructor(options: McpOAuthServiceOptions = {}) {
    this.store =
      options.store ??
      new JsonFileStore(
        options.nighthawkHomeDir === undefined ? undefined : mcpCredentialsDir(options.nighthawkHomeDir),
      );
