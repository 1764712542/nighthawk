 ...
 * `<NIGHTHAWK_HOME>/credentials/mcp/` (default
 * `~/.nighthawk/credentials/mcp/`).
 *
...
export function mcpCredentialsDir(nighthawkHomeDir: string): string {
  return join(nighthawkHomeDir, 'credentials', 'mcp');
}

export function defaultMcpCredentialsDir(): string {
  return mcpCredentialsDir(join(homedir(), '.nighthawk'));
}
