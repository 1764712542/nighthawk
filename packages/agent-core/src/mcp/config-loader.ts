...
import { resolveNighthawkHome } from '#/config/path';
...
export async function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): Promise<McpJsonPaths> {
  ...
  return {
    user: join(resolveNighthawkHome(input.homeDir), 'mcp.json'),
    projectRoot: join(projectRoot, '.mcp.json'),
    project: join(input.cwd, '.nighthawk', 'mcp.json'),
  };
}
...
/**
 * Load MCP server declarations from the user-global `~/.nighthawk/mcp.json`,
 * the project-root `<project root>/.mcp.json`, and the project-local
 * `<cwd>/.nighthawk/mcp.json`. Entries in later files override earlier files
 * with the same key, so a repo can specialise or replace a shared definition,
 * and NightHawk-specific project config wins over the Claude-compatible root file.
 *
...
async function readMcpJson(
  ...
  try {
    ...
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Failed to read ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Invalid JSON in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Invalid MCP server config in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
}
