...
import type { Tool } from '@nighthawk/kosong';
...
export const MCP_STARTUP_TIMEOUT_ENV = 'NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS';
export const MCP_TOOL_TIMEOUT_ENV = 'NIGHTHAWK_MCP_TOOL_TIMEOUT_MS';
 ...
 * timeout. Precedence: `NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS` (integer ms) →
 ...
 * `NIGHTHAWK_MCP_TOOL_TIMEOUT_MS` (integer ms) → `configMs`
...
export interface McpConnectionManagerOptions {
   ...
   * (wired by NighthawkCore, keyed to the session workDir). Powers config-aware
  ...
  async reconnect(name: string, config?: McpServerConfig): Promise<void> {
    ...
    if (entry === undefined) {
      ...
      if (resolved === undefined || resolved.config.enabled === false) {
        throw new NighthawkError(ErrorCodes.MCP_SERVER_NOT_FOUND, `Unknown MCP server: ${name}`);
      }
    ...
    if (config !== undefined) {
      ...
      if (config.enabled === false) {
        throw new NighthawkError(ErrorCodes.MCP_SERVER_DISABLED, `MCP server is disabled: ${name}`);
      }
      entry.config = config;
