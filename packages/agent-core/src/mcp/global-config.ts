...
export class GlobalMcpConfigStore {
  ...
  constructor(homeDir?: string) {
    this.path = join(resolveNighthawkHome(homeDir), 'mcp.json');
  }
...
export function normalizeServerName(name: string): string {
  ...
  throw new NighthawkError(ErrorCodes.REQUEST_INVALID, 'MCP server name cannot be empty');
}

function serverNotFound(name: string): NighthawkError {
  return new NighthawkError(ErrorCodes.MCP_SERVER_NOT_FOUND, `MCP server "${name}" was not found`);
}

function configError(message: string, cause?: unknown): NighthawkError {
  return new NighthawkError(ErrorCodes.CONFIG_INVALID, message, { cause });
}
