 ...
 *
 * The `NIGHTHAWK_SECONDARY_MODEL` / `NIGHTHAWK_SECONDARY_EFFORT` env vars override
 * `model` / `default_effort` in memory only: like the `NIGHTHAWK_MODEL_*`
...
export const SECONDARY_MODEL_ENV = 'NIGHTHAWK_SECONDARY_MODEL';
export const SECONDARY_MODEL_EFFORT_ENV = 'NIGHTHAWK_SECONDARY_EFFORT';
 ...
 */
export function applySecondaryModelConfig(config: NighthawkConfig, env: Env = process.env): NighthawkConfig {
 ...
 * — e.g. a `/secondary-model` pick made under `NIGHTHAWK_SECONDARY_MODEL` — does
