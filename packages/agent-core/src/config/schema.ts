...
const ModelAliasBaseSchema = z.object({
  ...
  // managed NightHawk models that declare `protocol: 'anthropic'`.
...
export const ThinkingConfigSchema = z.object({
  ...
  // NightHawk Preserved Thinking passthrough (`thinking.keep`). The value is
...
export const McpConfigSchema = z.object({
   ...
   * NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS env var both win over this value. Defaults
   * to 30s when unset.
   */
   ...
   * NIGHTHAWK_MCP_TOOL_TIMEOUT_MS env var both win over this value. Falls back to
...
export const ImageConfigSchema = z.object({
   ...
   * Overrides the built-in default; the NIGHTHAWK_IMAGE_MAX_EDGE_PX env var wins
   * over this value.
   */
   ...
   * NIGHTHAWK_IMAGE_READ_BYTE_BUDGET env var wins over this value. Explicit
...
export const NighthawkServiceConfigSchema = z.object({
  baseUrl: z.string().optional(),
  apiKey: z.string().optional(),
  oauth: OAuthRefSchema.optional(),
  customHeaders: StringRecordSchema.optional(),
...
export type NighthawkServiceConfig = z.infer<typeof NighthawkServiceConfigSchema>;

export const ServicesConfigSchema = z.object({
  nighthawkSearch: NighthawkServiceConfigSchema.optional(),
  nighthawkFetch: NighthawkServiceConfigSchema.optional(),
});
...
export const NighthawkConfigSchema = z.object({
...
export type NighthawkConfig = z.infer<typeof NighthawkConfigSchema>;
...
const NighthawkServiceConfigPatchSchema = NighthawkServiceConfigSchema.partial();
const ServicesConfigPatchSchema = z.object({
  nighthawkSearch: NighthawkServiceConfigPatchSchema.optional(),
  nighthawkFetch: NighthawkServiceConfigPatchSchema.optional(),
});

export const NighthawkConfigPatchSchema = z
  .object({
...
export type NighthawkConfigPatch = z.infer<typeof NighthawkConfigPatchSchema>;

export function getDefaultConfig(): NighthawkConfig {
  return {
    providers: {},
  };
}

export function validateConfig(config: unknown): NighthawkConfig {
  try {
    return NighthawkConfigSchema.parse(config);
  } catch (error) {
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Invalid configuration: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
