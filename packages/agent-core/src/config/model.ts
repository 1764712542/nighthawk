...
} from '@nighthawk/kosong/providers/anthropic-profile';
...
function withAnthropicProfile(model: ModelAlias, providerType?: ProviderType): ModelAlias {
  ...
  // protocol (catalog-imported NightHawk `k3`, GLM, …) must not advertise Claude
  // effort levels. NightHawk providers — including managed models routed through
  ...
  const profile =
    providerType !== undefined && providerType !== 'nighthawk' && protocol === 'anthropic'
