...
const DEFAULT_CONFIG_FILE_TEXT = `# ~/.nighthawk/config.toml
# Runtime settings for NightHawk.
# This file starts empty so built-in defaults can apply.
# Login will populate managed NightHawk provider and model entries.
`;
...
export function readConfigFile(filePath: string): NighthawkConfig {
 ...
 */
export function readConfigFileForUpdate(filePath: string): NighthawkConfig {
  try {
    return readConfigFile(filePath);
  } catch (error) {
    if (error instanceof NighthawkError && error.code === ErrorCodes.CONFIG_INVALID) {
      throw new NighthawkError(
        ErrorCodes.CONFIG_INVALID,
        `Cannot change settings while ${filePath} is invalid — fix it first (run \`nighthawk doctor\` for details).`,
        { cause: error },
      );
    }
    throw error;
 ...
 * synthesized from `NIGHTHAWK_MODEL_*` environment variables. Use this everywhere a
...
export interface RuntimeConfigLoadResult {
  ...
  /** Problems applying NIGHTHAWK_MODEL_* env overrides; the overlay was skipped. */
   ...
   * good config instead.
   */
  readonly fileError?: NighthawkError;
}
 ...
 * whole top-level section otherwise) and a bad NIGHTHAWK_MODEL_* env overlay is
...
export function loadRuntimeConfigSafe(
  ...
  let fileError: NighthawkError | undefined;
    ...
    if (data !== undefined) {
      ...
      if (salvaged.config === undefined) {
        ...
        if (salvaged.dropped.length > 0) {
          fileWarnings.push(
            `Ignored invalid config in ${filePath}: ${salvaged.dropped.join(', ')}. Run \`nighthawk doctor\` for details.`,
          );
        }
      }
    }
  ...
  try {
    ...
    envWarnings.push(
      `Ignoring NIGHTHAWK_MODEL_* environment overrides: ${describeUnknownError(error)}`,
    );
  }
...
interface SalvageResult {
  readonly config: NighthawkConfig | undefined;
...
function salvageConfigData(transformed: Record<string, unknown>): SalvageResult {
  ...
  for (;;) {
    const result = NighthawkConfigSchema.safeParse(transformed);
    if (result.success) {
...
export function parseConfigString(tomlText: string, filePath = 'config.toml'): NighthawkConfig {
  ...
  try {
    ...
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Invalid TOML in ${filePath}: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error,
    });
  }
...
function parseConfigData(data: Record<string, unknown>, filePath: string): NighthawkConfig {
  const raw = cloneRecord(data);
  const transformed = transformTomlData(data);
  transformed['raw'] = raw;

  try {
    return NighthawkConfigSchema.parse(transformed);
  } catch (error) {
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `Invalid configuration in ${filePath}: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
...
export async function writeConfigFile(filePath: string, config: NighthawkConfig): Promise<void> {
...
export function configToTomlData(config: NighthawkConfig): Record<string, unknown> {
  ...
  const scalarFields: (keyof NighthawkConfig)[] = [
    'defaultProvider',
    'defaultModel',
    'planMode',
    'yolo',
...
function servicesToToml(services: ServicesConfig, rawServices: unknown): Record<string, unknown> {
  ...
  if (services.nighthawkSearch !== undefined) {
    out['nighthawk_search'] = serviceToToml(services.nighthawkSearch);
  } else {
    delete out['nighthawk_search'];
  }
  if (services.nighthawkFetch !== undefined) {
    out['nighthawk_fetch'] = serviceToToml(services.nighthawkFetch);
  } else {
    delete out['nighthawk_fetch'];
  }
  return out;
}

function serviceToToml(service: NighthawkServiceConfig): Record<string, unknown> {
  const out: Record<string, unknown> = {};
