...
/**
 * Parse a floating-point environment value (e.g. `NIGHTHAWK_MODEL_TEMPERATURE`).
 * Returns `undefined` when unset/blank; throws `NighthawkError(CONFIG_INVALID)` on a
 * non-numeric value so a typo fails fast like the other `NIGHTHAWK_MODEL_*` vars.
...
export function parseFloatEnv(value: string | undefined, varName: string): number | undefined {
  ...
  if (!Number.isFinite(parsed)) {
    throw new NighthawkError(ErrorCodes.CONFIG_INVALID, `${varName} must be a number, got "${value}".`);
  }
  return parsed;
}
