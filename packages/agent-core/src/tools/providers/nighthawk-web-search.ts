/**
 * NighthawkWebSearchProvider — host-side `WebSearchProvider`.
 *
...
export interface NighthawkWebSearchProviderOptions {
...
interface NighthawkSearchResult {
  site_name?: string;
  title?: string;
  url?: string;
  snippet?: string;
...
interface NighthawkSearchResponse {
  search_results?: NighthawkSearchResult[];
}

export class NighthawkWebSearchProvider implements WebSearchProvider {
  ...
  constructor(options: NighthawkWebSearchProviderOptions) {
  ...
  async search(
    ...
    if (response.status === 401) {
      ...
      throw new Error(
        `NightHawk search request failed: HTTP 401 (auth/unauthorized). ${detail}`.trim(),
      );
    }
    ...
    if (response.status !== 200) {
      const detail = await safeReadText(response);
      throw new Error(
        `NightHawk search request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }

    const json = (await response.json()) as NighthawkSearchResponse;
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error(
      'NightHawk search service is not configured: missing API key or token provider.',
    );
  }
}
