/**
 * NighthawkFetchURLProvider — host-side UrlFetcher.
 *
 * Flow:
 *   1. Try NightHawk coding-fetch service (POST {url}, Bearer token from a
 *      narrow token provider, Accept: text/markdown, host-provided headers).
 *   2. NightHawk 200 → return the body as `extracted` content (the
 *      service has already extracted the main page text on its side).
 *   3. Any NightHawk failure — non-200, network error, or token
...
export interface NighthawkFetchURLProviderOptions {
...
export class NighthawkFetchURLProvider implements UrlFetcher {
  ...
  constructor(options: NighthawkFetchURLProviderOptions) {
  ...
  async fetch(url: string, options?: { toolCallId?: string }): Promise<UrlFetchResult> {
    try {
      const content = await this.fetchViaNighthawk(url, options?.toolCallId);
  ...
  private async fetchViaNighthawk(
    url: string,
    ...
    if (response.status !== 200) {
      ...
      throw new HttpFetchError(
        response.status,
        `NightHawk fetch request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error('NightHawk fetch service is not configured: missing API key or token provider.');
  }
}
