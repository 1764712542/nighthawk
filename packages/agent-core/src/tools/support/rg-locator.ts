 ...
 *   3. `<NIGHTHAWK_HOME>/bin/rg` — persistent cache for this app.
 *   4. CDN download to <NIGHTHAWK_HOME>/bin/ — one-off bootstrap
 *
...
async function downloadAndInstallRg(shareDir: string): Promise<string> {
  ...
  const tmp = await mkdtemp(join(tmpdir(), 'nighthawk-rg-'));
  try {
