...
export function resolveCompletionBudget(args: {
  ...
  const fromNew = parseEnvBudget(env['NIGHTHAWK_MODEL_MAX_COMPLETION_TOKENS']);
  if (fromNew !== 'absent') {
    return fromNew === 'disabled' ? undefined : { hardCap: fromNew };
  }
  const fromLegacy = parseEnvBudget(env['NIGHTHAWK_MODEL_MAX_TOKENS']);
 ...
 * in `NighthawkChatProvider._clone()`.
 */
