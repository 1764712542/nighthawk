...
describe('sessionSchema', () => {
  const fullSession: Session = {
    ...
    workspace_id: 'wd_nighthawk_0123456789ab',
    title: 'Test session',
    created_at: '2026-06-04T10:30:00.000Z',
...
describe('sessionCreateSchema', () => {
  ...
  it('parses a create with workspace_id only', () => {
    ...
    ).toEqual({ workspace_id: 'wd_nighthawk_0123456789ab' });
  });
  ...
  it('parses a create with BOTH workspace_id and metadata.cwd (route layer enforces agreement)', () => {
    ...
    expect(parsed.workspace_id).toBe('wd_nighthawk_0123456789ab');
  ...
  it('parses a full create with title + agent_config', () => {
    ...
    expect(parsed.agent_config?.model).toBe('nighthawk-v1-128k');
  });
...
describe('sessionUpdateSchema', () => {
  ...
  it('parses a partial agent_config patch', () => {
    expect(
      sessionUpdateSchema.parse({ agent_config: { model: 'nighthawk-v1-256k' } }),
    ).toEqual({ agent_config: { model: 'nighthawk-v1-256k' } });
  });
