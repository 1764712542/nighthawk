...
const sampleWorkspace: Workspace = {
  id: 'wd_nighthawk_0123456789ab',
  root: '/Users/foo/code/nighthawk',
  name: 'nighthawk',
...
describe('workspaceIdSchema', () => {
  it('accepts a wd_<slug>_<hash12> string', () => {
    expect(workspaceIdSchema.parse('wd_nighthawk_0123456789ab')).toBe('wd_nighthawk_0123456789ab');
  });

  it('accepts dots, dashes, underscores in slug', () => {
    expect(workspaceIdSchema.parse('wd_nighthawk.v2_0123456789ab')).toBe(
      'wd_nighthawk.v2_0123456789ab',
    );
  });

  it('rejects missing wd_ prefix', () => {
    expect(workspaceIdSchema.safeParse('nighthawk_0123456789ab').success).toBe(false);
  });

  it('rejects non-hex tail', () => {
    expect(workspaceIdSchema.safeParse('wd_nighthawk_xyzxyzxyzxyz').success).toBe(false);
  });

  it('rejects truncated hash', () => {
    expect(workspaceIdSchema.safeParse('wd_nighthawk_0123456789').success).toBe(false);
  });
});
...
describe('workspaceIdParamSchema', () => {
  it('accepts a wd_-shaped workspace_id', () => {
    expect(
      workspaceIdParamSchema.parse({ workspace_id: 'wd_nighthawk_0123456789ab' }).workspace_id,
    ).toBe('wd_nighthawk_0123456789ab');
  });
