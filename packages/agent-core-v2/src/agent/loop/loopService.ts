...
import { BugIndicatingError, ErrorCodes, Error2, isError2, toNighthawkErrorPayload } from '#/errors';
...
export class AgentLoopService extends Disposable implements IAgentLoopService {
  ...
  private async runTurn(
    ...
    try {
      ...
      if (result !== undefined) {
        const error = result.type === 'failed' ? toNighthawkErrorPayload(result.error) : undefined;
