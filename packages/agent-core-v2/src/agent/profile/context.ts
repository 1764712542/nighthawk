...
export function dotNighthawkAgentsMdPath(dir: string): string {
  return join(dir, '.nighthawk', 'AGENTS.md');
}

export function agentsMdCandidatePaths(dir: string): string[] {
  return [dotNighthawkAgentsMdPath(dir), ...AGENTS_MD_PLAIN_NAMES.map((name) => join(dir, name))];
}
...
export async function findAgentsMdInDir(
  ...
  const found: string[] = [];
  const dotNighthawk = dotNighthawkAgentsMdPath(dir);
  if (await isNonEmptyFile(deps, dotNighthawk)) found.push(dotNighthawk);
...
export async function loadAgentsMdForRoots(
  ...
  const brandDir = brandHome ?? join(realHome, '.nighthawk');
  ...
  for (const workDir of workDirs) {
    ...
    for (const dir of dirs) {
      await collect(dotNighthawkAgentsMdPath(dir));
      for (const fileName of AGENTS_MD_PLAIN_NAMES) {
