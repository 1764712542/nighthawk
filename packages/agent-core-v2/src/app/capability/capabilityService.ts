...
import { NIGHTHAWK_PROVIDER_NAME, resolveNighthawkRegion } from '@nighthawk/nighthawk-oauth';
...
import { createNighthawkCuEntry } from './entries/nighthawkCu';
import { createNighthawkWebbridgeEntry } from './entries/nighthawkWebbridge';
import type {
  CapabilityEntry,
  CapabilityId,
  ...
  constructor(
    ...
    if (entriesOverride !== undefined) {
      ...
      const ctx = {
        ...
        arch: process.arch,
        nighthawkHomeDir: bootstrap.homeDir,
        userHomeDir: homedir(),
        plugins,
        hostProcess,
        resolveRegion: () => {
          const oauth = providers.get(NIGHTHAWK_PROVIDER_NAME)?.oauth;
          return resolveNighthawkRegion({
            configuredOAuthHost: oauth?.oauthHost,
            configuredOAuthKey: oauth?.key,
            readMarker:
              (bootstrap.getEnv('NIGHTHAWK_REGION_MARKER') ??
      ...
      this.entries = new Map<CapabilityId, CapabilityEntry>([
        ['nighthawk-cu', createNighthawkCuEntry(ctx)],
        ['nighthawk-webbridge', createNighthawkWebbridgeEntry(ctx)],
      ]);
    }
  }
