...
export const WEB_SEARCH_BASE_URL_ENV = 'NIGHTHAWK_WEB_SEARCH_BASE_URL';
export const WEB_SEARCH_API_KEY_ENV = 'NIGHTHAWK_WEB_SEARCH_API_KEY';
export const WEB_FETCH_BASE_URL_ENV = 'NIGHTHAWK_WEB_FETCH_BASE_URL';
export const WEB_FETCH_API_KEY_ENV = 'NIGHTHAWK_WEB_FETCH_API_KEY';
...
const nighthawkSearchEnvBindings = envBindings(NighthawkServiceConfigSchema, {
  baseUrl: { env: WEB_SEARCH_BASE_URL_ENV, parse: nonBlankEnv },
  apiKey: { env: WEB_SEARCH_API_KEY_ENV, parse: nonBlankEnv },
});

const nighthawkFetchEnvBindings = envBindings(NighthawkServiceConfigSchema, {
...
export const servicesEnvBindings: EnvBindings<ServicesConfig> = envBindings(
  ...
  {
    nighthawkSearch: nighthawkSearchEnvBindings,
    nighthawkFetch: nighthawkFetchEnvBindings,
  },
);
...
const servicesCredentialEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    ...
    const nighthawkSearch = isolateEnvServiceCredentials(
      services['nighthawkSearch'],
      getEnv,
      WEB_SEARCH_BASE_URL_ENV,
      WEB_SEARCH_API_KEY_ENV,
    );
    const nighthawkFetch = isolateEnvServiceCredentials(
      services['nighthawkFetch'],
      getEnv,
      WEB_FETCH_BASE_URL_ENV,
      WEB_FETCH_API_KEY_ENV,
    );
    if (
      nighthawkSearch === services['nighthawkSearch'] &&
      nighthawkFetch === services['nighthawkFetch']
    ) {
      return [];
    }
...
const stripNighthawkSearchEnv = stripEnvBoundFields(nighthawkSearchEnvBindings);
const stripNighthawkFetchEnv = stripEnvBoundFields(nighthawkFetchEnvBindings);
...
export const stripServicesEnv: ConfigStripEnv<ServicesConfig> = (value, raw, getEnv) => {
  ...
  for (const [key, strip] of [
    ['nighthawkSearch', stripNighthawkSearchEnv],
    ['nighthawkFetch', stripNighthawkFetchEnv],
  ] as const) {
...
export const servicesToToml = (value: unknown, rawSnake: unknown): unknown => {
  ...
  writeService(out, 'nighthawk_search', value['nighthawkSearch']);
  writeService(out, 'nighthawk_fetch', value['nighthawkFetch']);
  return out;
};
