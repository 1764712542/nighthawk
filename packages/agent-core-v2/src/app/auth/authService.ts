...
export class OAuthService extends Disposable implements IOAuthService {
  ...
  getFlow(provider = NIGHTHAWK_PROVIDER_NAME): OAuthFlowSnapshot | undefined {
  ...
  cancelLogin(provider = NIGHTHAWK_PROVIDER_NAME): Promise<OAuthLoginCancelResponse> {
    const state = this.flows.get(provider);
  ...
  async logout(provider = NIGHTHAWK_PROVIDER_NAME): Promise<OAuthLogoutResponse> {
    const oauthRef =
      provider === NIGHTHAWK_PROVIDER_NAME
  ...
  async status(provider = NIGHTHAWK_PROVIDER_NAME): Promise<AuthStatus> {
  ...
  getManagedUsage(provider = NIGHTHAWK_PROVIDER_NAME): Promise<AuthManagedUsageResult> {
  ...
  getManagedUserInfo(provider = NIGHTHAWK_PROVIDER_NAME): Promise<AuthManagedUserInfoResult> {
    const configured = this.providerService.get(provider);
    const auth = resolveNighthawkRuntimeAuth({
      configuredBaseUrl: configured?.baseUrl,
      configuredOAuthRef: configured?.oauth,
  ...
  private async doRefreshOAuthProviderModels(): Promise<RefreshOAuthProviderModelsResponse> {
    ...
    const provider = current.providers[NIGHTHAWK_PROVIDER_NAME];
    ...
    try {
      ...
      const tokenProvider = this.resolveTokenProvider(NIGHTHAWK_PROVIDER_NAME, auth.oauthRef);
      if (tokenProvider === undefined) {
        throw new Error2(ErrorCodes.AUTH_TOKEN_MISSING, 'OAuth token provider is not configured.', {
          details: { provider_id: NIGHTHAWK_PROVIDER_NAME },
        });
      }
      ...
      restoreProviderAliases(
        next,
        preserveUserProviderAliases(current, NIGHTHAWK_PROVIDER_NAME, refreshedAliasKeys),
      );
      restoreDefaultSelection(next, current.defaultModel, current.thinking?.enabled);
      clampDanglingDefault(next);

      if (providerModelsEqual(current, next, NIGHTHAWK_PROVIDER_NAME, refreshedAliasKeys)) {
        ...
        changed.push({
          provider_id: NIGHTHAWK_PROVIDER_NAME,
  ...
  private readUserConfigShape(): ManagedNighthawkConfigShape {
    const providers =
    ...
    const services =
      this.config.inspect<ManagedNighthawkConfigShape['services']>(SERVICES_SECTION).userValue;
    const defaultModel = this.config.inspect<string>(DEFAULT_MODEL_SECTION).userValue;
    const thinking =
      this.config.inspect<ManagedNighthawkConfigShape['thinking']>(THINKING_SECTION).userValue;
    return {
      providers: { ...providers } as ManagedNighthawkConfigShape['providers'],
      models: { ...models } as ManagedNighthawkConfigShape['models'],
  ...
  getRegion(): NighthawkRegion {
    const oauth = this.providerService.get(NIGHTHAWK_PROVIDER_NAME)?.oauth;
    return resolveNighthawkRegion({
      configuredOAuthHost: oauth?.oauthHost,
      configuredOAuthKey: oauth?.key,
      readMarker:
        (this.bootstrap.getEnv('NIGHTHAWK_REGION_MARKER') ??
  ...
  private resolveLoginAuth(
    ...
    if (provider !== NIGHTHAWK_PROVIDER_NAME) {
      return { oauthRef: config?.oauth, baseUrl: undefined, oauthHost: undefined };
    }
    const hosts = region === undefined ? undefined : nighthawkRegionLoginHosts(region);
  ...
  private resolveRuntimeOAuthRef(provider: string, oauthRef?: OAuthRef): OAuthRef | undefined {
    if (provider !== NIGHTHAWK_PROVIDER_NAME) return oauthRef;
    const config = this.providerService.get(provider);
  ...
  private async finalizeAuthentication(state: FlowState): Promise<void> {
    try {
      ...
      if (state.status !== 'pending') return;
      if (state.provider === NIGHTHAWK_PROVIDER_NAME) {
  ...
  private async provisionProvider(
    ...
    if (oauthRef === undefined && provider !== NIGHTHAWK_PROVIDER_NAME) return;
    const baseUrl =
      loginBaseUrl ?? this.providerService.get(provider)?.baseUrl ?? nighthawkBaseUrl();
  ...
  private async deprovisionProvider(provider: string): Promise<void> {
    if (provider !== NIGHTHAWK_PROVIDER_NAME) return;
    const next = structuredClone(this.readUserConfigShape());
    const cleanup = clearManagedNighthawkConfig(next);
    if (
...
function restoreProviderAliases(
  ...
  config.models = {
    ...config.models,
    ...aliases,
  } as ManagedNighthawkConfigShape['models'];
}
...
class OAuthToolkitService extends NighthawkOAuthToolkit implements IOAuthToolkit {
  declare readonly _serviceBrand: undefined;
