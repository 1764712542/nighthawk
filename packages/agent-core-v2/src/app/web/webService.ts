...
import { NighthawkFetchURLProvider } from './providers/nighthawk-fetch-url';
...
export class WebFetchService implements IWebFetchService {
  ...
  private fromServicesConfig(): UrlFetcher | undefined {
    const fetchConfig = this.config.get<ServicesConfig>(SERVICES_SECTION)?.nighthawkFetch;
    ...
    const tokenProvider =
      fetchConfig.oauth === undefined
        ? undefined
        : this.oauth.resolveTokenProvider(NIGHTHAWK_PROVIDER_NAME, fetchConfig.oauth);
    return new NighthawkFetchURLProvider({
  ...
  private fromManagedOAuth(): UrlFetcher | undefined {
    ...
    const baseUrl = `${(provider.baseUrl ?? nighthawkBaseUrl()).replace(/\/+$/, '')}/fetch`;
    return new NighthawkFetchURLProvider({
      baseUrl,
      tokenProvider,
