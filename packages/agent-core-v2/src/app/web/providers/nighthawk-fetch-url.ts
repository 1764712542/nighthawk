...
export class NighthawkFetchURLProvider implements UrlFetcher {
  ...
  async fetch(
    ...
    try {
      const content = await this.fetchViaNighthawk(url, options?.toolCallId, options?.signal);
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error2(
      ...
      'NightHawk fetch service is not configured: missing API key or token provider.',
    );
  }
}
