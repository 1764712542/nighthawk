...
export async function readInstalled(nighthawkHomeDir: string): Promise<InstalledFile> {
  const filePath = path.join(nighthawkHomeDir, INSTALLED_REL);
  let text: string;
  try {
...
export async function writeInstalled(nighthawkHomeDir: string, data: InstalledFile): Promise<void> {
  const dir = path.join(nighthawkHomeDir, 'plugins');
