  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    try {
      if (resolved.kind === 'local-path') {
        ...
        zipTmpDir = await mkdtemp(path.join(tmpdir(), 'nighthawk-plugin-zip-'));
      ...
      managedCopy = await copyPluginToManagedRoot(this.nighthawkHomeDir, id, sourceRoot);
    ...
    for (const record of this.records.values()) {
      ...
      for (const hook of record.manifest.hooks ?? []) {
        out.push({
          ...
          cwd: record.root,
          env: {
            NIGHTHAWK_HOME: this.nighthawkHomeDir,
            NIGHTHAWK_PLUGIN_ROOT: record.root,
          },
        });
      }
    }
