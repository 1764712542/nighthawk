...
export class PluginService extends Service implements IPluginService {
  ...
  constructor(
    ...
    this.envBaseUrl = bootstrap.getEnv(NIGHTHAWK_BASE_URL_ENV);
    this.envOAuthHost =
      bootstrap.getEnv(NIGHTHAWK_OAUTH_HOST_ENV) ?? bootstrap.getEnv(NIGHTHAWK_OAUTH_HOST_ENV);
      ...
      this.enqueueMutation(async () => {
        try {
          ...
          throw new Error2(
            ...
            { cause: this.loadError, details: { nighthawkHomeDir: this.homeDir } },
          );
        }
      }),
    );
  ...
  enabledMcpServers(): Promise<Record<string, McpServerConfig>> {
    return this.runConsumptionRead({}, async () => {
      ...
      const managedEnv = await this.managedNighthawkEnvForPlugins();
      return withManagedNighthawkPluginEnv(pluginServers, managedEnv);
    });
  }
  ...
  mcpServerEntries(): Promise<readonly PluginMcpServerEntry[]> {
    return this.runManagementRead(async () => {
      ...
      const managedEnv = await this.managedNighthawkEnvForPlugins();
      return withManagedNighthawkPluginEnvOnEntries(entries, managedEnv);
    });
  }
  ...
  private async managedNighthawkEnvForPlugins(): Promise<Record<string, string>> {
...
function withManagedNighthawkPluginEnv(
...
function withManagedNighthawkPluginEnvOnEntries(
