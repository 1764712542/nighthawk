...
const SESSION_LOG_REL = 'logs/nighthawk.log';
const GLOBAL_LOG_REL = 'logs/global/nighthawk.log';
const WEB_LOG_REL = 'logs/nighthawk-web.jsonl';
const DESKTOP_LOG_REL = 'logs/nighthawk-desktop.log';
    ...
    return exportSessionDirectory({
      ...
      desktopLogPath:
        input.includeDesktopLog === true
          ? join(this.bootstrap.homeDir, 'logs', 'nighthawk-desktop.log')
          : undefined,
...
function defaultExportZipName(sessionId: string, now: Date): string {
  ...
  return `nighthawk-debug-${shortId}-${timestamp}.zip`;
}
