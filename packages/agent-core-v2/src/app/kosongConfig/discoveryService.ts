...
interface StaticExclusion {
  ...
  readonly thinking?: ManagedNighthawkConfigShape['thinking'];
}
...
export class ProviderDiscoveryService implements IProviderDiscoveryService {
  ...
  private computeStaticExclusion(): StaticExclusion {
    ...
    const thinking = this.config.inspect<ManagedNighthawkConfigShape['thinking']>(
      THINKING_SECTION,
    ).userValue;
    return {
  ...
  private readUserConfigShape(exclusion: StaticExclusion = EMPTY_EXCLUSION): ManagedNighthawkConfigShape {
    const providers =
      this.config.inspect<Record<string, ProviderConfig>>(PROVIDERS_SECTION).userValue ?? {};
    const models =
      this.config.inspect<Record<string, ModelRecord>>(MODELS_SECTION).userValue ?? {};
    ...
    return {
      providers: withoutKeys(providers, exclusion.providers) as ManagedNighthawkConfigShape['providers'],
      models: withoutKeys(models, exclusion.models) as ManagedNighthawkConfigShape['models'],
      defaultModel,
      thinking: thinking === undefined ? undefined : { ...thinking },
    };
  }

  private shapeWithoutProvider(providerId: string): Promise<ManagedNighthawkConfigShape> {
    ...
    return Promise.resolve({
      ...
      models: restModels,
    } as ManagedNighthawkConfigShape);
  }

  private async applyRefreshPatch(
    patch: ManagedNighthawkConfigShape,
    exclusion: StaticExclusion,
  ): Promise<ManagedNighthawkConfigShape> {
    const userProviders =
      this.config.inspect<Record<string, ProviderConfig>>(PROVIDERS_SECTION).userValue ?? {};
    const userModels =
      this.config.inspect<Record<string, ModelRecord>>(MODELS_SECTION).userValue ?? {};
    ...
    return {
      providers:
        patch.providers !== undefined
          ? ({ ...exclusion.providers, ...patch.providers } as ManagedNighthawkConfigShape['providers'])
          : (userProviders as ManagedNighthawkConfigShape['providers']),
      models:
        patch.models !== undefined
          ? ({ ...exclusion.models, ...patch.models } as ManagedNighthawkConfigShape['models'])
          : (userModels as ManagedNighthawkConfigShape['models']),
      defaultModel:
  ...
  private async resolveOAuthToken(
    providerName: string,
    oauthRef?: ManagedNighthawkOAuthRef,
  ): Promise<string> {
