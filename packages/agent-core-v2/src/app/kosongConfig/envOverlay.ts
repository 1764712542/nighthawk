...
export const nighthawkModelEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    const model = trimmed(getEnv('NIGHTHAWK_MODEL_NAME'));
    const temperature = parseFloatEnv(
      getEnv('NIGHTHAWK_MODEL_TEMPERATURE'),
      'NIGHTHAWK_MODEL_TEMPERATURE',
    );
    const topP = parseFloatEnv(getEnv('NIGHTHAWK_MODEL_TOP_P'), 'NIGHTHAWK_MODEL_TOP_P');
    const thinkingKeep = trimmed(getEnv('NIGHTHAWK_MODEL_THINKING_KEEP'));
    const maxCompletionTokens =
      parseCompletionTokens(getEnv('NIGHTHAWK_MODEL_MAX_COMPLETION_TOKENS')) ??
      parseCompletionTokens(getEnv('NIGHTHAWK_MODEL_MAX_TOKENS'));
    ...
    const maxContextRaw = trimmed(getEnv('NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE'));
    const maxContextSize =
      maxContextRaw === undefined
        ? DEFAULT_MAX_CONTEXT_SIZE
        : parsePositiveInt(maxContextRaw, 'NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE');

    const maxOutputRaw = trimmed(getEnv('NIGHTHAWK_MODEL_MAX_OUTPUT_SIZE'));
    const maxOutputSize =
      maxOutputRaw === undefined
        ? undefined
        : parsePositiveInt(maxOutputRaw, 'NIGHTHAWK_MODEL_MAX_OUTPUT_SIZE');
    const capabilities = parseCapabilities(getEnv('NIGHTHAWK_MODEL_CAPABILITIES')) ?? DEFAULT_CAPABILITIES;
    const displayName = trimmed(getEnv('NIGHTHAWK_MODEL_DISPLAY_NAME'));
    const reasoningKey = trimmed(getEnv('NIGHTHAWK_MODEL_REASONING_KEY'));
    const adaptiveThinking = parseBooleanVar(
      getEnv('NIGHTHAWK_MODEL_ADAPTIVE_THINKING'),
    ...
    const providerType =
      typeof envProvider['type'] === 'string' ? envProvider['type'] : 'nighthawk';
    ...
    if (envProvider['type'] === undefined) providerPatch['type'] = 'nighthawk';
...
registerConfigOverlay(nighthawkModelEnvOverlay);
