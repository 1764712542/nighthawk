...
export const cronEnvBindings: EnvBindings<CronConfig> = envBindings(cronConfigSchema, {
  debug: { env: 'NIGHTHAWK_CRON_DEBUG', parse: on },
  noJitter: { env: 'NIGHTHAWK_CRON_NO_JITTER', parse: on },
  noStale: { env: 'NIGHTHAWK_CRON_NO_STALE', parse: on },
  disabled: { env: 'NIGHTHAWK_DISABLE_CRON', parse: on },
  manualTick: { env: 'NIGHTHAWK_CRON_MANUAL_TICK', parse: on },
  clock: 'NIGHTHAWK_CRON_CLOCK',
  pollIntervalMs: { env: 'NIGHTHAWK_CRON_POLL_INTERVAL_MS', parse: parsePollIntervalMs },
});
