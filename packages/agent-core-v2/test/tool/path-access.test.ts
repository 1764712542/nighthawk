...
describe('extendWorkspaceWithSkillRoots', () => {
  ...
  it('appends roots outside the workspace and existing additional dirs', () => {
    expect(extendWorkspaceWithSkillRoots(workspace, ['/home/user/.nighthawk/skills'])).toEqual({
      workspaceDir: '/repo',
      additionalDirs: ['/extra', '/home/user/.nighthawk/skills'],
    });
  });
...
describe('resolvePathAccess shell path bridge', () => {
  const WIN_ENV = {
    ...
    shellPath: 'C:\\nighthawk-test-nonexistent\\Git\\bin\\bash.exe',
  };
