...
import { OAuthConnectionError, OAuthUnauthorizedError } from '@nighthawk/nighthawk-oauth';
...
describe('SessionTitleService', () => {
  ...
  beforeEach(() => {
    ...
    tokenCalls = [];
    flagEnabled = true;
    providers = { 'managed:nighthawk': MANAGED_PROVIDER };
  ...
  it('does nothing without a managed OAuth provider', async () => {
    delete providers['managed:nighthawk'];
    titlePrompts = ['hello'];
  ...
  it('includes environment custom headers', async () => {
    vi.stubEnv('NIGHTHAWK_CUSTOM_HEADERS', 'X-Proxy-Header: from-env\n');
    titlePrompts = ['hello'];
  ...
  it('pairs the environment endpoint with its credential slot when it overrides persisted config', async () => {
    vi.stubEnv('NIGHTHAWK_BASE_URL', 'https://api.env.example.test/coding/v1');
    vi.stubEnv('NIGHTHAWK_OAUTH_HOST', 'https://auth.env.example.test');
    titlePrompts = ['hello'];
