...
describe('resolveModelAuthMaterial', () => {
  ...
  it('reads env-bag credentials through the vendor endpoint declarations', () => {
    expect(
      authMaterial({
        model: { model: 'm' },
        provider: { type: 'nighthawk', env: { NIGHTHAWK_API_KEY: 'nighthawk-env-key' } },
      }),
    ).toEqual({ apiKey: 'nighthawk-env-key' });
    expect(
      authMaterial({
        model: { model: 'm' },
...
describe('effectiveModelConfig', () => {
  ...
  it('infers the Anthropic profile for non-trait-driven vendors only', () => {
    ...
    const nighthawkRouted = effectiveModelConfig({ model: 'kimi-k2', protocol: 'anthropic' }, 'nighthawk');
    expect(nighthawkRouted.supportEfforts).toBeUndefined();
    expect(nighthawkRouted.capabilities).toBeUndefined();
  });
});
