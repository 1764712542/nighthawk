...
describe('ModelService', () => {
  ...
  it('resolves ready on the first loadAll and exposes the default pointer', async () => {
    ...
    service.loadAll({ k1: { model: 'nighthawk-k2', maxContextSize: 262144 } }, 'k1');
    await service.ready;
  ...
  it('supports CRUD and diffs state changes into onDidChangeModels', async () => {
    ...
    const k1: ModelRecord = { provider: 'nighthawk', model: 'nighthawk-k2', maxContextSize: 262144 };
  ...
  it('fires the pointer event only on real pointer changes', async () => {
    const service = createService({ k1: { model: 'nighthawk-k2' } });
