...
describe('registry-driven vendor verdicts', () => {
  it('drivesThinkingThroughTraits: trait-driven vendors only, no string branches', () => {
    expect(drivesThinkingThroughTraits('nighthawk')).toBe(true);
  ...
  it('usesTraitDrivenThinking: native traits and the (nighthawk, anthropic) pair registration', () => {
    expect(usesTraitDrivenThinking(registry, 'openai', 'nighthawk')).toBe(true);
    expect(usesTraitDrivenThinking(registry, 'anthropic', 'nighthawk')).toBe(true);
    ...
    expect(usesTraitDrivenThinking(registry, 'google-genai', 'nighthawk')).toBe(false);
  });

  it('requiresStrictThinkingValidation: only the strict-validation thinking driver', () => {
    expect(requiresStrictThinkingValidation(registry, 'openai', 'nighthawk')).toBe(true);
    expect(requiresStrictThinkingValidation(registry, 'anthropic', 'nighthawk')).toBe(false);
...
describe('resolveThinkingEffortForModel', () => {
  ...
  it('normalizes unknown efforts back to the model default under nighthawk semantics', () => {
  ...
  it('keeps always-thinking models on under nighthawk semantics', () => {
    const always = {
