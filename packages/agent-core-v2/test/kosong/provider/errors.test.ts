...
import { nighthawkAnthropicTrait, nighthawkOpenAITrait } from '#/kosong/provider/providers/nighthawk/nighthawk.contrib';
import { classifyNighthawkQuotaError } from '#/kosong/provider/providers/nighthawk/nighthawk-errors';
...
function nighthawkQuota429(message: string, type?: string): OpenAIAPIError {
...
describe('classifyNighthawkQuotaError (NightHawk trait classifier)', () => {
  it('classifies a structured exceeded_current_quota_error body as quota-exhausted', () => {
    const error = classifyNighthawkQuotaError(
      nighthawkQuota429('Too many requests', 'exceeded_current_quota_error'),
    );
  ...
  it.each([QUOTA_MESSAGE, TOKEN_QUOTA_MESSAGE])(
    ...
    (message) => {
      const error = classifyNighthawkQuotaError(nighthawkQuota429(message));
      expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
      expect(isRetryableGenerateError(error)).toBe(false);
    },
  );
  ...
  it.each(['Too many requests', 'your token quota per minute was exceeded'])(
    ...
    (message) => {
      expect(classifyNighthawkQuotaError(nighthawkQuota429(message))).toBeUndefined();
    },
  );
  ...
  it('is declared as the convertError hook on both NightHawk traits', () => {
    const context: TraitContext = {
      config: { protocol: 'openai', providerType: 'nighthawk', modelName: '' } as ProtocolAdapterConfig,
      providerId: 'nighthawk',
    };
    const source = nighthawkQuota429('Too many requests', 'exceeded_current_quota_error');
    expect(nighthawkOpenAITrait.convertError!(source, context)).toBeInstanceOf(
      APIProviderQuotaExhaustedError,
    );
    expect(nighthawkAnthropicTrait.convertError!(source, context)).toBeInstanceOf(
      APIProviderQuotaExhaustedError,
    );
  });
});
...
describe('convertError hook consult at the OpenAI boundary', () => {
  const nighthawkContext: TraitContext = {
    config: { protocol: 'openai', providerType: 'nighthawk', modelName: '' } as ProtocolAdapterConfig,
    providerId: 'nighthawk',
  };

  it('prefers the composed hook over the vendor-neutral base classification', () => {
    const hooks = composeOpenAIChatHooks([{ trait: nighthawkOpenAITrait, context: nighthawkContext }]);
    const source = nighthawkQuota429(QUOTA_MESSAGE);
  ...
  it('binds convertError with last-declarer-wins semantics', () => {
    const bound = traitConvertError([
      { trait: { convertError: () => new ChatProviderError('first') }, context: nighthawkContext },
      { trait: { convertError: () => new ChatProviderError('second') }, context: nighthawkContext },
    ]);
...
describe('OpenAI base quota classification (vendor-neutral)', () => {
  ...
  it('keeps vendor billing wordings a retryable rate limit without a hook', () => {
    const error = convertOpenAIError(nighthawkQuota429(QUOTA_MESSAGE));
