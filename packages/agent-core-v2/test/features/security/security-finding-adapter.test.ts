...
import type { DepAuditFinding } from '#/features/security/engine/dep-audit';
import { depFindingToNormalized } from '#/features/security/engine/dep-audit';
...
describe('depFindingToNormalized', () => {
  it('maps cve with high cvss to critical', () => {
    ...
    expect(out.severity).toBe('critical');
    expect(out.evidence[0]!.source).toBe('external');
  ...
  it('sets evidence source to external for cve kind', () => {
    const f: DepAuditFinding = {
      ecosystem: 'npm',
      package: 'axios',
      version: '1.5.0',
