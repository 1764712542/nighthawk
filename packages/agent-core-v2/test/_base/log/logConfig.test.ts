...
describe('resolveLoggingConfig', () => {
  it('uses defaults when env is empty', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/home/nighthawk', env: {} });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalLogPath).toBe('/home/nighthawk/logs/nighthawk.log');
  ...
  it('reads level and sizes from env', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        NIGHTHAWK_LOG_LEVEL: 'debug',
        NIGHTHAWK_LOG_GLOBAL_MAX_BYTES: '1024',
        NIGHTHAWK_LOG_GLOBAL_FILES: '7',
        NIGHTHAWK_LOG_SESSION_MAX_BYTES: '2048',
        NIGHTHAWK_LOG_SESSION_FILES: '4',
      },
    });
  ...
  it('ignores invalid level and non-positive sizes', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        NIGHTHAWK_LOG_LEVEL: 'verbose',
        NIGHTHAWK_LOG_GLOBAL_MAX_BYTES: '-5',
        NIGHTHAWK_LOG_GLOBAL_FILES: 'abc',
      },
    });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalMaxBytes).toBe(DEFAULT_GLOBAL_MAX_BYTES);
  ...
  it('resolves the log path regardless of env', () => {
    ...
    expect(cfg.globalLogPath).toBe('/h/logs/nighthawk.log');
  });
});

describe('path resolution', () => {
  it('resolves the global log path under homeDir/logs', () => {
    expect(resolveGlobalLogPath('/home/nighthawk')).toBe('/home/nighthawk/logs/nighthawk.log');
  });

  it('resolves the session log path under sessionDir/logs', () => {
    expect(resolveSessionLogPath('/sessions/s1')).toBe('/sessions/s1/logs/nighthawk.log');
  });
});

describe('logSeed', () => {
  it('seeds ILogOptions into a App scope', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/h', env: { NIGHTHAWK_LOG_LEVEL: 'warn' } });
    const host = createScopedTestHost(logSeed(cfg));
    const opts = host.app.accessor.get(ILogOptions);
    expect(opts.level).toBe('warn');
    expect(opts.globalLogPath).toBe('/h/logs/nighthawk.log');
    host.dispose();
  });
});
