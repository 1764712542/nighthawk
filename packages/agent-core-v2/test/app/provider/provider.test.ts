...
describe('provider config section helpers', () => {
  it('declares NIGHTHAWK_MODEL_* bindings for the env provider', () => {
    expect(providersEnvBindings).toEqual({
      [ENV_MODEL_PROVIDER_KEY]: {
        apiKey: 'NIGHTHAWK_MODEL_API_KEY',
        type: 'NIGHTHAWK_MODEL_PROVIDER_TYPE',
        baseUrl: 'NIGHTHAWK_MODEL_BASE_URL',
      },
    });
  });
  ...
  it('strips only the env provider before write-back', () => {
    expect(
      stripProvidersEnv({
        user: { type: 'nighthawk', apiKey: 'sk-user' },
