...
describe('McpManagementService', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-home-'));
    vi.stubEnv('NIGHTHAWK_HOME', home);
  ...
  describe('read-only guards', () => {
    it.each([
      ...
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-read-only-'));
      ...
      await expect(mutate(project)).rejects.toMatchObject({
        code: ErrorCodes.REQUEST_INVALID,
        message: `MCP server "local" is read-only: it is defined in ${join(project, '.nighthawk', 'mcp.json')} — edit that file instead`,
      });
      await expect(store.list()).resolves.toEqual([]);
    });
  ...
  describe('redaction', () => {
    ...
    it('lists project-layer entries as read-only redacted views when a cwd is given', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-proj-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
      ...
      expect(local).toMatchObject({
        source: 'global',
        mutable: false,
        origin: join(project, '.nighthawk', 'mcp.json'),
      });
    ...
    it('hides project-layer entries when the workspace is untrusted', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-untrusted-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
  ...
  describe('testServer', () => {
    ...
    it('probes an inline stdio config without retaining the probe cwd workspace', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-cwd-'));
      tempDirs.push(cwd);
    ...
    it('probes a nested cwd against the containing workspace runtimes', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-nested-'));
      tempDirs.push(cwd);
    ...
    it('rejects a non-local runtime_id probe when no workspace contains the cwd', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-remote-miss-'));
      tempDirs.push(cwd);
    ...
    it('probes a non-local runtime_id through the containing workspace', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-remote-hit-'));
      tempDirs.push(cwd);
      findContaining.mockReturnValue({ id: 'test-workspace' } as unknown as WorkspaceInstance);
    ...
    it('keeps the transient local probe for an explicit local runtime_id', async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-local-explicit-'));
      tempDirs.push(cwd);
    ...
    it('does not execute a project server while the workspace is untrusted', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-untrusted-probe-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
    ...
    it('waits for config and identity readiness before starting a probe', async () => {
      ...
      const cwd = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-ready-'));
      tempDirs.push(cwd);
  ...
  describe('inspectServers', () => {
    it('includes trusted project-layer entries when cwd is provided', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-inspect-project-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
  ...
  describe('resolveServerByName', () => {
    it('resolves a project-layer-only name when cwd is provided', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-resolve-project-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
  ...
  describe('OAuth operations', () => {
    it('begins authorization against the project-layer URL when cwd is provided', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-begin-project-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
    ...
    it('resets credentials for the project-layer URL when cwd is provided', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-management-reset-project-'));
      tempDirs.push(project);
      await mkdir(join(project, '.nighthawk'), { recursive: true });
      await writeFile(
        join(project, '.nighthawk', 'mcp.json'),
