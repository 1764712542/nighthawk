...
function failedFiles(dir: string): string[] {
  const telemetryDir = join(dir, 'telemetry');
  if (!existsSync(telemetryDir)) return [];
  return readdirSync(telemetryDir).filter((f) => f.startsWith('failed_'));
}

describe('CloudAppender', () => {
  let homeDir: string;
  let savedTelemetryEndpoint: string | undefined;

  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'cloud-appender-'));
    savedTelemetryEndpoint = process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'];
    process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'] = TEST_ENDPOINT;
  });

  afterEach(() => {
    rmSync(homeDir, { recursive: true, force: true });
    if (savedTelemetryEndpoint === undefined) delete process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'];
    else process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'] = savedTelemetryEndpoint;
  });
  ...
  it('prefers an explicit endpoint option over the env default', async () => {
    const requests: CapturedRequest[] = [];
    const appender = new CloudAppender(
      baseOptions({
        homeDir,
        endpoint: 'https://telemetry-override.example.test/v1/event',
        fetchImpl: makeFetch((req) => {
          requests.push(req);
          return okResponse();
        }),
    ...
    expect(requests).toHaveLength(1);
    expect(requests[0]?.url).toBe('https://telemetry-override.example.test/v1/event');
  });

  it('silently drops events when no endpoint is configured', async () => {
    delete process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'];
    const requests: CapturedRequest[] = [];
    const appender = new CloudAppender(
      baseOptions({
        homeDir,
    ...
    expect(failedFiles(homeDir)).toHaveLength(0);
  });

  it('treats a blank NIGHTHAWK_TELEMETRY_ENDPOINT as unset and drops events', async () => {
    process.env['NIGHTHAWK_TELEMETRY_ENDPOINT'] = '   ';
    const requests: CapturedRequest[] = [];
    const appender = new CloudAppender(
      baseOptions({
        homeDir,
