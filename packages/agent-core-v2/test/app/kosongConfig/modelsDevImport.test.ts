...
const HOST_HEADERS = { 'User-Agent': 'nighthawk-test/1.0' };
...
describe('IModelsDevImportService', () => {
  ...
  it('imports a catalog entry as provider + aliases without touching the default pointers', async () => {
    ...
    expect(config.get('defaultProvider')).toBe('nighthawk');
  ...
  it('filters pool entries a catalog import drops, keeping a surviving default', async () => {
    ...
    const { config, imports } = createHost({
      ...
      models: {
        ...
        k2: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 131072 },
      },
      secondaryModel: {
        defaultModel: 'k2',
  ...
  it('imports a custom registry with a source blob and drops providers vanished upstream', async () => {
    ...
    const { config, imports } = createHost({
      providers: {
        ...
        nighthawk: { type: 'nighthawk', apiKey: 'sk-nighthawk' },
      },
      models: {
    ...
    expect(providers['nighthawk']).toMatchObject({ type: 'nighthawk' });
