...
async function createHost(
  ...
  const host = createScopedTestHost([
    ...
    [
      IBootstrapService,
      stubBootstrap('/tmp/nighthawk-home', {}, { requestHeaders: { 'User-Agent': 'nighthawk-test/1.0' } }),
    ],
    [
      IAgentIdentity,
      stubAgentIdentity({ hostRequestHeaders: { 'User-Agent': 'nighthawk-test/1.0' } }),
    ],
  ]);
...
describe('refreshProviderModels write behavior', () => {
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ 'User-Agent': 'nighthawk-test/1.0' }),
        }),
      );
    } finally {
      host.dispose();
  ...
  it('refreshes a hand-configured API-key provider at the managed endpoint', async () => {
        ...
        new Response(
          JSON.stringify({
            data: [
              ...
              { id: 'nighthawk-k2.5', context_length: 131072 },
            ],
          }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        ),
    ...
    const { host, config, discovery, events, providers, models } = await createHost({
      providers: {
        'my-nighthawk': { type: 'nighthawk', baseUrl, apiKey: 'sk-distributed-key' },
      },
      models: {
        'my-nighthawk/nighthawk-k2': {
      ...
      defaultModel: 'my-nighthawk/nighthawk-k2',
    });
    try {
      const result = await discovery.refreshProviderModels({ scope: 'all' });
      ...
      expect(result.failed).toEqual([]);
      expect(result.changed).toEqual([
        { provider_id: 'my-nighthawk', provider_name: 'my-nighthawk', added: 1, removed: 0 },
      ]);
      expect(events.published).toEqual([
        expect.objectContaining({ type: 'event.model_catalog.changed' }),
      ]);
      ...
      expect(providers.list()['my-nighthawk']).toEqual({
        type: 'nighthawk',
        baseUrl,
        apiKey: 'sk-distributed-key',
      });
      const modelRecords = models.list();
      expect(modelRecords['my-nighthawk/nighthawk-k2']?.displayName).toBe('Fresh K2');
      expect(modelRecords['my-nighthawk/nighthawk-k2.5']).toBeDefined();
      expect(config.get<string>('defaultModel')).toBe('my-nighthawk/nighthawk-k2');
    } finally {
      host.dispose();
    }
  });
  ...
  it('clears a stale defaultModel whose alias upstream dropped', async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            data: [{ id: 'nighthawk-k3', context_length: 1048576, supports_reasoning: true }],
          }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        ),
    );
    ...
    try {
      ...
      expect(result.failed).toEqual([]);
      expect(result.changed).toEqual([
        { provider_id: 'my-nighthawk', provider_name: 'my-nighthawk', added: 1, removed: 1 },
      ]);
      expect(config.get('defaultModel')).toBeUndefined();
      expect(config.get('thinking')).toBeUndefined();
      const modelRecords = models.list();
      expect(modelRecords['my-nighthawk/nighthawk-k3']).toBeDefined();
      expect(modelRecords['my-nighthawk/nighthawk-k2']).toBeUndefined();
    } finally {
      host.dispose();
    }
  });
  ...
  it('clears the subagent model pool when a refresh drops its default alias', async () => {
    ...
    const { host, config, discovery } = await createHost({
      ...
      models: {
        'my-nighthawk/nighthawk-k2': { provider: 'my-nighthawk', model: 'nighthawk-k2', maxContextSize: 262144 },
      },
      secondaryModel: {
        defaultModel: 'my-nighthawk/nighthawk-k2',
        models: { 'my-nighthawk/nighthawk-k2': 'fast and cheap' },
      },
    });
    try {
      const result = await discovery.refreshProviderModels({ scope: 'all' });
  ...
  it('filters pool entries a refresh dropped while keeping a surviving default', async () => {
    ...
    const { host, config, discovery } = await createHost({
      ...
      secondaryModel: {
        defaultModel: 's1',
        models: { s1: 'static fallback', 'my-nighthawk/nighthawk-k2': 'managed' },
      },
    });
    try {
      const result = await discovery.refreshProviderModels({ scope: 'all' });
