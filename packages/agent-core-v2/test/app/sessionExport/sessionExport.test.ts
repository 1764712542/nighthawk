...
describe('sessionExport', () => {
  ...
  it('exports a v2 session directory with per-agent wire activity and optional global log', async () => {
    ...
    await writeFile(join(sessionDir, 'logs', 'nighthawk.log'), '{"msg":"session"}\n', 'utf-8');
    await writeFile(
    ...
    const globalLogPath = join(tmp, 'logs', 'nighthawk.log');
    ...
    expect(result.entries).toEqual([
      ...
      'logs/nighthawk.log',
      'state.json',
      'logs/global/nighthawk.log',
    ]);
    expect(result.manifest).toMatchObject({
      sessionId: 'ses_demo',
      nighthawkVersion: '1.0.0-test',
      title: 'Demo',
      workspaceDir: '/workspace/demo',
      sessionFirstActivity: '2023-11-14T22:13:20.000Z',
      sessionLastActivity: '2023-11-14T22:13:25.000Z',
      sessionLogPath: 'logs/nighthawk.log',
      globalLogPath: 'logs/global/nighthawk.log',
    });
  });
  ...
  it('uses a timestamped default output path when outputPath is omitted', async () => {
    ...
    try {
      ...
      expect(basename(result.zipPath)).toMatch(/^nighthawk-debug-ses_defa-\d{8}-\d{6}\.zip$/);
  ...
  it('keeps the session log bound when it rotates as wire scanning starts', async () => {
    ...
    const logPath = join(sessionDir, 'logs', 'nighthawk.log');
    ...
    try {
      ...
      expect(result.entries).toContain('logs/nighthawk.log');
      await expect(readZipEntry(outputPath, 'logs/nighthawk.log')).resolves.toEqual(log);
    } finally {
  ...
  it('keeps the global log bound when it rotates as wire scanning starts', async () => {
    ...
    try {
      ...
      expect(result.entries).toContain('logs/global/nighthawk.log');
      await expect(readZipEntry(outputPath, 'logs/global/nighthawk.log')).resolves.toEqual(log);
    } finally {
      fsOpenHook.afterOpen = undefined;
    }
  });
  ...
  it('closes pre-opened logs when manifest creation fails before writer ownership', async () => {
    ...
    const sessionLogPath = join(sessionDir, 'logs', 'nighthawk.log');
    const globalLogPath = join(tmp, 'logs', 'nighthawk.log');
    const wirePath = join(sessionDir, 'agents', 'main', 'wire.jsonl');
    ...
    expect((await readdir(tmp)).filter((entry) => entry.startsWith('.nighthawk-session-export-'))).toEqual(
      [],
    );
  });
  ...
  it('omits the optional global log when the configured file is missing', async () => {
    ...
    expect(result.entries).not.toContain('logs/global/nighthawk.log');
  });
  ...
  it('closes a pre-opened source when it conflicts with the output path', async () => {
    ...
    await expect(
      writeExportZip({
        ...
        sessionDir: tmp,
        sessionFiles: [],
        extraEntries: [{ source, target: 'logs/global/nighthawk.log' }],
      }),
    ).rejects.toMatchObject({ code: 'session.export_output_conflict' });
  ...
  it('archives a bound session log after its path is rotated', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'session-export-test-'));
    const logPath = join(tmp, 'logs', 'nighthawk.log');
    const rotatedPath = `${logPath}.1`;
    ...
    ).resolves.toContain('logs/nighthawk.log');
    await expect(readZipEntry(outputPath, 'logs/nighthawk.log')).resolves.toEqual(original);
    expect(closeCalls).toBe(1);
  });
  ...
  it('includes a bounded Web log in the exported archive', async () => {
    ...
    expect(result.entries).toContain('logs/nighthawk-web.jsonl');
    expect(result.manifest.webLogPath).toBe('logs/nighthawk-web.jsonl');
    await expect(readZipEntry(outputPath, 'logs/nighthawk-web.jsonl')).resolves.toEqual(
  ...
  it('includes the desktop app log when given', async () => {
    ...
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'state.json'), '{}\n', 'utf-8');
    const desktopLogPath = join(tmp, 'logs', 'nighthawk-desktop.log');
    await mkdir(join(tmp, 'logs'), { recursive: true });
    ...
    expect(result.entries).toContain('logs/nighthawk-desktop.log');
    expect(result.manifest.desktopLogPath).toBe('logs/nighthawk-desktop.log');
    await expect(readZipEntry(outputPath, 'logs/nighthawk-desktop.log')).resolves.toEqual(
  ...
  it('skips a missing desktop app log silently', async () => {
    ...
    expect(result.entries).not.toContain('logs/nighthawk-desktop.log');
