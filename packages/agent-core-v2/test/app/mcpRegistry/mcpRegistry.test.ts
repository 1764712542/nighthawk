...
describe('McpRegistryService', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-registry-home-'));
  ...
  async function makeProject(): Promise<{ project: string; sub: string }> {
    const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-registry-proj-'));
  ...
  describe('list', () => {
    ...
    it('resolves a relative non-git cwd before checking workspace trust', async () => {
      const project = mkdtempSync(join(tmpdir(), 'nighthawk-mcp-registry-non-git-'));
      tempDirs.push(project);
      await writeJson(join(project, '.mcp.json'), {
