...
import { NighthawkFetchURLProvider } from '#/app/web/providers/nighthawk-fetch-url';
...
describe('WebFetchService', () => {
  ...
  it('yields the local fetcher when the managed provider is not an OAuth nighthawk provider', () => {
  ...
  it('builds a NightHawk fetcher from the managed provider oauth ref', () => {
    providers = {
      [OAUTH_PROVIDER]: {
        type: 'nighthawk',
    ...
    expect(fetcher()).toBeInstanceOf(NighthawkFetchURLProvider);
  ...
  it('builds a NightHawk fetcher from the services.nighthawk_fetch api_key config', async () => {
    servicesConfig = {
      nighthawkFetch: {
  ...
  it('sends the configured identity to a services-config endpoint', async () => {
    identitySlug = 'acme';
    servicesConfig = {
      nighthawkFetch: { baseUrl: 'https://fetch.example.com/fetch', apiKey: 'fetch-key' },
    };
  ...
  it('prefers the services.nighthawk_fetch config over the managed oauth provider', () => {
    servicesConfig = {
      nighthawkFetch: { baseUrl: 'https://config.example.com/fetch', apiKey: 'config-key' },
    };
    providers = {
      [OAUTH_PROVIDER]: {
        type: 'nighthawk',
  ...
  it('builds a NightHawk fetcher from the services.nighthawk_fetch oauth ref', () => {
    servicesConfig = {
      nighthawkFetch: {
        baseUrl: 'https://fetch.example.com/fetch',
        oauth: { storage: 'file', key: 'oauth/nighthawk' },
  ...
  it('yields the local fetcher when services.nighthawk_fetch has no baseUrl and no managed oauth', () => {
    servicesConfig = { nighthawkFetch: { apiKey: 'fetch-key' } };
    expect(fetcher()).toBeInstanceOf(LocalFetchURLProvider);
    expect(resolveTokenProvider).not.toHaveBeenCalled();
  });
});
