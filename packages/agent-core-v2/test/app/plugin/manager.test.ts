...
describe('PluginManager', () => {
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        command: 'echo stop',
        cwd: root,
        env: { NIGHTHAWK_HOME: home, NIGHTHAWK_PLUGIN_ROOT: root },
      },
    ]);
  ...
  it('installs a local-path plugin into the managed root', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'nighthawk.plugin.json'), JSON.stringify({ name: 'other' }), 'utf8');
      const manager = new PluginManager({ nighthawkHomeDir: home });
  ...
  it('installs a zip-url plugin', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'nighthawk.plugin.json'), JSON.stringify({ name: 'zip-plugin' }), 'utf8');
  ...
  it('installs a github plugin through codeload', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'nighthawk.plugin.json'), JSON.stringify({ name: 'github-plugin' }), 'utf8');
      execFileSync('zip', ['-qr', zipPath, '.'], { cwd: sourceRoot });
