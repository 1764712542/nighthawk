...
describe('PluginService (plugin boundary)', () => {
  ...
  async function makeHome(): Promise<string> {
    const home = await mkdtemp(path.join(tmpdir(), 'nighthawk-home-'));
  ...
  it('restores the previous managed copy when reinstall persistence fails', async () => {
    ...
    try {
      ...
      await expect(
        readFile(path.join(previous.root, 'nighthawk.plugin.json'), 'utf8'),
  ...
  it('injects the managed NightHawk endpoint env into stdio plugin MCP servers only', async () => {
    const home = await makeHome();
    await writeValidInstalledFile(home);
    const host = makeHost(
      home,
      stubProviderService({
        [NIGHTHAWK_PROVIDER_NAME]: {
          ...
          oauth: { storage: 'file', key: 'nighthawk', oauthHost: 'https://auth.example.test' },
        },
      }),
    );
    try {
      ...
      expect(servers['plugin-demo:finance']).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            NIGHTHAWK_BASE_URL: 'https://api.example.test/',
            NIGHTHAWK_OAUTH_HOST: 'https://auth.example.test',
            CUSTOM: '1',
            NIGHTHAWK_HOME: home,
            NIGHTHAWK_PLUGIN_ROOT: await realpath(managedRoot),
          }),
        }),
      );
      expect(JSON.stringify(servers['plugin-demo:docs'])).not.toContain('NIGHTHAWK_BASE_URL');
    } finally {
      host.dispose();
    }
  });

  it('merges the managed NightHawk endpoint env into stdio MCP server entries with provenance', async () => {
    const home = await makeHome();
    await writeValidInstalledFile(home);
    const host = makeHost(
      home,
    ...
    try {
      ...
      expect(finance?.config).toEqual(
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            ...
            CUSTOM: '1',
            NIGHTHAWK_HOME: home,
            NIGHTHAWK_PLUGIN_ROOT: managedRoot,
          }),
        }),
      );
      ...
      expect(JSON.stringify(docs?.config)).not.toContain('NIGHTHAWK_BASE_URL');
    } finally {
      host.dispose();
    }
  });
  ...
  it('waits for provider config before injecting persisted managed endpoints', async () => {
    ...
    try {
      ...
      providerConfigs[NIGHTHAWK_PROVIDER_NAME] = {
        baseUrl: 'https://ready.example.test/',
        oauth: { storage: 'file', key: 'nighthawk', oauthHost: 'https://auth.ready.example.test' },
      };
      ...
      await expect(servers).resolves.toMatchObject({
        'plugin-ready-demo:finance': {
          env: {
            NIGHTHAWK_BASE_URL: 'https://ready.example.test/',
            NIGHTHAWK_OAUTH_HOST: 'https://auth.ready.example.test',
          },
        },
      });
    } finally {
  ...
  it('prefers explicit NIGHTHAWK_BASE_URL / NIGHTHAWK_OAUTH_HOST env over the persisted provider', async () => {
    const home = await makeHome();
    await writeValidInstalledFile(home);
    const host = makeHost(
      home,
      ...
      {
        NIGHTHAWK_BASE_URL: 'https://env.example.test/',
        NIGHTHAWK_OAUTH_HOST: 'https://legacy.example.test',
      },
    );
    try {
      const svc = host.app.accessor.get(IPluginService);
  ...
  it('does not inject managed env when neither env nor the nighthawk provider supplies it', async () => {
    const home = await makeHome();
    await writeValidInstalledFile(home);
    const host = makeHost(home);
    try {
      ...
      expect(env).not.toHaveProperty('NIGHTHAWK_BASE_URL');
      expect(env).not.toHaveProperty('NIGHTHAWK_OAUTH_HOST');
    } finally {
      host.dispose();
    }
  });
