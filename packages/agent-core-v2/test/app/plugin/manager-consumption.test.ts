...
async function isolatedTmpdir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), 'nighthawk-isolated-tmp-'));
  vi.stubEnv('TMPDIR', dir);
  return dir;
}

async function zipTempLeftovers(dir: string): Promise<readonly string[]> {
  return (await readdir(dir)).filter((entry) => entry.startsWith('nighthawk-plugin-zip-'));
}
...
async function makePlugin(
  ...
  await writeFile(path.join(root, 'nighthawk.plugin.json'), JSON.stringify(manifest), 'utf8');
  return realpath(root);
}
...
describe('PluginManager consumption plane', () => {
  ...
  it('pluginSkillRoots() excludes plugins in error state', async () => {
    ...
    await writeFile(
      path.join(await managedPluginRoot(manager, 'demo'), 'nighthawk.plugin.json'),
      '{ not json',
      'utf8',
    );
  ...
  it('removes the zip temp dir and reports the original source when a zip plugin has no manifest', async () => {
    ...
    expect(message).not.toContain('nighthawk-plugin-zip');
    expect(await zipTempLeftovers(isolated)).toEqual([]);
  ...
  it('mcpServerEntries() applies the stdio runtime transforms to every entry', async () => {
    ...
    expect(JSON.stringify(docs?.config)).not.toContain('NIGHTHAWK_PLUGIN_ROOT');
  });
  ...
  it('install() from /tree/<tag-shaped-ref> pins the resolved commit', async () => {
    ...
    await writeFile(
      path.join(sourceRoot, 'nighthawk.plugin.json'),
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    await writeFile(
      path.join(cdnSource, 'nighthawk.plugin.json'),
    ...
    await writeFile(
      path.join(ghSource, 'nighthawk.plugin.json'),
