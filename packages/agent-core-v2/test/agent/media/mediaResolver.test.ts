...
import { buildNighthawkFileUrl } from '#/agent/media/nighthawkFileUrl';
...
function requester(opts: {
  ...
  return {
    model: {
      ...
      providerName: 'p',
      providerType: opts.providerType ?? 'nighthawk',
...
describe('AgentMediaResolverService video strategy', () => {
  it('uploads a nighthawk-file video once and reuses the cached reference on later steps', async () => {
    ...
    const message = videoMessage(buildNighthawkFileUrl(FILE_ID));
  ...
  it.each<TagCase>([
    ...
    const out = await resolver(files, sessionDir).resolve(
      [videoMessage(buildNighthawkFileUrl(fileId))],
      req(upload),
    );
  ...
  it('rethrows an auth failure so it can drive credential refresh', async () => {
    ...
    await expect(
      res.resolve([videoMessage(buildNighthawkFileUrl(FILE_ID))], requester({ uploadVideo: upload })),
  ...
  it('emits an unavailable placeholder when a stale reference has no canonical copy', async () => {
    const out = await resolver(new Map()).resolve(
      [videoMessage(buildNighthawkFileUrl('missing'))],
  ...
  it.each([
    {
      ...
      message: videoMessage(buildNighthawkFileUrl(FILE_ID)),
    ...
    },
    {
      ...
      message: imageMessage(buildNighthawkFileUrl(FILE_ID)),
...
describe('AgentMediaResolverService image strategy', () => {
  it('inlines a daemon-ref image as a canonical base64 data url, leaving other parts untouched', async () => {
    ...
    const message = imageMessage(buildNighthawkFileUrl(FILE_ID), tagPart, remotePart);
  ...
  it('rethrows a cancelled image read instead of degrading to a tag', async () => {
    ...
    await expect(
      res.resolve(
        [imageMessage(buildNighthawkFileUrl(FILE_ID))],
        requester({}),
        controller.signal,
      ),
  ...
  it.each([
    ...
    const message = imageMessage(buildNighthawkFileUrl(fileId));
  ...
  it.each([
    {
      ...
      files: new Map([[FILE_ID, { name: 'pic.png', bytes: PNG_BYTES }]]),
      url: buildNighthawkFileUrl(FILE_ID),
      imageIn: false,
    ...
    },
    {
      ...
      files: new Map<string, { name: string; bytes: Buffer }>(),
      url: buildNighthawkFileUrl('missing'),
      imageIn: true,
      expected: IMAGE_UNAVAILABLE_TEXT,
    },
  ...
  it('memoizes an inlined image across resolves without re-reading the bytes', async () => {
    ...
    const message = imageMessage(buildNighthawkFileUrl(FILE_ID));
  ...
  it('evicts the least-recently-hit memo entry once the total byte budget is exceeded', async () => {
    ...
    for (const id of ids.slice(0, 8)) {
      await res.resolve([imageMessage(buildNighthawkFileUrl(id))], req);
    }
    await res.resolve([imageMessage(buildNighthawkFileUrl('file_0'))], req);
    await res.resolve([imageMessage(buildNighthawkFileUrl('file_8'))], req);
    expect(counting.gets).toBe(9);

    await res.resolve([imageMessage(buildNighthawkFileUrl('file_0'))], req);
    expect(counting.gets).toBe(9);
    await res.resolve([imageMessage(buildNighthawkFileUrl('file_1'))], req);
  ...
  it('keeps a legacy persisted tag as text and degrades the reference with a synthesized tag', async () => {
    ...
    const message: Message = {
      ...
      content: [
        ...
        { type: 'video_url', videoUrl: { url: buildNighthawkFileUrl(FILE_ID) } },
      ],
    };
