...
describe('daemon file URL', () => {
  it('builds and parses a bare reference', () => {
    expect(buildDaemonFileUrl('file_1')).toBe('nighthawk-file://file_1');
    expect(parseDaemonFileUrl('nighthawk-file://file_1')).toEqual({ fileId: 'file_1' });
  });

  it('strips a legacy `?path=` query at parse time', () => {
    expect(parseDaemonFileUrl('nighthawk-file://file_1?path=%2Fa%20b%2Fclip.mp4')).toEqual({
      fileId: 'file_1',
    });
    expect(parseDaemonFileUrl('nighthawk-file://file_1?path=')).toEqual({ fileId: 'file_1' });
    expect(parseDaemonFileUrl('nighthawk-file://file_1?path=%E0%A4%A')).toEqual({ fileId: 'file_1' });
  });

  it('rejects non-daemon URLs and empty file ids', () => {
    expect(isDaemonFileUrl('nighthawk-file://file_1')).toBe(true);
    ...
    expect(parseDaemonFileUrl('nighthawk-file://')).toBeUndefined();
    expect(parseDaemonFileUrl('nighthawk-file://?path=%2Fa')).toBeUndefined();
  });
