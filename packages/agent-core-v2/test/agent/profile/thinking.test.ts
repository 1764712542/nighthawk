...
const nighthawkEffortModel = { ...effortModel, protocol: 'openai', providerType: 'nighthawk' };
const nighthawkBooleanModel = { ...booleanModel, protocol: 'openai', providerType: 'nighthawk' };
...
describe('resolveThinkingEffortForModel', () => {
  ...
  it('falls back to the model default for an unsupported NightHawk effort', () => {
    expect(resolveThinkingEffortForModel('ultra', undefined, nighthawkEffortModel, true)).toBe(
      'medium',
    );
  });

  it('projects a concrete effort to on for a boolean-only NightHawk model', () => {
    expect(resolveThinkingEffortForModel('ultra', undefined, nighthawkBooleanModel, true)).toBe('on');
  });

  it('reports unsupported concrete efforts only for NightHawk effort models', () => {
    expect(modelSupportsThinkingEffort('ultra', nighthawkEffortModel, true)).toBe(false);
