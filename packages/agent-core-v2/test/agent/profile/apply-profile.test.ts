...
describe('AgentProfileService.applyProfile', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-apply-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'nighthawk-apply-work-'));
  });
  ...
  it('maps prompt context roots through the bound runtime workspace view', async () => {
    const mappedDir = await mkdtemp(join(tmpdir(), 'nighthawk-apply-mapped-'));
    const localExtra = await mkdtemp(join(tmpdir(), 'nighthawk-apply-extra-local-'));
    const mappedExtra = await mkdtemp(join(tmpdir(), 'nighthawk-apply-extra-mapped-'));
    try {
...
function pluginStub(
  ...
  return {
    ...
    pluginToolRoots: async () => [],
    pluginProfileRoots: async () => [],
    pluginConfigSectionRoots: async () => [],
