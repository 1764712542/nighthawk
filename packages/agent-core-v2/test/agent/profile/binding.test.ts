...
describe('AgentProfileService.bind', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-bind-home-'));
  });
  ...
  it('binds a profile + model atomically and becomes runnable', async () => {
    ...
    expect(svc.getSystemPrompt()).toContain('NightHawk Code CLI');
  });
  ...
  it('persists the complete binding in one journal record', async () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      thinkingEffort: 'on',
      systemPrompt: expect.stringContaining('NightHawk Code CLI'),
  ...
  it('keeps the system prompt frozen after a default bind when AGENTS.md changes', async () => {
    const workDir = await mkdtemp(join(tmpdir(), 'nighthawk-bind-work-'));
    try {
  ...
  it('rejects an unsupported thinking effort atomically before first bind', async () => {
    ctx = createTestAgent(
      {
        initialConfig: {
          providers: {
            nighthawk: { type: 'nighthawk', apiKey: 'test-key', baseUrl: 'https://api.example.test/v1' },
          },
          models: {
            'nighthawk/nighthawk': {
              provider: 'nighthawk',
    ...
    expect(svc.data().profileName).toBeUndefined();
    await svc.bind({ profile: DEFAULT_AGENT_PROFILE_NAME, model: 'nighthawk/nighthawk' });
    expect(svc.data().profileName).toBe(DEFAULT_AGENT_PROFILE_NAME);
  });
...
describe('AgentToolPolicyService tool denylist', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-deny-home-'));
  });
...
describe('AgentToolPolicyService global [tools] config', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-tools-config-home-'));
  });
...
describe('AgentToolPolicyService.setSessionDisabledTools', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-session-deny-home-'));
  });
...
describe('AgentToolPolicyService executor enforcement', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-executor-policy-home-'));
  });
...
describe('AgentProfileService tool-pattern warnings', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-tool-pattern-home-'));
  });
...
describe('agentsMdReminder seeding', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-seed-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'nighthawk-seed-work-'));
  });
