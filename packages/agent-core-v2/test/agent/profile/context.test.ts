...
beforeEach(async () => {
  homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-agents-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'nighthawk-agents-work-'));
  extraDirs = [];
  fs = createFs();
});
...
describe('loadAgentsMd user-level discovery', () => {
  it('loads user-level branded and generic files before project-level', async () => {
    await mkdir(join(homeDir, '.nighthawk'), { recursive: true });
    await writeFile(join(homeDir, '.nighthawk', 'AGENTS.md'), 'user branded', 'utf-8');
  ...
  it('does not load the same file twice when the work dir is the home dir', async () => {
    await mkdir(join(homeDir, '.nighthawk'), { recursive: true });
    await writeFile(join(homeDir, '.nighthawk', 'AGENTS.md'), 'home branded', 'utf-8');
...
describe('loadAgentsMd symlinked files', () => {
  it('follows symlinks when loading user-level and project-level AGENTS.md', async () => {
    const targetDir = await mkdtemp(join(tmpdir(), 'nighthawk-agents-target-'));
    ...
    await mkdir(join(homeDir, '.nighthawk'), { recursive: true });
    await symlink(brandTarget, join(homeDir, '.nighthawk', 'AGENTS.md'));
...
describe('loadAgentsMd unreadable paths', () => {
  it('warns when an instruction file exists but is a dangling symlink', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'nighthawk-agents-brand-'));
...
describe('loadAgentsMd brand home (NIGHTHAWK_HOME)', () => {
  let brandHome: string;

  beforeEach(async () => {
    brandHome = await mkdtemp(join(tmpdir(), 'nighthawk-agents-brand-'));
  });
  ...
  it('ignores the real-home .nighthawk/AGENTS.md when the brand home is elsewhere', async () => {
    await writeFile(join(brandHome, 'AGENTS.md'), 'brand wins', 'utf-8');
    await mkdir(join(homeDir, '.nighthawk'), { recursive: true });
    await writeFile(join(homeDir, '.nighthawk', 'AGENTS.md'), 'stale real-home brand', 'utf-8');
  ...
  it('falls back to the real-home .nighthawk/AGENTS.md when no brand home is given', async () => {
    await mkdir(join(homeDir, '.nighthawk'), { recursive: true });
    await writeFile(join(homeDir, '.nighthawk', 'AGENTS.md'), 'fallback branded', 'utf-8');
...
describe('loadAgentsMd nested project hierarchy', () => {
  it('loads AGENTS.md from the project root down to the cwd in root→leaf order', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'nighthawk-agents-project-'));
...
describe('prepareSystemPromptContext additional directories', () => {
  it('includes additional directory listings without loading their AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'nighthawk-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDir = await mkdtemp(join(tmpdir(), 'nighthawk-agents-extra-'));
  ...
  it('loads user-level AGENTS.md once and skips additional directory AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'nighthawk-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDirA = await mkdtemp(join(tmpdir(), 'nighthawk-agents-extra-a-'));
    const extraDirB = await mkdtemp(join(tmpdir(), 'nighthawk-agents-extra-b-'));
...
describe('loadAgentsMdDetailed discovered paths', () => {
  ...
  it('returns the normalized paths of every injected file in collection order', async () => {
    ...
    await writeFile(join(workDir, '.nighthawk', 'AGENTS.md'), 'dot nighthawk', 'utf-8');
    await writeFile(join(workDir, 'AGENTS.md'), 'project instructions', 'utf-8');

    const result = await loadAgentsMdDetailed({ fs, homeDir }, workDir);

    expect(result.paths).toEqual([
      normalize(join(homeDir, '.nighthawk', 'AGENTS.md')),
      normalize(join(workDir, '.nighthawk', 'AGENTS.md')),
