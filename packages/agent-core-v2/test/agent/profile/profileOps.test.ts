...
function createTestModel(
  ...
  return {
    ...
    maxContextSize: 1000,
    supportEfforts: providerType === 'nighthawk' ? ['low', 'medium', 'high', 'max'] : undefined,
    defaultEffort: providerType === 'nighthawk' ? 'high' : undefined,
...
function createProtocolRegistryStub(): IProtocolAdapterRegistry {
  return {
    ...
    resolveAdapterIdentity: (protocol: Protocol, providerType?: string) => ({
      baseId: protocol,
      traits:
        providerType === 'nighthawk' && protocol === 'openai'
          ? [
              {
          ...
          : providerType === 'nighthawk' && protocol === 'anthropic'
...
describe('AgentProfileService (wire-backed config.update)', () => {
  ...
  it('applies thinking.keep model override when thinking is enabled', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
    ...
    host.svc.update({ modelAlias: 'nighthawk', thinkingLevel: 'high' });
  ...
  it('uses the resolved NightHawk effort instead of the configured default', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
  ...
  it('forces the environment NightHawk effort instead of the resolved effort', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
  ...
  it('does not leak a forced NightHawk effort when switching to a non-NightHawk model', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
  ...
  it('forces NightHawk effort through Anthropic without NightHawk generation kwargs', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ protocol: 'anthropic', providerType: 'nighthawk' }),
    });
  ...
  it('defaults thinking.keep to "all" when thinking is enabled on NightHawk', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
  ...
  it('treats an off env thinking.keep override as disabled on NightHawk', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
  ...
  it('does not apply thinking.keep model override when thinking is off', () => {
    ...
    host.svc.update({ modelAlias: 'nighthawk', thinkingLevel: 'off' });
  ...
  it('uses the session id as a NightHawk prompt cache hint', () => {
    modelCatalog = createModelCatalogStub({
      'nighthawk': createTestModel({ providerType: 'nighthawk' }),
    });
  ...
  it('resolves the session cache-key intent for non-NightHawk protocols too', () => {
    modelCatalog = createModelCatalogStub({
