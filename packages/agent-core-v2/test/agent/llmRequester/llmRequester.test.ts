...
describe('LLMRequester service migration coverage', () => {
  describe('wire observability records', () => {
    ...
    it('records the resolved NightHawk thinking keep default when thinking is enabled', async () => {
      ctx.configure({
        modelCapabilities: {
          image_in: false,
          video_in: false,
    ...
    it('records the env-forced NightHawk effort used by the provider', async () => {
      await ctx.dispose();
