...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'nighthawk-reminder-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'nighthawk-reminder-work-'));
...
describe('agentsMdReminder path-carrying tools', () => {
  ...
  it('discovers the .nighthawk/AGENTS.md variant alongside the plain one', async () => {
    const h = createHarness();
    const subDir = join(workDir, 'packages', 'kap-server');
    const dotNighthawk = normalize(join(subDir, '.nighthawk', 'AGENTS.md'));
    await writeAgentsMd(join(subDir, '.nighthawk'), 'dot nighthawk instructions');
    ...
    expect(outputText(result)).toBe('original result');
    const text = reminderText(h);
    expect(text).toContain(dotNighthawk);
...
describe('agentsMdReminder probing boundaries', () => {
  ...
  it('probes only the immediate directory outside any project', async () => {
    const h = createHarness();
    const outside = await mkdtemp(join(tmpdir(), 'nighthawk-reminder-outside-'));
  ...
  it('discovers a symlinked directory through the link at its lexical address', async () => {
    const h = createHarness();
    const target = await mkdtemp(join(tmpdir(), 'nighthawk-reminder-target-'));
