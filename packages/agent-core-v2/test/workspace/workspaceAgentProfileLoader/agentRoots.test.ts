...
describe('agentRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .nighthawk/agents directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.nighthawk/agents'), { recursive: true });

      const roots = await projectAgentRoots(hostFs, root);

      expect(
        roots.some((r) => r.path.endsWith('.nighthawk/agents') && r.source === 'project'),
      ).toBe(true);
    });
    ...
    it('falls back to the generic .agents/agents directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.nighthawk/agents'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.nighthawk/agents'))).toBe(true);
    });
    ...
    it('orders the brand directory before the generic directory', async () => {
      ...
      const roots = await projectAgentRoots(hostFs, root);
      const brandIdx = roots.findIndex((r) => r.path.endsWith('.nighthawk/agents'));
