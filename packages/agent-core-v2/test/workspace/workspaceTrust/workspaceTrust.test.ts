...
describe('WorkspaceTrustService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'nighthawk-workspace-trust-home-'));
    cwd = mkdtempSync(join(tmpdir(), 'nighthawk-workspace-trust-cwd-'));
  ...
  it('tracks different roots independently', async () => {
    const other = mkdtempSync(join(tmpdir(), 'nighthawk-workspace-trust-other-'));
    try {
      const { service: first } = createService(cwd);
      await first.ready;
      await first.trust();
