...
describe('WorkspaceMcpConfigService', () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), 'nighthawk-workspace-mcp-config-cwd-'));
    homeDir = mkdtempSync(join(tmpdir(), 'nighthawk-workspace-mcp-config-home-'));
