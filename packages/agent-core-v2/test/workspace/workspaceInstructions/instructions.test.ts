...
describe('WorkspaceInstructionsService', () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), 'nighthawk-instructions-work-'));
    osHomeDir = mkdtempSync(join(tmpdir(), 'nighthawk-instructions-os-'));
    brandHomeDir = mkdtempSync(join(tmpdir(), 'nighthawk-instructions-brand-'));
