...
import type {
  ...
  SessionSummary,
} from '@nighthawk/klient';
import { ErrorCodes, isError2 } from '@nighthawk/agent-core-v2';
import { RPCError } from '@nighthawk/klient';
...
export class AcpServer {
  ...
  async authenticate(params: AuthenticateRequest): Promise<AuthenticateResponse | void> {
    ...
    // Re-check the gate; clients spawn `nighthawk login` themselves via the
...
export function createAcpAgentApp(getServer: () => AcpServer): AgentApp {
  return agent({ name: 'nighthawk-acp' })
