/**
 * acp-server bootstrap — wires `@nighthawk/agent-core-v2` (the DI × Scope
 * engine) into an ACP (Agent Client Protocol) stdio server.
 *
 * Composition root: `bootstrap()` builds the App `Scope`; a `@nighthawk/
...
export async function runAcpServerWithStream(
  ...
  // `NIGHTHAWK_PLATFORM` from `@nighthawk/nighthawk-oauth`, which this
  ...
  const { app: core } = bootstrap(
    {
      ...
      configPath,
      clientIdentity: {
        productName: opts.agentInfo?.name ?? 'nighthawk-acp',
