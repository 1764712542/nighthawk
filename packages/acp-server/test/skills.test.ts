...
import type { SkillSummary } from '@nighthawk/agent-core-v2';
...
describe('acp-server skills / available commands', () => {
   ...
   * `<cwd>/.nighthawk/skills/acp-fixture/SKILL.md` (the engine's project
  ...
  async function bootWithFixtureSkill(): Promise<TestClient> {
    ...
    await mkdir(join(homeDir, '.nighthawk', 'skills', 'acp-fixture'), { recursive: true });
    await writeFile(
      join(homeDir, '.nighthawk', 'skills', 'acp-fixture', 'SKILL.md'),
