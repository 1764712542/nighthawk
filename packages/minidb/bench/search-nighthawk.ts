// bench/search-nighthawk.js
//
// Import ~/.nighthawk sessions (useful extraction) and full-text-search for a
// query, printing hits with context snippets.
//
// Run:  node bench/search-nighthawk.js <query>  [--full]
...
if (!query) {
  console.error('usage: node bench/search-nighthawk.js <query> [--full]');
  process.exit(1);
}

const DATA = path.join(os.homedir(), '.nighthawk');
