...
//
// "Without minidb" search baseline over the SAME real ~/.nighthawk sessions
// used by import-nighthawk.ts. Loads every session's extracted text into an
...
// range). Paired with import-nighthawk.ts, this shows what minidb's indexes
// buy you on real data.
//
// Run:  node --import tsx bench/search-baseline.ts [--data ~/.nighthawk]
...
const DATA = path.resolve(arg('data', path.join(os.homedir(), '.nighthawk')));
const FULL = argv.includes('--full'); // also index full tool results (matches import-nighthawk --full)
