// bench/import-nighthawk.js
//
// Import all workspaces + sessions from ~/.nighthawk into minidb and build a
// full-text index over the session content, then measure import + search speed.
//
// Run:  node bench/import-nighthawk.js [--data ~/.nighthawk] [--out <dir>]
...
const OUT = path.resolve(arg('out', path.join(os.tmpdir(), 'minidb-nighthawk-' + Date.now())));
